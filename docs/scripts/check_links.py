...
def find_markdown_files(root_dir):
    ...
    return [
        p
        for p in list(root.rglob("*.md")) + list(root.rglob("*.qmd"))
        if not (p.parent == root and p.name in ("index.md", "index-scuba.md", "SUMMARY.md"))
    ]
        ...
        for match in link_pattern.finditer(content):
            ...
            if base:
                ...
                if not target_path.exists():
                    errors.append(f"{filepath}: broken link [{link_text}]({link_target})")
