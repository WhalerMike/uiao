...
![UIAO Governance OS](../../images/canonical/UIAO-FIG-001-governance-os-overview.png){fig-alt="High-level overview of the UIAO Governance OS — identity, mission, and value proposition for federal agencies running Microsoft 365 in GCC-Moderate." width="100%"}
...
Per [ADR-079](https://github.com/WhalerMike/uiao/blob/main/src/uiao/canon/adr/adr-079-governance-principle-reconciliation.md),
three further principles — Two-Brain Execution, Boundary Enforcement, and
Tenant Agnosticism — bind only at specific [ADR-076](https://github.com/WhalerMike/uiao/blob/main/src/uiao/canon/adr/adr-076-tier-conformance-model.md)
conformance tiers (3+, 4+, and 5 respectively). They are declared in
`substrate-manifest.yaml` under `tier_specific_principles:` and apply per
the agency's declared tier. Tier 1 / 2 adopters honor the three universal
principles above without taking on the tier-specific commitments.
...
- **[UIAO Governance OS whitepaper](../whitepapers/uiao-governance-os-whitepaper.qmd)** —
  the long-form treatment of the operating model summarized here.
- **[Architecture Series — Six-Plane Architecture](../architecture-series/six-plane-architecture.qmd)** —
  how the substrate decomposes into planes.
- **[Universal-Enterprise Positioning](universal-enterprise-positioning-overview.qmd)** —
  why the engine is vertical-agnostic and federal is one adapter pack.
- **[Adapter Ecosystem Overview](adapter-ecosystem-overview.qmd)** — how
  consumer surfaces reach incumbent systems.
