...
aliases:
  - /customer-documents/reference-architecture/directory-migration.html
---

{{< lifecycle-banner >}}

Directory migration is the flagship journey of **OrgMod (Organizational
Modernization)** — the modernization member of the Org family, alongside
OrgPath (governance) and OrgComp (compliance). Like its siblings, OrgMod is
designed to be adopted together with them or entirely on its own.

![The Org family: four operational expressions — OrgPath (governance), OrgComp (Federal Organizational Compliance), OrgMod (Organizational Modernization), and OrgLink (external interconnection) — standing on one governance substrate of identity, addressing, evidence, and policy-bearing control planes. OrgComp connects to the substrate through a dashed Evidence & Authorization Contract seam, and a footer band states the four are adoptable together or each on its own.](../orgcomp-series/figs/org-family-substrate.png){width="100%"}
...
Every adapter follows the same six-phase governance-driven migration:
...
| **6. Decommission** | Formally retire legacy infrastructure after validation passes a stability window | Legacy systems offline; rollback window expired with no incidents; provenance archived for audit |

Adapter migrations that add a net-new capability without retiring any legacy infrastructure may declare Phase 6 as `N/A — net-new capability` in their registry entry with a brief justification. See [ADR-081](https://github.com/WhalerMike/uiao/blob/main/src/uiao/canon/adr/adr-081-directory-migration-phase-canonical-model.md) for the canonical phase model and the customer-side mapping pattern (DNS 11→6, PKI 6→6).

## Adapter Registration Schema

Every adapter registers in the `migration-adapter-registry.yaml` with standardized metadata. The contract is declared in [`src/uiao/schemas/migration-adapter-registry/migration-adapter-registry.schema.json`](https://github.com/WhalerMike/uiao/blob/main/src/uiao/schemas/migration-adapter-registry/migration-adapter-registry.schema.json) (ADR-081 Phase 2):

- **Adapter ID and name**
- **Priority tier** (CRITICAL / HIGH / MEDIUM)
- **Migration phase** (current state in the six-phase model per [ADR-081](https://github.com/WhalerMike/uiao/blob/main/src/uiao/canon/adr/adr-081-directory-migration-phase-canonical-model.md); valid values: `Discover`, `Normalize`, `Map`, `Migrate`, `Validate`, `Decommission`, or `N/A — net-new capability`)
- **Dependencies** (which other adapters must complete first)
- **Validation criteria** (machine-checkable success conditions)
- **Canon crosswalk** (links to relevant UIAO_xxx canon documents; the legacy MOD_xxx namespace was retired into UIAO_xxx per [ADR-060](https://github.com/WhalerMike/uiao/blob/main/src/uiao/canon/adr/adr-060-mod-namespace-flatten-into-uiao-canon.md))
...
Companion documents in the module:

- [`src/uiao/modernization/directory-migration/eight-core-concepts.md`](https://github.com/WhalerMike/uiao/blob/main/src/uiao/modernization/directory-migration/eight-core-concepts.md) — the eight universal architectural concepts grounding every adapter
- [`src/uiao/modernization/directory-migration/ad-dependency-inventory.md`](https://github.com/WhalerMike/uiao/blob/main/src/uiao/modernization/directory-migration/ad-dependency-inventory.md) — comprehensive inventory of the eleven AD-dependent object types and their migration risks
