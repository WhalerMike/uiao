...
[IMAGE-02: Windows Hello for Business enrollment screen mockup. A laptop screen rendering of the Windows 11 Settings → Accounts → Sign-in options panel, with three highlighted enrollment cards: "Face recognition (Windows Hello)" with a small camera icon, "Fingerprint recognition (Windows Hello)" with a fingerprint icon, and "PIN (Windows Hello)" with a numeric-pad icon — each card showing a teal "Set up" button. A side callout labeled "Tied to this device only" points to the PIN card; a second callout labeled "Hardware-backed by the TPM" points to the laptop chassis. Clean Windows-11 UI illustration style, dark navy (#0D1B2E) and teal (#1E8C8C) accents on white background. No photographs, no human faces, purely a UI mockup.]
...
[IMAGE-03: Three-step Microsoft Authenticator setup flow, arranged left to right. Step 1: a smartphone outline showing the Authenticator app's "+" button being tapped, label "Install & open the app". Step 2: a laptop showing a QR code on the mysignins.microsoft.com Security info page with the same smartphone scanning it via its camera, label "Scan the QR code". Step 3: the smartphone showing a single bold "Approve" button beneath a two-digit number-match prompt, label "Approve sign-in with one tap". Light teal arrows connect the three steps. Clean engineering illustration style, dark navy (#0D1B2E) and teal (#1E8C8C) on white background. No photographs, no human figures, purely diagrammatic.]
...
[IMAGE-04: Conditional Access block-page mockup. A browser window centered on a Microsoft-styled "You can't access this right now" message, with a sub-line "Your sign-in was successful, but doesn't meet the criteria to access this resource." At the bottom of the page, a "Correlation ID:" label with a sample GUID, outlined in amber and pointed to by a callout arrow labeled "Share this ID with IT — it helps them diagnose the issue quickly." Clean Microsoft 365 sign-in UI styling, dark navy (#0D1B2E) and teal (#1E8C8C) accents on white background, with amber (#D4A017) reserved for the correlation-ID highlight only. No photographs, no human figures, purely a UI illustration.]
...
[IMAGE-06: Microsoft Intune Company Portal main-screen mockup, Windows 11 styling. A windowed application with two visible tabs in the left sidebar: "Apps" (currently active, highlighted by a teal accent bar) and "Devices". The main content area under the Apps tab shows a 4×2 grid of application tiles — each tile has an app icon and a name (examples: Microsoft Teams, Adobe Acrobat, Cisco AnyConnect, Power BI, Visio, Microsoft Project, Edge for Business, Outlook). The Devices tab in the sidebar carries a small "Compliant" badge next to the device name. Clean Windows-11 UI illustration style, dark navy (#0D1B2E) and teal (#1E8C8C) accents on white background. No photographs, no human figures, purely a UI mockup.]
