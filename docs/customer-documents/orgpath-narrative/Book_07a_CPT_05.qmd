...
![One Fabric Domain per OrgPath boundary node, with workspaces and OneLake mirrors inheriting the domain that matches their segment](images/book07acpt05-diagram-01-one-fabric-domain-per-orgpath-bo.png){#fig-book-07a-cpt-05-diagram-01 fig-alt="A teal OrgPath tree shows a node flagged FabricDomainBoundary at monospace path NCR / Finance, joined by an amber connector to a federal navy Fabric Domain named NCR / Finance. A Fabric workspace box assigns into that domain, and inside it a OneLake mirrored database of an operational SQL instance inherits the domain default sensitivity context. A navy note reads \"operational SQL keeps running; the mirror makes it governable, not retired\". A small amber DRIFT-PROVENANCE marker flags the check when a workspace domain does not match its OrgPath. White background, 16:9, engineering blueprint style, monospace labels for paths and flags, no human figures." width="100%"}
