...
```{mermaid}
graph TB
    REG["Partner Registry<br/>(JSON in governance repo, PR-gated)"]
    INV["Invitation-time Azure Function<br/>(Event Grid + Managed Identity)"]
    STUB["Guest OrgPath stub<br/>/EXTERNAL/B2B/{domain} or /UNREGISTERED"]
    REG --> INV
    INV --> STUB
    subgraph "Dynamic group library (external subtree)"
        G1["ORG-EXTERNAL-B2B-UNREGISTERED"]
        G2["ORG-EXTERNAL-B2B-{partner} (Tier2)"]
        G3["ORG-EXTERNAL-B2B-{partner} (Tier1)"]
    end
    STUB --> G1
    STUB --> G2
    STUB --> G3
    subgraph "Conditional Access tiers"
        T3["Tier 3 - UNREGISTERED<br/>MFA every time + compliant device AND<br/>no persistent browser"]
        T2["Tier 2 - Standard registered<br/>MFA OR compliant device<br/>8h sign-in frequency"]
        T1["Tier 1 - High-trust attested<br/>MFA OR compliant OR hybrid-joined<br/>24h sign-in frequency"]
    end
    G1 --> T3
    G2 --> T2
    G3 --> T1
    REG -.->|"per-partner trust"| CTAS["Cross-Tenant Access Settings<br/>(MFA + device claim trust by tier)"]
```
