...
[DIAGRAM-01: Two signals — OrgPath validity and credential health — combine into a four-level risk score that drives remediation. An engineering matrix sets OrgPath status across the top in teal columns reading Valid, Missing, Invalid, Archived, and credential health down the left in navy rows reading Healthy, ExpiringSoon, Expired. Cells shade by risk: green for Low, amber for Medium and High, and red for Critical where Missing or Invalid crosses Expired. An amber arrow routes Critical cells to a box "disable the service principal and open a decommissioning inquiry" and routes High cells to a box "governance ownership triage". White background, 16:9, engineering blueprint style, monospace labels for the status and health values, red reserved for the Critical cells, no human figures.]
