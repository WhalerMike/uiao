...
![Node groups versus branch groups over an OrgTree subtree](images/book05cpt02-diagram-01-node-groups-versus-branch-groups.png){#fig-book-05-cpt-02-diagram-01 fig-alt="Engineering blueprint diagram on a neutral slate background showing how a node group and a branch group draw their membership from the same OrgTree subtree. On the left, a vertical hierarchy of four federal navy (#1F3A5F) nodes with monospace path labels: /Root/Finance at the top, branching to /Root/Finance/AccountsPayable and /Root/Finance/Payroll, with /Root/Finance/AccountsPayable/Invoicing beneath AccountsPayable. On the right, two stacked teal (#1A9E8F) group boxes. The top box labeled ORG-NODE-FINANCE with rule device.OrgPath -eq /Root/Finance draws a single solid connector to ONLY the /Root/Finance node. The bottom box labeled ORG-BRANCH-FINANCE with rule device.OrgPath -startsWith /Root/Finance/ draws solid connectors to all four nodes and encloses the entire subtree in a dashed teal boundary. An amber (#D4A017) marker notes that the trailing slash is mandatory. Monospace labels for all paths and group names, no human figures, 16:9 landscape." width="85%"}
