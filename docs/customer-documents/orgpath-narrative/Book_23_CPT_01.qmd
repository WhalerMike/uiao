...
**ADR-NNN** refers to an Architecture Decision Record: UIAO's numbered, binding governance specification. Each ADR governs a specific aspect of the platform. This book cites ADR-102 (LocPath Physical Location Codebook), ADR-104 (E911 Compliance Layer over LocPath), ADR-092 (Active Governance), ADR-062 (OrgPath Leaf Naming), ADR-040 (Drift Engine), and ADR-035 (OrgPath Codebook). Full titles and one-paragraph descriptions for each cited ADR appear in the **Glossary and References** appendix at the end of this document.

**UIAO_NNN** refers to a UIAO Specification document — an operational schema, field mapping, or scanner interface specification. UIAO_194 (cited in this book) is the LocPath Codebook specification defining the physical-location record shape, civic address fields, and sub-address precision requirements for E911 compliance.

**GOV-LOCPATH-NNN** is a LocPath drift finding code identifying a specific E911 governance gap. Examples: `GOV-LOCPATH-012` (dispatchable location missing — P1) and `GOV-LOCPATH-013` (PSAP routing gap — P2). A complete table of finding codes appears in the Glossary.

**LocPath** is UIAO's physical-location addressing dimension — a hierarchy of Site, Building, Floor, and Space nodes carrying civic address fields and sub-address precision data. LocPath is the governance substrate that makes E911 dispatchable location a managed obligation rather than a best-effort PBX configuration record.

**Dispatchable Location** is the legally required output of the E911 compliance obligation under 47 CFR Part 9 §9.16: a civic address plus sufficient sub-address detail — floor, suite, room — to allow first responders to locate a 911 caller without additional assistance.

**PSAP** (Public Safety Answering Point) is the emergency communications center that receives 911 calls and dispatches first responders. The PSAP database maps civic addresses and sub-address records to dispatch jurisdictions; a LocPath node's civic address must be validated against the PSAP database that serves its geographic location.

**OrgPath** is UIAO's organizational placement model — a colon-separated namespace path that anchors every identity and location object to its position in the federal bureau hierarchy (e.g., `DOJ:BOP:FCI-Cumberland` for the Federal Correctional Institution at Cumberland within the Bureau of Prisons).
