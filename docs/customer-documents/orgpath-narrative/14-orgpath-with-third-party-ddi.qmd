...
```{mermaid}
graph TB
    REG["OrgTree Registry<br/>(DnsBoundary, NetworkCidr, MigrationPhase)"]
    PIPE["Governance pipeline<br/>(orchestrator, per-platform adapters)"]
    REG --> PIPE
    subgraph "Third-party DDI (unified hybrid management plane)"
        NIOS["Infoblox NIOS Grid<br/>Extensible Attributes<br/>(WAPI, Basic auth)"]
        BLOX["Infoblox BloxOne DDI<br/>tags<br/>(CSP API, Bearer token)"]
        BAM["BlueCat Address Manager<br/>User-Defined Fields<br/>(REST v1, session token)"]
    end
    subgraph "Azure-native (target state)"
        AZDNS["Azure DNS Private Zones<br/>+ Private Resolver"]
    end
    PIPE --> NIOS
    PIPE --> BLOX
    PIPE --> BAM
    PIPE --> AZDNS
    subgraph "MigrationPhase state machine (per zone)"
        P1["Assessment<br/>(DDI tagged, no Azure zone)"]
        P2["Migration<br/>(Azure zone created, idle)"]
        P3["Parallel<br/>(both serving live traffic)"]
        P4["Complete<br/>(Azure sole authority, DDI retired)"]
    end
    P1 --> P2 --> P3 --> P4
    P3 -.->|"webhook on record change<br/>(NIOS Outbound API / BAM events)"| AZDNS
```
