...
[DIAGRAM-01: Reconstructing OU-style policy inheritance with a prefix-matched dynamic group library. Left side shows the Active Directory model — a GPO linked to a division OU cascading automatically to every child OU and object beneath it. Right side shows the cloud equivalent — a position attribute (e.g. /Org/Finance/FieldServices/RegionalWest) on each identity, and a dynamic group whose rule is "position startsWith /Org/Finance" automatically capturing every user and device at or below that node; an Intune / Conditional Access / Azure Policy assignment targets that one group. A note reads "add a team under Finance → its members match the prefix automatically → coverage updates with no membership edit". The group library is shown maintained by the registry (new node → new group). Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for the AD OU tree, teal (#1A9E8F) for the dynamic group library, amber (#D4A017) for the registry-driven maintenance, monospace path strings, no human figures, 16:9 landscape.]
