...
```{mermaid}
graph LR
    OPATH["OrgPath substrate<br/>users, devices, servers"]
    subgraph "Four operational integrations"
        PIM["PIM eligibility groups<br/>(ORG-BRANCH-{id} per node)"]
        MOVE["Lifecycle Workflow mover<br/>OrgPath change to realign + review"]
        SENT["Sentinel KQL queries<br/>activations + drift by segment"]
        RBAC["Azure RBAC via tag-based assignments<br/>(resource plane)"]
    end
    OPATH --> PIM
    OPATH --> MOVE
    OPATH --> SENT
    OPATH --> RBAC
    AUDIT["Versioned governance record<br/>audit + compliance evidence"]
    PIM --> AUDIT
    MOVE --> AUDIT
    SENT --> AUDIT
    RBAC --> AUDIT
