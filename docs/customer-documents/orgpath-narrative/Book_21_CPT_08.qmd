...
::: {.callout-note}
Federal identity governance has a well-understood model for human identities: someone joins the agency, moves between roles, and eventually leaves. Every step triggers a defined set of credential operations. Federal AI systems need exactly the same model — but driven by entirely different sources. The hire event is an ATO grant. The job change is a re-authorization or bureau transfer. The resignation is a retirement entry in the OMB inventory. This chapter shows how that model works, why the annual OMB inventory vintage cycle is the engine that drives it, and how credential lifecycle automation — today via `Invoke-AIInventoryScan.ps1` and in the target architecture via the SailPoint machine-identity adapter — makes compliance durable rather than dependent on institutional memory.
:::
...
::: {.callout-note}
The SailPoint machine-identity adapter described in this section is a **target architecture** — it represents the full-automation end-state that the governance model is designed to reach. The adapter slot is reserved in the UIAO adapter registry (phase-planning) pending the Option-B boundary-expansion decision in ADR-059. The `ai-inventory-governance` scanner and `Invoke-AIInventoryScan.ps1` are available today and produce the same OrgPath records and drift findings; the SailPoint integration adds automated credential lifecycle management for agencies that reach phase-3 adoption.
:::
...
[Chapter 8a — The Centralized Deployment Model: CDM for the AI Identity Surface →](Book_21_CPT_08a.qmd)
:::
