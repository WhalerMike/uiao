...
::: {.callout-note}
## In this chapter
PIM activation logs annotated only by identity, role, and time are structurally
incomplete — Sentinel cannot tell a routine Tier0 activation from the same role
activated by someone in Finance. This chapter shows how making OrgPath a
queryable Sentinel dimension changes that, the audit-log and sign-in-log data
paths that carry OrgPath into the log model, and three production KQL queries —
a daily organizational dashboard, a sensitive-segment alert rule, and an
eligibility-versus-position drift detector — that together form a layered
operational model.
:::
...
> Without OrgPath, Sentinel cannot distinguish between these two events. They are both activations of the same role, at the same time of day, with similarly worded justification strings — the detection logic has nothing to work with except the identity and the role.
...
The three queries form a layered operational model, each serving a distinct consumer, cadence, and routing destination:

| Query | Consumer | Output | Routes to |
| :--- | :--- | :--- | :--- |
| **Query One** — Activations by OrgPath | Governance team | Workbook tile (no incidents) | No queue — daily review |
| **Query Two** — Sensitive-segment activations | Security operations | High-severity incident | Security operations queue |
| **Query Three** — Eligibility-vs-position drift | Governance team | Governance finding above threshold | Identity governance queue |

Query One is the governance team's daily situational awareness tool: it shows where in the organizational hierarchy privileged access is being activated, at what volume, and by how many unique identities. It is not an alert query — it produces no incidents and routes to no queue. It is a workbook tile that a governance manager reviews each morning to understand the privileged access activity of the past day in organizational terms.
