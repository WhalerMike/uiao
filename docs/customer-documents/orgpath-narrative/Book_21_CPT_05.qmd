...
This is the identity governance implication of an ATO gap. M-25-21 does not itself impose an AI-specific ATO gate — it directs agencies to keep applying their existing authorization requirements — but those requirements bite here precisely because the authorization boundary is what legitimizes credential issuance. Without it, the credential has no defined scope. The system accesses whatever it can reach, authorized only by whatever the provisioning administrator happened to configure at deployment time.

## The 596 gap: how the scanner finds it

The UIAO AI identity governance scanner (UIAO_196) reads the OMB inventory against the ATO enumeration scope defined in ADR-054 and extended to the AI surface by ADR-112 §2. The `have_ato` field is the primary signal; `system_name_ato` is the join key that links an inventory entry to its ATO record in the agency's authorization system.
...
This posture reflects the agency's own authorization requirement directly: ATO before operational deployment, not ATO before the next available audit cycle. M-25-21's contribution is the inventory that makes the gap enumerable, not a gate of its own. The finding's creation date records when the gap was detected; if a system was deployed without an ATO, the finding is backdated to the deployment date for remediation timeline reporting. Agencies with large numbers of ATO gap findings can see exactly how long each ungoverned system has been operating.
