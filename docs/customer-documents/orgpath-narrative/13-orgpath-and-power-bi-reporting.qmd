...
```{mermaid}
graph LR
    subgraph "Six governance source surfaces"
        S1["OrgTree registry JSON<br/>(authoritative dimension)"]
        S2["Intune Graph API<br/>device compliance"]
        S3["Microsoft Sentinel<br/>drift / PIM / alerts / DLP"]
        S4["Defender for Cloud<br/>+ Resource Graph"]
        S5["Azure Policy Guest Config<br/>compliance state"]
        S6["Entra ID Graph<br/>B2B guest accounts"]
    end
    PIPE["Export orchestrator<br/>(PowerShell, daily, cert auth)"]
    STAGE["Staging directory<br/>(timestamped + -latest JSON files)"]
    GW["Power BI Gateway"]
    MODEL["Semantic Model<br/>(7 tables, OrgTree dimension as spine)"]
    DAX["DAX measure library<br/>(KPIs + RAG status)"]
    subgraph "Report pages"
        R1["Per-division Governance Posture"]
        R2["Executive Summary (RAG)"]
        R3["Trend Analysis"]
    end
    S1 --> PIPE
    S2 --> PIPE
