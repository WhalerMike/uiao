...
![Two axes woven into one fabric — and the threads that do not belong are trimmed at the edge](images/book19cpt10-image-01-the-woven-fabric.png){#fig-book-19-cpt-10-image-01 fig-alt="Illustrative 16:9 scene on a white background. A loom-like weave fills the center: six vertical navy warp threads descend from a navy top beam labeled OrgPath, and five horizontal teal weft threads run rightward from a teal left beam labeled LocPath. The threads interlace over and under in an alternating weave, and a soft pale-teal glow radiates behind the densest part of the fabric. At the fabric right edge three loose grey threads hang outside the weave, drooping down and each ending in a clean flat cut marked by a small slanted tick; tiny monospace labels beside the cut ends read IWMS, network, and 911. A pair of stylized grey shears with two ring handles points its open blades at the middle cut thread, showing that strands that do not belong to the two governed axes are trimmed at the edge." width="100%"}
