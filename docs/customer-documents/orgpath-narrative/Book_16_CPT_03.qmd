...
[DIAGRAM-03: Engineering blueprint, white background, 16:9 landscape. Left panel labeled REJECTED — two authorities shows two federal-navy server boxes side by side, each accepting a commit arrow, their histories drifting apart into a red fork labeled divergent truth. Right panel labeled CANON — one authority plus standby shows a single federal-navy server box labeled AUTHORITY, the one that says yes, with all commit arrows converging on it, plus a standby box connected by a dashed amber arrow labeled automatic quorum-gated failover, never active-active. A small amber witness box sits above both, labeled quorum witness, only the quorum holder may bind commits, and a tiny amber tag on the standby path reads demoted node fenced. Below, a teal band labeled scale lives here points to a separate cluster of small boxes labeled TARGET SURFACES — Entra, Intune, Azure Arc, fan out freely. Federal navy #1F3A5F for authority, teal #1A9E8F for the target-surface band, amber #D4A017 for the failover path and witness, red #C0392B only on the rejected fork, monospace labels, no human figures.]
