...
![OrgPath-aware incident response playbook flow](images/book06cpt07-diagram-01-orgpath-aware-incident-response.png){#fig-book-06-cpt-07-diagram-01 fig-alt="Engineering blueprint flow diagram on a white background. Far left, a federal navy (#1F3A5F) trigger box \"Sentinel incident created\". An arrow leads to a navy box \"Playbook reads affected identity, retrieves OrgPath via Microsoft Graph\". An arrow leads to a teal (#1A9E8F) decision box \"Match OrgPath against response-matrix.json (most-specific prefix first; loaded from governance repo)\". Three teal branch boxes fan right by tier: \"NCR / IT / InfraOps → auto-isolate, revoke PIM, P1, SEC-TIER0-CRITICAL\", \"NCR / Finance → analyst-approval isolation, P2, SEC-FINANCE-STANDARD\", and \"no path (default) → no auto-isolate, P3, SEC-STANDARD\". Each branch emits an amber (#D4A017) action cluster \"annotate incident, route ITSM queue, notify escalation group, trigger access review\". A red (#C0392B) accent marks the Tier 0 auto-isolation action. Monospace labels for paths, queues, and the matrix filename, no human figures, 16:9 landscape." width="100%"}
