...
```{mermaid}
graph TB
    subgraph "Tier0 - Substrate (DCs, KMS, PKI, Entra Connect)"
        T0A["BitLocker: TPM + PIN"]
        T0B["WDAC: enforcement mode"]
        T0C["RDP off, WinRM only"]
        T0D["Credential Guard on"]
        T0E["Audit: maximum verbosity"]
        T0F["LAPS: 8 hour rotation"]
        T0G["SMBv1 / LLMNR / NetBIOS off"]
        T0H["PS Constrained Language Mode"]
        T0I["OrgPath integrity check: 6h"]
    end
    subgraph "Tier1 - Production workloads"
        T1A["BitLocker: TPM only"]
        T1B["RDP restricted to admin group"]
        T1C["Audit: elevated (logon, account mgmt)"]
        T1D["LAPS: 24 hour rotation"]
        T1E["SMBv1 / LLMNR off"]
        T1F["OrgPath integrity check: 24h"]
    end
    subgraph "Tier2 - Standard organizational"
        T2A["BitLocker: TPM only"]
        T2B["Defender real-time on"]
        T2C["Audit: standard subcategories"]
        T2D["LAPS: 72 hour rotation"]
        T2E["SMBv1 off"]
        T2F["OrgPath integrity check: 72h"]
    end
```
...
```{mermaid}
graph LR
    DSC["PowerShell DSC config<br/>OrgPath_TierN_Baseline.ps1"]
    MOF["Compiled MOF<br/>declarative state document"]
    PKG["Guest Configuration package<br/>.zip archive"]
    BLOB["Azure Blob Storage<br/>stable URL + SHA256 hash"]
    POL["Azure Policy definition<br/>references URL + hash"]
    ASSIGN["Policy Assignment<br/>scoped by OrgTier tag"]
    EXT["Arc Guest Configuration extension<br/>on each Arc-enrolled machine"]
    STATE["Compliance state<br/>Compliant / NonCompliant / Pending"]
    DASH["Azure Policy compliance<br/>+ Resource Graph + Sentinel"]
    DSC -->|"Configuration cmdlet"| MOF
    MOF -->|"New-GuestConfigurationPackage"| PKG
    PKG -->|"upload"| BLOB
    BLOB -->|"referenced by"| POL
    POL -->|"assigned at MG scope"| ASSIGN
    ASSIGN -->|"OrgTier match condition"| EXT
    EXT -->|"applies DSC + reports back"| STATE
    STATE --> DASH
```
