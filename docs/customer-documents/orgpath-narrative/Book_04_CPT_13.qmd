...
![The infrastructure dependency map — initialization, record schema, and resolution lifecycle](images/book04cpt13-diagram-01-the-infrastructure-dependency-ma.png){#fig-book-04-cpt-13-diagram-01 fig-alt="Left \"Initialization\": three assessment sources merge into the map — Azure Migrate (network-flow dependencies), BloodHound/SharpHound (identity and ACL relationships), and the AD service-account inventory (identity-to-service mapping). Center \"Dependency record\": the six-field schema for one record (id e.g. INFRA-COMM-001, provider OrgPath, consumer OrgPath, dependencyType [Protocol/Application/Infrastructure/Identity" width="85%"}, mechanism [Kerberos/LDAP/NTLM/DNS/…], status). Right "Lifecycle": a three-state track Active → InRemediation → Resolved, with the Resolved state adding resolvedDate, resolvedBy, and migrationTarget. A separate dashed arrow labeled "weekly discovery" loops new Azure Migrate flows back into the map as draft Active records. Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for the record schema, teal (#1A9E8F) for the three sources, amber (#D4A017) for the lifecycle states, monospace field names and ids, no human figures, 16:9 landscape.]
...
The six mandatory fields of each dependency record are summarized below.

| Field | Meaning |
|-------|---------|
| `id` | Unique record id — provider id + consumer id + sequence number (e.g. `INFRA-COMM-001`) |
| `provider` | OrgPath of the unit that owns the depended-upon infrastructure component |
| `consumer` | OrgPath of the unit that owns the consuming identity, application, or service |
| `dependencyType` | Protocol, Application, Infrastructure, or Identity |
| `mechanism` | Kerberos, LDAP, NTLM, CertificateEnrollment, DNS, DatabaseConnection, WebServiceCall, NFS, SMB, RPC, or ServiceAccountCredential |
| `status` | Active, InRemediation, or Resolved (Resolved adds `resolvedDate`, `resolvedBy`, `migrationTarget`) |
