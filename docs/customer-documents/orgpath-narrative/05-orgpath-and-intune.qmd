...
```{mermaid}
graph LR
    REG["OrgTree Registry<br/>(Git, versioned)"]
    PIPE["ETL Pipeline<br/>(derives + stamps)"]
    ATTR["Device OrgPath attribute<br/>(Entra extension schema)"]
    REG --> PIPE
    PIPE --> ATTR
    subgraph "Dynamic group library (per OrgTree node)"
        NG["ORG-NODE-{id}<br/>OrgPath -eq prefix"]
        BG["ORG-BRANCH-{id}<br/>OrgPath -startsWith prefix"]
    end
    ATTR --> NG
    ATTR --> BG
    subgraph "Intune policy surfaces (all group-targeted)"
        CONF["Configuration profiles"]
        COMP["Compliance policies"]
        ESEC["Endpoint Security<br/>(AV, FW, ASR, EDR)"]
        UPD["Update rings"]
        APP["App assignments"]
    end
    BG --> CONF
    ...
    BG --> UPD
    BG --> APP
    NG -.->|"node-only exceptions"| CONF
    CA["Conditional Access<br/>(compliance signal)"]
    COMP --> CA
```
