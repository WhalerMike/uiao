...
```{mermaid}
graph TB
    subgraph "On-prem (must be decommissioned cleanly)"
        ADDNS["AD-integrated DNS<br/>SRV records in directory partition"]
        ADCS["AD Certificate Services<br/>templates in Configuration NC"]
        IPAM["Windows Server IPAM role<br/>(transition inventory baseline)"]
    end
    subgraph "Azure-native (target state)"
        AZDNS["Azure DNS Private Zones<br/>tagged OrgPath / OrgNodeId / OrgTier / MigrationPhase"]
        RESOLVE["Azure DNS Private Resolver<br/>hybrid forwarding ruleset"]
        PKI["Microsoft Cloud PKI<br/>SCEP, PKCS, Trusted Root profiles"]
        INT["Intune certificate deployment<br/>strong-mapping per KB5014754"]
    end
    REG["OrgTree Registry<br/>(DnsBoundary, CertificateProfiles, MigrationPhase)"]
    REG --> AZDNS
    REG --> PKI
    ADDNS -->|"zone-by-zone migration"| AZDNS
    IPAM -.->|"baseline inventory"| AZDNS
    ADCS -->|"template to profile mapping"| PKI
    PKI --> INT
    AZDNS --> RESOLVE
    POL["Azure Policy + drift detection<br/>tag compliance + cert-OrgPath alignment"]
    AZDNS --> POL
    PKI --> POL
    SCORE["Decommissioning Readiness Score<br/>(five-dimension quantified gate)"]
    POL --> SCORE
```
