...
graph LR
    ...
    ATTR["OrgPath<br/>on users, devices, Arc tags"]
    REG --> ATTR
    subgraph "Defender suite (siloed signals without OrgPath)"
        MDE["Defender for Endpoint<br/>device groups + RBAC scope"]
        MDI["Defender for Identity<br/>entity profiles + Sentinel rules"]
        MDA["Defender for Cloud Apps<br/>session controls + anomaly"]
        MDC["Defender for Cloud + Servers<br/>Azure Policy by OrgTier tag"]
    end
    XDR["Defender XDR + Microsoft Sentinel<br/>(cross-product correlation)"]
    PLAY["Logic App playbooks<br/>OrgPath-aware response matrix"]
    ATTR -->|"dynamic group membership"| MDE
    ATTR -->|"identity org context"| MDI
    ATTR -->|"per-segment baselines"| MDA
    ATTR -->|"resource tag targeting"| MDC
    MDE --> XDR
    MDI --> XDR
    MDA --> XDR
    MDC --> XDR
