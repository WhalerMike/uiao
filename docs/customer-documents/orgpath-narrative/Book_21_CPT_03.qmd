...
::: {.callout-note}
## OrgPath governs the AI system with or without SailPoint

It is worth being precise about what depends on what. **OrgPath governs an AI
system on its own** — the `agency_bureau` → OrgPath leaf placement, the owner
anchor, the lifecycle, and the drift monitoring all run from the core substrate
and the Microsoft/Entra estate, with no SailPoint involvement. SailPoint NERM
(ADR-059) is an **optional add-on that deepens one branch only**: the
non-employee / vendor / partner relationship behind `vendor_name`, which
Microsoft has no FedRAMP-Moderate product to govern. Turn NERM off and the AI
system is still fully governed as an identity subject; you lose only the deep
non-employee lifecycle behind the vendor. The two are complementary —
`agency_bureau → OrgPath` places the system; `vendor_name → NERM` governs the
people behind it — not a single coupled dependency.
:::
