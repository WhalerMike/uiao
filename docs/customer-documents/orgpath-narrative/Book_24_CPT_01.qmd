...
**ADR-NNN** refers to an Architecture Decision Record: UIAO's numbered, binding governance specification. Each ADR governs a specific aspect of the platform. This book cites ADR-043 (FedRAMP Conformance Adapter), ADR-054 (Single-ATO Reciprocity), ADR-092 (Active Governance), ADR-006 (Evidence Fabric), ADR-016 (Evidence Bundle Lifecycle), ADR-040 (Drift Engine), and ADR-106 (FedRAMP 20x). Full titles and one-paragraph descriptions for each cited ADR appear in the **Glossary and References** appendix at the end of this document.

**KSI (Key Security Indicator)** is a specific, machine-readable security outcome measurement continuously emitted by a cloud service provider under the FedRAMP 20x model — not a narrative of intent, but a current-state measurement formatted in OSCAL and consumed directly by authorizing officials.

**cATO (Continuous Authorization to Operate)** is the authorization model introduced by FedRAMP 20x in which authorization status is a live function of the KSI stream rather than a point-in-time letter. A cATO is current as long as all required KSIs pass; it narrows automatically when a KSI fails.

**OSCAL (Open Security Controls Assessment Language)** is a NIST-developed, machine-readable language for representing security controls, assessment plans, and results. FedRAMP 20x requires KSI evidence bundles to be structured in OSCAL so they can be consumed programmatically by agency authorizing officials and automated compliance pipelines.

**FedRAMP 20x pathway** refers to the authorization track a cloud service provider follows: *Pathway 1 (Modernized)* relies on continuous KSI emission and automated evidence review; *Pathway 2 (Traditional)* retains the 3PAO-led assessment model. UIAO's Evidence Fabric supports Pathway 1 by providing OrgPath-resolved KSI assertions as OSCAL evidence bundles.

**DRIFT-CLASS::subtype** is a drift finding code identifying the governance-gap category and specific condition. Examples: `DRIFT-COMPLIANCE::ai-ato-gap` (P2 — system deployed without a current ATO) and `DRIFT-COMPLIANCE::ato-reciprocity-gap` (P2 — authorized system not yet recognized reciprocally by a consuming agency). A complete table of drift codes appears in the Glossary.

**OrgPath** is UIAO's organizational placement model — a colon-separated namespace path that anchors every identity object (human, device, or cloud system) to its position in the federal bureau hierarchy. OrgPath is what makes KSI assertions precise: instead of stating "MFA coverage is 99% organization-wide," an OrgPath-resolved KSI states "MFA coverage is 100% for `DOD:DISA::cloud-identity-service` privileged accounts as of the stated timestamp."
