...
repo-path: docs/customer-documents/uiao-orgcomp-integration/10-scuba-technical-spec.qmd
...
aliases:
  - /customer-documents/uiao-aan-integration/10-scuba-technical-spec.html
  - /customer-documents/compliance/evidence-telemetry/scuba-technical-spec.html
family: uiao-orgcomp-integration

---

![Three-hop mapping chain (PolicyId to NIST to KSI)](images/scuba-technical-spec-image-03-three-hop-mapping-chain-policyid.png){#fig-scuba-technical-spec-image-03 fig-alt="Three-hop mapping chain (PolicyId to NIST to KSI). Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for primary structural elements, teal (#1A9E8F) for secondary/integration elements, amber (#D4A017) for governance/audit/registry overlays, monospace labels for identifiers, no human figures, 16:9 landscape." width="100%"}
...
![A 16:9 muted-blue schematic showing the 4-plane SCuBA compliance pipeline.](images/scuba-technical-spec-image-01-a-169-muted-blue-schematic-showi.png){#fig-scuba-technical-spec-image-01 fig-alt="A 16:9 muted-blue schematic showing the 4-plane SCuBA compliance pipeline: input normalization, IR transform, KSI evaluation, and OSCAL artifact generation, framed by the UIAO Governance OS boundary." width="100%"}
...
![A 16:9 muted-blue architectural diagram showing the UIAO Governance OS boundary containing four sequential planes.](images/scuba-technical-spec-diagram-01-a-169-muted-blue-architectural-d.png){#fig-scuba-technical-spec-diagram-01 fig-alt="A 16:9 muted-blue architectural diagram showing the UIAO Governance OS boundary enclosing the four sequential SCuBA pipeline planes (input normalization, IR transform, KSI evaluation, evidence bundle, OSCAL artifacts), with the certificate-anchored provenance chain depicted across the bottom." width="100%"}
...
![A 16:9 muted-blue sequence diagram showing the Orchestrator invoking each plane with retry and auto-normalization.](images/scuba-technical-spec-diagram-02-a-169-muted-blue-sequence-diagra.png){#fig-scuba-technical-spec-diagram-02 fig-alt="A 16:9 muted-blue sequence diagram showing the Orchestrator invoking Plane 1 through Plane 4 in sequence, with retry loops, exponential backoff, auto-normalization of raw ScubaGear input, and a run manifest aggregating PlaneResult records at the end." width="100%"}
...
This section is retained per UIAO governance mandate. All content in this document is derived from canonical artifacts in the UIAO repository. Provenance traces to: src/uiao/adapters/scuba/ (normalizer, transformer), src/uiao/ksi/evaluate.py (evaluator), src/uiao/evidence/builder.py (evidence builder), src/uiao/generators/ (OSCAL, POA&M, SSP), orchestrator/orchestrator.py (orchestration layer), src/uiao/adapters/scubagear_adapter.py (policy mapping), and rules/ksi/ (KSI rule definitions).

**Appendix D: References**

All references are user-provided or derived from the UIAO codebase:
...
**REF-05:** UIAO repository --- Canonical governance framework source
...
\[\^2\]: The KSI count reflects the rules/ksi/ directory (191 as of July 2026, tracked by the regen-gated index.yaml; 163 at the April 2026 measurement). This number changes as the UIAO governance framework evolves.
...
All sections validated against UIAO source artifacts.
