...
When both events complete, every NAC-admitted device has **name, address, and key bound together** — the same compound guarantee that [Vol I Book 01](Vol_I_Book_01_FedAAN_SSA_Landing_Zone_IPAM_FedRAMP.qmd) achieves for cloud workloads at provisioning time, extended to the endpoint population. The machine certificate architecture is generalized in [Vol I Book 03](Vol_I_Book_03_FedAAN_Certificates_Tokens_Cryptographic_Identity.qmd). The design decision for the hostname-to-cert binding must be validated with OIS.
:::
