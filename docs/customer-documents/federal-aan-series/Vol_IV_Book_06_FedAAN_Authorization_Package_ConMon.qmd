...
The UIAO Conformance Adapter evaluates all 29 internal KSI rules against the
...
- [ ] Run `uiao oscal bundle --system-name "{{< meta agency.short >}} GCC Moderate"` and confirm 29/29 self-assessed satisfied against the series' internal KSI decomposition (not the CR26 catalog)
...
|---------|-------|-------------------|
| **CA-2** | Control Assessments | OSCAL AR (`ksi-ar.json`) is the formal security assessment record for all 29 internal KSI-decomposition rules; generated by `uiao oscal bundle` on a monthly cadence |
...
| **SA-15** | Development Process, Standards, and Tools | The slot evaluator, series origin, and diagram pipeline are documented design decisions of record covering the development process used to build the evidence pipeline; the `uiao` CI/CD gate (ruff, mypy, pytest 5109 tests) is the SA-15 toolchain evidence |
