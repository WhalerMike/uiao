---
title: "GPO Sunset Program"
subtitle: "Existing-fleet GPO replacement — 7-phase migration to Intune (per ADR-080)"
...
repo-path: docs/customer-documents/operational-guides/target-surface/intune-policy-templates.qmd
...
![Intune policy-family architecture (Compliance/Configuration/Endpoint Security/Ap...](images/intune-policy-templates-image-01-intune-policy-family-architectur.png){#fig-intune-policy-templates-image-01 fig-alt="Intune policy-family architecture (Compliance/Configuration/Endpoint Security/App Protection). Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for primary structural elements, teal (#1A9E8F) for secondary/integration elements, amber (#D4A017) for governance/audit/registry overlays, monospace labels for identifiers, no human figures, 16:9 landscape." width="100%"}

![OrgPath-driven dynamic-group targeting model](images/intune-policy-templates-image-02-orgpath-driven-dynamic-group-tar.png){#fig-intune-policy-templates-image-02 fig-alt="OrgPath-driven dynamic-group targeting model. Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for primary structural elements, teal (#1A9E8F) for secondary/integration elements, amber (#D4A017) for governance/audit/registry overlays, monospace labels for identifiers, no human figures, 16:9 landscape." width="100%"}

![GPO-to-Intune deployment-and-coexistence sequence](images/intune-policy-templates-image-03-gpo-to-intune-deployment-and-coe.png){#fig-intune-policy-templates-image-03 fig-alt="GPO-to-Intune deployment-and-coexistence sequence. Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for primary structural elements, teal (#1A9E8F) for secondary/integration elements, amber (#D4A017) for governance/audit/registry overlays, monospace labels for identifiers, no human figures, 16:9 landscape." width="100%"}


::: {.callout-note}
## About this program

**GPO Sunset Program** is one of three distinct programs in the UIAO governance corpus that touch Intune. Per [ADR-080](../../../adr/adr-080-intune-first-scope-disambiguation.qmd) (Intune-First Scope Disambiguation), do not conflate them:

- **[Intune-First Asset Onboarding](../../../customer-documents/reference-architecture/intune-first.qmd)** — the canon doctrine for **net-new devices** (procurement-time governance, 5-phase Procure→Validate). Not this page.
- **GPO Sunset Program** *(this page)* — the **existing-fleet** program. 7-phase replacement of GPOs on currently-deployed endpoints, ending in formal GPO retirement. ~34-week timeline.
- **[Policy Transformation Pattern](../client-server-to-hybrid-cloud/05-policy-transformation.qmd)** ("Regenerate, Not Copy") — the **philosophy** governing how this program authors replacement policies. Not a separate program.

The 50-row GPO→Intune mapping later in this document is a **Semantic Translation Reference** — engineers' lookup card for "what's the closest Intune equivalent for this GPO setting." It is *not* a literal 1:1 copy prescription. The "regenerate, not copy" philosophy applies to policy authoring decisions; this reference applies to setting-level translation.
:::

GPO Sunset Program

Existing-fleet GPO Replacement --- 7-Phase Migration to Intune (per ADR-080)
...
7.  GPO→Intune Semantic Translation Reference
...
All policies in this document are targeted via dynamic Azure AD (Entra ID) groups built from the OrgPath extension attribute schema. This eliminates OU-based GPO targeting and replaces it with attribute-based, cloud-native group membership:
  ...
  **Extension Attribute**   **Purpose**             **Example Values**
  ...
  (not a facet)             Site / Location         LocPath axis - see ADR-102 D5      
...
- **Filters:** Intune assignment filters provide an additional layer of targeting (e.g., *device.extensionAttribute11 -eq \"Tier0\"*) applied at the assignment level, not the group level.
...
Intune filters allow granular targeting at the assignment level. Filters are evaluated after group membership and can include or exclude devices from a policy assignment:
  ...
  -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  // Example: Include only Tier 0 devices (device.extensionAttribute11 -eq \"Tier0\") // Example: Exclude Dev environment devices (device.extensionAttribute13 -ne \"Dev\") // Example: Target Kiosk devices in East region (device.extensionAttribute12 -eq \"Kiosk\") -and (device.extensionAttribute1 -eq \"EASTUS\")
...
**OrgPath Targeting:** Dynamic group --- device.extensionAttribute11 -eq \"Tier0\"
...
**OrgPath Targeting:** Dynamic group --- device.extensionAttribute11 -eq \"Tier2\"
...
**OrgPath Targeting:** Dynamic group --- device.extensionAttribute11 -eq \"Tier1\"
...
**OrgPath Targeting:** Dynamic group --- device.extensionAttribute12 -eq \"Kiosk\"
...
**Policy ID: ES-006 \| Blade: Endpoint Security \> Account protection \| Target: Varies by tier**
  ...
  **Setting**                            **Value**                **Target**
  -------------------------------------- ------------------------ ----------------------------------------------------------------
  Credential Guard                       Enabled with UEFI lock   Tier 0 devices only (device.extensionAttribute11 -eq \"Tier0\")
...
::: {.callout-important}
## Reference, not prescription (per ADR-080)

The table below is a **Semantic Translation Reference** — engineers' lookup card for "what is the closest Intune surface for this GPO setting?" Per [ADR-080](../../../adr/adr-080-intune-first-scope-disambiguation.qmd), it is **not** a literal 1:1 copy prescription. Use it to find the right Intune target surface for a given GPO setting; do not use it as authority for what the policy *value* should be in the new system.

The "**regenerate, not copy**" philosophy — defined in [Client-Server-to-Hybrid-Cloud chapter 05](../client-server-to-hybrid-cloud/05-policy-transformation.qmd) — governs the policy-authoring decision. Replacement policies are authored from current policy intent (compliance baseline, persona model, Conditional Access strategy, OrgPath-scoped dynamic group targeting), with this reference as one input among several. Faithfully porting old GPO conflict-resolution semantics or OU-filter targeting into Intune reproduces ancient drift in a new surface; the table tells you where to land, not what to land with.
:::

The following table maps the top 50 most commonly deployed GPO settings in enterprise environments to their nearest Intune surface, including the UIAO policy ID where the setting is configured.
...
Tier 1 Servers
  ...
  -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  New-MgGroup -DisplayName \"UIAO-Devices-Tier1\" \` -Description \"Tier 1 Servers (Azure Arc enrolled)\" \` -MailEnabled:\$false \` -MailNickname \"uiao-devices-tier1\" \` -SecurityEnabled:\$true \` -GroupTypes @(\"DynamicMembership\") \` -MembershipRule \'(device.extensionAttribute11 -eq \"Tier1\")\' \` -MembershipRuleProcessingState \"On\"
...
Tier 2 Standard Workstations
  ...
  ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  New-MgGroup -DisplayName \"UIAO-Devices-Tier2\" \` -Description \"Tier 2 Standard End-User Workstations\" \` -MailEnabled:\$false \` -MailNickname \"uiao-devices-tier2\" \` -SecurityEnabled:\$true \` -GroupTypes @(\"DynamicMembership\") \` -MembershipRule \'(device.extensionAttribute11 -eq \"Tier2\")\' \` -MembershipRuleProcessingState \"On\"
...
8.2 Environment Groups
  ...
  -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  \# Production Devices New-MgGroup -DisplayName \"UIAO-Devices-Production\" \` -Description \"Production environment devices\" \` -MailEnabled:\$false -MailNickname \"uiao-devices-prod\" \` -SecurityEnabled:\$true -GroupTypes @(\"DynamicMembership\") \` -MembershipRule \'(device.extensionAttribute13 -eq \"Production\")\' \` -MembershipRuleProcessingState \"On\" \# Staging Devices New-MgGroup -DisplayName \"UIAO-Devices-Staging\" \` -Description \"Staging environment devices\" \` -MailEnabled:\$false -MailNickname \"uiao-devices-staging\" \` -SecurityEnabled:\$true -GroupTypes @(\"DynamicMembership\") \` -MembershipRule \'(device.extensionAttribute13 -eq \"Staging\")\' \` -MembershipRuleProcessingState \"On\" \# Dev Devices New-MgGroup -DisplayName \"UIAO-Devices-Dev\" \` -Description \"Development environment devices\" \` -MailEnabled:\$false -MailNickname \"uiao-devices-dev\" \` -SecurityEnabled:\$true -GroupTypes @(\"DynamicMembership\") \` -MembershipRule \'(device.extensionAttribute13 -eq \"Dev\")\' \` -MembershipRuleProcessingState \"On\"
...
8.3 Role-Based Groups
  ...
  -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  \# Kiosk Devices New-MgGroup -DisplayName \"UIAO-Devices-Kiosk\" \` -Description \"Kiosk and shared devices\" \` -MailEnabled:\$false -MailNickname \"uiao-devices-kiosk\" \` -SecurityEnabled:\$true -GroupTypes @(\"DynamicMembership\") \` -MembershipRule \'(device.extensionAttribute12 -eq \"Kiosk\")\' \` -MembershipRuleProcessingState \"On\"
...
8.4 Region, Site, and Department Groups
  ...
  ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  \# Region --- East New-MgGroup -DisplayName \"UIAO-Devices-Region-East\" \` -Description \"Devices in East region\" \` -MailEnabled:\$false -MailNickname \"uiao-devices-east\" \` -SecurityEnabled:\$true -GroupTypes @(\"DynamicMembership\") \` -MembershipRule \'(device.extensionAttribute1 -eq \"EASTUS\")\' \` -MembershipRuleProcessingState \"On\" \# Site --- Annapolis HQ. Site is the LocPath axis, not an OrgPath facet: per ADR-102 D5, LocPath claims no extensionAttribute slot, so a site group has no membership rule to write. It is planned by the LocPath Entra exposure as an assigned-membership group, with members read from the governed Primary-LocPath assignments. New-MgGroup -DisplayName \"LocPath-Site-ANNAPOLIS-HQ-Users\" \` -Description \"Devices and users at the Annapolis HQ site (LocPath /USA/MD/ANNAPOLIS-HQ)\" \` -MailEnabled:\$false -MailNickname \"locpath-site-annapolis-hq\" \` -SecurityEnabled:\$true \` -MembershipRuleProcessingState \"On\" \# Department --- Governance New-MgGroup -DisplayName \"UIAO-Devices-Dept-Governance\" \` -Description \"Devices assigned to Governance department\" \` -MailEnabled:\$false -MailNickname \"uiao-devices-governance\" \` -SecurityEnabled:\$true -GroupTypes @(\"DynamicMembership\") \` -MembershipRule \'(device.extensionAttribute3 -eq \"Governance\")\' \` -MembershipRuleProcessingState \"On\"
