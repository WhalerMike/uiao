...
![Workflow B — Admin Consent request. The request is sponsored by the Application Owner and travels as a documented package to the Identity Governance Authority, which holds sole, non-delegable approval authority.](images/enterprise-application-access-governance-diagram-02-admin-consent-workflow.png){#fig-consent-workflow fig-alt="A left-to-right blueprint pipeline. Requester initiates, Departmental Owner validates business need, Application Owner sponsors and attaches scope and risk assessment, then a teal arrow passes through an amber gate labelled Identity Governance Authority — sole approval to a navy Entra ID consent action box. A red side note: no approval below Layer 4. Footer: the IGO never receives a cold request." width="90%"}
