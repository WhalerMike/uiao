...
800-53 (AC-2, AC-6), and the FedRAMP 20x **KSI-IAM** indicators anchored on
those controls — `KSI-IAM-ELP` (ensuring least privilege) and `KSI-IAM-AAM`
(automating account management) in particular — require that access decisions
be traceable to accountable individuals acting within defined roles.
...
- **Compliance exposure:** Inability to produce an auditable access decision
  ...
  FedRAMP 20x assessment contexts. Under 20x the exposure surfaces sooner:
  `KSI-IAM-ELP` and `KSI-IAM-AAM` are continuously validated, so an
  unattributable grant reads as an unevidenced indicator immediately rather
...
| **Compliance readiness** | Unable to produce complete, role-attributable access decision trails for audit | Complete, exportable governance records in SAM supporting federal audit, IG review, and FedRAMP 20x KSI-IAM evidence |
