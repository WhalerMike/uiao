...
### How this replaces OU-based helpdesk delegation

In Active Directory, helpdesk delegation was scoped by placing the target users in an OU and granting rights on that OU — implicit containment. Entra ID has no OU tree. The equivalent is an **Administrative Unit (AU)** combined with a scoped Entra role:

| AD model | Entra equivalent in this guide |
|---|---|
| Helpdesk delegated on `OU=Agency-Users` | Authentication Administrator scoped to agency AU |
| Password reset rights on `OU=Field-Offices` | Authentication Administrator scoped to field-office AU (OrgPath rule) |
| OU ACL giving group membership write | Groups Administrator scoped to department AU |
| Standing delegated admin (persistent) | PIM-eligible assignment — never standing active |

AU membership is driven by OrgPath facet predicates (e.g., `extensionAttribute2 -eq "IT"`), so scope tracks org structure automatically without manual OU management. The PAG and PIM configuration below operationalize the role-binding side of this model.
