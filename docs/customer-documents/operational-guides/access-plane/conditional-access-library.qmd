...
repo-path: docs/customer-documents/operational-guides/access-plane/conditional-access-library.qmd
...
![CA policy tier architecture (baseline, device, environment, region, app, governa...](images/conditional-access-library-image-01-ca-policy-tier-architecture-base.png){#fig-conditional-access-library-image-01 fig-alt="CA policy tier architecture (baseline, device, environment, region, app, governance). Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for primary structural elements, teal (#1A9E8F) for secondary/integration elements, amber (#D4A017) for governance/audit/registry overlays, monospace labels for identifiers, no human figures, 16:9 landscape." width="100%"}

![GPO-to-CA migration matrix as a flow](images/conditional-access-library-image-02-gpo-to-ca-migration-matrix-as-a.png){#fig-conditional-access-library-image-02 fig-alt="GPO-to-CA migration matrix as a flow. Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for primary structural elements, teal (#1A9E8F) for secondary/integration elements, amber (#D4A017) for governance/audit/registry overlays, monospace labels for identifiers, no human figures, 16:9 landscape." width="100%"}

![break-glass account procedure swimlane](images/conditional-access-library-image-03-break-glass-account-procedure-sw.png){#fig-conditional-access-library-image-03 fig-alt="break-glass account procedure swimlane. Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for primary structural elements, teal (#1A9E8F) for secondary/integration elements, amber (#D4A017) for governance/audit/registry overlays, monospace labels for identifiers, no human figures, 16:9 landscape." width="100%"}
...
- Compliance alignment to NIST 800-53 and the FedRAMP 20x / CR26 Key Security Indicators anchored on those controls (Appendix D.2)
...
**Description:** Applies maximum security controls to Tier 0 Privileged Access Workstations (PAWs). Requires phishing-resistant MFA, enforces 1-hour sign-in frequency, and disables persistent browser sessions to minimize credential exposure on the highest-value assets.
  ...
  **Configuration**                  **Value**
  ---------------------------------- -------------------------------------------------------------------------------------------------------------
  **Users --- Include**              Dynamic group: DG-Devices-Tier0 (device.extensionAttribute11 -eq \"Tier0\")

  **Users --- Exclude**              SG-BreakGlass-Exclusion

  **Cloud Apps**                     All cloud apps

  **Conditions --- Device Filter**   Include filtered devices: device.extensionAttribute11 -eq \"Tier0\"
...
**Description:** Controls access from Tier 1 server administrator identities to Azure management and server administration tools. Requires compliant devices and MFA to prevent lateral movement between tiers.
  ...
  **Configuration**                  **Value**
  ---------------------------------- -------------------------------------------------------------------------------------------------------
  **Users --- Include**              Dynamic group: DG-Devices-Tier1 (device.extensionAttribute11 -eq \"Tier1\")

  **Users --- Exclude**              SG-BreakGlass-Exclusion

  **Cloud Apps --- Include**         Microsoft Azure Management, Windows Virtual Desktop, Azure Arc-enabled servers

  **Conditions --- Device Filter**   Include filtered devices: device.extensionAttribute11 -eq \"Tier1\"
...
**Description:** Applies standard security controls to Tier 2 end-user workstations. Allows access from either Intune-compliant or Hybrid Azure AD joined devices, providing flexibility during the hybrid transition period.
  ...
  **Configuration**                  **Value**
  ---------------------------------- ---------------------------------------------------------------------------------------
  **Users --- Include**              Dynamic group: DG-Devices-Tier2 (device.extensionAttribute11 -eq \"Tier2\")

  **Users --- Exclude**              SG-BreakGlass-Exclusion

  **Cloud Apps --- Include**         Office 365 (all apps)

  **Conditions --- Device Filter**   Include filtered devices: device.extensionAttribute11 -eq \"Tier2\"
...
**Description:** Restricts kiosk devices to a limited set of applications with extended sign-in frequency and app-enforced restrictions. Kiosk devices are identified by extensionAttribute12 = \"Kiosk\" and are intended for shared-use scenarios.

  ---------------------------------------------------------------------------------------------------------------
  **Configuration**                  **Value**
  ---------------------------------- ----------------------------------------------------------------------------
  **Users --- Include**              Dynamic group: DG-Devices-Kiosk (device.extensionAttribute12 -eq \"Kiosk\")

  **Users --- Exclude**              SG-BreakGlass-Exclusion

  **Cloud Apps --- Include**         Select apps (limited kiosk app set defined per site)

  **Conditions --- Device Filter**   Include filtered devices: device.extensionAttribute12 -eq \"Kiosk\"
...
**Description:** Applies the highest level of protection to production environment access. Requires compliant devices, MFA, and approved client applications. Blocks access from unapproved client apps to prevent data leakage through unauthorized tools.
  ...
  **Configuration**       **Value**
  ----------------------- ------------------------------------------------------------------------------------------------------------------------------
  **Users --- Include**   Dynamic group: DG-Env-Production (user.extensionAttribute13 -eq \"Production\")
...
**Description:** Restricts staging environment access to internal networks only and requires MFA. Staging environments contain near-production configurations and must be protected from external access.
  ...
  **Configuration**              **Value**
  ------------------------------ -----------------------------------------------------------------------------------
  **Users --- Include**          Dynamic group: DG-Env-Staging (user.extensionAttribute13 -eq \"Staging\")
...
**Description:** Provides developers with more flexible access while maintaining baseline MFA requirements. Extended sign-in frequency reduces friction during development workflows while preserving identity assurance.
  ...
  **Configuration**            **Value**
  ---------------------------- ------------------------------------------------------------------
  **Users --- Include**        Dynamic group: DG-Env-Dev (user.extensionAttribute13 -eq \"Dev\")
...
A.4 CA-010 --- JSON Template
  ...
  -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  { \"displayName\": \"CA-010: Tier 0 - Privileged Access Workstation Enforcement\", \"state\": \"enabledForReportingButNotEnforced\", \"conditions\": { \"users\": { \"includeGroups\": \[\"{DG-Devices-Tier0-ObjectId}\"\], \"excludeGroups\": \[\"{SG-BreakGlass-Exclusion-ObjectId}\"\] }, \"applications\": { \"includeApplications\": \[\"All\"\] }, \"devices\": { \"deviceFilter\": { \"mode\": \"include\", \"rule\": \"device.extensionAttribute11 -eq \\\"Tier0\\\"\" } } }, \"grantControls\": { \"operator\": \"AND\", \"builtInControls\": \[\"compliantDevice\"\], \"authenticationStrength\": { \"id\": \"00000000-0000-0000-0000-000000000004\", \"displayName\": \"Phishing-resistant MFA\" } }, \"sessionControls\": { \"signInFrequency\": { \"value\": 1, \"type\": \"hours\", \"isEnabled\": true }, \"persistentBrowser\": { \"mode\": \"never\", \"isEnabled\": true } } }
...
D.2 FedRAMP 20x / CR26 Mapping

FedRAMP 20x expresses assessment expectations as **Key Security Indicators** rather than control narratives. The NIST 800-53 controls listed in D.1 remain correct — CR26 anchors each indicator to a set of them — but the artifact an assessor consumes is indicator evidence. This policy library is almost entirely a **KSI-IAM** surface; the mapping below is the one to cite in a 20x package.

  --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  **CR26 indicator**                                       **What the indicator asks**                                                                                          **CA Policy Reference**
  -------------------------------------------------------- -------------------------------------------------------------------------------------------------------------------- --------------------------------------
  **KSI-IAM-APM** --- Adopting Passwordless Methods        Modern, phishing-resistant authentication in place of shared secrets                                                 CA-001, CA-004, CA-010

  **KSI-IAM-ELP** --- Ensuring Least Privilege             Access granted no broader than the role requires                                                                     CA-023, CA-010 through CA-012, CA-043

  **KSI-IAM-JIT** --- Authorizing Just-in-Time             Privilege held only for the window it is needed                                                                      CA-010, PIM (Section 11)

  **KSI-IAM-SNU** --- Securing Non-User Authentication     Service and workload identities authenticate without interactive MFA, securely                                       CA-005, Appendix C

  **KSI-IAM-SUS** --- Responding to Suspicious Activity    Detected risk drives an automated access response                                                                    CA-061 through CA-064

  **KSI-IAM-AAM** --- Automating Account Management        Account lifecycle is automated rather than ticket-driven; dynamic group targeting throughout                         OrgPath-scoped assignment

  **KSI-MLA-RVL** --- Reviewing Logs                       Logs are reviewed, not merely retained                                                                               CA-060, Section 12

  **KSI-CMT-LMC** --- Logging Changes                      Configuration changes are logged and attributable; drift detection integration                                       Compliance Amendment (`CM-3`)
  --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

Note what this library does **not** evidence. CR26 indicators outside KSI-IAM — recovery planning (KSI-RPL), supply chain risk (KSI-SCR), cybersecurity education (KSI-CED), incident response process (KSI-INR) — are not addressable by Conditional Access policy and are evidenced elsewhere. A package that cites this library against those indicators is overclaiming.

D.2a Deployment and residency considerations

The following considerations apply to this policy library within the GCC-Moderate deployment boundary. They are deployment facts; what is *assessed* is decided per information resource by the Minimum Assessment Scope inclusion test.

  --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  **Deployment requirement**                               **Implementation**                                                                                                   **CA Policy Reference**
  -------------------------------------------------------- -------------------------------------------------------------------------------------------------------------------- --------------------------------------
...
## Compliance Amendment — Device Compliance Enforcement and Policy Change Tracking (`AC-3`, `CM-3`)

### Device compliance enforcement (`AC-3`)

A Conditional Access policy requires that devices accessing in-boundary resources are marked compliant (Intune) or are hybrid/Entra-joined, enforcing access only from managed, compliant devices. This complements the user- and risk-based policies in this library and is part of the `AC-3` access-enforcement posture.

### Policy change tracking (`CM-3`)

Changes to any Conditional Access policy follow change control (`CM-3`): each modification is recorded with author, rationale, and approval, and the policy set is captured as governed configuration so that an unauthorized or out-of-band change surfaces as drift. Policy state is reviewed on the quarterly cadence noted above.
