...
.DESCRIPTION
    Implements the six drift classes defined in UIAO_196 / ADR-112:
    ...
    if ($rec.AgentClass -eq 'agentic-ai' -and $rec.AtoStatus -ne 'yes') {
        $findings.Add([PSCustomObject]@{
            ...
            Finding     = "Agentic AI system deployed without ATO (have_ato=$($rec.AtoStatus)). " +
                          "Per ADR-112 §4, agentic AI without ATO is the highest-risk identity class. " +
...
<div class="note">
  <strong>Scan basis:</strong> UIAO_196 / ADR-112. Six drift classes, L1 (observe only).
...
<p style="margin-top:2rem;font-size:0.75rem;color:#888">
  Generated by Invoke-AIInventoryScan.ps1 v1.0.0 &mdash; UIAO_196 / ADR-112 &mdash;
