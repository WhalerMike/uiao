...
UIAO_154 enumerates every AU in a three-tier structure mirroring the
hierarchy facets:
...
OrgPath dynamic-group rule. `AU-NCR-Finance` contains exactly the same users
as `OrgTree-NCR-Finance-Users`.
...
A member of `OrgTree-NCR-Finance-Admins` can reset passwords, create groups,
and manage users — but only for users in `AU-NCR-Finance`. Outside that
...
policy targeting `OrgTree-EASTUS-Users`, whose rule is the region facet
alone (`user.extensionAttribute1 -eq "EASTUS"`). Users in that region get
the policy; everyone else doesn't.
...
recomputed from it; every downstream group, AU, CA scope,
and license assignment follows automatically. No hand edits; no
ticket; no drift window.

### Leaver

Termination writes a `leaver` attribute that the workflow catches;
the user's `term_date` facet (`extensionAttribute8`) is stamped and the
hierarchy facets are cleared, so the derived path empties; dynamic
...
| **Hierarchy Drift** | An AU prefix names a facet value no longer in the codebook |
| **Orphan Drift** | A security group exists that isn't in UIAO_152 |
| **Phantom Drift** | The codebook defines Division `Governance` but zero users carry it |
