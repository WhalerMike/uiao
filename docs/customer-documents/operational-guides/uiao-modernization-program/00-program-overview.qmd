...
services. Azure services consumed by the architecture operate under the
commercial FedRAMP authorizations appropriate to their deployment
context.

Under FedRAMP 20x, what gets assessed is no longer settled by drawing an
authorization boundary around that estate. The **Minimum Assessment
Scope** standard asks a two-pronged question of each information
resource --- does it handle federal information, and/or does it likely
impact the confidentiality, integrity, or availability of federal
information? --- and resources (and most metadata) that answer no to both
sit outside the assessed scope. The deployment facts above still describe
*where* UIAO runs; the inclusion test decides *what is assessed*.
...
UIAO aligns with established frameworks: FedRAMP 20x and its
**Consolidated Rules for 2026 (CR26)**, Zero Trust (per Executive Order
14028 lineage), TIC 3.0, and NIST SP 800-53. UIAO does not invent new
compliance requirements --- it provides the architectural substrate that
makes existing requirements operationally achievable. By embedding
compliance into the architecture rather than bolting it on after
deployment, UIAO eliminates the chronic gap between documented policy and
operational reality that plagues traditional modernization efforts.

The 20x alignment is dated, not aspirational. CR26 became optionally
adoptable on **2026-07-04**, opened the 20x Class B/C pipelines on
**2026-08-31**, is **mandatory for all FedRAMP stakeholders on
2027-01-01**, and closes new Rev5 certifications on **2027-06-11**. Every
phase in this program is planned against that calendar: the artifact a
UIAO-governed estate produces for an assessor is a machine-readable KSI
evidence payload keyed to the CR26 catalog (10 themes, 46 indicators),
not a hand-written control narrative. See
[ADR-126](../../../adr/adr-126-fedramp-cr26-official-rules-adoption.html)
for the authority pin and
[ADR-106](../../../adr/adr-106-fedramp-20x-integration.html) for the
substrate's emission contract.
...
organization subject to FedRAMP 20x, CMMC, SOC 2, HIPAA, or no formal
...
mappings to match its regulatory context. The KSI emission surface
described throughout this program is the federal expression of a
general mechanism --- continuously validated, machine-readable evidence
keyed to a published indicator catalog. Swap the catalog and the same
substrate serves a non-federal regime unchanged.
...
- **UIAO Repository on GitHub** --- The canonical source for
...
Contributions, issues, and pull requests are welcome via the UIAO
GitHub repository.
