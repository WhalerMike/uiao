...
::: {.callout-note title="Deployment boundary vs. Minimum Assessment Scope"}
Everything in this section describes **where services run** --- a
deployment fact that governs data residency, feature availability, and
contractual posture. It does not by itself decide **what is assessed**.
Under FedRAMP 20x that question is answered per information resource by
the Minimum Assessment Scope inclusion test: does the resource handle
federal information, and/or does it likely impact the confidentiality,
integrity, or availability of federal information? An Azure Commercial
service that meets either prong is in the assessment scope despite
sitting outside GCC-Moderate; a GCC-Moderate service that meets neither
is not automatically in scope for being there. Keep the two axes
separate when reading §8.3 below.
:::
...
- **Assessment-scope classification:** For each intersection above,
  which side's information resources meet the Minimum Assessment Scope
  inclusion test, and which are `metadata-out-of-scope`? This question
  is separable from the five above and resolves faster: it is decided
  by what a resource does with federal information, not by which cloud
  it sits in. Record the classification per resource in the register
  described in §8.4 --- an intersection can be unresolved on data
  residency while its scope classification is already settled.
...
1.  **Never Break Authentication:** Identity synchronization (Entra
...
2. **Policy Parity Before Cutover:** Intune policies must achieve
...
3. **Compliance Continuity:** Devices must never exist in an ungoverned
...
4. **Reversibility Windows:** Each migration wave includes a documented
...
5. **Evidence-Based Advancement:** Advancement from one wave to the
...
Appendix A --- Glossary
  ...
  **FedRAMP**        Federal Risk and Authorization
                     ...
                     services. Its current framework is
                     FedRAMP 20x, expressed as the
                     Consolidated Rules for 2026 (CR26),
                     mandatory for all stakeholders from
                     2027-01-01.

  **FedRAMP 20x**    The automation-first FedRAMP framework
                     that replaces narrative authorization
                     packages with continuously validated,
                     machine-readable evidence. Its rules
                     surface is CR26; its evidence
                     vocabulary is the Key Security
                     Indicator catalog; its scoping rule is
  ...
  **Key Security     A CR26 indicator of security outcome,
  Indicator (KSI)**  evidenced by a machine-readable,
                     continuously validated payload rather
                     than a control narrative. CR26 defines
                     46 indicators across 10 themes (CED,
                     CMT, CNA, IAM, INR, MLA, PIY, RPL,
                     SCR, SVC), each anchored to NIST
  ...
  **Minimum          The FedRAMP 20x replacement for the
  Assessment Scope** authorization boundary. An information
                     resource is in scope if it handles
                     federal information, and/or likely
                     impacts the confidentiality,
                     integrity, or availability of federal
                     information. Resources and most
                     metadata meeting neither prong are
                     outside it.
