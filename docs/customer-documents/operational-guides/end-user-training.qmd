...
repo-path: docs/customer-documents/operational-guides/end-user-training.qmd
...
![Two-panel before-and-after comparison diagram for end-user identity modernization](images/end-user-training-image-01-two-panel-before-and-after-compa.png){#fig-end-user-training-image-01 fig-alt="Left panel labeled \"Before\" shows a workstation with sticky-note passwords on the monitor, a VPN client icon, three mapped network drives labeled H:, S:, T:, and four separate sign-in prompts to different apps — friction points highlighted in muted red (#C74040). Right panel labeled \"After\" shows the same workstation now with a fingerprint icon at the sign-in screen, a smartphone with an Authenticator \"Approve\" button, a single OneDrive cloud-sync icon replacing the mapped drives, and a single \"Signed in everywhere\" badge. A bold horizontal arrow between the panels is labeled \"Modern authentication & cloud services\". Clean engineering blueprint style, dark navy (#0D1B2E) and teal (#1E8C8C) on white background, with red (#C74040) reserved for the legacy-state friction points only. No photographs, no human figures, purely diagrammatic." width="100%"}
...
1. **Look for the enrollment prompt.** After your organization enables Windows Hello, you\'ll see a prompt during sign-in that says *\"Your organization requires Windows Hello.\"* Follow the on-screen instructions to get started.

2. **Set up your PIN first.** Click *\"Set up PIN\"* and create a PIN that meets your organization's minimum length (commonly 6 digits or more). This PIN only works on *this specific device* --- it\'s not like a regular password.

3. **Set up fingerprint recognition (if available).** If your device has a fingerprint reader, go to: **Settings → Accounts → Sign-in options → Fingerprint recognition → Set up**. Follow the prompts to scan your fingerprint.

4. **Set up facial recognition (if available).** If your device has an infrared (IR) camera, go to: **Settings → Accounts → Sign-in options → Facial recognition → Set up**. Look at the camera when prompted to register your face.

5. **Test it!** Lock your PC by pressing **Win + L**, then sign back in using your face, fingerprint, or PIN. If it works --- you\'re all set!
...
![Windows Hello for Business enrollment screen mockup](images/end-user-training-image-02-windows-hello-for-business-enrol.png){#fig-end-user-training-image-02 fig-alt="A laptop screen rendering of the Windows 11 Settings → Accounts → Sign-in options panel, with three highlighted enrollment cards: \"Face recognition (Windows Hello)\" with a small camera icon, \"Fingerprint recognition (Windows Hello)\" with a fingerprint icon, and \"PIN (Windows Hello)\" with a numeric-pad icon — each card showing a teal \"Set up\" button. A side callout labeled \"Tied to this device only\" points to the PIN card; a second callout labeled \"Hardware-backed by the TPM\" points to the laptop chassis. Clean Windows-11 UI illustration style, dark navy (#0D1B2E) and teal (#1E8C8C) accents on white background. No photographs, no human faces, purely a UI mockup." width="100%"}
...
1. **Install the app.** Download *\"Microsoft Authenticator\"* from the **Apple App Store** (iPhone) or **Google Play Store** (Android).

2. **Add your work account.** Open the app, tap the **\"+\"** button, and select **\"Work or school account.\"**

3. **Sign in when prompted.** Enter your work email address and follow the sign-in prompts.

4. **Register the app with your account.** On your computer, open a browser and go to **https://mysignins.microsoft.com**.

5. **Add the sign-in method.** Click **\"Security info\"** → **\"Add sign-in method\"** → select **\"Authenticator app\"** from the dropdown.

6. **Scan the QR code.** Follow the on-screen wizard. When a QR code appears, open the Authenticator app on your phone and scan it.

7. **Approve the test notification.** A test notification will be sent to your phone. Tap **\"Approve\"** to confirm everything is connected.

8. **Enable phone sign-in.** In the Authenticator app, tap on your work account and select **\"Enable phone sign-in.\"** This lets you sign in by matching a number on your phone instead of typing a password.
...
![Three-step Microsoft Authenticator setup flow, arranged left to right](images/end-user-training-image-03-three-step-microsoft-authenticat.png){#fig-end-user-training-image-03 fig-alt="Step 1: a smartphone outline showing the Authenticator app's \"+\" button being tapped, label \"Install & open the app\". Step 2: a laptop showing a QR code on the mysignins.microsoft.com Security info page with the same smartphone scanning it via its camera, label \"Scan the QR code\". Step 3: the smartphone showing a single bold \"Approve\" button beneath a two-digit number-match prompt, label \"Approve sign-in with one tap\". Light teal arrows connect the three steps. Clean engineering illustration style, dark navy (#0D1B2E) and teal (#1E8C8C) on white background. No photographs, no human figures, purely diagrammatic." width="100%"}
...
1. **Get your key from IT.** Your IT team will provide you with a FIDO2-compatible security key (USB-A or USB-C, depending on your device).

2. **Register the key.** Go to **https://mysignins.microsoft.com** → click **\"Security info\"** → **\"Add sign-in method.\"**

3. **Select \"Security key.\"** Choose **\"USB device\"** when prompted.

4. **Insert and tap.** Insert your security key into the USB port when prompted, then tap the button or sensor on the key.

5. **Create a key PIN.** You\'ll be asked to create a PIN for the key itself (this serves as a backup verification method).

6. **You\'re done!** To sign in going forward: insert the key → tap → done.
...
1. **Check the basics.** Are you on your work device? Are you connected to the office network or VPN?

2. **Check for Windows Updates.** Go to **Settings → Windows Update** (Windows 11) or **Settings → Update & Security → Windows Update** (Windows 10) and install any pending updates.

3. **Update your Authenticator app.** Open your phone\'s app store and make sure Microsoft Authenticator is up to date.

4. **Contact IT if still blocked.** Reach out to the IT Help Desk and be ready to share:
...
![Conditional Access block-page mockup](images/end-user-training-image-04-conditional-access-block-page-mo.png){#fig-end-user-training-image-04 fig-alt="A browser window centered on a Microsoft-styled \"You can't access this right now\" message, with a sub-line \"Your sign-in was successful, but doesn't meet the criteria to access this resource.\" At the bottom of the page, a \"Correlation ID:\" label with a sample GUID, outlined in amber and pointed to by a callout arrow labeled \"Share this ID with IT — it helps them diagnose the issue quickly.\" Clean Microsoft 365 sign-in UI styling, dark navy (#0D1B2E) and teal (#1E8C8C) accents on white background, with amber (#D4A017) reserved for the correlation-ID highlight only. No photographs, no human figures, purely a UI illustration." width="100%"}
...
- For apps that use certificate-based authentication, your device usually selects the right certificate automatically (IT can configure browsers to auto-select via Intune policy). You may not see any prompt at all.

- If a **\"Select a certificate\"** popup *does* appear, choose the certificate that **matches your work email address**. The issuer name will typically include your organization\'s name.
...
1. **A notification appears** saying *\"Your IT administrator has enabled backup of your important folders.\"*

2. **Click \"Start backup\"** (or it may happen automatically, depending on your organization\'s settings).

3. **Look for the OneDrive cloud icon** in your system tray (bottom-right corner of your screen, near the clock).

4. **A blue sync icon** will appear briefly on your files as they upload to the cloud.

5. **When complete, your files show a green checkmark** --- they\'re safely backed up!
...
![OneDrive sync status reference card](images/end-user-training-image-05-onedrive-sync-status-reference-c.png){#fig-end-user-training-image-05 fig-alt="Four labeled cards arranged horizontally on a clean white background, each showing one OneDrive File-Explorer status icon at large size with its label and a one-line meaning underneath: (1) green circle with white checkmark — \"Saved on this PC and in the cloud — fully synced\"; (2) blue cloud outline — \"Available online only — saves disk space; downloads when opened\"; (3) blue circular-arrows icon — \"Currently uploading or downloading — give it a moment\"; (4) red circle with white X — \"Sync error — click the OneDrive icon in the system tray for details\". The four status colors (green, blue, blue, red) are preserved as the authentic OneDrive palette. Overall page uses dark navy (#0D1B2E) and teal (#1E8C8C) for borders and labels. Clean engineering reference-card style. No photographs, no human figures, purely a UI illustration." width="100%"}
...
- **Shared/team drives (S:, etc.)** are moving to **SharePoint team sites**. Your team\'s files will be accessible at your tenant SharePoint URL (your IT team will share the exact address --- it looks like `https://<your-tenant>.sharepoint.com`) or by syncing SharePoint libraries to File Explorer.
...
2. Enter your **work email address** and follow the on-screen prompts.

3. **Company Portal will install automatically** once enrollment is complete.

4. Your device will begin receiving **security policies** and **approved apps** from your IT team.

![Microsoft Intune Company Portal main-screen mockup, Windows 11 styling](images/end-user-training-image-06-microsoft-intune-company-portal.png){#fig-end-user-training-image-06 fig-alt="A windowed application with two visible tabs in the left sidebar: \"Apps\" (currently active, highlighted by a teal accent bar) and \"Devices\". The main content area under the Apps tab shows a 4×2 grid of application tiles — each tile has an app icon and a name (examples: Microsoft Teams, Adobe Acrobat, Cisco AnyConnect, Power BI, Visio, Microsoft Project, Edge for Business, Outlook). The Devices tab in the sidebar carries a small \"Compliant\" badge next to the device name. Clean Windows-11 UI illustration style, dark navy (#0D1B2E) and teal (#1E8C8C) accents on white background. No photographs, no human figures, purely a UI mockup." width="100%"}
...
1. Open your browser and go to **https://mysignins.microsoft.com**.

2. Click **\"Security info.\"**

3. Make sure you have **at least TWO methods** registered from the following:
...
1. On the Windows sign-in screen, click **\"I forgot my password\"** (or *\"Reset password\"*).

2. Enter your **work email address**.

3. **Complete verification:** Approve the notification on your Authenticator app, or enter the SMS code sent to your phone.

4. **Create a new password.** Your new password must meet complexity requirements: **14+ characters**, with a mix of uppercase letters, lowercase letters, numbers, and special characters.
...
2. Follow the same verification and password creation steps as above.
...
1. **Wait for smart-lockout to expire.** Microsoft Entra ID's smart-lockout starts at about one minute and increases with repeated bad attempts. After it expires, try signing in again.

2. **Or use SSPR** to reset your password, which also unlocks your account immediately.

3. **If neither works,** contact the IT Help Desk for assistance.
...
3. You\'ll see all groups you currently belong to.
...
*Print this page and keep it at your desk for quick access to the most common tasks and links.*
  ...
  **SharePoint team sites**                https://\<your-tenant\>.sharepoint.com (ask IT for your exact URL)
...
## Compliance Amendment — IT Staff Training, Records, and Phishing Simulation (`AT-3`, `AT-4`)

This end-user material is complemented by the role-based requirements in the UIAO Security Awareness and Training Program (`UIAO_187`).

### IT staff security training (`AT-3`)

Personnel in IT and security-significant roles (administrators, operators, developers) complete role-based security training beyond the end-user baseline — covering privileged-access governance, incident-response duties, and secure operations — before assuming the role and annually thereafter.

### Training records (`AT-4`)

Training completion is tracked per individual with completion and next-due dates; outstanding training is reported to the system owner and retained as assessment evidence.

### Phishing simulation

A phishing-simulation program runs on a defined cadence; click and report rates are tracked, and repeat clickers receive targeted follow-up training.
