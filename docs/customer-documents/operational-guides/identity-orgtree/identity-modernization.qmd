...
repo-path: docs/customer-documents/operational-guides/identity-orgtree/identity-modernization.qmd
...
Under **FedRAMP 20x**, those control anchors resolve forward to CR26 Key Security Indicators, which are what a 20x package actually evidences. Compliance Manager's templates are Rev5-shaped and do not emit indicator evidence, so treat them as a readiness check rather than as the deliverable:

| Controls above | CR26 indicator | Sections in this guide |
|---|---|---|
| AC-2 | `KSI-IAM-AAM` — Automating Account Management | 9.1, 9.3 |
| AC-6 | `KSI-IAM-ELP` — Ensuring Least Privilege; `KSI-IAM-JIT` — Authorizing Just-in-Time | 5.3, 7 |
| IA-2, IA-5 | `KSI-IAM-APM` — Adopting Passwordless Methods | 8, 11.2, 11.3, 6.4 |
| AU-2 | `KSI-MLA-LET` — Logging Event Types; `KSI-MLA-RVL` — Reviewing Logs | 12.2, 12.3 |
...
Generated: 11 May 2026 \| Author: Michael\
...
## Compliance Amendment — Phishing-Resistant Authenticators and Privileged Access Workstations (`IA-2(6)`, `AC-6(7)`)

### FIDO2 / PIV / CAC mapping (`IA-2(6)`)

Privileged and high-assurance access requires phishing-resistant, hardware-bound authenticators. Authenticator types map to Entra ID methods as follows:

- **FIDO2 security keys** — the primary phishing-resistant method for cloud-native administrators; registered as a FIDO2 method and required by Conditional Access for privileged roles.
- **PIV / CAC smart cards** — federal-issued credentials map via certificate-based authentication (CBA) to Entra ID, satisfying `IA-2(6)` (access to privileged accounts using a hardware-based authenticator).
- Password and SMS/voice methods are disallowed for privileged roles.

### Privileged Access Workstation (PAW) guidance (`AC-6(7)`)

Administration of privileged roles is performed only from a Privileged Access Workstation — a hardened, separately-governed device with no general productivity or browsing surface. PAWs enforce device-compliance Conditional Access, are excluded from standard user policy, and are reviewed for least-privilege role assignment per `AC-6(7)`. Administrative actions from non-PAW devices are blocked.
