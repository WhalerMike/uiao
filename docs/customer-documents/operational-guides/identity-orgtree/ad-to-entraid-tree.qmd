...
> **Model C — 15-facet multi-attribute (per [ADR-078](../../../adr/adr-078-orgpath-attribute-schema-15-facet.qmd)).** This guide was originally authored against Model B (composite-slash path, `extensionAttribute1: CORP/US/EAST/BALTIMORE/IT`). Model B is superseded; the canonical OrgPath schema is now 15 facets across `extensionAttribute1`–`15`. Section 2 below has been rewritten for Model C; the manager-spine / governance / delegation / lifecycle material in sections 3+ remains operationally correct (those patterns are independent of the attribute model) but the inline `CORP/...` examples will be replaced with facet-tuple equivalents in a follow-up ADR-078 Phase 4 PR. For the Model C-native treatment of the codebook, dynamic groups, AUs, and drift detection, see [Reference Architecture → OrgPath Codebook](../../reference-architecture/codebook.qmd), [Dynamic Groups](../../reference-architecture/dynamic-groups.qmd), [Delegation Matrix](../../reference-architecture/delegation.qmd), and [Drift Walk-through](../../reference-architecture/drift-walkthrough.qmd).
