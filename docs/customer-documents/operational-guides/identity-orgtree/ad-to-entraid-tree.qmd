...
<!-- orgpath-slot-allow-file: this document IS the Model B -> Model C translation reference. Its left-hand columns and the A.2 format section quote the retired composite-path binding on purpose, next to the Model C form that replaces it; rewriting them would delete the translation.
     orgpath-codebook-allow-file: likewise for the composite-path literals
     themselves. The sibling slot and prefix guards run over this file
     unmarked, so a genuine Model A slot binding introduced here is still
     caught. -->
