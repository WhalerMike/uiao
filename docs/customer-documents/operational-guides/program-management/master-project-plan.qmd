...
repo-path: docs/customer-documents/operational-guides/program-management/master-project-plan.qmd
...
![Seven-phase program Gantt with milestone diamonds](images/master-project-plan-image-01-seven-phase-program-gantt-with-m.png){#fig-master-project-plan-image-01 fig-alt="Seven-phase program Gantt with milestone diamonds. Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for primary structural elements, teal (#1A9E8F) for secondary/integration elements, amber (#D4A017) for governance/audit/registry overlays, monospace labels for identifiers, no human figures, 16:9 landscape." width="85%"}

![five-pillar workstream swim lanes](images/master-project-plan-image-02-five-pillar-workstream-swim-lane.png){#fig-master-project-plan-image-02 fig-alt="five-pillar workstream swim lanes. Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for primary structural elements, teal (#1A9E8F) for secondary/integration elements, amber (#D4A017) for governance/audit/registry overlays, monospace labels for identifiers, no human figures, 16:9 landscape." width="85%"}

![assessment-before-action artifact dependency tree](images/master-project-plan-image-03-assessment-before-action-artifac.png){#fig-master-project-plan-image-03 fig-alt="assessment-before-action artifact dependency tree. Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for primary structural elements, teal (#1A9E8F) for secondary/integration elements, amber (#D4A017) for governance/audit/registry overlays, monospace labels for identifiers, no human figures, 16:9 landscape." width="85%"}

![phase-gate review/approval flow](images/master-project-plan-image-04-phase-gate-reviewapproval-flow.png){#fig-master-project-plan-image-04 fig-alt="phase-gate review/approval flow. Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for primary structural elements, teal (#1A9E8F) for secondary/integration elements, amber (#D4A017) for governance/audit/registry overlays, monospace labels for identifiers, no human figures, 16:9 landscape." width="85%"}
...
M-002 --- IIS + Gitea Operational (Week 2)
  ...
  **Task**   **Description**                                                                                         **Hours**   **Dependencies**               **Reference**
  ...
  T-002.3    Download Gitea binary, install as Windows service, generate app.ini with PostgreSQL backend (DB_TYPE = postgres, pg01.corp.contoso.com, database uiao_gitea)   4h          T-002.1                        Platform Server Build Guide §6
...
## Compliance Amendment — Control Traceability and Risk Register (`PL-2`, `RA-3`)

### Milestone-to-control-family traceability (`PL-2`)

Each program milestone is mapped to the NIST 800-53 control families it advances, so that program progress is traceable to control coverage. This traceability complements the SSP (`UIAO_185`), which holds the authoritative per-control disposition; the program plan references control families at the milestone level.

### Risk register (`RA-3`)

The program maintains a risk register recording identified program and security risks, their likelihood/impact rating, owner, mitigation, and status. Security risks with residual exposure are cross-referenced to POA&M items (`UIAO_189`); the register is reviewed at each phase gate per `RA-3`.
