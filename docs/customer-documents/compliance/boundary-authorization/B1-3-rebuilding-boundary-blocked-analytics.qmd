...
| # | Signal (gap-matrix id) | Disposition | Verdict | Impact of the fix / not-fix |
|---|---|---|---|---|
| 1 | Communication patterns — chat vs email (`communication-patterns-chat-vs-email`) | confirmed | Partial — substitute | Chat-vs-email ratios rebuilt from the unified audit log; recovers the insider digital-disengagement baseline, not the Adoption Score aggregate. |
| 2 | Content collaboration — attachments vs cloud links (`content-collaboration-attachments-vs-cloudlinks`) | confirmed | Partial — substitute | Derived from audit + message-trace; DLP-bypass regression detectable via custom rules. |
| 3 | Mobility — multi-platform usage (`mobility-multiplatform-usage`) | confirmed | Partial — substitute | Platform/device read from `SigninLogs` — well-rebuildable; unusual-platform takeover signal recovered. |
| 4 | Meetings participation (`meetings-participation`) | confirmed | Partial — substitute | From Teams audit / CQD; coarser collaboration-shift baseline. |
| 5 | Apps health — version / channel (`apps-health-version-channel`) | confirmed | Partial — substitute | From Intune + WUfB device export; outdated-app trend rebuildable. |
| 6 | Network connectivity scores per location (`network-connectivity-scores-per-location`) | confirmed | Partial — substitute (network-plane) | Per-location quality via SD-WAN / SASE PEP + CQD (ADR-057); agency-owned, not Microsoft's score. |
| 7 | Endpoint readiness baselines, tenant-wide (`endpoint-readiness-baselines-tenant-wide`) | confirmed | Partial — substitute | Tenant posture view from device-inventory export + Phase 3 models. |
| 8 | INR real-time path metrics (`inr-realtime-path-metrics`) | confirmed | Partial — substitute (network-plane) | Canonical substitute: SD-WAN / SASE / ThousandEyes (ADR-057) recovers path / MITM / DNS detection via agency-owned PEP telemetry — not Microsoft's path intelligence. |
| 9 | Device boot / GP / desktop-load performance (`device-boot-performance`) | inferred | Partial — substitute | Boot/GP/load times from Graph device export + Defender `DeviceEvents`; depends on exportable device telemetry. |
| 10 | App reliability — crash / hang / stability (`app-reliability-crashcount-hangrate`) | inferred | Partial — substitute | Crash/hang trends from device export; exploit-attempt clusters partially recovered. |
| 11 | Resource performance — CPU / memory (`resource-performance-cpu-memory`) | inferred | Partial — substitute | High-value: Phase 3 anomaly model recovers the grey-ware / miner / exfil signal that bypasses AV. |
| 12 | Stop-error restarts (`stop-error-restarts`) | inferred | Partial — substitute | Restart-anomaly detection from device events; moderate value. |
| 13 | Battery health / WFA readiness (`battery-health-wfa-readiness`) | inferred | Partial — substitute | Low-value substitute; remote-endpoint trust baseline partially recoverable. |
| 14 | Device performance anomalies / regression (`device-performance-anomalies-regression`) | inferred | Partial — substitute | Post-config / OS-regression detection via Phase 3 model; core device-integrity signal partially recovered. |
| 15 | Entra real-time risk / Identity Protection (`entra-identity-protection-realtime-risk`) | inferred | **Fixes** | Headline win: impossible-travel + atypical-IP + leaked-cred + spray rebuilt → Identity Advanced. Detection minutes → hours, no global TI. |
| 16 | Cross-tenant access telemetry — B2B / Direct Connect (`cross-tenant-access-telemetry`) | inferred | **Not fixed** (mostly) | Own-tenant B2B events are in the agency's logs (visible), but cross-tenant *correlation* is irreducible — the agency never holds the other tenant's telemetry. |
| 17 | Real-time CAE revocation, sub-second (`cae-realtime-revocation-paths`) | inferred | Partial — bounded | SOAR revocation + short token lifetimes bound the token-theft window to minutes, not sub-second; Optimal unreachable. |
| 18 | App-protection policy-violation analytics (`app-protection-policy-violation-analytics`) | inferred | **Fixes** | Phase 2 rules over compliance-state deltas restore violation aggregation — snapshot-driven, not real-time behavioral. |
| 19 | WUfB deployment-health trends (`wufb-deployment-health-trends`) | inferred | **Fixes** | Graph update-state export + trend queries restore fleet patch posture, with a reporting lag vs Microsoft's real-time view. |
| 20 | EPM elevation operational analytics (`epm-elevation-operational-analytics`) | inferred | **Fixes** (if collectible) | Elevation-as-risk rebuildable *where* EPM events are exportable; insider elevation patterns scored centrally. |
| 21 | Office "Optional" diagnostic data (`office-optional-diagnostic-data`) | restricted | **Not fixed** (mostly) | Suppressed at source — GCC defaults to Required-only, so client telemetry is never collected and cannot be rebuilt. Only a server-side substitute remains; client-side "contextual glue" stays lost. |
| 22 | Copilot / AI prompt-response telemetry (`copilot-ai-prompt-response-telemetry`) | inferred | Partial | Purview audit gives basic incident counts only; rich AI-usage behavioral patterns largely unrecoverable. |
| 23 | Sensitivity-label usage analytics (`sensitivity-label-usage-analytics`) | inferred | **Fixes** | Phase 2 mass-label-change / mass-decrypt detection from Purview audit; label-strip insider signal recovered → Data Advanced. |
| 24 | DLP behavioral richness — override / near-miss (`dlp-behavioral-richness`) | inferred | **Fixes** (pattern-level) | Override-pattern / near-miss rules from Purview / Management-Activity; no content-fingerprint model — pattern rules only. |
| 25 | Long-term CQD EUII (`cqd-euii-long-term`) | retention-limited | **Fixes** (retention) | Scheduled CQD export before the 28-day purge defeats the forensic cliff — fully fixable (retention, not analytics). |
| 26 | Cross-product 12+ month historical pivots (`m365-usage-analytics-historical-pivots`) | retention-limited | **Fixes** (retention) | Agency long-retention store (L2) restores 12+ month historical pivots and low-and-slow visibility. |
...
| Partial (substitute or bounded) | 16 | 1–14, 17, 22 |
