...
| `PG_ADMIN_PASSWORD` | Postgres admin password (store in Key Vault too) |
...
# Update the Container App to point at the restored server
az containerapp secret set -g <rg> -n uiao-saas \
  --secrets "database-url=postgresql+asyncpg://<user>:<pwd>@uiao-saas-pg-restored.postgres.database.azure.com/uiao"
