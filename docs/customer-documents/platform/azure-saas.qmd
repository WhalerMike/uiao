...
`GET /control/v1/audit`. When a database is configured (`UIAO_SAAS_DATABASE_URL`)
the trail is persisted to an append-only `saas_audit_events` table (sharing the
registry's passwordless connection); without one it falls back to the in-memory
ring buffer. Toggle with `UIAO_SAAS_AUDIT_ENABLED`.
