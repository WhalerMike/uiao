...
[`01-platform-foundation.qmd`](../modernization/client-server-to-hybrid-cloud/01-platform-foundation.html)
...
[DIAGRAM-01: Left-to-right architecture diagram of the UIAO Git Smart HTTP server. A "Git client on developer workstation (C:\Users\whale\git\uiao)" box on the far left issues an HTTPS request over port 443 into a central "Windows Server 2025 / IIS" host box. Inside the IIS host, the request flows through three internal stages drawn as nested boxes: TLS termination at the IIS site binding; CGI handler mapping; the git-http-backend.exe CGI process. The CGI process reads and writes to a "Bare repository — D:\GitRepos\uiao.git" box on the right. Below the bare repo, a horizontally aligned cluster of three server-side hooks: pre-receive, update, post-receive (each shown as a small subordinate box attached to the bare repo). A dotted-line upstream sync arrow leaves the bare repo and curls back to a top-right cloud "https://github.com/WhalerMike/uiao" box, labeled "scheduled pull". Engineering blueprint style, neutral slate background with federal navy (#1F3A5F) for the IIS host and teal (#1A9E8F) for client and upstream, monospace filesystem paths and process names, 900x450 landscape (16:9 acceptable).]
...
\# Download and install URL Rewrite Module 2.1 \$rewriteInfoUrl = \'https://www.iis.net/downloads/microsoft/url-rewrite\' \$rewriteInstaller = \"\$env:TEMP\\rewrite_amd64.msi\" Write-Host \"Download the current x64 MSI from \$rewriteInfoUrl and save it to \$rewriteInstaller before continuing.\" -ForegroundColor Yellow Start-Process msiexec.exe -ArgumentList \"/i \`\"\$rewriteInstaller\`\" /quiet /norestart\" -Wait Remove-Item \$rewriteInstaller -Force \# Verify installation if (Test-Path \"\$env:SystemRoot\\System32\\inetsrv\\rewrite.dll\") { Write-Host \"URL Rewrite Module installed successfully\" -ForegroundColor Green } else { Write-Error \"URL Rewrite Module installation failed\" }
