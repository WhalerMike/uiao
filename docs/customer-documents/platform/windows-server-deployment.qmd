...
| Permission | Scope | Why |
|---|---|---|
| Write `extensionAttribute1-3`, `extensionAttribute6`, `extensionAttribute9`, `extensionAttribute10` | All computer objects | The projected governance-layer facets (ADR-078 schema, ADR-121 projection subset) |
| Write `extensionAttribute15` | All computer objects | The derived OrgPath, recomputed from the hierarchy facets on every stamp (ADR-127) |
| The same two rows | All user objects | Identical facet and derived-path slots, if user tagging is in scope |

Seven slots, not one: the whole-path-in-`extensionAttribute1` form is the
retired Model A binding. Grant the facet slots and the derived-path slot
together — a delegation covering only some of them leaves the derived path
unwritable and every stamp lands as drift.
