...
**Problem.** Buyers assume UIAO is a monolith you adopt whole, which makes
build-vs-buy look binary. Without a legible map of what is active, proposed,
or not applicable per identity target and capability pack, the *deployed*
surface and the *designed* surface get confused in both directions.

**Solution.** OrgPath (UIAO_151) is the à-la-carte core; everything else is an
additive profile or adapter (ADR-098). Two independent axes — where identity
lives, which capabilities are switched on — laid out as a status-labelled grid
with three worked stacks and the dependency truths: OrgPath needs no single
vendor, SailPoint is an optional vertical, and enforcement reads facets
through whichever storage profile is present.

**Ask.** Weight the matrix by status, not by row count — today's write-capable
path is Microsoft/Entra plus read-only assessment. Pick a stack from the grid,
and where a target you need is marked *Proposed*, ask for its live-tenant
conformance run before treating it as delivered.
:::
