...
subtitle: "How the open UIAO canon composes Evidence Act, M-25-05, CDO, DAMA-DMBOK, Zero Trust, TIC 3.0, NIST 800-63 and FedRAMP 20x obligations over the identity layer — SSOT as the architectural response, not a mandate"
doc-type: whitepaper
...
canon-reference:
  ...
  - "src/uiao/canon/adr/adr-030-pre-uiao-terminology-reconciliation.md"
...
**Problem.** Agencies carry Evidence Act, OPEN Government Data Act, and
M-25-05 data-governance obligations *and* an AD → Entra ID migration
imperative, and run them as two unrelated programs. No statute mandates Single
Source of Truth — but a credible data inventory cannot be maintained from
sources that disagree, and the identity layer beneath every one of those
decisions is exactly what the migration is about to change.

**Solution.** Treat the identity namespace as the root of the SSOT hierarchy
and the migration as a governance event. The open UIAO canon encodes SSOT
designations, the drift taxonomy, and a provenance chain so that FedRAMP-grade
evidence is a byproduct of the migration rather than assembled afterward —
beneath Entra ID, Purview, Fabric, Defender, and Sentinel, not in place of
them.

**Ask.** Run the two programs as one. Adopt the phased posture (assess →
govern → migrate → validate → operate) with governance in place before any
identity object moves, and give the Chief Data Officer and the identity team a
single, shared SSOT designation register.
:::

### Executive summary

Federal civilian agencies carry a set of data-governance obligations — through
the Foundations for Evidence-Based Policymaking Act, the OPEN Government Data
Act, and OMB's implementation guidance, currently
[M-25-05](https://www.whitehouse.gov/omb/information-for-agencies/memoranda/)
(Phase 2, January 2025) — to maintain comprehensive data inventories, designate
Chief Data Officers, and publish open-by-default in machine-readable form.

**None of those authorities mandates Single Source of Truth**, and this paper
does not claim otherwise. There is no federal SSOT mandate, no statutory
requirement to designate authoritative systems of record, and no OMB CDO
directive imposing one. The Federal Data Strategy was a strategy rather than a
mandate, and its action plans ended with the 2021 edition.

SSOT is the **architectural response** a great many agencies adopt *in order to*
satisfy those obligations. You cannot maintain a credible data inventory across
fragmented operational environments without deciding which system is
authoritative for a given fact, and you cannot publish open-by-default from
sources that disagree. That is an engineering conclusion, not a legal one — and
stating it that way is what makes the rest of this paper survive a reader who
knows the statutes. A paper that asserts a mandate its cited authority does not
impose has handed that reader the fastest possible reason to stop reading. At the same time, those
...
([07a — UIAO Beneath the Azure SSOT Stack](../orgpath-narrative/Book_07a.qmd))
of the UIAO OrgPath series. This whitepaper establishes the federal
mandate alignment that makes that chapter necessary.

![Two federal program tracks converging into the UIAO open canon as a single identity SSOT substrate](images/federal-ssot-alignment-image-01-two-federal-program-tracks-conve.png){#fig-federal-ssot-alignment-image-01 fig-alt="Two-track program convergence diagram. Left lane labeled \"Evidence Act / OPEN Data Act / CDO — data obligations\" with five boxes: MDM hub / golden records, cloud consolidation, data catalog + lineage, CDO governance board, federated standards. Right lane labeled \"AD to Entra ID Migration Imperative\" with five boxes: AD forest (legacy), sync engine (hybrid), Entra ID (target), ZT / TIC 3.0 coherence, FedRAMP 20x. Two arrows converge — captioned \"treated as one governance event, not two programs\" — into a navy band labeled \"UIAO Open Canon — Identity SSOT Substrate\" carrying six teal pills for the six governance domains (Identity, Authorization, Compliance, Migration, Evidence, Operational) with a note that these are this paper's federated reading, distinct from the canon's six control planes (ADR-030). A teal arrow exits downward to a box labeled \"Continuous FedRAMP-grade Evidence Bundle — regenerated deterministically, not assembled at assessment time\". Title above: \"Two Programs, One Substrate\". Clean engineering blueprint style, dark navy (#0D1B2E) and teal (#1E8C8C) on white background. No photographs, purely diagrammatic." width="100%"}
...
![Rosetta-stone mapping of federal SSOT mechanisms to UIAO canon capabilities](images/federal-ssot-alignment-image-02-rosetta-stone-mapping-of-federal.png){#fig-federal-ssot-alignment-image-02 fig-alt="Two vertical columns under headers \"Federal Mandate / Mechanism\" and \"UIAO Canon Mechanism\", with five paired rows joined by bidirectional teal arrows: MDM Golden Records to COR Schema (UIAO_007 + OrgPath); Data Lineage and Provenance to Immutable Provenance Chain feeding the Evidence Bundle; Redundancy and Conflict Audit to Drift Engine — 6 DRIFT classes (P1–P4) including DRIFT-SSOT-CONTENTION per ADR-074; Federated Governance to Six Governance Domains (federated, central-standards: Identity, Authorization, Compliance, Migration, Evidence, Operational); Layer-on-Legacy (no rip-and-replace) to Nine Adapter Interfaces (IPAM, PKI, RADIUS, LDAP-proxy, sync-engine, device-mgmt, NTP, DFS, SQL Server per migration-adapter-registry.yaml). Footer band: \"Each mapping is mechanical, not aspirational — every row anchors to a named canon artifact\". Clean engineering blueprint style, dark navy (#0D1B2E) and teal (#1E8C8C) on white background. No photographs, purely diagrammatic." width="100%"}
...
[UIAO OrgPath](../orgpath-narrative/index.qmd) develops the substrate
...
defines six drift classes — `DRIFT-SCHEMA`, `DRIFT-SEMANTIC`,
`DRIFT-PROVENANCE`, `DRIFT-AUTHZ`, `DRIFT-IDENTITY`, and
`DRIFT-SSOT-CONTENTION` (ADR-074) — each with severity
...
This is the automated equivalent of the redundancy auditing an agency has to
perform to keep a data inventory honest — a practice the inventory obligation
implies rather than a phrase any statute uses — applied in real time to the
identity layer, with the
drift class and severity carried as schema rather than narrative.

#### 3.4 Six governance domains → federated governance with central standards

To be explicit up front, because the count is easy to conflate: this
paper reads UIAO's substrate through six **governance domains** —
Identity, Authorization, Compliance, Migration, Evidence, and Operational.
These are **not** a competing or independent six-part taxonomy. They are
this paper's governance-ownership reading of the canon's own six **control
planes** — Identity, Addressing, Overlay, Telemetry, Management, Governance,
defined in
[ADR-030](../../../src/uiao/canon/adr/adr-030-pre-uiao-terminology-reconciliation.md)
— asking one question of each plane: *who owns accountability for it?* The
control planes describe what the substrate acts on; the governance domains
describe who governs that action, framed for the federated governance model
federal agencies already run (centralized standards, federated execution
across CDO, ISSO, and program-office ownership).

The correspondence is not one-to-one, and presenting it as a clean 1:1 table
would overstate it. Identity is a direct match on both sides. Everywhere
else, a governance domain typically draws its accountability from more than
one control plane, or a control plane's accountability splits across more
than one domain:

| UIAO governance domain | Federal governance equivalent | Draws accountability primarily from (ADR-030 control plane) |
|---|---|---|
| Identity domain | CDO-designated identity SSOT | Identity |
| Authorization domain | Data access policy standards | Governance, with Identity supplying the assurance context it enforces against |
| Compliance domain | FedRAMP / NIST continuous monitoring | Governance, with Telemetry supplying the monitoring signal it evaluates |
| Migration domain | Transition governance and rollback authority | Identity, Addressing, and Overlay jointly — migration is the event where all three change state together |
| Evidence domain | Audit, lineage, and reporting | Telemetry for emission, Management for bundle assembly and OSCAL regeneration |
| Operational domain | Regional / silo operational continuity | Management, with Addressing and Overlay for the legacy infrastructure that domain keeps running |

Put plainly: Authorization and Compliance both lean primarily on the
Governance plane rather than each owning a distinct plane of their own;
Migration and Operational are each an accountability reading spread across
three planes rather than one. That is expected, not a flaw in the
overlay — a governance-ownership reading is not obligated to partition the
same six things the control planes partition; it exists to answer a
different question (who is accountable) over the same six-plane substrate.
Each domain is governed independently but references the same COR-based
SSOT. This is the "hybrid governance" model: centralized standards,
federated execution.
...
instances rather than replacing them wholesale. The nine adapter interfaces
under
...
- Microsoft Entra ID tenant control planes are shared with non-GovCloud
  tenants
- Location signal of the fidelity TIC 3.0's inspection model assumes is not
  available at the GCC Moderate boundary

This creates a three-way compliance conflict:

1. **TIC 3.0** positions the agency's TIC to inspect cloud service traffic —
   but GCC Moderate telemetry bypasses the inspection point. TIC 3.0 is
   guidance and prescribes no product class, so this is a **capability gap
   against its Security Capabilities Catalog**, not an agency failing a
   directive; stated as a directive violation it would not survive review
...
[`B1-gcc-moderate-boundary-model`](../orgcomp-series/boundary/B1-gcc-moderate-boundary-model.qmd)
documents the structural shape, and its dedicated leaf
[B1-1 — the GCC-Moderate three-way conflict](../orgcomp-series/boundary/B1-1-gcc-moderate-three-way-conflict.qmd)
formalizes this section's conflict as the standing finding
`GCC-BOUNDARY-3WAY-001`. UIAO's Compliance governance domain is the
...
obligation maps to a specific UIAO capability that exists in canon. Read the
left column as *what the agency owes* and the middle as *how this substrate
discharges it* — not as a claim that the authority prescribes the mechanism.
Only the Zero Trust and FedRAMP rows carry directive force with their own
clocks; the data-governance rows are obligations whose method is the agency's
to choose.

| Federal obligation | UIAO capability | Canon anchor |
|---|---|---|
| Evidence Act — Data Asset Inventories | COR schema as authoritative identity inventory | `UIAO_007`, `substrate-manifest.yaml` |
| M-25-05 — Open Data Plans, machine-readable publication | Identity namespace as root SSOT; no parallel copies. **SSOT is UIAO's architectural answer to this obligation, not a requirement the memo imposes** | UIAO OrgPath §1–4 |
| CDO Data Governance Board requirements | Six governance domains with ownership and accountability assignments | `substrate-manifest.yaml` |
...
| TIC 3.0 | Compliance governance domain models GCC boundary constraints | `B1-gcc-moderate-boundary-model` |
...
2. **Govern** — Governance domains activated; SSOT designations encoded;
...
[Chapter 07a of the UIAO OrgPath](../orgpath-narrative/Book_07a.qmd).
