...
![Architecture comparison diagram with two panels: Left panel 'Active Directory...](images/ad-to-entraid-migration-problem-image-01-architecture-comparison-diagram.png){#fig-ad-to-entraid-migration-problem-image-01 fig-alt="Architecture comparison diagram with two panels: Left panel 'Active Directory..." width="85%"}
...
The reason this matters for your code decisions right now is: if gos is truly that broader product, then the federal compliance machinery in UIAO (OSCAL generation, KSI engine, FedRAMP artifacts) is a \*\*vertical add-on\*\* to gos, not the other way around. The core governance engine should be clean and universal, with a federal compliance module on top.
...
That means gos could legitimately be an \*\*adapter\*\* in the UIAO framework --- the same way ServiceNow, Entra, and Palo Alto are adapters in UIAO today. The difference is that instead of being a data source adapter, gos is a \*\*governance domain adapter\*\* that brings the directory migration problem under the UIAO governance envelope.
