...
![Consume-vs-build decision tree](images/uiao-vs-native-tools-image-04-consume-vs-build-decision-tree.png){#fig-uiao-vs-native-tools-image-04 fig-alt="Consume-vs-build decision tree. Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for primary structural elements, teal (#1A9E8F) for secondary/integration elements, amber (#D4A017) for governance/audit/registry overlays, monospace labels for identifiers, no human figures, 16:9 landscape." width="100%"}


::: {.callout-important}
## Problem, solution, ask

**Problem.** "Doesn't Microsoft already do this?" Partially — across twelve
disconnected tools that, without significant custom glue, cover an estimated
30% of the AD assessment discovery surface and roughly 22% once the governance
pipeline, cross-domain correlation, and drift detection are counted. OU
hierarchy and delegation, DNS, PKI health, and trusts are essentially
uncovered.

**Solution.** Position UIAO as a complementary orchestration layer, not a
replacement — the same pattern as UIAO SCuBA and CISA ScubaGear. Consume
Microsoft and third-party output (Azure Migrate, Group Policy Analytics,
Defender for Identity, ScubaGear, ADRecon) through the UIAOImportAdapters
module; keep the PowerShell-first assessment module for the discovery
Microsoft does not do; treat Defender for Identity as the good-enough security
layer.

**Ask.** Do not frame the build-vs-buy decision as UIAO *versus* Microsoft.
Route native tool output into one governance pipeline, run the assessment
module against a representative forest to replace this paper's illustrative
percentages with your own numbers, and carry the complementary positioning
into the FedRAMP CWGs, CISA/FSCAC, and the Microsoft partner ecosystem (§10).
:::
...
Microsoft's tools collectively cover approximately **30%** of what the UIAO Assessment Module captures in raw technical discovery. The remaining **70%** of the discovery surface --- OU hierarchy, DNS, PKI health, trusts, delegation --- plus governance provenance, canonical artifact management, drift detection, cross-domain correlation, and pipeline orchestration is where UIAO provides unique value. When governance requirements are factored in, Microsoft's effective coverage drops to approximately **22%** of the total UIAO assessment surface. These percentages are illustrative estimates derived from this paper's own domain coverage matrix (Section 4); the canonical gap doctrine deliberately carries no coverage percentages.
:::

::: {.callout-warning}
## ILLUSTRATIVE — replace before external release

The 30% / 22% figures above are derived from the domain coverage matrix in
Section 4, not from a single reproducible assessment run. Before external
release, replace this block with a worked example that anchors the headline
number in an actual (or representative) assessment:

Applied to a representative `[N]`-object AD forest assessment run on
`[date]`, Microsoft's native tooling (the tools inventoried in Section 2)
surfaced `[X]` of `[Y]` governance-relevant findings across the domains
enumerated in §3's capability matrix; UIAO's methodology, applied to the
same forest, surfaced `[X2]` of `[Y]` findings. The `[Y]` findings should be
enumerated (or referenced) so the count is auditable, and `[X]` / `[X2]`
should map back to specific rows of the §3 matrix so a reader can verify
which findings each tooling approach did and did not surface.
:::

## 2. The Microsoft Native Tool Landscape

The following inventory catalogs every Microsoft tool that touches Active Directory assessment, migration, or modernization. (The [OrgPath narrative's Book 01](../orgpath-narrative/Book_01.qmd) walks the same stack chapter by chapter from the governance side; this paper's lens is assessment coverage.) For each tool, we document its current status, capabilities, limitations, licensing requirements, and output format --- with specific attention to governance pipeline compatibility. Product statuses and version details in this inventory reflect the June--July 2026 snapshot and move quickly; verify current status before relying on any individual entry.
...
For the governance lens on what this tool's ~30–60% support percentage
actually *costs* — the policy-plane capabilities lost in the GPO-to-Intune
shift and where OrgPath restores each — see
...
::: {.callout-important title="Overall Assessment"}
Microsoft tools cover approximately **22%** of the total UIAO assessment surface when governance pipeline, cross-correlation, and drift detection requirements are factored in (the unweighted average of all ten categories above). For pure technical discovery without governance, Microsoft covers approximately **30%** (the unweighted average of the seven discovery categories, excluding cross-domain correlation, governance pipeline, and drift detection). Both figures are illustrative estimates derived from this paper's domain matrix, not canonical measurements.
:::
...
::: {.callout-tip title="The Metaphor"}
...
### 9.1 Import Adapters (Implemented)

The import adapters described conceptually in earlier revisions of this paper are now implemented as the **UIAOImportAdapters** PowerShell module (`tools/powershell/UIAOImportAdapters/`, shipped as `.psd1` + `.psm1` with Pester tests wired into CI), governed by the canon specification **UIAO_182** and positioned by ADR-094 as a producer in the assessment-to-plan toolchain. This closes the gap the OrgPath narrative named in [Book 15, "The Gap That Remains"](../orgpath-narrative/Book_15_CPT_23.qmd). The module normalizes heterogeneous third-party assessment output into the UIAO schema so that downstream correlation, drift detection, and plan generation operate over one canonical assessment shape rather than vendor-specific exports.

The architectural contract is deliberately file-path-based: every importer consumes an already-produced report via `-ReportPath` and writes normalized JSON via `-OutputPath`. **The module never reaches a live API.** An operator (or another tool) exports the report from Azure Migrate, Intune, Defender for Identity, ScubaGear, or ADRecon; the adapter ingests the exported file. This keeps the ingestion surface read-only, credential-free, and auditable within the M365 GCC-Moderate boundary.

The canonical function roster (per UIAO_182):

| **Function** | **Source Consumed** | **Normalized Target** |
|---|---|---|
| Import-UIAOAzureMigrateReport | Azure Migrate assessment export | UIAO ComputerInventory |
| Import-UIAOGPOAnalyticsReport | Intune Group Policy Analytics export | GPOMigrationTracker |
| Import-UIAODefenderFindings | Defender for Identity Secure Score export | SecurityAssessment overlay |
| Import-UIAOSCuBAReport | CISA ScubaGear compliance output | UIAO conformance evidence |
| Import-UIAOADReconReport | ADRecon Excel output | UIAO ComputerInventory / identity |
| Merge-UIAOAssessmentSources | Multiple normalized sources | Correlated assessment bundle |

SCT PolicyAnalyzer, Entra Connect Health, and Azure Policy compliance output remain candidates for future adapters; the consumption patterns in Section 6.2 describe where their exports would land.

### 9.2 Import Module Specification

Each importer wraps its normalized records in a sealed provenance envelope (`schema: uiao.import.v1`) whose provenance block carries:

- **source** --- name of the third-party tool consumed

- **version** --- source export version (recorded when supplied, "unknown" otherwise)

- **timestamp** --- ISO 8601 UTC timestamp of import execution

- **content_hash** --- integrity seal over the normalized data, computed by the canonical hasher so it is byte-identical to the Python `canonical_hash` --- making every imported artifact directly `DRIFT-PROVENANCE`-classifiable by `src/uiao/governance/drift.py`

- **source_file** --- path of the ingested report

- **actor**, **import_module**, **import_module_version** --- adapter identity for the provenance chain

`Merge-UIAOAssessmentSources` correlates multiple normalized envelopes (`-SourcePaths`, `-MergeStrategy`) into a single assessment bundle, validating each source's provenance completeness and integrity seal first: an envelope that is incomplete, or whose hash no longer matches its data, is a `DRIFT-PROVENANCE` finding by definition (UIAO_182).

Outputs are UIAO-schema JSON artifacts suitable as input to UIAOPlanGenerators and as canon-anchored evidence through the UIAO IR / OSCAL pipeline. Consistent with the program's `classification: Public` posture, the envelope carries provenance metadata rather than a classification marking; Authenticode signing occurs at release packaging.
...
1. **Do not position UIAO as competing with Microsoft's tools** --- the SCuBA pattern (complementary orchestration) is the correct positioning.

2. **Route Microsoft and third-party tool output through the implemented UIAOImportAdapters module** --- Azure Migrate, Group Policy Analytics, Defender for Identity, ScubaGear, and ADRecon exports all normalize through its file-based importers (Section 9, canon UIAO_182).

3. **Maintain the PowerShell-first assessment module** --- it provides the roughly 70% of technical discovery coverage that no Microsoft tool offers (OU, DNS, PKI, trusts, cross-correlation; an illustrative estimate per the Section 4 matrix).

4. **Leverage Defender for Identity as a "good enough" security layer** --- focus UIAO's unique value on governance, not security tooling.

5. **Use ADRecon as a validation reference** --- UIAO's data collection should produce at least everything ADRecon produces, plus governance metadata.
...
## Appendix B --- License and Cost Comparison
  ...
  **UIAO Governance OS**        **Open Source (Apache-2.0)**                **$0**                                                        **Free. Requires PowerShell, RSAT, and Gitea (also free/open-source). All governance pipeline capabilities included.**
  ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

::: {.callout-note title="Cost Analysis"}
...
UIAO vs. Microsoft Native Tools --- AD Assessment and Modernization Gap Analysis | Version 1.1 | July 2026\
Classification: Public | Boundary: GCC-Moderate\
