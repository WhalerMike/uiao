...
The worked substrate for this principle — the governed facet schema,
dynamic-group library, and AU delegation model — is the
[OrgPath](../../orgpath/index.qmd) pillar, told end to end in the
[OrgPath narrative series](../orgpath-narrative/index.qmd).
...
| **Modernization adapters** | `modernization-registry.yaml` (28 entries) declares change-making adapters — including `bluecat-address-manager` and `infoblox` from the retired `uiao-gos`, now first-class modernization adapters. Each maps a piece of pre-migration infrastructure to its post-migration target. |
