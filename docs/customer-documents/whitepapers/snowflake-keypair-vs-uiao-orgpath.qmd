...
**Problem.** Snowflake is switching passwords off on a published schedule, and
service accounts, pipelines, and dbt jobs have to land somewhere. The usual
answer — key-pair authentication — gets treated as the finish line, and
warehouse-role grants stay hand-authored in a ticket queue, so nothing
computes who should reach which role.

**Solution.** Three altitudes of one migration: Snowflake forces the
credential change; UIAO governs *what the credential becomes* (password
eliminated → stored key pair as a dated, rotated, compensated bridge → IdP
federation with no long-lived secret, per ADR-004); OrgPath governs *who the
credential is for*, rendering access from facet predicates and turning a stray
grant into a `DRIFT-AUTHZ` finding. This is an illustrative reading: UIAO
ships no Snowflake adapter, and Snowflake sits outside the GCC-Moderate
boundary.

**Ask.** Three steps that need no adapter: classify every Snowflake principal
by hand today, rotate stored key pairs on a defined cadence and track it as a
compliance signal, and settle the boundary question (a gating ADR in the
ADR-059/ADR-033 pattern, or `commercial-general` per ADR-129) before the
automation question. See the Call to Action.
:::
