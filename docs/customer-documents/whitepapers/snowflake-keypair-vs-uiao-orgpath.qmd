...
one of the two named Commercial exceptions (Amazon Connect, SailPoint); any
real Snowflake coverage would require a gating boundary ADR in the pattern of
`ADR-059` / `ADR-033`. Treat the patterns here as *how the substrate would
...
not supply on its own: a multi-facet schema (`ADR-078`, the 15-facet model)
materialized across Entra `extensionAttribute` slots — region, division,
...
deployment boundary**, and it is not one of the two named Commercial exceptions
(Amazon Connect, SailPoint). Before any Snowflake adapter could exist, the
boundary question is a **gating ADR decision**: a Snowflake exception would need
its own `gcc-boundary` enum value added in lockstep with an authorizing ADR — the
pattern `ADR-059` set for SailPoint and `ADR-033` defines for boundary drift.
Nothing in this whitepaper authorizes that step.
