...
**Problem.** Forty years of federal architecture — the mainframe,
decentralization, the security era, the breaking point — conflated the system
of record with the in-path enforcer, first in the mainframe and then in Active
Directory. Today's estates run session-based identity over hairpinned WANs
across four environments, and the cost shows up as duplicate payments and
unreconcilable records, not slow screens. The commercial sector made this
transition in 2015–2020; the agency's own clocks — M-22-09, BOD 25-01, CA-7
under A-130 — have already run.

**Solution.** Rebuild the foundation around one structural move — separate
truth from enforcement, as NIST SP 800-207 and OMB M-22-09 require — and the
five principles that follow: a single source of truth, application-aware
networking, token-based identity, active governance, and fewer synchronous
round-trips; with a PassTicket/Identity Propagation token bridge for the
mainframe.

**Ask.** Six steps this quarter: inventory the actual architecture, run a
physics-based latency audit before the next migration decision, assign an
owner to each principle, put the agency's own deadlines on the calendar, pilot
the mainframe token bridge on one low-risk transaction class, and hand this
paper unchanged to the CIO/CISO, network, identity, and mainframe teams. The
full list is in the Call to Action.
:::
