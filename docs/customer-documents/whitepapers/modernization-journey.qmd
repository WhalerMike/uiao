...
the mainframe's stark single source of truth, through the Great
...
A note on what "your agency" is: Acts 1–3 are a narrative synthesis of an
agency modernization arc (mainframe → regional decentralization →
security-layer accretion → multi-cloud breaking point); the specific
institutional details (twelve regional domains, the named data-center
anchors) are the agency's own and are not asserted as UIAO canon. The full
Provenance callout at the end of this document gives the line-by-line
mapping of Act 4's claims to their canon anchors.
:::
...
The model was starkly elegant: **one source of truth.** The mainframe *was* the
...
That's the lie. Latency is physics — but the physics has to be stated at the
right order of magnitude. Portland to Maryland is roughly 2,400 miles; fibre
propagation puts the theoretical round-trip floor near 40 ms, and real paths
land at 60–80 ms. Active Directory has no millisecond latency requirement; the
widely-quoted five-minute figure is Kerberos clock-skew tolerance, not a
latency budget. What the distance actually costs is the DC-dependent
operations Microsoft documents as needing line-of-sight to an on-premises
domain controller: initial TGT acquisition at logon, SYSVOL/GPO retrieval,
password changes, and hybrid Entra join registration. Add session-breaking
security appliances on top of that distance and those operations compound.
Logon times creep up. GPO processing stalls.
...
transport plane and adopts **token-bound,
...
review; published to the site as a Draft-status work.
:::
