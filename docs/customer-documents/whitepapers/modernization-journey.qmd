...
Version 2.0 (2026-09-09) consolidates two earlier editions of this same
forty-year arc into a single paper. The argument is the vendor- and
product-neutral one: it cites only NIST, OMB, CISA and FedRAMP, and asserts
nothing that depends on any particular product, platform, or vendor. The
...
::: {.callout-warning}
## Figure correction pending
The figure below renders a “Kerberos timeout limit: 5ms” and a “16x OVER
limit” comparison. **There is no 5 ms Kerberos limit** — that number conflates
the five-*minute* clock-skew tolerance with a latency budget, a factor of
60,000. The alt text below describes the image as it currently stands rather
than contradicting it; the figure is queued for regeneration against the
corrected mechanism above.
:::
...
translation issues"* — tolerating a changed source address is a design property
...
> tolerance, not a latency budget — but the DC-dependent operations are the ones
...
**Everything above this appendix is vendor- and product-neutral** — it cites
only NIST, OMB, CISA and FedRAMP, and the argument stands without anything
below. This appendix is separable: it exists for readers who want to know
where Act 4's claims come from in the UIAO substrate, and it can be dropped
when the paper is handed to an audience that is not evaluating a platform.
