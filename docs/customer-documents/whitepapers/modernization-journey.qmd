...
![Act 2 — When Security Ate the Architecture: a field-office request traverses five stacked inline security devices, and the added latency degrades the four operations that must reach a domain controller](images/federal-application-aware-networking-architecture-image-04-security-stacking.png){#fig-security-stacking fig-alt="An engineering diagram on a white background titled 'Act 2 — When Security Ate the Architecture', subtitled 'Stacked inline inspection did not expire tickets. It degraded the operations that must reach a domain controller.' On the left, a 'Field office workstation' box feeds down through five navy boxes — Firewall +15 ms, NAT +8 ms, IDS inline probe +12 ms, Proxy/DLP +20 ms, Compliance scanner +25 ms — into an amber box reading '80 ms added, one way, on top of the propagation floor'. A red arrow points right into a bordered panel headed 'What this actually degrades: the DC-dependent operations', listing four red-tinted entries: initial TGT acquisition at logon (the client must reach a KDC before the desktop is usable); SYSVOL/GPO retrieval (many small file reads, so latency multiplies per round trip); password change (written to a writable DC, not a local cache); and hybrid Entra join registration (documented as needing on-premises DC reach). A note beneath observes that steady-state application traffic is largely unaffected, which is why the problem presents as slow logons rather than an outage. A teal panel on the right headed 'What does not happen' states that there is no Kerberos latency limit — RFC 4120 sets a five-minute clock-skew tolerance, which is a clock tolerance and not a latency budget, with no millisecond threshold anywhere in the protocol; that NAT does not invalidate tickets because Windows KILE ships ClientIpAddresses = 0 for stated DHCP and network-address-translation reasons; and that address checking, where enabled, is performed by the application server via KRB_AP_ERR_BADADDR rather than by the KDC. A navy footer reads 'The argument survives being stated correctly — and is stronger for it', noting that stacking inline latency in front of a domain controller makes logons crawl and GPO processing stall. A caption records that the latency figures are illustrative of a typical stacked-inspection path while the operation list and Kerberos behaviour are not." width="100%"}
...
**The radio mechanism:** 5G's scalable numerology shortens the slot as
...
perfectly well) but because it carries **no device or session context**: an
...
Consolidated Rules define the KSI assessment as a follow-on assessment *of a
cloud service offering*, coordinated by *the provider*, and they place
agencies on the other side of the transaction as the *customer* using that
offering. An agency's own AD domains, SD-WAN fabric, DDI grid, and mainframe
...
> CISA ZTMM stage as a production requirement — ZTMM is a maturity model, and
...
| Machine-readable KSI evidence; no manual evidence assembly | FedRAMP 20x Consolidated Rules — **binds cloud service providers, not agencies**; relevant to the agency as a purchaser and as direction of travel | FedRAMP 20x JSON evidence format; 46 published indicators across 10 KSI families |
