...
**Problem.** "All sign-in activity comes from managed devices" is scored T3 —
a multi-quarter program — yet converting 50,000 domain-joined devices to Entra
hybrid join is a two-setting tenant switch. Taken ungoverned, the fleet
converts itself and silently spends a loss ledger: duplicate objects,
unattributable device-state changes, devices Conditional Access cannot see —
with nobody having decided anything.

**Solution.** The governed path reaches the same end state under UIAO's
active-governance control plane: enter at L1 Observe in report-only mode,
sequence OrgPath-scoped waves, and gate every L3 enforcement class on coverage
data and AO sign-off, so enforcement never runs ahead of the inventory it
depends on.

**Ask.** Find out whether the ungoverned path is already live (SCP + GPO
auto-registration, and since when); run the fleet against the loss ledger in
§3 before any Conditional Access policy enforces; and name the wave owner who
holds the plan, the sign-offs, and the rollback. The four steps are in the
Call to Action.
:::
...
| The wave's plan/apply mechanism, and its permanent human-review set | [ADR-084 §C4–C5](../../../src/uiao/canon/adr/adr-084-phase5-consumer-architecture.md) | `EntraDynamicGroupAdapter.plan()` / `apply(dry_run=...)` and `governance_review_ops` are the code-level shape of the L1/L2/L3 rungs above. |
...
- Companion narrative: [*Modernization Journey*](modernization-journey.qmd) — the control-plane/data-plane arc this paper sits inside.
