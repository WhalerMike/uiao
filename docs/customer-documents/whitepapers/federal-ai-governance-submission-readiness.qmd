...
2. **High-impact §V checklist.** Systems affecting rights, safety, or access to
   critical services must satisfy five requirements — pre-deployment testing,
   independent assessment, ongoing monitoring, a failsafe/override, and an
   appeal process — captured as six schema fields in the inventory (assessment
   and independent review are recorded as separate attestations).
...
![M-25-21 coverage ledger — deployed versus proposed](images/federal-ai-governance-submission-readiness-diagram-01-m2521-coverage-ledger.png){#fig-federal-ai-diagram-01 fig-alt="Nine-row status ledger titled M-25-21 Coverage Ledger — Deployed vs. Proposed, columns Requirement/Capability, UIAO Mechanism, State. Four teal Active rows above a red dashed \"deployed core above this line\" divider: owner anchor / OrgPath placement (Active, Microsoft), drift detection via the six-phase drift engine (Active), provenance-anchored evidence (Active), and SaaS deployment — Azure Container Apps ADR-096 with AWS CDK ADR-117 (Active Azure / IaC landed AWS). Below the divider, grey rows: AI system as governed identity record — AISystemRecord UIAO_196 with the ai_inventory L1 scanner (Scanner built, ADR-112 Proposed); ATO gate on credential issuance (Active / Proposed); multi-cloud / non-MS IdP storage via binding profiles (Proposed / Profile-only); agentic-AI P1 finding (Proposed); and an ice-blue Reserved row for the SailPoint NERM non-employee branch. Bottom evaluator note: demo surface today is Microsoft/Entra plus drift plus evidence; SailPoint and GCP/VMware bindings are roadmap; the AI scanner is built and awaits ADR-112 activation. White background, navy/teal/ice palette." width="100%"}

## Coverage ledger — deployed vs. proposed

| Requirement / capability | UIAO mechanism | State |
|---|---|---|
| AI system as governed identity record | AISystemRecord (UIAO_196) | Spec drafted (Spec4-AI-D1.1); L1 scanner implemented (`src/uiao/governance/ai_inventory/`); ADR-112 still Proposed |
...
| Multi-cloud / non-MS IdP storage | Binding profiles (ADR-098/099/113) | Proposed (IdP) / Profile-only (GCP, VMware) |
| Provenance-anchored evidence | Evidence Fabric, schema-enforced canon | **Active** |
| SaaS deployment | Azure Container Apps (ADR-096); AWS CDK (ADR-117) | **Active (Azure)** / IaC landed (AWS) |
...
- **Much of the AI/multi-cloud/partner story is `proposed`/`reserved`.** A demo
  ...
  AISystemRecord L1 scanner is implemented but its governing ADR-112 remains
  Proposed; SailPoint and the GCP/VMware bindings are pre-activation.
...
- **SaaS cloud footprint.** The Azure SaaS plane is active (ADR-096); an AWS
  CDK deployment surface landed under ADR-117 (ECS Fargate + ALB + RDS + S3)
  but has no operational tenant history yet, and there is no GCP IaC. UIAO
  reaches other clouds as an API client, not yet as a proven multi-cloud host.
