...
owner anchor, `agency_bureau` is an OrgPath node. UIAO's approach treats the
inventory as a governed discovery feed that gates the credential lifecycle, in
contrast to spreadsheet-style tracking that stops at reporting.
...
| ATO gate on credential issuance | ATO registry + reciprocity (ADR-054) | Active (reciprocity) / Proposed (AI gate) | Reciprocity is live in production; the AI-specific gate on top of it is not yet wired |
