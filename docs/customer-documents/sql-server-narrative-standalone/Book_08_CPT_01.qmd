...
::: {.callout-note}
## In this chapter
Why SQL Server's authentication transformation is incomplete until the upstream applications that consume it are addressed, how the LDAP-dependent-application migration plan's four-class taxonomy categorizes those applications by modernization capability, what each class means for both the user-authentication track and the SQL-connection track, and how the application-audit script produces the per-application classification data that drives the triage.
:::
...
The LDAP-dependent-application migration plan, accepted 2026-05-13, provides the four-class taxonomy for categorizing upstream applications by modernization capability, in a strict preference order: Class 3 (OIDC) preferred, then Class 2 (SAML), then Class 1 (App Proxy), then Class 4 (Entra Domain Services) as the last resort. The application-audit script (`Get-LDAPBindAccountInventory.ps1`) is the discovery instrument that produces the per-application classification data this chapter depends on.

![The four application classes in the LDAP-dependent-application migration plan's preference order and the two migration tracks each one carries](images/book08cpt01-diagram-01-four-class-pattern.png){#fig-book-08-cpt-01-diagram-01 fig-alt="A horizontal engineering blueprint on a white background showing four application-class boxes arranged left to right in preference order: Class 3 OIDC labeled 'preferred — full LDAP elimination' in teal, Class 2 SAML labeled 'Entra as SAML IdP', Class 1 App Proxy labeled 'cannot modernize directly', and Class 4 Entra Domain Services labeled 'last resort — preserves AD dependency' in red. Below each class box two parallel horizontal tracks run: an upper track labeled 'user authentication' and a lower track labeled 'SQL connection — Managed Identity or service principal'. The Class 3 box shows both tracks merging into a single navy 'no AD, no LDAP, no stored credential' endpoint; the Class 4 box shows the SQL connection track blocked by a red bar labeled 'blocks Entra-only posture'. Engineering blueprint style, white background, federal navy (#1F3A5F) for boxes, teal (#1A9E8F) for OIDC and preferred annotations, red (#C0392B) for Class 4 and least-preferred annotations, monospace labels, no human figures, 16:9 landscape." width="100%"}
...
That record is the input to the LDAP-dependent-application migration plan triage and to the instance-level migration planning in Book 04. The two audit scripts must be correlated to produce a complete picture: `Get-SQLServerAuthAudit.ps1` enumerates SQL Server instances, and the application-audit script enumerates the upstream applications. Only when both are joined does each SQL Server instance reveal its full authentication chain — every application that depends on it, and the class each one falls into.

::: {.callout-note title="What Book 08 establishes — Chapter 1"}
SQL Server's authentication transformation is incomplete until the upstream applications that consume it are addressed as a full chain — user to application to database. The LDAP-dependent-application migration plan's four-class taxonomy ranks those applications by modernization capability: Class 3 (OIDC, full LDAP elimination) preferred, then Class 2 (SAML, Entra as IdP per the SAML identity-provider standard), then Class 1 (App Proxy, cannot modernize directly), then Class 4 (Entra Domain Services, last resort, which preserves the AD dependency and blocks the host instance from Entra-only posture). Every class carries two independent tracks — user authentication and the SQL connection — and the application-audit script produces the per-application classification data that drives the triage.
:::
