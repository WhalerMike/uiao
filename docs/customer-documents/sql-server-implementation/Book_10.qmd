...
subtitle: "Run the real Spec3-D1.10 certificate audit to find SQL Server's three ADCS dependencies — TLS, certificate-mapped logins, and CBA as the NTLM-replacement posture — and sequence the replacement-CA work, before any ADCS decommission date is set"
description: "How to discover, ahead of an ADCS decommission, the certificate dependencies that break SQL Server if the certificate authority is removed without a replacement. Documents the shipped Spec3-D1.10 Get-CertBasedAuthAudit audit with its real parameters, the six analysis sections, and its machine/user-certificate and PKI output schema, then maps the output to SQL Server's three dependency classes and gives the dependency-ordered CA-migration planner (New-UIAOPKIMigrationPlan). Analytical and motivational — UIAO_135 §3.3 marks the ADCS-to-Cloud-PKI transformation Not Yet Defined and no normative ADR governs it; the SQL-specific cert extraction and the CBA-issuance bridge are marked [to build]."
...
`[to build]` — a SQL-specific extraction that, for each instance from the Book 03 D1.8 audit, reads the engine's bound TLS certificate (subject, SAN, **issuing CA**, expiry) and queries `sys.certificates` / `sys.server_principals` for type-C certificate-mapped logins and their issuing CA, then joins to the D1.10 PKI inventory to flag every ADCS-issued certificate the instance depends on. The instance-side queries are simple reads:
...
Narrative Book 10 Chapter 1 names three distinct SQL Server capabilities that depend on ADCS-issued certificates. The D1.10 output (the issuing CA on every certificate) plus the `[to build]` instance-side reads classify each:
...
This entire book is **read-only**: the D1.10 audit reads AD and (with `-IncludePKIAudit`) CA configuration; the `[to build]` instance-side queries are inventory reads; `New-UIAOPKIMigrationPlan` reads an assessment and emits a plan. Nothing here issues, binds, or revokes a certificate, and nothing decommissions a CA — those are governed actions awaiting the ADCS-migration ADR.

---

*Companion to [narrative Book 10](../sql-server-narrative/Book_10.qmd). Discovery script-backed: `tools/discovery/Spec3-D1.10`; CA-sequencing via `tools/powershell/UIAOPlanGenerators` (`New-UIAOPKIMigrationPlan`). Analytical — no normative ADCS-migration ADR exists; the SQL-specific cert extraction and the CBA-issuance bridge are [to build]. Canon: UIAO_135 §3.3, ADR-068.*
