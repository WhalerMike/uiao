...
subtitle: "The Phase-0 runbook for building the estate inventory and the retain / consolidate / retire classification that gates every later book — with the general-purpose discovery instruments, because the canonical Spec does not exist yet"
description: "How to assess and consolidate a twenty-year SQL Server estate before any login is touched. Documents the real discovery the audit family already provides (Spec3-D1.8 instance enumeration, Spec1-D1.5 MSSQLSvc SPN trail) and gives the verified pattern for the broader estate sweep that has no canonical Spec yet — dbatools Find-DbaInstance, the SPN trail, Azure Migrate / MAP / DMA — then the utilization, version, and dependency lenses that produce the retain / consolidate / retire decision ADR-091 §1 makes the gate for the whole series. Read-only discovery; consolidation execution is review-required."
...
::: {.callout-important}
## Gap notice — pattern, with partial script backing
There is **no canonical `Spec3-D1.x` estate-discovery instrument** — narrative Book 02 Chapter 2 names this as the ADR-091 §1 **Phase 0** gap. The authentication audit (`Spec3-D1.8`, Book 03) enumerates instances that are already known and measures their *auth posture*; estate discovery runs earlier and casts wider to establish the **denominator**. This book documents the real coverage the shipped scripts already give you, then gives the verified **pattern** for the broader sweep, marking the to-be-built automation **[to build]**.
:::
...
### 1b — The broader sweep (pattern — no canonical Spec yet)

The shipped scripts find Kerberos-configured and registry-enumerable instances. They do **not** find a SQL Express instance an application installer dropped on an app server, nor do they capture per-database utilization. Close that gap with the general-purpose instruments narrative Book 02 Chapter 2 names:
...
`[to build]` — the canonical **Phase 0** `Spec3-D1.x` estate-discovery script: a single pass that fuses `Find-DbaInstance`, the `MSSQLSvc` SPN trail, and per-database last-access into one inventory object whose output feeds the **same CCM-BIR ingestion pipeline** as the authentication audit (the way D1.8 / D1.5 already do). Until it exists, the sweep is a deliberate **program artifact**, not a canonical telemetry record — record it as such.
...
Every script and query in this book is **read-only** discovery against live instances and AD. The shipped `Spec3-D1.8` (discovery-only) and `Spec1-D1.5` change nothing; `Find-DbaInstance`, `setspn -Q`, and the DMV queries are inventory reads. Nothing here alters a production security posture — that begins in Book 04 (Arc) and Book 05 (logins), and only on the **retain** / **target** set this book defines.

---

*Companion to [narrative Book 02](../sql-server-narrative/Book_02.qmd) and ADR-091 §1. Partial script backing — `tools/discovery/Spec3-D1.8` (discovery-only), `Spec1-D1.5`; the fused Phase-0 estate-discovery instrument is [to build]. Canon: ADR-091 §1, UIAO_135 §3.2.*
