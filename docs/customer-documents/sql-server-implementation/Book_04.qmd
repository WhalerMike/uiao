...
description: "A pattern runbook for Azure Arc enablement of SQL Server hosts: prerequisites and egress for the GCC-Moderate boundary, azcmagent enrollment, IMDS managed-identity token validation, the Azure extension for SQL Server, and a verification checklist. This step has no shipped automation yet — the book gives the exact commands, validation, and rollback, and marks the to-be-built automation explicitly."
...
::: {.callout-important}
## Gap notice — pattern, not shipped script
Unlike the audit (Book 03), there is **no Arc-deployment script in the repository yet**. This book is the verified *pattern* — the exact commands, the validation steps, and the rollback — written so it can be executed by hand today and codified into a runbook module later. Where automation is the right end state, it is called out as **[to build]**.
:::
...
`[to build]` — wrap this in an idempotent onboarding function (skip if already `Connected`, emit a sealed evidence record) consistent with the `UIAO*` module pattern in `tools/powershell/`.
...
`[to build]` — a verification wrapper that confirms the extension provisioning state and that the instance reports Entra-auth capable.
...
*Companion to [narrative Book 04](../sql-server-narrative/Book_04.qmd). No shipped script yet — verified pattern; automation marked [to build]. Canon: ADR-002, ADR-004, UIAO_135 Transformation #7.*
