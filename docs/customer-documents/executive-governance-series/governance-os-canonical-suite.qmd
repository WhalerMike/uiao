...
| `Update-OrgAttributes` | Stamps the governance facets and the derived path in one Graph update --- `Update-MgUser` with `onPremisesExtensionAttributes` for users, `Update-MgDevice` with `extensionAttributes` for devices, because the property differs by object type |
