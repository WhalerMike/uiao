...
![Four structural failure-mode silos with missing-substrate bar](images/index-image-02-four-structural-failure-mode-sil.png){#fig-index-image-02 fig-alt="Four structural failure-mode silos with missing-substrate bar. Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for primary structural elements, teal (#1A9E8F) for secondary/integration elements, amber (#D4A017) for governance/audit/registry overlays, monospace labels for identifiers, no human figures, 16:9 landscape." width="100%"}
...
**At a glance — the four failure modes and where the fix actually lives:**

| Failure mode | Looks like a… | Actually requires (substrate-level) |
|---|---|---|
| Fragmented identity context | directory problem | Canonical resolution rules that cross all pillars |
| Uncorrelated telemetry | SIEM problem | A canonical claim shape every adapter emits before the SIEM |
| Point-in-time evidence | GRC-tool problem | Deterministic regeneration from canon plus telemetry |
| Tool-shaped policy | policy-engine problem | Separating policy intent (canon), mapping (registry), and enforcement (runtime) |
...
Next: [Chapter 2 — The Compliance Paradox](../ch02-compliance-paradox/index.qmd) — why compliance-driven programs accumulate risk *between* audit cycles, and how deterministic evidence collapses the gap.
:::
