...
![Drift hierarchy with five-class taxonomy strip](images/index-image-02-drift-hierarchy-with-five-class.png){#fig-index-image-02 fig-alt="Drift hierarchy with five-class taxonomy strip. Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for primary structural elements, teal (#1A9E8F) for secondary/integration elements, amber (#D4A017) for governance/audit/registry overlays, monospace labels for identifiers, no human figures, 16:9 landscape." width="85%"}

![Detect > classify > remediate > emit-as-evidence loop with adapter and CI feeds](images/index-image-03-detect-classify-remediate-emit-a.png){#fig-index-image-03 fig-alt="Detect > classify > remediate > emit-as-evidence loop with adapter and CI feeds. Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for primary structural elements, teal (#1A9E8F) for secondary/integration elements, amber (#D4A017) for governance/audit/registry overlays, monospace labels for identifiers, no human figures, 16:9 landscape." width="85%"}
...
> **The fix is not "watch for the events" — it is to make drift itself a
> first-class signal so the events have nowhere to hide.**
...
| Severity | Typical class | Response |
|---|---|---|
| **P1** | Authorization or identity drift | Halt and alert immediately |
| **P2** | High-severity drift | Auto-remediate when deterministic; otherwise escalate within an hour |
| **P3 / P4** | Medium / low drift | Queue for auto-remediation within 24 / 72 hours |
...
Next: [Chapter 4 — Deterministic Governance](../ch04-deterministic-governance/index.qmd) — what it takes to make every drift finding assessor-reproducible, byte-for-byte.
:::
