...
of which **19 map 1:1** to a CR26 indicator (`OrgComp_CR26_Reconciliation.md`). The
...
series' internal KSI decomposition"; mapping to CR26 is 19 of 46.
  ...
  3. NUMBER->THEME MISBINDING (blocking): a KSI-0NN indicator number bound to the
     ...
     is derived from OrgComp_CR26_Reconciliation.md (the SSOT), for KSI-011..029
...
# number->theme map is derived from OrgComp_CR26_Reconciliation.md (the SSOT), so
...
def _recon_rows() -> list[tuple[str, list[str]]]:
    recon = HERE / "OrgComp_CR26_Reconciliation.md"
            ...
            for mm in rx.finditer(line):
                ...
                if canon and canon != theme:
                    errors.append(
                        ...
                        f"here to {theme} — see OrgComp_CR26_Reconciliation.md: "
                        f"«{line.strip()[:70]}»"
                    )
            ...
            for num in re.findall(r"KSI-0\d\d", mm.group(2)):
                ...
                if canon and canon != theme:
                    errors.append(
                        ...
                        f"'{theme}' row claims it — see OrgComp_CR26_Reconciliation.md: "
                        f"«{line.strip()[:70]}»"
                    )
                ...
                for m in IND_RE.finditer(line):
                    ...
                    if m.group(0) not in valid:
                        errors.append(
                            ...
                            f"(catalog has {len(valid)}; see OrgComp_CR26_Reconciliation.md)"
                        )
                ...
                for tm in THEME_RE.finditer(line):
                    ...
                    if theme not in canon:
                        errors.append(
                            ...
                            f"see OrgComp_CR26_Reconciliation.md: «{line.strip()[:60]}»"
                        )
