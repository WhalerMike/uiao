---
title: "Vol IV Book 05 — Federal Organization Compliance — Cybersecurity Training & Awareness"
...
aliases:
  - /customer-documents/federal-aan-series/Vol_IV_Book_05_FedAAN_Cybersecurity_Training_Awareness.html
...
::: {.aan-note}
**Scope — control closure here; the academy in Volume V** — This book specifies the **AT-family training program and its control closure** (AT-2, AT-3, AT-4, CP-3, IR-2) and the shape of the KSI-CED training record. The full curriculum, labs, assessment rubrics, and certification are delivered by **Volume V — Training & Certification** (`Vol_V_Book_00_OrgComp_Training_Certification_Overview.qmd`), which operationalizes this program and produces the `training-effectiveness-record` that evidences the KSI-CED closure. This book remains the AT-family control-closure reference; Volume V is the enablement layer that teaches and certifies it. The two cross-reference rather than duplicate.
:::
...
dimensions. This is the last remaining KSI that the series
...
1. **Training Program Architecture** — the four training dimensions this
   program decomposes KSI-CED-RAT review into, mapped to AT-2, AT-3, AT-4,
   CP-3, and IR-2
...
4. **Training Records and LMS Integration (AT-4)** — machine-readable
   ...
   export format for the governance substrate
...
Together these artifacts close five base NIST SP 800-53 Rev 5 controls plus
four enhancements (nine closure rows) across the AT, CP, and IR families, activate the final scaffold KSI in the internal
...
The four training dimensions this program decomposes KSI-CED-RAT review into:
...
completion is recorded in a form that the governance substrate can
...
| **AT-2(2)** | Insider Threat | Insider threat module within annual awareness program; included in LMS completion record |
...
| **AT-4** | Training Records | LMS export in JSON/CSV format; monthly snapshot to governance substrate |
...
| **HR / Training Coordinator** | Manages LMS enrollment, completion tracking, and export to governance substrate |
...
| **Insider Threat Awareness** | Behavioral indicators, reporting obligations, NITTF minimum standards / agency insider-threat program (EO 13587) | AT-2(2), EO 13587 |
...
|----------|-----------------|------|-----------------|
| **Microsoft Viva Learning** | Native GCC / GCC High | M365 E3/E5 included | JSON via Graph API; exports to governance substrate directly |
...
| **SANS Security Awareness** | Commercial; FedRAMP-authorized | Licensed | API export; direct governance substrate integration possible |
...
be automated into the governance substrate on a monthly pull cadence.
...
: Tier 1 Sysadmin Training — OrgComp-Aligned Curriculum {.striped .hover}
...
These four quarterly exercises test the CP-2 Contingency Plan established in
Vol IV Book 01. Q3 is the full CP-3 tabletop (contingency-role training);
Q1, Q2, and Q4 are functional failover tests that support CP-4 (Contingency
Plan Testing):
...
: CP-3 / CP-4 Quarterly Exercise Calendar {.striped .hover}
...
(Vol IV Book 03) and the CP-3 evidence record that the governance substrate ingests.
...
machine-readable — the governance substrate needs to query whether all
...
This means the LMS must produce an exportable record that the governance engine can
...
The training-effectiveness-record exported to the governance substrate follows
this schema:

```json
{
  "record_type": "training-effectiveness-record",
  "snapshot_date": "2026-10-01",
  "generated_by": "Microsoft Viva Learning Graph API export",
  "overall_completion_pct": 98.3,
  "cp3_tabletop_date": "2026-08-14",
  "ir2_drill_date": "2026-09-09",
  "role_tiers": [
    { "tier_name": "sysadmins",         "tier_completion_pct": 100.0, "last_completion_date": "2026-09-20", "nist_controls": ["AT-3", "AT-3(3)"] },
    { "tier_name": "privileged_access", "tier_completion_pct": 96.4,  "last_completion_date": "2026-09-22", "nist_controls": ["AT-3", "AT-3(5)"] },
    { "tier_name": "developers",        "tier_completion_pct": 97.8,  "last_completion_date": "2026-09-18", "nist_controls": ["AT-3", "AT-3(3)"] },
    { "tier_name": "ir_responders",     "tier_completion_pct": 100.0, "last_completion_date": "2026-09-09", "nist_controls": ["IR-2", "CP-3"], "ir2_drill_date": "2026-09-09" },
    { "tier_name": "data_stewards",     "tier_completion_pct": 96.8,  "last_completion_date": "2026-09-25", "nist_controls": ["AT-3"] },
    { "tier_name": "executives",        "tier_completion_pct": 100.0, "last_completion_date": "2026-09-15", "nist_controls": ["AT-2", "AT-3"] }
  ],
...
![AT-4 Evidence Collection Pipeline — governance.training-effectiveness-record](figs/ced-fig-02-evidence-pipeline.png){fig-alt="Flow diagram showing four stages of the AT-4 evidence collection pipeline. Stage 1 (dark navy): LMS (Viva Learning or equivalent) stores enrollment and completion records. Stage 2 (federal blue): JSON Schema Export produces the training-effectiveness-record document with six role_tier entries carrying completion percentages and dates. Stage 3 (dark green): Governance Substrate ingests the record via the Conformance Adapter. Stage 4 (dark red): KSI-CED-RAT Verdict is produced by calling slot_binding(KSI-CED-RAT, confidence=high) — result is PASS when all thresholds are met. Below the flow, a detail panel shows required schema fields: top-level fields include overall_completion_pct (must be at least 95), snapshot_date, cp3_tabletop_date (within 12 months), ir2_drill_date (within 12 months), and role_tiers array; per-tier fields include tier_name, tier_completion_pct (must be at least 95 per tier), last_completion_date (within 12 months), nist_controls array, and ir2_drill_date for IR responder tier. Six tiers required: sysadmins, privileged_access, developers, ir_responders, data_stewards, executives." width="100%"}
...
# Filter by course IDs in the OrgComp training program
$aanCourses = $response.value | Where-Object {
    $_.courseId -in @("OrgComp-AT2-2026", "OrgComp-AT3-SYSADMIN-2026",
                       "OrgComp-AT3-DEV-2026", "OrgComp-AT3-IR-2026")
}

# Export to JSON for governance substrate ingestion
$aanCourses | ConvertTo-Json -Depth 5 | Out-File "training-record-$((Get-Date).Year)Q$([math]::Ceiling((Get-Date).Month/3)).json"
```

### Automated Monthly Export

The training completion export should run on a monthly schedule so the
governance substrate always reflects current completion status. The recommended
pattern:

1. **Logic App or Azure Automation runbook** triggers on the 1st of each month
2. **Graph API call** retrieves completion records for all OrgComp training courses
3. **Transform** maps Graph API response to the `training-effectiveness-record` schema
4. **Storage** writes the JSON to the governance record store (Azure Storage, GCC-compliant)
5. **governance substrate pull** ingests the record on the next evidence collection cycle
...
| **AT-4** | Training records | LMS-agnostic `governance.training-effectiveness-record` JSON schema (the evidence contract); current adapter: Viva Learning Graph API export; monthly automated pull to governance substrate |
...
| **Total** | | **9 closure rows** (5 base + 4 enhancements) |
...
rule in the internal CR26 ruleset. Its activation requires a
...
When the monthly LMS export is running and the governance substrate is ingesting
...
`training-effectiveness-record` is ingested by the governance substrate:
...
- **CR26 indicator coverage:** **19 of 46** CR26 indicators explicitly mapped 1:1
  ...
  [`OrgComp_CR26_Reconciliation.md`](OrgComp_CR26_Reconciliation.md).
...
eleven-volume Federal Organization Compliance series is **self-assessed against
...
: OrgComp Series — Volumes I–IV Control-Closure Summary as of This Book {.striped .hover}
...
- [ ] Integrate export into governance substrate
...
|----------|-------------|
| OrgComp — {{< meta agency.short >}} Landing Zone IPAM / FedRAMP | Vol I Book 01: InfoBlox DDI, four-environment landing zone, NIST control mapping, FedRAMP 20x KSI evidence |
| OrgComp — Federal Telecommunications Modernization | Vol I Book 06: Amazon Connect, Teams Phone, SD-WAN/SASE, G.114 compliance |
| OrgComp — Network Modernization | Vol I Book 05: AD-to-Entra migration, ZTNA, Active Governance, FedRAMP 20x alignment |
| OrgComp — SQL Server Authentication Modernization | Vol II Book 01: Arc Managed Identity, Kerberos/NTLM elimination, login migration |
| OrgComp — SQL Server Implementation Guide | Vol II Book 02: 10-book runbook — estate discovery, Arc deployment, monitoring |
| OrgComp — Database Consolidation and Network Physics | Vol II Book 03: SSOT architecture, speed-of-light floor, SD-WAN for database traffic |
| OrgComp — Privileged Access Management | Vol III Book 01: Entra PIM JIT, PAW architecture, AC-6 least privilege |
| OrgComp — Vulnerability Management | Vol III Book 02: Defender MDVM, CISA KEV, BOD 26-04 SLAs, RA-5 continuous scanning |
| OrgComp — Data Protection and Purview | Vol III Book 05: Purview DLP, sensitivity labels, Key Vault CMK |
| OrgComp — SIEM / XDR / Detection Engineering | Vol III Book 06: Sentinel connectors, KQL analytics, UEBA, Logic App playbooks |
| OrgComp — Business Continuity | Vol IV Book 01: CP-2 Contingency Plan, BIA, SQL AG and DDI DR integration |
| OrgComp — Supply Chain Risk Management | Vol IV Book 02: Syft SBOM, Grype VDR, OpenVEX, GitHub Actions pipeline |
| OrgComp — Program Management and Governance | Vol IV Book 03: ISPP, POA&M, SCRM Charter, PM-30 closure |
| OrgComp — PII Processing and Transparency | Vol IV Book 04: PT family templates, Privacy Program Plan, Purview Data Map |
