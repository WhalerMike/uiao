...
single compliance spine (orgcomp-compliance-spine.yml).
...
SPINE = Path(__file__).with_name("orgcomp-compliance-spine.yml")
...
def render_book(spine: dict, book_id: str) -> str:
    ...
    lines = [f"<!-- authorities:{book_id} — generated from orgcomp-compliance-spine.yml; do not hand-edit -->"]
    lines.append(header)
    lines.append(sep)
