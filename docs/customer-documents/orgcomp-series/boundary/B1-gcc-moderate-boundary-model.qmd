...
**Closed 2026-09-09 — and the answer is High, not Moderate.** The Marketplace
listing was read directly (see
[FINDING-PGM-006](../../../findings/gcc-marketplace-listing-class-d-in-process.qmd)).
**Microsoft 365 Government Community Cloud & Supporting Services**, package
`MSO365MTA`, is listed at **Class D (High)** with status **FedRAMP In Process**
as of 2026-07-20 and **zero authorizations** — having moved *from* FedRAMP
Certified on 2026-07-01. The label "GCC-Moderate" is therefore wrong about the
level, not merely unqualified about it, and the earlier report that GCC is
listed as "Office 365 (Commercial)/Multitenant" is outdated.

Two cautions carry forward. The parenthetical "(High)" is FedRAMP's own
Marketplace rendering of Class D; FedRAMP separately cautions that the 2026
classes are not one-for-one replacements for the legacy impact levels, so cite
the **class**. And a Marketplace listing is a snapshot of a moving process —
this one changed twice in July 2026 — so re-read it before it carries an ATO
narrative.
:::
