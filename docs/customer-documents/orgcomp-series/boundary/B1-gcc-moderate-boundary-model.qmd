...
**Closed 2026-09-09 — the level is Moderate, and this doc's usage was right.**
The Marketplace was read directly (see [FINDING-PGM-006](../../../findings/gcc-marketplace-listing-class-d-in-process.qmd)).
The certified offering is package **`MSO365MT`** — *Microsoft 365 Government
Community Cloud & Supporting Services*, **Class C (Moderate)**, FedRAMP
Certified since 2014-11-20, with **99 authorizations** across roughly ninety
agencies. So the standing caution here is about **vocabulary, not level**:
"GCC Moderate" is not a Microsoft product name — Microsoft's term is Government
Community Cloud — but Moderate is the right level for the certified service.

**Two things this doc did not know.** A *second* listing shares the identical
name, `MSO365MTA`, at **Class D (High)**, FedRAMP In Process, zero
authorizations — Microsoft appears to be pursuing a High certification for GCC
alongside the existing Moderate one. And **GCC High** is a third package,
`FR1824057433`, Class D (High), certified 2024-12-26.

Two cautions carry forward. FedRAMP renders Class C as "(Moderate)" in its own
UI while separately cautioning that the 2026 classes are not one-for-one
replacements for legacy levels — so cite the **class**. And a Marketplace record
is a snapshot: `MSO365MTA` changed status twice in July 2026.
:::
