---
title: "Vol VII Book 02 — Federal Organization Compliance — M365 Federal Control Compliance Automation"
...
aliases:
  - /customer-documents/federal-aan-series/Vol_VII_Book_02_FedAAN_M365_Control_Compliance_Automation.html
...
Generated from the series compliance spine (`orgcomp-compliance-spine.yml`) by `render_authorities_table.py`; not hand-edited here. These are *coordination* closures — the M365 control state operated as tracked work — not a re-closure of the identity/data mechanisms Volumes I, III, and VI already own.
...
Draft. Microsoft 365, Graph, SCuBA/ScubaGear, and Purview are deployed examples; the detect→raise→actuate-native→close→evidence loop is the invariant. Vendor claims cite vendor documentation (see References) and are dated, not permanent — see the [Vendor Currency Doctrine](Vol_0_Book_00_OrgComp_Executive_Summary.qmd#vendor-currency). The compliance-loop figure is authored as committed house-style SVG; further per-book figures follow in the PPTX→DOCX sync pass.
