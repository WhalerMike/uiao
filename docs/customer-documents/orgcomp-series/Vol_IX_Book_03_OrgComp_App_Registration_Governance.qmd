---
title: "Vol IX Book 03 — Federal Organization Compliance — App Registration Governance"
...
aliases:
  - /customer-documents/federal-aan-series/Vol_IX_Book_03_FedAAN_App_Registration_Governance.html
...
This book governs the whole lifecycle as catalog work, closing AC-2, IA-5(2), SC-17, and AC-6, and feeding Vol VII Book 04 attestation.

# The Lifecycle

![The app-registration lifecycle — request, scoped consent, short-lived credential, owner-approved rotation, attestation](figs/vol9b03-fig-01-appreg-lifecycle.png){#fig-vol9b03-lifecycle fig-alt="A house-style five-step pipeline titled App Registration Governance (Vol IX Book 03), subtitle 'App registrations as machine JML — request, consent, credential, rotate, attest', on a white background. Five navy-headed cards joined left to right by teal arrows: Step 1 Request — owner, purpose, and the specific scopes needed (AC-2); Step 2 Consent — scoped admin consent, privileged scopes gated by a security approver (AC-6); Step 3 Credential, drawn with a teal header and an amber ring labeled 'short-lived — no long-lived secrets' — a short-lived certificate (ACME) or a secret with a mandatory expiry; Step 4 Rotate — raised automatically before expiry, the owner approves, not a human's memory; Step 5 Attest — ownership and scope re-attested, orphaned or expired registrations flagged (feeds CA-7). A footer states no long-lived secrets in config — certificate-based, short-lived workload identity (IA-5(2) / SC-17), Moderate and GCC Moderate. White background. Authored house-style diagram." width="100%"}
...
| **2. Consent** | Scoped, admin-consented permissions only — no blanket delegation; any privileged or write scope requires a **security approver**, least privilege the default | AC-6 |
| **3. Credential** | A **short-lived certificate (ACME)** or a secret with a **mandatory expiry** — no long-lived secrets in config | IA-5(2), SC-17 |
| **4. Rotate** | Rotation is raised automatically before expiry and approved by the owner — the app never depends on a human *remembering* | IA-5(2) |
| **5. Attest** | Ownership and scope re-attested on a cadence; orphaned or expired registrations flagged | AC-6, CA-7 |

::: {.aan-important}
**No long-lived secrets.** The single highest-value rule in this book: credentials are short-lived and rotated on schedule. A client secret with a two-year expiry sitting in a pipeline variable is the credential an attacker wants most; certificate-based, short-lived identity removes the target. This is the IA-5(2) / SC-17 closure, applied to workload identity.
:::

# App Registrations Are Machine JML

The lifecycle above is deliberately the same shape as the human joiner/mover/leaver catalog (Book 01): a request with an owner, a governed change of scope, and an evidenced retirement. The **leaver** step is the one most often skipped for apps — an orphaned service principal with live credentials is exactly the ghost-credential risk that human de-provisioning is meant to close. Attestation (stage 5) is the machine-identity equivalent of an access review.
...
<!-- authorities:book-day2-appreg â€” generated from orgcomp-compliance-spine.yml; do not hand-edit -->
...
| IA-5(2) | Public Key-Based Authentication | App-registration credentials issued as short-lived certs (ACME) with mandatory expiry and scheduled, owner-approved rotation â€” no long-lived secrets in config | FedRAMP Moderate | NIST SP 800-53 Rev 5; EO 14028 | Identity | KSI-IAM | No |
...
: Authorities Closed Here â€” App Registration Governance (request Â· consent Â· secret/cert lifecycle) (â€  = Closure-Necessity anchor: no alternate closure path) {.striped .hover}
