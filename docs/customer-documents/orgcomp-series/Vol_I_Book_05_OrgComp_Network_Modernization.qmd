---
title: "Vol I Book 05 — From Mainframe to Application-Aware Modernization"
...
date: "2026-07-18T00:33:00-04:00"
format:
  html:
    css: lz-style.css
...
aliases:
  - /customer-documents/federal-aan-series/Vol_I_Book_05_FedAAN_Network_Modernization.html
...
::: {.aan-note}
**Relationship to the published whitepaper.** This narrative is also
published, in provenance-anchored form, as the whitepaper
[Modernization Journey](../whitepapers/modernization-journey.qmd),
whose closing section maps each Act-4 claim line-by-line to the governance
engine's decision records. That publication owns the narrative and its
provenance mapping; this book owns the federal control-closure framing
(NIST 800-53 mappings, FedRAMP 20x KSI alignment, TIC 3.0) and the
Appendix A federal implementation reference.
:::
...
> **The physics that cannot be negotiated:** Portland to Washington is approximately 4,500 km in a straight line. Actual fiber routing adds roughly 20%, bringing the real fiber distance to approximately 5,400 km. At 200,000 km/second — the speed of light through glass fiber — that distance sets an unavoidable propagation floor of 27 ms one-way, 54 ms round-trip minimum, before any queue delay, security inspection, or application processing. Active Directory has **no millisecond latency requirement** — the widely-quoted five-minute Kerberos figure is clock-skew tolerance, not a latency budget — but the DC-dependent operations feel this distance directly: initial TGT acquisition at logon, SYSVOL/GPO retrieval, password changes, and hybrid Entra join registration. **No amount of bandwidth changes this. Bandwidth is volume. Latency is distance. You cannot buy your way out of geography.**
...
![Asymmetric routing — broken with perfect latency, structurally removed by the overlay](figs/mf-fig-17-asymmetric-routing.svg){fig-alt="Diagram of the asymmetric routing failure. A client sends a SYN along the outbound path through stateful firewall A, which creates a session entry, to the server. The server's SYN-ACK returns along a different route through stateful firewall B, which sees a SYN-ACK with no session entry and drops it — the TCP three-way handshake never completes. Right panel, the SD-WAN fix: the overlay tunnel is one logical connection — both directions enter and exit the same edge device, and asymmetry below the tunnel is structurally removed. Bottom panel: independent killers that compound — latency kills sessions by exceeding protocol timeouts; asymmetric routing kills them by preventing the handshake from completing at all, even with perfect sub-millisecond latency on both paths; in environments where both conditions exist simultaneously — the federal standard — they compound. White background. Source: Vol I Book 05 Network Modernization, Date Code 2026-07-28." width="100%"}

## Where the Inline Tier Went: The Dissolution of the Load Balancer

Before leaving Act 2, follow its signature artifact to its ending. The load balancers that became inline proxies — and the forward proxies, VPN concentrators, and hardware application delivery controllers stacked beside them — do not survive into the modern architecture. But they do not migrate to any single successor either, and that is where modernization plans most often make a category error. The inline tier **dissolves into three separate functional planes**, and each of its legacy functions lands in a different one:

| Legacy function | Dissolves into | Functional plane |
|---|---|---|
| Forward proxy / VPN concentrator / secure web gateway appliance | SASE/SSE — cloud-delivered SWG, CASB, ZTNA | Policy enforcement plane |
| Hardware ADC / reverse proxy / TLS offload | Cloud-native delivery services provisioned in the landing zone (application gateways, cloud load balancers, global front doors) | Application delivery plane — a landing-zone function |
| GSLB / global traffic steering | DNS-based traffic steering — health-checked, policy-weighted authoritative DNS responses | Naming and addressing plane — a DDI control-plane function ([Vol I Book 01](Vol_I_Book_01_OrgComp_Cloud_Landing_Zone_IPAM_FedRAMP.qmd)) |

: The Dissolution of the Load-Balancer / Proxy Tier {.striped .hover}

The category error this table pre-empts: "SD-WAN replaces the load balancer." It does not, and claiming it does hands skeptics an easy rebuttal. **SD-WAN does not replace the load balancer; it retires the WAN edge. The load-balancer tier dissolves upward into cloud-native delivery services and downward into the DNS control plane.** The remote-access half of the tier — the forward proxies and VPN concentrators that extended per-network trust — dissolves sideways into the SASE/SSE policy enforcement plane described in Act 4. The global-steering half was always a DNS decision executed by an appliance; returned to the naming plane, it inherits the DNSSEC signing, IaC management, and audit logging that plane already carries. Nothing on the list needs a like-for-like appliance replacement, because none of the functions was ever really about the box.
...
> **The three-step every DIA conversation must survive.** (1) *What DIA solves:* exactly one thing — the geographic hairpin. The latency budget is a physics constraint, and local breakout is the only fix. (2) *What DIA satisfies:* none of the protection controls claimed near it. DIA is a pipe; SC-7, SC-8, AC-4, SI-4, and CA-7 are satisfied by the SD-WAN, SASE, and TIC 3.0 stack that governs the pipe, never by the pipe itself — the one closure transport selection provides is CP-8 (Telecommunications Services), through the multi-transport diversity this book's multipath design supplies. (3) *What DIA breaks if deployed bare:* the TIC boundary. A bare DIA circuit is an SC-7 regression against the MPLS/TIC baseline it replaced — from the assessor's chair, it is *worse* than the hairpin it fixed.
...
> **The vacuum advantage:** Light travels through optical fiber at approximately 200,000 km/second — two-thirds the speed of light in vacuum, due to the refractive index of glass. Light travels through the vacuum of space at 299,792 km/second. For the same physical path length, a signal through space travels **50% faster** than a signal through fiber — it covers the same distance in about one-third less time. This is not a technology gap that better fiber will close. It is physics.

For the longest domestic paths — Pacific Northwest to mid-Atlantic — the terrestrial fiber route may traverse 4,000 km or more of physical cable. The LEO vacuum path traverses 550 km up, geometrically direct inter-satellite laser link hops through space, and 550 km down at propagation speed 50% higher than fiber. **Under the right conditions the satellite path is not a compromise — it is the physics-optimal path.** Two caveats bound the claim: inter-satellite laser routing is not guaranteed on domestic paths (traffic may be bent down to a nearby ground station and ride terrestrial backbone anyway), and satellite-to-satellite handovers introduce jitter that matters for voice — the G.114 jitter-buffer analysis in Vol I Book 06 §2.3 applies. The overlay's continuous per-path measurement is what makes LEO usable despite both: it steers voice off the satellite path when jitter degrades and back when it recovers.

## 5G: Lower Latency Through Architecture, Not Just Physics

5G reduces latency through two distinct mechanisms:

**The radio mechanism:** 5G's shorter radio slot duration reduces the fundamental waiting time — approximately 0.5 ms slots at the mid-band sub-6 GHz numerology (30 kHz subcarrier spacing), shrinking to 0.125 ms at mmWave (120 kHz), versus 1 ms in LTE. Practical mid-band sub-6 GHz 5G achieves 5–10 ms across the air interface.
...
![Why Kerberos Breaks in the Cloud: The Session Contract Assumption](figs/mf-fig-10-kerberos-breaks.png){fig-alt="Two-panel comparison diagram. Left panel, Kerberos 1988 design on-premises: an AD domain controller and a user at a desk share the same LAN; a Kerberos ticket is issued and authentication succeeds on a short round-trip to the KDC. Right panel, Kerberos in the 2020+ cloud world: the user now works in a cloud tenancy while the domain controller remains on-premises; the authentication path crosses the WAN and inline proxies at 100-150ms round-trip, so DC-dependent operations — initial TGT acquisition at logon, SYSVOL/GPO retrieval, password changes — slow or time out, and where the KDC is unreachable Windows can fall back to NTLM, which is unaudited and replayable. NAT itself does not invalidate a ticket: Windows KILE writes no client addresses into tickets by default. Bottom box, token-based solution: an Entra ID token travels via HTTPS with no DC roundtrip, works across any cloud, any WAN path, any latency — authentication succeeds regardless of RTT. Navy, green, red palette on white background." width="100%"}

> Note the mechanism in the figure's right panel: the failure is *KDC unreachability at ticket issuance* and *middlebox session breaks* — not the multipath overlay, which resequences packets and holds the transport session across path switches. [Vol I Book 03](Vol_I_Book_03_OrgComp_Certificates_Tokens_Cryptographic_Identity.qmd) develops the full corrected argument.
...
> **Closure Necessity — SC-8 on DIA circuits.** FedRAMP's Rev 5 SC-8 guidance is protocol-agnostic: it requires FIPS 140-validated encryption edge-to-edge, names no protocol, and admits three closure paths — FIPS-validated cryptography of any protocol (TLS included), physical protection under SC-8(5) (a Controlled Access Area / Protected Distribution System), and inheritance for traffic contained within an authorized IaaS/PaaS boundary. So SD-WAN is not the *only* way to close SC-8 in general. But on a **field-office DIA circuit egressing over the public internet**, the SC-8(5) physical path and boundary inheritance do not apply, and application-layer TLS closes SC-8 only for the flows that negotiate it — DNS, legacy protocols, and misconfigured apps ride bare. The mechanism that closes SC-8 for *every* flow on that circuit at once is a network-layer overlay; among overlay classes (site-to-site IPsec, ZTNA tunnels, WireGuard-class), SD-WAN is the one that *also* delivers the SC-5 traffic-class and SI-4 telemetry closures this document bundles with it. The claim is coverage-completeness, not exclusivity.
>
> **Co-located rebuttal (the strongest counter-source).** A reviewer will cite FedRAMP's own post, "The Rev. 5 Approach to SC-8, and Protecting Data-in-Transit" (2023-07-13), which established the protocol-agnostic reading above and moved physical protection to SC-8(5) as a Controlled Access Area. That post does not undercut the necessity argument here — it narrows it. FIPS-validated TLS is a valid SC-8(1) closure per flow; the overlay's contribution is uniform coverage of *all* flows on an ungoverned public-internet circuit, plus the co-delivered SC-5/SI-4 closures. State the FedRAMP guidance up front and claim only what survives it.
...
| SC-8 Transmission Confidentiality and Integrity | Confidentiality and integrity for every flow the circuit carries — per-flow TLS covers only the flows that negotiate it | SD-WAN IPsec tunnel — DIA alone encrypts nothing |
| SI-3 Malicious Code Protection | Inline malware inspection at appropriate locations | SWG with SSL inspection in SASE stack |
| SI-4 System Monitoring | Application-layer telemetry — user, device, app, data, behavioral pattern | AAN application telemetry |
| AU-2 / AU-12 Event Logging / Audit Record Generation | User Z on device D accessed application A, performed action V on data type C — not just IP flows | Application-layer logging via AAN |
...
TIC 3.0 is the policy frame that turns this from good engineering advice into a compliance requirement. Under TIC 3.0, an agency may move enforcement from the consolidated TIC access point to a **distributed policy enforcement point** — but only if the TIC security capabilities catalog is satisfied *at that PEP*. The SASE stack is that distributed PEP. This is what makes DIA lawful: the pipe is permissible precisely and only when the enforcement point travels with it. A DIA circuit deployed without the distributed PEP has not modernized the TIC boundary — it has deleted it, and deleted boundaries surface as findings. The use-case-by-use-case mapping of the TIC 3.0 catalog onto distributed egress is developed in [Vol I Book 06](Vol_I_Book_06_OrgComp_Federal_Telecommunications_Modernization.qmd).
...
| Automated check: all DNS encrypted or integrity-protected | DNSSEC on all zones; DoH/DoT on Grid Members |
| Automated check: no CIDR overlap between environments | Terraform InfoBlox provider blocks overlapping allocation at request time |
| Automated check: every workload has machine identity | SCEP/EST auto-enrollment triggered at IP provisioning |
...
| Continuous monitoring; automated KSI enforcement | FedRAMP 20x Consolidated Rules (Jun 25, 2026); OMB Circular A-130 | NIST SP 800-53 Rev 5 CA-7 (RMF continuous-monitoring step per SP 800-37 Rev 2); FedRAMP 20x KSIs; mandatory Jan 1, 2027 |
...
# Appendix A — Cisco Catalyst SD-WAN and Microsoft 365 Informed Network Routing: Federal Implementation Reference {#appendix-cisco-inr}

::: {.aan-note}
**Canonical split.** The DNS-resolution interaction of this combination — how Cisco Catalyst SD-WAN DNS interception meets the Infoblox DDI resolution chain, and the DNSSEC forwarder-position analysis — is developed canonically in the [Next Generation DNS whitepaper, Parts 8–9](../whitepapers/infoblox-hybrid-dns-unified-ddi.qmd); this appendix owns the federal implementation reference (boundary partitioning, KSI evidence contracts, deployment sequence).

**Scope: FedRAMP Moderate civilian agencies using Microsoft 365 GCC.** This appendix is scoped to **FedRAMP Moderate** — civilian federal agencies operating M365 GCC tenants. GCC High, DoD IL4/IL5, and FedRAMP High authorizations are out of scope. Vol I Book 05's main narrative describes SD-WAN in platform-neutral terms. This appendix addresses the specific combination that most FedRAMP Moderate agencies encounter: **Cisco Catalyst SD-WAN** as the branch transport platform and **Microsoft 365 Informed Network Routing (INR)** as the M365 traffic optimization layer.
:::

::: {.aan-important}
**INR telemetry is a commercial-cloud capability — it is unavailable in this appendix's own scope.** Microsoft 365 Informed Network Routing is not offered in the GCC Moderate cloud. Microsoft Learn states the feature "supports tenants in WW Commercial cloud but not the GCC Moderate, GCC High, DoD, Germany, or China clouds," and the substrate encodes this as a machine-enforced contract fixture — **FINDING-001** (`tests/fixtures/contract/m365/informed-network-routing-unavailable-gcc-moderate.yaml`), which requires the adapter to return a `CapabilityUnavailable` claim rather than assume INR telemetry.

**What this means for the architecture below.** The INR-fed telemetry channel — Microsoft sharing per-endpoint path and performance signals with the SD-WAN fabric — is **not** available to a GCC Moderate agency. Everything else in this appendix holds: the branch still runs **Cloud OnRamp for SaaS**, which probes the published M365 GCC Optimize/Allow endpoints directly and steers to the best local-breakout path from its own measurements. The control table, KSI evidence contracts, and TIC alignment are satisfied by probe-based steering; only the Microsoft-fed telemetry enrichment is lost. Where the sections below reference INR telemetry, read them as the commercial-cloud target state and Cloud OnRamp probe-based steering as the GCC Moderate implementation. Cite FINDING-001 in the boundary documentation so an assessor sees the constraint was designed for, not missed.

**Confirming it in your own tenant.** The check is first-party and takes five minutes, and it is worth running before the architecture is drawn rather than after. In the Microsoft 365 admin center, open **Health → Network connectivity → Settings**. A commercial tenant offers an **SD-WAN solution** configuration pane there; a GCC Moderate tenant does not. Absence of that pane is tenant-specific evidence an assessor can act on, which a documentation citation alone is not.
:::
...
![Cisco Catalyst SD-WAN — Four Functional Planes](figs/mf-fig-15-cisco-sdwan-planes.png){fig-alt="Architecture diagram showing Cisco Catalyst SD-WAN's four functional planes side by side: Management (SD-WAN Manager/vManage) for policy authoring and telemetry; Control (vSmart Controller) distributing routing policy via OMP; Orchestration (vBond Validator) for zero-touch provisioning; Data (WAN Edge cEdge/ISR/ASR) carrying all agency traffic. FedRAMP boundary note at bottom: only SD-WAN Manager is cloud-hosted in the cloud-delivered model; data plane stays inside the agency boundary." width="100%"}
...
## A.2 — Microsoft 365 Informed Network Routing: What It Is

**Microsoft 365 Informed Network Routing (INR)** is Microsoft's named capability for sharing M365 path telemetry with partner SD-WAN platforms — and, more broadly, the operational model through which Microsoft routes M365 traffic across its global private backbone. Understanding it matters for Cisco Catalyst SD-WAN deployments because it changes the answer to a specific question federal IT architects regularly get wrong: *where should agency SD-WAN terminate M365 traffic?*
...
> **Optimize-category bypass and SC-8 — reconciled, not in tension.** The Optimize category's "no inspection, no proxy, no detour" requirement means Teams media steered by Cloud OnRamp may egress outside the IPsec overlay entirely. That is not an SC-8 gap; it is the per-flow closure the coverage-completeness doctrine already concedes — Teams media is SRTP-encrypted and M365 sessions ride FIPS-validated TLS — while the overlay continues to close SC-8 for every flow that does not carry its own validated encryption. What the WAN Edge adds is that the exemption is **deliberate and attestable**: the DPI classification that steers an Optimize flow around the overlay is itself a logged, per-flow record of which traffic rests on application-layer encryption and which rides the IPsec tunnel — SI-4 telemetry over flows the overlay does not carry. State the split this way in the boundary documentation and Microsoft's connectivity principles and the SC-8 closure claim never collide.

![Microsoft 365 Informed Network Routing — Local Breakout vs. Backhaul](figs/mf-fig-16-inr-routing.png){fig-alt="Side-by-side comparison of two routing paths for M365 traffic. Left panel (Legacy Backhaul): Portland field office sends traffic via MPLS to Agency DC in Washington DC, then to internet, entering Microsoft network at suboptimal Ashburn PoP — 80ms+ total latency. Right panel (AAN Local Breakout): Portland field office sends traffic directly via DIA or 5G to Microsoft's nearest Seattle front door PoP — 18ms total latency. Bottom section shows M365 endpoint categories: Optimize (Teams media, no proxy, maximum priority), Allow (general M365, proxy OK), Default (standard inspection). Satisfies SC-8, AC-4, TIC 3.0 Use Case E." width="100%"}
...
::: {.aan-important}
**The failure mode that looks like success: resolution that is not local to the egress.** Cloud OnRamp measures the path to whichever front door the branch actually resolves. Microsoft's connectivity principles require name resolution to occur **close to the network egress**, and name remote lookups as a cause of network hairpin — the Distributed Service Front Door steers on where the *resolver* appears to be, not where the client is. A branch that egresses locally but resolves through a datacenter resolver is therefore given a datacenter-proximate front door, and then reaches it across the local circuit.

That defeats the local-breakout benefit while leaving every indicator green. The probes report the path healthy because it *is* healthy; it is simply the wrong front door, and no SD-WAN telemetry says so. The per-location network scores that would expose it are part of the admin-center layer GCC does not have.

Two consequences for the build. First, A.4.2's Umbrella requirement closes SC-20/SC-21 and the TIC DNS capability, but the geographic question is a separate one and has to be answered on its own terms: confirm that the resolver a branch actually uses yields a front door near that branch, measured **from the branch** rather than inferred from configuration. Second, audit conditional forwarders — a rule returning Microsoft 365 queries to a core resolver reintroduces the hairpin however the branch is otherwise built.
:::

## A.4 — FedRAMP-Specific Considerations

The Cisco Catalyst SD-WAN + Microsoft INR architecture introduces four categories of FedRAMP-specific compliance questions that do not arise in the platform-neutral SD-WAN narrative.
...
| Cisco ThousandEyes for Government, where the ADR-057 rebuild path is adopted | **Outside** — FedRAMP-authorized external service | ThousandEyes for Government package reference; evidence that agents registered to the government instance and not the commercial platform |
...
::: {.aan-important}
**A ThousandEyes agent registers to the commercial platform unless it is told not to.** Where [ADR-057](../../adr/adr-057-thousandeyes-networks-pillar-scope.qmd)'s rebuild path puts Cisco ThousandEyes in the architecture, note that **ThousandEyes for Government is a separate platform instance**, not a setting inside the commercial one. Agents have to be directed to it: for a Linux-package agent that means adding the `-m advanced` option to the installation script with the value set to `FEDRAMP`. Docker installs carry their own government-instance prerequisites, taken from the ThousandEyes for Government documentation rather than the general installation pages.

An agent installed without that step registers to the **commercial** platform, placing agency branch-path telemetry outside the authorized boundary. That is a data-placement incident rather than a configuration defect, and a component inventory will not catch it — the agent is present, healthy and reporting, just to the wrong platform. Make registration an explicit build step with a post-install check that each agent appears in the government console and nowhere else.
:::
...
| **AU-2 / AU-12** Event Logging / Audit Record Generation | Every flow traceable to source, destination, application, policy decision | SD-WAN Manager retains full flow telemetry: WAN Edge ID, source/destination IP, application ID, path selected, policy applied, timestamp; exported via syslog or streaming telemetry to SIEM |
...
### A.4.4 — Branch Egress Ranges and the Scope of the Optimize Exemption

A.4.2's second condition exempts the Optimize category from proxy inspection. That exemption has a scope, and the scope is a list of circuits — which makes the per-circuit public egress range a boundary artifact rather than a network-engineering detail. Record one row per **circuit**, not per site: a site with two internet circuits occupies two rows, because attribution and steering both operate per circuit.

Five rules govern the range column. Each describes an error that yields a fabric which looks correctly configured while attributing traffic to the wrong place:

1. **Record the post-NAT public address the branch presents to the internet** — not the WAN interface address, and not the inside pool. The address Microsoft and the probe endpoints observe is the one that identifies the circuit.

2. **Give each circuit a unique range.** Where two circuits at a site share a NAT pool, per-circuit attribution is not possible. Fix the NAT design before listing the site, or record why path comparison at that site is not meaningful.

3. **Ranges must be static and assigned to the agency** for the life of the exemption. A dynamically assigned or ISP-rotated address moves a site outside the approved scope without anyone deciding to move it.

4. **Verify from the branch, then reconcile to IPAM** — not the reverse. The site is authoritative and the record is not; a register built from IPAM alone inherits whatever has drifted.

5. **List the IPv6 range wherever the circuit is dual-stacked.** An IPv6-capable client egressing on an unlisted range sits outside the recorded scope even though its site is listed.

Completed, this register is a map of branch egress — public addressing, circuit topology, and which sites carry reduced inline inspection. Mark and handle it under the agency's own rules for network architecture information.

## A.5 — FedRAMP 20x KSI Evidence Contracts

The following table maps the OrgComp evidence layer to FedRAMP 20x KSI themes for the Cisco Catalyst SD-WAN + Microsoft INR configuration.
...
| **KSI-MLA** | KSI-MLA-OSM | SD-WAN Manager alert correlation: path quality degradation + flow reroute event = correlated security event | Cisco XDR or SIEM correlation rule |
...
::: {.aan-note}
**The OrgComp slot binding for this appendix.** The evidence contracts in the table above are satisfied by OrgComp slot 03 (network control plane), which binds to Vol I Book 05 (SD-WAN/SASE/UC) in the Conformance Adapter. The Cisco Catalyst SD-WAN + Microsoft INR configuration is a specific implementation of the generic SD-WAN evidence described in slot 03. No additional slot binding is required — the slot-03 controls (KSI-CNA, KSI-MLA, KSI-SVC, KSI-CMT) are already satisfied when the platform-specific configuration in this appendix is in place.
:::
...
**Step 6 — FedRAMP 20x KSI evidence validation.** Using the evidence contracts in §A.5, verify that SIEM receives the required structured events for each KSI. Confirm the evidence passes the format and completeness requirements of the governance engine's OSCAL assessment-results generator (any engine satisfying the series evidence contract).
