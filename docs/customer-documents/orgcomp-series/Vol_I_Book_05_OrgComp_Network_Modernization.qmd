...
![Why Kerberos Breaks in the Cloud: The Session Contract Assumption](figs/mf-fig-10-kerberos-breaks.png){fig-alt="Two-panel comparison diagram. Left panel, Kerberos 1988 design on-premises: an AD domain controller and a user at a desk share the same LAN at 5ms round-trip; Kerberos ticket issued, authentication succeeds — the protocol assumed client and DC in the same room. Right panel, Kerberos in the 2020+ cloud world: the user now works in a cloud tenancy while the domain controller remains on-premises; the authentication path crosses the WAN and inline proxies/NAT, where sessions break and round-trips run 100-150ms; KDC unreachable — ticket issuance fails, and Windows silently falls back to NTLM, which is unaudited and replayable. Bottom box, token-based solution: an Entra ID token travels via HTTPS with no DC roundtrip, works across any cloud, any WAN path, any latency — authentication succeeds regardless of RTT. Navy, green, red palette on white background." width="100%"}
