---
title: "Vol IV Book 01 — Federal Organization Compliance"
...
aliases:
  - /customer-documents/federal-aan-series/Vol_IV_Book_01_FedAAN_Business_Continuity.html
...
The Volume I network-and-identity books, the Volume II database books, and the early Volume III security books built the infrastructure substrate for a resilient federal network: DNS with Grid replication (Vol I Book 01, Cloud Landing Zone, IPAM/DDI), SD-WAN with dual underlay circuits and automatic path failover (Vol I Book 06, Federal Telecommunications Modernization), identity with break-glass access procedures (Vol I Book 03, Certificates & Tokens and Vol III Book 01, Privileged Access Management), SQL Server Always On Availability Groups with geo-replicated disaster recovery replicas (Vol II Book 02, SQL Server Implementation Guide and Vol II Book 03, Database Consolidation & Network Physics), and a centralized logging and alerting fabric (Vol III Book 06, SIEM / XDR Detection). Each of those books produced runbooks, configuration templates, and tested recovery procedures. A FedRAMP assessor reviewing all of them would find a technically impressive architecture. They would also find that the agency cannot demonstrate compliance with CP-2.

The reason is structural. CP-2 is not satisfied by capable infrastructure. CP-2 is satisfied by a document: a Contingency Plan that has been approved by a designated official, distributed to all recovery personnel, tested on a defined cycle, and updated when the system changes or after every test. The runbook in Vol II Book 02 that walks a DBA through failing over an Availability Group is a recovery procedure. It is not a Contingency Plan. The SD-WAN configuration in Vol I Book 06 that automatically steers traffic onto the backup DIA circuit when MPLS degrades is a resilience capability — one that lives in the overlay's probe-driven path selection, not in the circuit. It is not a Contingency Plan either. An assessor who opens the Plan of Action and Milestones and sees CP-2 open can turn off the screens showing a perfectly functioning DR failover and write a High finding, because the artifact does not exist.

This document closes that gap. It assembles the infrastructure components from the Volume I, II, and III books into a formal CP-2 Contingency Plan structure aligned to NIST SP 800-34 Rev 1, adds the organizational and testing layers that infrastructure alone cannot provide, and provides the template and evidence framework to close CP-2 through CP-10 (CP-1 policy and procedures is an agency-level artifact out of scope for this system document). Agencies can use this document as a direct template: the section outlines are populated with enough specificity to guide authors, the Business Impact Analysis methodology produces the RTO/RPO values that anchor the plan, and the testing framework produces the artifacts an assessor expects to see.
...
The DR runbooks in Vol II Book 02 and Vol II Book 03 are Section 5 content for the Contingency Plan. They are not the Contingency Plan itself. The difference is visible in what each artifact contains.
...
NIST SP 800-34 Rev 1 structures an Information System Contingency Plan around an Introduction, a Concept of Operations, and three sequential phases — Activation/Notification, Recovery, and Reconstitution — with supporting appendices. This document organizes that structure into the seven working sections below, with sufficient detail for agency authors to populate each. The runbooks from Vol II Book 02 and Vol II Book 03 are explicitly referenced as appendix content that satisfies the Recovery-phase procedure requirements.
...
Appendix A — Network Recovery Procedures (SD-WAN failover and DDI Grid failover; derived from the runbooks in Vol I Book 01 and Vol I Book 06 of this series)

Appendix B — Database Recovery Procedures (SQL AG failover and point-in-time restore; derived from the runbooks in Vol II Book 02 and Vol II Book 03 of this series)

Appendix C — Identity Recovery Procedures (break-glass account activation and Conditional Access adjustment; derived from Vol I Book 03 and Vol III Book 01 of this series)

Appendix D — Application and Cloud Service Recovery Procedures (application tier failover to secondary region)

Appendix E — Audit Logging Recovery Procedures (Sentinel workspace validation and log analytics continuity; derived from Vol III Book 06 of this series)
...
For cloud contact center platforms (such as the Amazon Connect deployment referenced in Vol I Book 06), CP-7 requires documented multi-region failover. The failover mechanism uses DNS-based health checks with automatic routing policy changes.
...
> **Closure Necessity — CP-8 on the dual underlay.** Most of the CP family closes with authored artifacts, not technology: CP-2 is a signed plan, CP-3 is a training record, CP-4 is a documented test. No network purchase produces any of those, and this document exists precisely because infrastructure alone cannot. CP-8 is the one place in the family where the mechanism matters. The control requires alternate telecommunications services that support *resumption of operations* within the RTO. Two circuits from two carriers satisfy the "alternate services" clause on paper; by themselves they resume nothing. Traditional dual-homing fails over on routing-protocol reconvergence: it is all-or-nothing, blind to brownouts (loss, latency, and jitter that never drop the link), carries no per-class policy, and produces no recorded evidence of path health. Those are the gaps that leave "alternate services on paper" short of *resuming operations* to a documented standard, against a 15-minute RTO and a 30-minute MTD for the network layer. The mechanism that closes them is SD-WAN active-active multipath: probe-driven path selection, per-class steering that reacts to brownout in under a second, and continuous recorded path telemetry that becomes the CP-8 test evidence. A second DIA circuit without the overlay adds bandwidth, not resilience.
...
CP-4 requires annual testing of the contingency plan. Confirm the FedRAMP-defined CP-4 test type for your baseline before relying on a tabletop-only year — at Moderate, tabletop exercises are generally accepted as the minimum, with functional testing expected for critical systems. What assessors specifically look for:
...
## Multi-Cloud Continuity and Disaster Recovery

The CP-6 / CP-7 / CP-8 architecture above is realized on the Microsoft estate
(Azure Site Recovery, Azure Backup). A multi-cloud agency must also satisfy
**CP-9 (backup)** and **CP-10 (recovery)** for workloads on **AWS GovCloud**,
**Oracle OCI**, and **VMware** — each with its own native DR/backup service, and
each producing its own RTO/RPO evidence.

| Platform | Backup / DR service |
|---|---|
| Microsoft | Azure Backup + Azure Site Recovery |
| AWS | AWS Backup + AWS Elastic Disaster Recovery (DRS) |
| Oracle OCI | OCI Full Stack Disaster Recovery |
| VMware | Site Recovery Manager / vSphere Replication (recovery target may be Azure or AWS) |

**Doctrine:** DR *actuation* is platform-native — the same split the
Management-Plane Coordination Doctrine draws for patching. The unifying layer is
the **evidence contract**: RTO/RPO test results, backup-success status, and
replication-lag records from every platform normalize into the Multi-Cloud
Evidence Fabric, so **CP-9 / CP-10** evidence is one correlated picture rather
than five disconnected dashboards. Microsoft-preference: ASR / Azure Backup is the
default for the Azure estate; cross-cloud DR is native per-platform, and
cross-cloud recovery targets (recovering VMware into Azure or AWS) are a
deliberate design choice, not an automatic one. CP-4 test records and the CP-9/10
metrics roll up through the fabric to CISA CDM and the continuity KSIs.

## Integration with the Volume I, II, and III Books
...
|---|---|---|
| Vol I Book 01 (DDI — DNS/DHCP/IPAM) | Grid replication with alternate Grid member in DR region | CP-6: alternate storage for DNS zone data; CP-7: alternate processing for name resolution; recovery-sequencing dependency: DNS must be restored before CP-7 alternate-processing cutover |
| Vol I Book 03 (Certificates and Tokens — cryptographic identity) | Cloud-hosted globally redundant identity; Conditional Access; certificate issuance and revocation continuity | CP-7: identity is inherently multi-region, and certificate authority continuity ensures TLS-dependent services can be restored at the alternate site; CP-2: break-glass accounts ensure identity access independent of Conditional Access availability |
| Vol I Book 06 (SD-WAN) | Dual underlay (MPLS + DIA) governed by active-active overlay multipath with automatic path failover | CP-8: primary and alternate telecommunications services — failover, resequencing, and per-class path policy live in the overlay; CP-7: alternate network path to cloud processing site |
| Vol II Book 02 (SQL — on-premises) | AG synchronous replica for RPO near zero | CP-6: synchronous replica as alternate storage; CP-7: synchronous failover for minimal RTO SQL recovery |
| Vol II Book 03 (SQL — cloud DR) | AG async replica in secondary region | CP-6(1): geographically separated alternate storage; CP-7(1): geographically separated alternate processing; CP-9: backup of structured data |
| Vol III Book 01 (Privileged Access and MFA) | Break-glass accounts for emergency access | CP-2: ensures recovery teams can authenticate during Conditional Access disruption or MFA outage |
| Vol I Book 02 and Vol I Book 06 (NAC policy tiers; SD-WAN/SASE policy) | Network admission and egress policy backup — 802.1X policy tiers, SD-WAN templates, SASE FWaaS rules | CP-6: configuration baseline for admission and egress policy stored in alternate region; CP-7: network policy can be restored at alternate site |
| Vol IV Book 06 (Authorization Package and ConMon) | OSCAL bundle, ScubaDrift ledger, and compliance baseline exports | CP-6: configuration and compliance baselines (device management policy exports, compliance rules, ConMon history) stored in alternate Key Vault |
| Vol III Book 06 (Sentinel/SIEM) | Geo-redundant Log Analytics workspace | CP-9: audit logs replicated to alternate region; AU-9: integrity of audit records maintained during primary site outage |
...
| CP-3 | Contingency Training | No annual training requirement enforced; personnel may be unaware of their roles | Tabletop participation provides the annual training component of CP-3; attendance record evidences it; onboarding training within the org-defined period of assuming a contingency role must be added to fully satisfy the control; plan includes training requirement in Section 7 | NIST SP 800-53 Rev 5 CP-3 |
...
| CP-9 | System Backup | Addressed in Vol II Book 02 and Vol II Book 03; extended here to audit logs and config baselines | Audit log backup via geo-redundant Log Analytics (Vol III Book 06) explicitly referenced in CP-2 as CP-9 evidence; config baseline exports in secondary Key Vault documented | NIST SP 800-53 Rev 5 CP-9 |
...
: NIST SP 800-53 Rev 5 Control Closure — Vol IV Book 01 Business Continuity {.striped .hover}
...
**KSI-RPL (Recovery Planning) — CP-6, CP-7, CP-9.** A FedRAMP 20x-ready implementation provides machine-readable evidence of backup recency and integrity, not just a policy document claiming backups occur. The SQL AG replication lag metric, the Log Analytics geo-redundancy configuration, and the secondary Key Vault contents are all query-able via API. Agencies preparing for 20x assessment should expose these metrics through a compliance dashboard that answers: "What is the current RPO for each critical system, and when was the last successful recovery test?"

**KSI-RPL (Recovery Planning) and KSI-INR (Incident Response) — CP-2, CP-4 → KSI-RPL; IR-4 → KSI-INR.** FedRAMP 20x assessors will look for evidence that contingency plans are living documents connected to operational telemetry, not static PDFs filed at authorization time. The connection points in this framework are the Sentinel alerts (Vol III Book 06) that trigger the activation threshold evaluation, the SD-WAN monitoring dashboards that evidence CP-8 path health, and the AAR tracking table that shows findings from prior tests are being resolved. Automate the evidence collection: a scheduled export of circuit uptime, replication lag, and backup age to the ISCM continuous monitoring system satisfies KSI evidence requirements without manual artifact assembly at assessment time.

The Contingency Plan document is the assessor-facing artifact. The monitoring, replication, and failover capabilities from the Volume I, II, and III books are the automated evidence substrate. Vol IV Book 01 closes the gap between having the capability and being able to demonstrate it.
:::
