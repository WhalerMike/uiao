...
# Requires the bundled pandoc (pypandoc-binary) + orgcomp-reference.docx + orgcomp-callouts.lua.
set -uo pipefail
cd "$(dirname "$0")"
HERE="$(pwd)"
...
echo "== pandoc: $PANDOC =="
REF="orgcomp-reference.docx"; LUA="orgcomp-callouts.lua"
STAGE="/tmp/aanbuild/kit"
DATECODE="$(date +'%Y-%m-%d_%H%MET')"
ZIP="OrgComp_Federal_Series_Complete_${DATECODE}.zip"
OLDZIP="$(ls -1 OrgComp_Federal_Series_Complete_*.zip 2>/dev/null | grep -v "$DATECODE" | head -1)"
...
declare -A V8MAP=(
  ["01-azure"]="Vol_VIII_Book_01_OrgComp_DDI_Azure"
  ["02-aws"]="Vol_VIII_Book_02_OrgComp_DDI_AWS"
  ["03-gcp"]="Vol_VIII_Book_03_OrgComp_DDI_GCP"
  ["04-oci"]="Vol_VIII_Book_04_OrgComp_DDI_OCI"
  ["05-vmware"]="Vol_VIII_Book_05_OrgComp_DDI_VMware"
  ["06-cross-platform-operations"]="Vol_VIII_Book_06_OrgComp_DDI_Cross_Platform_Operations"
  ["07-servicenow-orchestration"]="Vol_VIII_Book_07_OrgComp_DDI_ServiceNow_Orchestration"
  ["08-servicenow-led-implementation"]="Vol_VIII_Book_08_OrgComp_DDI_ServiceNow_Led_Implementation"
  ["appendix-A-sizing-cost-ipv6-dhcp"]="Vol_VIII_Book_A_OrgComp_DDI_Appendix_Sizing_Cost_IPv6_DHCP"
)
for src in "${!V8MAP[@]}"; do
...
# Driven from orgcomp-compliance-spine.yml so the zip carries EVERY registered kit
...
done < <(python3 - "$HERE/orgcomp-compliance-spine.yml" <<'PY'
import sys, os, yaml
...
render "federal-orgcomp-conmon-gap-roadmap.md" "$STAGE/federal-orgcomp-conmon-gap-roadmap.docx" "." && ok=$((ok+1)) || fail=$((fail+1))
[ -f "federal-orgcomp-conmon-gap-roadmap.pptx" ] && cp "federal-orgcomp-conmon-gap-roadmap.pptx" "$STAGE/"
render "federal-orgcomp-governance-ownership-model.md" "$STAGE/federal-orgcomp-governance-ownership-model.docx" "." && ok=$((ok+1)) || fail=$((fail+1))
...
this zip as complete source, under <Volume>/kits/<name>/:
  ...
  - Vol V:   the companion training academy curriculum (OrgComp-Training-Program).
...
Every AAN deliverable is bound by the compliance spine (orgcomp-compliance-spine.yml):
...
FOUO and executive-summary blocks) produced with Pandoc + orgcomp-reference.docx +
orgcomp-callouts.lua. See BUILD-DERIVATIVES.md and build_distribution_kit.sh.
...
if [ -z "$OLDZIP" ]; then
  ...
  if [ -z "${ALLOW_NO_SEED:-}" ]; then
    ...
    echo "   Restore the prior OrgComp_Federal_Series_Complete_*.zip, or set ALLOW_NO_SEED=1"
