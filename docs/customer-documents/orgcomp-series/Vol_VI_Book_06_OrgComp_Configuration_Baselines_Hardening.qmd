---
title: "Vol VI Book 06 — Federal Organization Compliance — Configuration Baselines & Hardening"
...
aliases:
  - /customer-documents/federal-aan-series/Vol_VI_Book_06_FedAAN_Configuration_Baselines_Hardening.html
...
This book authors the intended state as machine-enforced policy, detects drift against it continuously, and reconciles or reports it. It operationalizes, through CM-6(1)/(2) automation, the enforce-detect-reconcile loop over base CM-6: an enforced configuration, a detection of deviation, and a documented reconciliation — not a spreadsheet of settings someone once applied.
...
Volume III proves conformance tooling *measures drift from an intended state; it cannot author one.* This book authors the intended state as code and enforces it. Base CM-6 requires an established, documented, and implemented baseline; this book operationalizes it through CM-6(1) automated management and verification and CM-6(2) response to unauthorized changes, so the baseline is machine-enforced and drift is detected and dispositioned — a spreadsheet of settings no pipeline applies closes nothing, and a scanner with no authored baseline has nothing to measure against.
:::
...
| CM-7 | Least Functionality | Host hardening roles |
