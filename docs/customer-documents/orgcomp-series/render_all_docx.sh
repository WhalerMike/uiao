...
# (orgcomp-reference.docx — carries the callout/banner paragraph styles) and the
# orgcomp-callouts.lua filter (maps each callout/banner div class to its style).
...
# orgcomp-callouts.lua). Pandoc is kept because it is ~6x faster across the ~56-book
...
REF="orgcomp-reference.docx"
LUA="orgcomp-callouts.lua"
OUTDIR="${1:-.}"
mkdir -p "$OUTDIR"
