---
title: "Vol V Book 03 — Federal Organization Compliance — Assessment, Rubrics & Certification"
subtitle: "The four-level rubric ladder, the two capstones' scoring, the KSI Closure Necessity Matrix as the exam, and the OrgComp Practitioner / Assessor credentials"
...
aliases:
  - /customer-documents/federal-aan-series/Vol_V_Book_03_FedAAN_Assessment_Certification.html
...
the authoritative one-row-per-family table that doubles as the certification exam;
and the definition of the two credentials the Academy confers — **OrgComp
Practitioner** and **OrgComp Assessor**.
...
|---|---|---|
| Books | Vol IV Bk 01 | — |
| Controls | CP-2, CP-3, CP-4, CP-6, CP-7, CP-8, CP-9, CP-10 with enhancement counts right (CP-5 withdrawn in Rev 5, incorporated into CP-2) | — |
...
slots, backed by a real `<engine> oscal ksi-ar` run. Score each slot on the ladder,
...
| The verdict is fed | `<engine> oscal ksi-ar` shows the module's slot verdict changing because of *your* artifact |
...
rule family: the family, its closing technology (function, not product), whether
...
and reconciles with Vol 0 Book 00 and Vol IV Book 06 at the totals and the
tool-attestable split: **10 attestable by tooling alone, 19 architecture-bound,
29 total.** This is the *theme-axis* view: the KSI-001..010 row is the ScuBA
baseline layer, and the theme rows beneath it are the architecture-bound
remainder of each theme (the 19). Vol 0 Book 00 holds the authoritative
*per-rule-number* decomposition (KSI-011..029 grouped by rule range), which is
what an assessor reconciles against rule-by-rule.
...
| KSI-PIY | inventory | Authoritative IPAM/DDI + device identity | No | 2, 5 | Vol I Bk 01/02 · Vol III Bk 01/03/04 · Vol IV Bk 04 |
...
# Certification — OrgComp Practitioner and OrgComp Assessor
...
- **OrgComp Practitioner** (implementation). Requires: the Track B capstone at pass
  ...
  KSI reads.
- **OrgComp Assessor** (compliance). Requires: the Track A capstone at pass (all
...
*Federal Organization Compliance — Vol V Book 03 of the series. Volume V is
the enablement layer; this book is its assessment and certification instrument,
grading the capstones of Vol V Books 01–02 and conferring the OrgComp Practitioner
and OrgComp Assessor credentials. The KSI Closure Necessity Matrix reconciles with
...
internal KSI decomposition (modeled on the FedRAMP 20x CR26 KSI schema), pending
