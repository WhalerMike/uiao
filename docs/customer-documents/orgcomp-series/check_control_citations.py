...
_TITLE_ROW_GLOBS = ["Vol_*_Book_*.qmd", "OrgComp_Spine_Crosswalk.md"]
...
def main() -> int:
    ...
    for pattern in PROSE_GLOBS + MAP_GLOBS + ["orgcomp-compliance-spine.yml"]:
    ...
    for i, c in enumerate(spine["closures"]):
        ...
        if _norm(claimed) != _norm(official):
            errors.append(
                f"orgcomp-compliance-spine.yml closure[{i}] {cid}: title says '{claimed}', Rev 5 catalog says '{official}'"
            )
        ...
        for f in sorted(HERE.glob(pattern)):
            for ln, line in enumerate(f.read_text(encoding="utf-8", errors="replace").splitlines(), 1):
                ...
                if len(ids) != len(titles) or not all(CTRL_RE.fullmatch(i) for i in ids):
                    continue
                for cid, prose in zip(ids, titles, strict=False):
                    official = catalog.get(cid)
                    if official is None:
