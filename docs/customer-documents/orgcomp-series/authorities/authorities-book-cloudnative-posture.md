...
| CM-6 | Configuration Settings | Cloud resource misconfiguration detection across CSPs (CSPM) — Defender for Cloud (Azure/AWS/GCP, Moderate default); AWS Config + Security Hub (AWS, owned, primary for AWS — High-covers-Moderate in AWS GovCloud); VMware Aria Operations for Compliance (VMware, owned, boundary-inherited — not independently FedRAMP-authorized); OCI Cloud Guard (OCI, residual gap closer, High-covers-Moderate); Prisma Cloud/Wiz (High, alternative) | FedRAMP Moderate | NIST SP 800-53 Rev 5; CISA SCuBA | Security / supply chain | KSI-CMT, KSI-SVC | No |
