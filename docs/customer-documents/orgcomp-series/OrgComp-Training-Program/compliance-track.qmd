---
title: "OrgComp Track A — Compliance"
subtitle: "The 20-book OrgComp series as a FedRAMP 20x / NIST SP 800-53 Rev 5 compliance curriculum"
date: "2026-07-18"
date-format: "MMMM D, YYYY"
description: "Compliance track of the OrgComp Training Program: nine modules (orientation plus the eight OrgComp evidence slots) that teach which controls each of the 20 OrgComp books closes, which FedRAMP 20x KSI category evaluates the closure, and how evidence flows through the engine's conformance pipeline."
aliases:
  - /customer-documents/federal-aan-series/AAN-Training-Program/compliance-track.html
...
1. **Read** — the OrgComp book(s) bound to the slot.
...
`<engine>` is a substitution token, not a product — see the
[tooling note](index.html#tooling-stack) for what it stands for, what each
command produces, and what the Tier F fixture path actually runs.
...
- **Learn:** the FedRAMP 20x timeline (Class A pipeline opens August 3, 2026;
...
- **Controls:** CM-2, AC-4 anchors; SR-1/2/3/5/6/8/10/11 from Book 15; PM
  family from Book 16.
- **Evaluate:** **KSI-CMT** (configuration / change-management evidence) and
  **KSI-SCR** (supply-chain-risk evidence); the Book 15 SBOM/VDR/VER evidence
  contract activated KSI-011–014 and, via its SR-family evidence, KSI-015/016
  (KSI-SCR-MIT / KSI-SCR-MON) — the BOD 26-04 submission (December 7, 2026)
  remains the operational cadence Book 19 runs.
...
Produce a single document (or an annotated `<engine> oscal ksi-ar` export) that,
for **all eight slots**:
...
3. shows the per-KSI verdict (satisfied / not-satisfied / not-evaluated)
   from a real `<engine> oscal ksi-ar` run, and
