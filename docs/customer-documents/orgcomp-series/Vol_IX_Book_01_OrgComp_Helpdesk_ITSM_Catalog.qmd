...
# Identity MACD-R — The Five Verbs at the Help Desk {#identity-macdr}

Every identity operation the catalog fulfills is one of five verbs — a
**Move, Add, Change, Deletion, or Reset** — and each verb travels the full
authorized path of the series' MACD-R Lifecycle Doctrine (Vol 0 Book 00):
SSOT-originated, authoritatively authorized, executed under a just-in-time
scoped role, closed with provenance, and emitting evidence automatically.
The verbs differ in what originates them, what the closure probe must
verify, and which controls they exercise:

| Verb | Identity meaning | SSOT origin | Closure probe verifies | Controls exercised |
|---|---|---|---|---|
| **Add** | Joiner: account provisioned, groups/licenses derived, MFA registered, SCIM fan-out to federated apps | HR joiner event | Account exists with exactly the derived entitlements; MFA method registered; every SCIM-connected app provisioned | AC-2, IA-4, IA-5 |
| **Move** | Mover: role/unit change → attributes updated, group and license membership re-derived, app access re-evaluated | HR mover event | New derivation matches the SSOT record; *stale* entitlements from the old role removed | AC-2, AC-6 |
| **Change** | Entitlement or attribute change outside a role move: access requests, group adds, delegation grants | Governed request resolved against the SSOT record | The granted scope equals the approved scope — nothing broader | AC-2, AC-6 |
| **Deletion** | Leaver: disable, revoke sessions and tokens, deprovision every federated app, reassign owned objects — including the leaver's **anchored non-human estate** (owned service accounts and app registrations transfer per the Human-Anchor Rule) | HR leaver event | Account unusable everywhere; zero live sessions; SCIM deprovisioned in each connected app; no orphaned objects or ownerless non-human identities | AC-2, AC-2(3), IA-4 |
| **Reset** | Credential events: password reset, MFA method reset, session revocation | Governed request against the SSOT identity | Re-proofing performed to policy before issuance; new credential meets IA-5; old credential unusable | IA-4, IA-5, AC-2 |

::: {.aan-warning}
**Reset is the highest-risk verb, and it is the one attackers order from
the help desk.** A password or MFA reset re-issues the keys to an
identity on the strength of a phone call — the classic help-desk
social-engineering entry. The catalog's Reset items therefore gate on
**identity re-proofing to policy** (IA-4 discipline: verified against the
SSOT record, not caller-supplied facts), execute under a scoped
just-in-time role, and close only on a probe confirming the old
credential is dead and the new one meets policy. A Reset closed manually,
with no re-proofing payload, is precisely the closure the Closure
Provenance Doctrine refuses to count.
:::

The matrix is the doctrine made operational: the five verbs are the same
five the SSOT Registry (Vol 0 Book 00) governs for every object class —
what changes at the help desk is only the class (human identities here,
their anchored non-human estate on the Deletion row) and the probes.
