---
title: "Vol IX Book 00 — Federal Organization Compliance — Volume IX Overview: ServiceNow Day-2 Operations"
...
date: "2026-07-19T17:05:00-04:00"
format:
  html:
    css: lz-style.css
...
aliases:
  - /customer-documents/federal-aan-series/Vol_IX_Book_00_FedAAN_Day2_Operations_Overview.html
...
Volume IX runs on the same ServiceNow Government Cloud coordination hub as Volume VII, which holds **FedRAMP High** authorization. The High-on-Moderate boundary treatment — why a High-authorized hub coordinating a Moderate boundary is a conservative, documented posture — is stated once in **Vol VII Book 00 §"Authorization Boundary Treatment — ServiceNow High-on-Moderate"** and inherited here without restatement. The platform-optionality position — ServiceNow is the **recommended, not mandatory** coordination platform; the requirement is the coordination contract, and named alternatives (authorized workflow/CMDB platform, or a composed Azure DevOps/GitHub + Logic Apps + Sentinel pipeline) satisfy it — is likewise stated once in **Vol VII Book 00 §"ServiceNow is the recommended coordination platform — not a mandatory one"** and inherited here.
:::
...
- **[`ticket-to-machine-not-ticket-to-human` whitepaper](../whitepapers/ticket-to-machine-not-ticket-to-human.qmd) §5.1** — cites this volume's `MacdrOrchestrator.js` as a working, tested reference implementation of the paper's Closure Necessity loop, applied to identity lifecycle (AC-2/AC-3/AC-6) rather than configuration drift.
...
- **`orgcomp-compliance-spine.yml`** — the SSOT registering this volume (`vol-9`) and its closures.
