---
title: "Book 00 — Federal Organization Compliance Series"
subtitle: "Executive Summary — Complete Control Closure Across the OrgComp Series (NIST · FedRAMP 20x · CISA · EO/OMB)"
author: "Federal Cloud Services"
date: "2026-07-29T14:17:00-04:00"
format:
  html:
    css: lz-style.css
...
aliases:
  - /customer-documents/federal-aan-series/Vol_0_Book_00_FedAAN_Executive_Summary.html
...
[Vol 0 Book 00a — Executive Brief](Vol_0_Book_00a_OrgComp_Executive_Brief.qmd)**,
...
This series was developed by the **CSI Team (Cloud Services Infrastructure)** as a technical planning document for {{< meta agency.short >}}'s FedRAMP 20x compliance transition. It represents an {{< meta agency.short >}}-wide analysis of the architectural work required to satisfy NIST SP 800-53 Rev 5 controls and FedRAMP 20x Key Security Indicators under the Consolidated Rules effective January 1, 2027.
...
- **Organizational leaders** — operational feasibility, resource capacity, and accountability mapping across {{< meta agency.short >}}'s organizational units
...
- **"29 KSIs"** is unchanged — the series' internal 29-rule KSI decomposition. The extension books add *evidence* for existing network, endpoint, telemetry, and security slots; none adds a new KSI rule, and because their evidence is architecture-generated rather than conformance-tool-attestable, they reinforce the "19 of 29 require the architecture" finding below.
:::
...
and a final integration volume, **Volume X**, has since brought it to the current
**eleven (Volumes 0–X)**.
...
- **Volume X — Governance-Substrate Integration.** The optional closing layer:
  where everything the volumes above produce lands as machine-checked evidence
  on the governance substrate — adoptable late or never, with no rework (see
  the Vol X row in the series table below and the separability statement that
  follows it).
...
- **"34 distinct controls" and "29 KSI rules"** remain the closure arithmetic of
  the *architecture* corpus (the original six tracks and the series' internal
  KSI decomposition).
...
This series comprises twenty-three companion implementation guides for the Federal Organization Compliance program, organized across Volumes I–IV:

**Vol I Books 01–04 — Substrate** (naming, identity, and trust foundations):

- **Vol I Book 01** — DDI/IPAM naming-and-addressing plane — 18 NIST SP 800-53 Rev 5 controls
...
**Vol IV Book 05 — Training and Awareness** — AT family cybersecurity training program — activates KSI-022 (KSI-CED-RAT), making all 29 rules in the series' internal KSI decomposition evaluable at high confidence.

**Vol IV Book 06 — Authorization Package and ConMon** — OSCAL AP+AR+POA&M bundle (29/29 rules self-assessed, pending independent security control assessment (SCA)), continuous monitoring program, and FedRAMP VDR/VER submission (NTC-0014) — closing CA-2, CA-5, CA-6, CA-7, RA-3, and SA-15.

---

**Together, the twenty-three implementation books constitute the complete evidence-generation and authorization-package layer for the agency's RMF authorization, with continuous-monitoring evidence structured in the FedRAMP 20x KSI idiom.** Each book maps its implementations directly to the NIST SP 800-53 Rev 5 controls it closes, and Vol IV Book 06 assembles those mappings into a machine-readable OSCAL authorization package ready for independent SCA. The FedRAMP cloud services {{< meta agency.short >}} consumes (M365 GCC, AWS GovCloud, DDI SaaS) are inherited as leveraged authorizations; {{< meta agency.short >}} itself does not submit to the FedRAMP PMO.

**Beyond the architecture corpus, five further volumes make it buildable and operable** (the eleven-volume series, 0–X): **Volume VI** deploys the architecture as code (IaC, detection and SOAR libraries, baselines, the evidence pipeline); **Volume VII** coordinates M365/Azure control state into tracked ServiceNow tasks and KSI evidence; **Volume VIII** realizes the DDI naming plane across every cloud (Azure, AWS, GCP, OCI, VMware — the one deliberate multi-CSP exception); and **Volume IX** governs routine day-2 operations (helpdesk, landing-zone front door, app-registration, Teams telephony) through the same ServiceNow discipline; **Volume X** documents where the series’ evidence lands on the governance substrate — an **optional, separable binding** the series never depends on. These volumes deploy, coordinate, and operate the controls the architecture closes rather than closing new ones — see *Series Extension II* above and the Series Control Crosswalk appendix for the full per-control view.

Three technologies recur throughout the series as the **only** closure mechanisms for their control sets — not the preferred options among several: **IPAM/DDI** (the authoritative naming-and-addressing plane), **SD-WAN** (the encrypted, application-aware transport overlay), and **TIC 3.0** (the distributed policy enforcement that makes direct internet access lawful). The Closure Necessity Doctrine below states the claim; the KSI Closure Necessity Matrix quantifies it against all 29 rules in the series' internal KSI decomposition.
...
**The twenty-three implementations described in this series are not optional modernization projects.** They are the structural prerequisites for an agency Moderate authorization whose evidence survives the FedRAMP 20x transition. CR26 is mandatory for the cloud service offerings {{< meta agency.short >}} inherits effective January 1, 2027; {{< meta agency.short >}}'s obligation is to ensure inherited services are on the 20x path and to structure its own ConMon evidence on the same KSI schema so inheritance stays clean.
:::
...
![Federal Organization Compliance Series, architecture layer — Volumes I–IV, 23 implementation books: Substrate, Modernization, Operations, One Compliance Architecture](figs/es-fig-01-series-overview.png){fig-alt="Three-band blueprint diagram on a white background. The subtitle reads five volumes, 23 implementation books. The top band, labeled Substrate — naming, identity, and trust foundations (Vol I Books 01–04), contains four navy columns: Vol I Book 01 Landing Zone IPAM / DDI / PKI (InfoBlox DDI, DNSSEC, RPZ, certificates — 18 NIST controls closed), Vol I Book 02 Network Access Control (802.1X, RADIUS, device identity at the port), Vol I Book 03 Certificates and Tokens (cryptographic identity, PKI lifecycle, token issuance), and Vol I Book 04 HRIT Identity and Org SSOT (HR system of record, joiner-mover-leaver lifecycle). Teal arrows point down to the middle band, Modernization Tracks (Vol I Books 05–07 and Vol II), with four ice-blue boxes: Vol I Book 05 Network and Identity Modernization (Entra ID, ZTNA, Active Governance — 13 NIST controls closed), Vol I Book 06 Telecommunications Modernization (SD-WAN plus SASE — 7 NIST controls closed), Vol I Book 07 Network Enforcement Substrate (NIAP, DISA STIG, FIPS 140-3, DoDIN APL — an extension book, the accreditation substrate, drawn with a dashed border), and Vol II Books 01–03 SQL Server and Database Architecture (Azure Arc, Entra authentication, network physics — 7 additional controls closed). Teal arrows continue down to the bottom band, Operations and Governance (Vol III and Vol IV), with four white boxes: Vol III plus Vol IV Book 01 Security Operations (PAM, VulnMgmt, Purview, SIEM/XDR, Business Continuity, plus the extension books Patch and Systems Management, Cloud-Native Posture, and the Multi-Cloud Evidence Fabric — about 55 controls closed or deepened), Vol IV Books 02–04 Risk and Program Governance (SCRM, PM governance, PII processing transparency), Vol IV Book 05 Training and Awareness (role-based curriculum, KSI-CED evidence), and Vol IV Book 06 Authorization Package (OSCAL bundle, continuous monitoring CA-7). A footer badge states that the six original tracks — Vol I Books 01, 05–06 and Vol II, each marked with a teal dot — close 34 distinct NIST SP 800-53 Rev 5 controls, that four extension books (Vol I Book 07 and Vol III Books 03, 04, 07) close additional controls, and that all 23 implementation books generate machine-readable KSI evidence for FedRAMP 20x." width="100%"}

| Designation | Document | Domain | Core Technology |
|------|----------|--------|-----------------|
| Vol I Book 01 | OrgComp — {{< meta agency.short >}} Landing Zone IPAM / FedRAMP | DNS · DHCP · IPAM · PKI · Zero Trust | InfoBlox DDI |
| Vol I Book 02 | OrgComp — Network Access Control: 802.1X and Device Identity | 802.1X EAP-TLS · RADIUS/CoA · DHCP fingerprinting · NAC policy tiers | 802.1X + RADIUS + Intune SCEP |
| Vol I Book 03 | OrgComp — Certificates & Tokens: Cryptographic Identity | PKI lifecycle · ACME automation · OAuth 2.0/OIDC · CBA · workload identity | Entra ID CBA + ACME + managed identity |
| Vol I Book 04 | OrgComp — OPM HRIT and the Identity & Organizational SSOT | HR system of record · joiner/mover/leaver lifecycle · org hierarchy as queryable truth · HR-driven provisioning | Oracle HCM Cloud + Entra inbound provisioning |
| Vol I Book 05 | OrgComp — Network Modernization | Identity · Governance · Active Directory modernization | Entra ID · ZTNA · Active Governance |
| Vol I Book 06 | OrgComp — Federal Telecommunications Modernization | Amazon Connect · Teams Phone · SD-WAN · SASE | SD-WAN / SASE stack |
| Vol I Book 07 | OrgComp — Network Enforcement Substrate | Customer-owned NGFW/edge · distributed policy enforcement · accreditation gate | Cisco · Palo Alto · Juniper |
| Vol II Book 01 | OrgComp — SQL Server Authentication Modernization | SQL credential elimination · Arc Managed Identity · NTLM elimination | Azure Arc + Entra ID for SQL |
| Vol II Book 02 | OrgComp — SQL Server Implementation Guide | 10-book runbook: estate audit, Arc deployment, login migration, monitoring | Azure Arc + Entra ID (implementation) |
| Vol II Book 03 | OrgComp — Database Consolidation and Network Physics | SSOT architecture · SD-WAN for database traffic · consistency class alignment | SQL + Network Physics |
| Vol III Book 01 | OrgComp — Privileged Access Management | Entra PIM JIT activation · PAW architecture · AC-6 least privilege | Entra PIM + PAW |
| Vol III Book 02 | OrgComp — Vulnerability Management | Defender VM · CISA KEV integration · RA-5 continuous scanning | Defender MDVM + KEV |
| Vol III Book 03 | OrgComp — Patch & Systems Management | Flaw remediation · configuration state · SI-2 / CM closure | Azure Update Manager · multi-CSP patch |
| Vol III Book 04 | OrgComp — Cloud-Native Security Posture & Containers | CSPM · CNAPP · Kubernetes runtime | Defender for Cloud · CSPM / CNAPP |
| Vol III Book 05 | OrgComp — Data Protection and Purview | Purview DLP · sensitivity labels · Key Vault CMK · MP/SC controls | Purview + Key Vault |
| Vol III Book 06 | OrgComp — SIEM / XDR / Detection Engineering | Sentinel connectors · KQL analytics rules · UEBA scoring · Logic App playbooks | Microsoft Sentinel + Defender XDR |
| Vol III Book 07 | OrgComp — Multi-Cloud Evidence Fabric | Unified evidence contract · telemetry → CDM → KSI | Multi-CSP evidence pipeline |
| Vol IV Book 01 | OrgComp — Business Continuity | CP-2 contingency plan · BIA RTO/RPO/MTD · SQL AG and DDI DR integration | Azure Site Recovery + DR runbooks |
| Vol IV Book 02 | OrgComp — Supply Chain Risk Management | Syft CycloneDX SBOM · Grype vulnerability scan · OpenVEX exploitability · GitHub Actions pipeline | EO 14028 §4 / SR-family (remediation SLAs: BOD 26-04) |
| Vol IV Book 03 | OrgComp — Program Management and Governance | ISPP · POA&M · SCRM Charter · Leadership RACI · PM-1/PM-9/PM-30 closure | PM-family governance artifacts |
| Vol IV Book 04 | OrgComp — PII Processing and Transparency | PT family templates · Privacy Program Plan · authority-to-process · Purview Data Map | PT-1–PT-7 GCC Moderate |
| Vol IV Book 05 | OrgComp — Cybersecurity Awareness and Training | AT-2/AT-3 general and role-based training · LMS evidence schema · CP-3 tabletop calendar · IR-2 drill documentation | AT/CP/IR families · KSI-CED-RAT |
| Vol IV Book 06 | OrgComp — Authorization Package and ConMon | Agency RMF authorization package (ConMon evidence modeled on the FedRAMP 20x KSI schema) · OSCAL AP+AR+POA&M bundle · FedRAMP VDR/VER submission (NTC-0014) · CA-7 ConMon program | CA-2/5/6/7 · RA-3 · SA-15 |
| Vol V Book 00–04 | OrgComp — Training & Certification | Enablement · two-track model · eight evidence slots as curriculum · rubric ladder · vendor labs | OrgComp Practitioner/Assessor credentials · KSI-CED |
| Vol VI Book 00 | OrgComp — Implementation (Overview) | The deployable layer — architecture operationalized as code | IaC · CI/CD · CM-2/CM-3 |
| Vol VI Book 01 | OrgComp — Landing Zone & Network as Code | SC-7/SC-20/21/22, CM-8 realized as reviewable infrastructure code | Terraform / Bicep |
| Vol VI Book 02 | OrgComp — Identity & Access as Code | AC-2/5/6, IA-2/3/5, SC-17 as policy-as-code | Entra · SCIM · policy-as-code |
| Vol VI Book 03 | OrgComp — Detection Engineering | AU-6, SI-4 analytics rule library | Sentinel KQL rules |
| Vol VI Book 04 | OrgComp — Response Automation (SOAR) | IR-4/6, AC-12 playbook library | Logic Apps / SOAR |
| Vol VI Book 05 | OrgComp — Data Protection Deployment | MP-2–5, SC-12/28, AU-10 deployment | Purview · Key Vault |
| Vol VI Book 06 | OrgComp — Configuration Baselines & Hardening | CM-2/3/6/7, SI-2 baselines | Machine Config · hardening |
| Vol VI Book 07 | OrgComp — Evidence & ConMon Pipeline | AU-6/12, CA-5/7 evidence wiring | OSCAL emitter pipeline |
| Vol VI Book 08 | OrgComp — Validation & Test Harnesses | CA-2/7, SI-4 validation | Conformance test harnesses |
| Vol VII Book 00 | OrgComp — ServiceNow Automation (Overview) | The coordination layer for M365/Azure/AWS/VMware control compliance | ServiceNow (Gov) + in-boundary MID Server |
| Vol VII Book 01 | OrgComp — CMDB Reconciliation & Asset Identity | CM-8 (reconciled to IPAM/DDI), CM-3 | ServiceNow Discovery / Service Graph |
| Vol VII Book 02 | OrgComp — M365 Control-Compliance Automation | CM-6, AC-2, CA-7 drift → tracked task | Graph · SCuBA · ServiceNow |
| Vol VII Book 03 | OrgComp — Azure Control-Compliance Automation | CM-6, SI-2, RA-5 → remediation task | Azure Policy · Defender · ServiceNow |
| Vol VII Book 04 | OrgComp — Control Attestation, Evidence & KSI | CA-2, CA-5, CA-7 → OSCAL/KSI evidence | ServiceNow IRM/GRC · OSCAL |
| Vol VII Book 05 | OrgComp — The ServiceNow Compliance App | Scoped app · Flows · ATF · Update Set | ServiceNow scoped app |
| Vol VII Book 06 | OrgComp — AWS Control-Compliance Automation | CM-6, SI-2, RA-5 → remediation task | AWS Config · Security Hub · ServiceNow |
| Vol VII Book 07 | OrgComp — VMware Control-Compliance Automation | CM-6, SI-2, RA-5 → remediation task | vSphere Config Profiles · VCF Operations · ServiceNow |
| Vol VII Book 08 | OrgComp — Native Cloud IAM/RBAC Access-Governance Automation | AC-2, AC-3, AC-6 → remediation task | SailPoint ISC Governance (CIEM) · ServiceNow |
| Vol VIII Book 00–A | OrgComp — Multi-Cloud DDI Landing-Zone Automation | SC-20/22, CM-8, CM-3/CM-5, AU-2 across Azure/AWS/GCP/OCI/VMware (multi-CSP exception) | InfoBlox DDI + ServiceNow front door |
| Vol IX Book 00 | OrgComp — Day-2 Operations (Overview) | The governed catalog for routine Entra/M365/Azure day-2 work | ServiceNow · Graph via in-boundary MID |
| Vol IX Book 01 | OrgComp — Helpdesk & ITSM Catalog | AC-2, IA-5, AC-6 as governed catalog items | ServiceNow catalog over Entra |
| Vol IX Book 02 | OrgComp — Landing Zone Front Door | CM-2, CM-8, CM-3 (catalog → IaC → CMDB) | Terraform / Bicep · CMDB |
| Vol IX Book 03 | OrgComp — App Registration Governance | IA-5(2), SC-17, AC-6 · consent + secret/cert lifecycle | ACME short-lived certs |
| Vol IX Book 04 | OrgComp — Teams Telephony Catalog & SCuBA Drift | CM-3, CM-6, AU-2 · E911 validation | Teams Phone · SCuBA loop |
| Vol IX Book 05 | OrgComp — SaaS Integration Governance | SA-9 †, CA-3, AC-20 · verify authorization, then configure | Entra gallery · FedRAMP Marketplace · ServiceNow |
| Vol IX Book 06 | OrgComp — Identity & Access Governance | AC-2, AC-5, AC-6, IA-4, IA-5, AU-2 · event-driven JML · certification campaigns | HRIT SSOT · ServiceNow (Kit Phase 2, gated) |
| Vol X Book 00 | OrgComp — Governance-Substrate Integration (overview) | — (no new closures: routes and explains) | Conformance layer · surface slots · evidence contract — **optional binding** |

: Federal Organization Compliance Series — Vols 0–X {.striped .hover}

The series closes with **Volume X — Governance-Substrate Integration**: where
everything the volumes above produce lands as machine-checked evidence on a
governance substrate, and why that binding is **optional**. The series stands
complete at FedRAMP Moderate on its own; the wider Org family — **OrgPath**
(governance), **OrgComp** (this series), and **OrgMod** (modernization) — is
designed to be implemented **together or entirely independently**, in any
order, with no expression a prerequisite for another.
...
| **Aug 31, 2026** | Class B + C opens | Class C (Moderate) CSO certifications open — the pipeline {{< meta agency.short >}}'s inherited providers enter |
| **Dec 7, 2026** | FedRAMP VDR/VER rules mandatory (NTC-0014); BOD 26-04 remediation timelines in force | FedRAMP's Vulnerability Detection & Response (VDR) and Vulnerability Evaluation & Reporting (VER) rule sets become mandatory (NTC-0014), aligned to the same date CISA BOD 26-04's risk-tiered remediation timelines take effect; KSI-011–014 become evaluable |
...
including the elapsed baseline (M-21-31 logging — rescinded 22 May 2026 by M-26-14 — M-21-07 IPv6, M-22-09 Zero
Trust, BOD 25-01 SCuBA) — is maintained in the companion document
*OrgComp / ConMon Compliance Gap Roadmap*, which is the series' single
...
| Jan 1, 2027 | CNSA 2.0 acquisition requirement (NSS-scoped) | New NSS acquisitions must **support** ML-KEM-1024 + ML-DSA-87 unless an exception is explicitly noted in the acquisition documentation (NSA CNSA 2.0 FAQ); civilian PQC trajectory follows via M-23-02 |
| Feb / Aug windows | Future NIST SP 800-53 releases | NIST issues **minor** releases in its February and August windows (up to twice yearly) and a **major** release/Revision every 2 years; Release 5.2.0 (Aug 27, 2025) was a *minor* release adding SA-15(13), SA-24, SI-2(7) — not yet mapped in Vol III Book 02 or Vol IV Book 02; tracked in the roadmap |
...
across these eleven volumes belongs to one of seven functional planes — including the
...
| **SD-WAN** — encrypted, application-aware transport overlay | SC-8, SC-8(1) on DIA circuits (network-layer encryption); SC-5 (traffic-class separation); SI-4 (application-layer telemetry on transport) | A pipe has no encryption, no path intelligence, and no telemetry. Physics gives DIA lower latency; only the overlay gives it a control surface. FIPS-validated application-layer TLS closes SC-8 per flow, but only a network-layer overlay closes it for *every* flow on the circuit at once — and among overlay classes SD-WAN is the one that also delivers the SC-5 traffic-class and SI-4 telemetry closures. (On a field-office circuit egressing over the public internet, the SC-8(5) physical path and intra-boundary inheritance do not apply, so the flows TLS does not reach are the gap the overlay fills.) |
| **TIC 3.0** — distributed policy enforcement for the pipes | SC-7, SC-7(7) (boundary protection on distributed egress); AC-4 (information flow enforcement outside the legacy TIC access point) | Under TIC 3.0, DIA requires a distributed enforcement point (the SASE stack) providing protection *commensurate with the agency's risk tolerance* — in practice the applicable security-capabilities catalog entries. An ungoverned DIA circuit is not a modernization; it is a finding. |

: The Three Necessity Anchors {.striped .hover}

> **Baseline caveat:** These closure claims are made against the NIST SP 800-53 Rev 5 control baseline and FedRAMP 20x CR26 rules published June 25, 2026 (optional adoption July 4, 2026; mandatory January 1, 2027). Changes to the control baseline, new FedRAMP-authorized product entries, or revised assessment guidance may alter these closure paths. Validate with OIS and the agency's independent assessor before using these claims as the basis for authorization decisions.

::: {.aan-important}
## The SC-8 Row Against Its Strongest Counter-Sources

The SC-8 row above is a **coverage-completeness** claim, not an exclusivity
claim, and it is written to survive the two objections a well-sourced
reviewer will raise.

**"FedRAMP accepts TLS."** Correct. FedRAMP's Rev 5 data-in-transit guidance
("The Rev. 5 Approach to SC-8, and Protecting Data-in-Transit," fedramp.gov,
July 2023) requires FIPS 140-validated encryption and names no protocol:
FIPS-validated TLS closes SC-8 for the flow that negotiates it, Rev 5 moved
physical protection to SC-8(5) (a Controlled Access Area path satisfiable
through PE-family controls), and SC-8 / SC-8(1) are inheritable when traffic
originates and terminates inside an authorized boundary. None of those paths
reaches a field-office DIA circuit: the traffic originates outside every
authorized boundary, no Controlled Access Area encloses the public internet,
and TLS closure must then be demonstrated **flow by flow** — while DNS
lookups, legacy protocols, and misconfigured applications ride the same
circuit bare. The overlay is the mechanism that converts an unbounded set of
per-flow attestations into one circuit-level attestation.

**"Our traffic is M365, and M365 is already encrypted."** Also correct —
Microsoft's service endpoints negotiate TLS with FIPS-validated cryptography
available, and Microsoft's network connectivity principles recommend that
Optimize-category traffic (Teams media above all) egress locally and bypass
tunnels and inspection, for exactly the ITU-T G.114 latency reasons this
series cites. Neither fact rebuts the row; both are instances of it. M365's
TLS is the conceded per-flow closure, and the circuit still carries the DNS
queries that resolve those very endpoints plus every non-M365 flow. Where
the agency follows Microsoft's bypass guidance, the SD-WAN edge is what
makes the exemption **deliberate and attestable**: application-aware
steering records which flows ride the IPsec overlay and which rest on the
application's own FIPS-validated TLS, turning a split-tunnel exception from
an invisible gap into per-flow evidence — the same SI-4 telemetry closure
the row already claims.

Mechanism-class honesty cuts both ways: plain site-to-site IPsec, ZTNA
tunnels, and other network-layer overlays also encrypt every flow on the
circuit without being SD-WAN. The necessity claim is the bundle — among
overlay classes, SD-WAN is the one that co-delivers the SC-5 traffic-class
separation and SI-4 application-layer telemetry closures alongside SC-8.
:::

# The Closure Provenance Doctrine

Closure Necessity answers *which mechanisms can close a control*. Its twin
answers *what counts as closed*: **no closure counts unless it carries its
proof.** A remediation task, a daily operational check, a POA&M milestone, or
a control test closed by a human clicking "complete" — with no
machine-readable verification attached to the closing record — is prose, not
evidence. It asserts that work happened; it proves nothing an assessor or a
KSI scorer can consume.

This series already applies the rule to itself. Every "Authorities Closed
Here" table, the spine crosswalk, and the claim→evidence→test matrix are
**generated and drift-gated**, because hand-maintained numbers silently
falsify. A manually closed task is the operational hand-maintained number:
the dashboards, SLA reports, control tests, and KSI feeds built on a
manual-closure stream inherit its central defect — every row is an
unverified claim, so the reporting layer is a compilation of assertions.
That is why manual closure fails to create reporting. Reporting is not
something added after closure; it is something a well-formed closure
**emits**.

The doctrine in three requirements:

1. **Every closure carries provenance.** The closing update on a task
   carries the verification payload that justified it — the probe result,
   the posture query, the re-test output — with a timestamp and the identity
   of the checking mechanism. The close *is* the contract: a closure record
   without a verification payload does not validate.
2. **Human-only closure is an exception state, not a path.** It remains
   possible — operations need an escape hatch — but it is visibly flagged,
   counted separately, and never silently aggregated into control tests, KSI
   evidence, or SLA attainment. An attestation pipeline that cannot
   distinguish probe-backed closures from attested-only ones attests
   fiction.
3. **Reports are derivations, never compilations.** Every report in the
   coordination plane — ConMon roll-ups, POA&M aging, patch SLA attainment,
   the KSI feed — is derived from the closure stream by a generator that can
   be re-run and diffed, exactly as this series' own tables are. A report
   that cannot be regenerated from closure records is an artifact of effort,
   not of state.

Closure provenance is also the one place a capped deployment gets full
automation. Verifying a closure is a **read** — probe the estate, attach the
result, gate the close. Under the day-2 actuation ceiling ("no undeclared
autonomous estate writes"), closure verification and auto-closure of the
coordination record are lawful at full automation even where the estate-side
write requires a human approver: the write already happened under its own
authority, and recording *proof* that it happened requires none.

The series measures itself against this doctrine. The claim→evidence→test
matrix binds a falsifying test to each closure claim, and today only
**13 of 106** claims yet carry a named test. That ratio is the series' own
closure-provenance indicator — generated, gated, and expected to rise; Vol
VI Book 08 (validation harnesses) and Vol VII Book 04 (control attestation)
are the books that raise it, and the day-2 scoped app enforces the closure
rules at the task layer (Vol VII Books 04–05).

# The Vendor Currency Doctrine — A Roadmap Is Not a Commitment {#vendor-currency}

This series names specific vendor products and capabilities on purpose — Closure Necessity requires naming a real mechanism, not a category, so a book says "AWS Config remediation actions" rather than "some CSPM tool." That specificity has a cost: the market this series describes moves in weeks, not review cycles. Two concrete instances from this series' own drafting, both discovered the same week: a ServiceNow-native FedRAMP 20x KSI product (C1Secure SmartRAMP 20x) did not exist when Vol VII Book 05 was first scoped and shipped roughly two weeks later; native ServiceNow↔VMware connectors Vol VII Book 07 initially treated as unbuilt turned out to already exist as vendor-published integrations. Neither claim was wrong when written — both were current, sourced, and dated. Both were also stale within a month.

The doctrine, stated once here for every book and whitepaper in the series to cite rather than restate:

1. **Every vendor capability claim in this series is dated, not permanent.** A document's Date Code marks when its vendor claims were last verified — never an assertion that they remain current.
2. **Before any claim in this series becomes the basis of a Project Plan, procurement action, or acquisition decision, the current roadmap must be confirmed directly with the vendor** — ServiceNow, SailPoint, VMware/Broadcom, and each named CSP (Microsoft, AWS, GCP, OCI) — not assumed from this series' snapshot.
3. **A closed gap can reopen and an open gap can close between two drafting sessions in the same series.** Treat every "not yet built" and every "ships natively" claim as equally perishable; neither is more durable than the vendor's own next release note.

> **Vendor currency caveat:** Vendor product names, capabilities, integration claims, and FedRAMP authorization status throughout this series reflect the date each book or whitepaper was last updated (its Date Code) and move quickly. **Confirming current vendor roadmaps directly with ServiceNow, SailPoint, VMware/Broadcom, and each CSP is essential before any implementation Project Plan is built on a specific claim in this series.** Validate with OIS and the agency's independent assessor, as with every other closure claim in this series.

# The MACD-R Lifecycle Doctrine — Every Operation Travels the Authorized Path {#macdr-lifecycle}

Closure Provenance governs the end of an operation's life; the full law
covers the whole path. Every operational change in the estate — every
**Move, Add, Change, Deletion, and Reset (MACD-R)** — travels five clauses,
and an operation that skips one is unauthorized by construction:

1. **It originates with the SSOT.** Work is created by divergence between
   the source of truth and the estate — a joiner in the HR system of
   record, a control gap against the compliance spine, a migration wave in
   the modernization plan. A task with no SSOT origin is an ad-hoc
   assertion: nothing authoritative asked for it, so nothing authoritative
   can attest it.
2. **It carries an authoritative authorization in the path.** The approval
   resolves from the authority model, not from whoever is nearby: the owner
   and approver derive from the same SSOT that originated the work (the
   manager/owner routing the identity substrate rebuilds), and the
   pre-authorized lanes (A0/A1) are standing authorizations — recorded
   grants with scope and expiry, never the absence of approval.
3. **It executes under least privilege, elevated just-in-time.** The
   executing actor — human or automation — activates a time-boxed, scoped
   role at execution (PIM) and holds no standing privilege. The activation
   record is part of the operation's evidence, binding *who could act* to
   *what was done* (AC-6, and the PAM discipline of Vol III Book 01).
4. **It closes with provenance.** The Closure Provenance Doctrine above: no
   closure counts unless it carries its proof.
5. **It emits verified, secured evidence — automatically.** The closure
   record, integrity-protected and carrying its SSOT lineage (originating
   record, authorization, PIM activation, verification payload), flows into
   the continuous-monitoring stream with no human assembly. Evidence a
   person compiles arrives late and unverifiable; evidence the lifecycle
   emits arrives continuously and carries its own chain of custody (CA-7).

The five clauses are one chain — **SSOT-originated → authoritatively
authorized → least-privilege executed → provenance-closed →
evidence-emitted back to the SSOT's reporting plane** — and the chain is
the Org family's common law, not this series' alone: OrgPath applies it to
identity and attribute governance, OrgComp (this series) to control
remediation and attestation, OrgMod to migration and modernization moves.
Three pillars, one lifecycle, one evidence contract.

## The SSOT Registry — Where Every MACD-R Originates {#ssot-registry}

Clause 1 demands a source of truth per object class — including the
**non-human** classes, where most estates have never designated one.
Exactly one SSOT is designated per class; everything downstream —
directory objects, CMDB records, management-console state — is a
**projection** whose divergence from the SSOT is drift, never an
alternative truth (the DNS split-brain law, generalized):

| Object class | SSOT — where its MACD-R originates | Projections (never truth) |
|---|---|---|
| Employees | HR system of record | Directory accounts, mailboxes |
| Non-employee humans (contractors, vendors, partners) | Non-employee registry | Guest accounts, sponsored credentials |
| Network names, addresses, segments | IPAM/DDI | Zone dumps, spreadsheets, scanner output |
...
| Devices / endpoints (lifecycle) | Asset-lifecycle (ITAM/procurement) registry, keyed to the join key | MDM enrollment state (enforcement, not truth) |
| Service accounts, workload identities, app registrations | The **non-human identity registry** — purpose, scope, expiry, owner | Directory service principals, key vault entries |
| AI systems | The agency AI use-case inventory, with a full joiner/mover/leaver lifecycle | Deployment manifests, model endpoints |
| Applications | Application portfolio registry | App registrations, service-catalog entries |
| Certificates | The issuance ledger of the PKI plane | Local cert stores, binding configs |
| Infrastructure declared state | The IaC repository (declared state in version control) | Deployed resources, consoles |
| Org structure / physical location | The governed org-path and location codebooks | Directory OU trees, site topology |

: The SSOT Registry — one source of truth per class {.striped .hover}

Two rules make the table a doctrine rather than a list. **The Human-Anchor
Rule:** every non-human item anchors to a human owner resolvable in the HR
SSOT — the ownership chain terminates in a person, or the item is
**orphaned**: clause 2 cannot resolve an approver for it, so no operation
against it can be authorized, and nothing can attest it. The HR SSOT is
therefore the root of the ownership chain for *every* governed object, and
a leaver event fans out to the leaver's entire anchored non-human estate.
**Non-humans get JML too:** provision is a joiner, rescope or re-owner is
a mover, decommission is a leaver — with **Reset** (credential, secret,
and key rotation) as the high-risk verb for non-human identities, because
rotation is where standing privilege hides. An estate item with no SSOT
record, or an owner no longer resolvable, is an orphan finding — the
highest-value output any discovery scan can produce, because a scan can
only measure drift from a truth; it can never create one.
...
alternatives matrix in the companion *OrgComp / ConMon Compliance Gap
...
2. **What DIA satisfies:** none of the protection controls claimed near it.
   ...
   confidentiality, malware inspection, monitoring, and audit are satisfied
   by the SD-WAN/SASE/TIC 3.0 stack that governs the pipe — never by the
   pipe. The one closure transport selection itself provides is **CP-8
   (Telecommunications Services)**, through provider and path diversity —
   an argument for the multi-transport menu, never for the bare circuit.
...
: OrgComp Modernization Sequence — Architecture Before Assessment {.striped .hover}
...
The authoritative closure-necessity view for all 29 rules in the series'
internal KSI decomposition. Each row states what closes the rule set, whether conformance
...
> matrix are the series' internal KSI decomposition, not the published CR26 Moderate KSI
> catalog. The rule-ID-by-rule-ID diff against the CR26 catalog committed in-repo
> now exists — see **`OrgComp_CR26_Reconciliation.md`** (generated from the rule
...
| KSI-011..014 (4) | CMT — change-management evidence | SBOM/VEX pipeline (Syft, Grype, OpenVEX) under EO 14028 §4 / SR-family; every dependency change logged, immutably redeployed, and validated per deployment | No | slot-06 security | Vol IV Book 02 |
...
| KSI-023..029 (7) | INR / RPL | Sentinel/XDR incident playbooks + DR/continuity architecture | No | slot-07 continuity, slot-04 telemetry | Vol III Book 06 and Vol IV Book 01 |

: KSI Closure Necessity Matrix — 29 internal KSI rules {.striped .hover}
...
| **SC-8: Network-layer encryption absent on DIA circuits** | DIA circuits carry traffic unencrypted at the network layer; confidentiality rests on per-flow application-layer TLS, which covers only the flows that negotiate it — DNS, legacy protocols, and misconfigured applications ride bare — with no circuit-level attestation | No IPsec overlay on DIA circuits | SD-WAN IPsec overlay — encrypts every flow on the circuit at the network layer, converting an unbounded set of per-flow TLS attestations into one circuit-level closure |
...
# Vol I Book 01 — Landing Zone DDI: InfoBlox DDI Closes 18 NIST Controls

## What This Document Addresses

Every federal cloud landing zone — whether on AWS GovCloud, Azure Government, Oracle OCI, or on-premises VMware — requires DNS, DHCP, and IP Address Management (DDI) to function. {{< meta agency.short >}} operates four such environments simultaneously. Without a unified DDI control plane, each environment generates its own IP inventory, its own DNS logs, and its own certificate records. FedRAMP compliance evidence must be assembled separately from each.

**InfoBlox DDI** is one of a small number of FedRAMP Moderate-authorized DDI platforms — but that DDI-specific authorization is six days old as of this writing, not the December 2022 vintage most of this series cites. CSO FR2017257053 was originally agency-authorized December 15, 2022 via the U.S. Census Bureau / Dept. of Commerce for **BloxOne Threat Defense Federal Cloud** — a DNS-security/threat-intelligence service, not DDI/IPAM. On **July 22, 2026**, the same CSO — rebranded **Infoblox Government Cloud**, hosted on AWS GovCloud — was recertified FedRAMP Moderate with an expanded boundary adding **Universal DDI Management, NIOS-X Servers, and Universal Asset Insights**. Verify current authorization scope, sponsor, and package status on the FedRAMP Marketplace at time of procurement — this is a newly expanded boundary. A single InfoBlox deployment spanning all four {{< meta agency.short >}} environments closes **18 NIST SP 800-53 Rev 5 controls** that would otherwise require manual evidence assembly from four separate sources.
...
| **SC-7(7)** | Split Tunneling for Remote Devices | InfoBlox RPZ | No DNS-layer control over split tunnel behavior | RPZ blocks resolution of split-tunnel-bypass domains; enforces GCC-only Microsoft 365 resolution | RPZ policy export; block event log |
...
| **AU-2 / AU-12** | Event Logging / Audit Record Generation | InfoBlox Syslog | DNS, DHCP, and IP allocation events logged per-environment in incompatible formats; no unified SIEM feed | Single syslog stream from all Grid Members across all four environments; RFC 5424 format; 3-year retention configured | SIEM dashboards; retention policy; pre-built Sentinel/Splunk queries |
...
Domain-controller-tethered authentication died with the perimeter: Kerberos
depends on continuous reachability to a KDC, and under slow active-passive WAN
failover that dependency surfaces as the 30-to-45-second logon field offices
report when the path to the domain controller drops (Vol I Book 06 documents
the mechanism; active-active multipath removes the failover gap). The ticket
itself is not network-path-bound — Windows omits client addresses from tickets
by default — so the modernization case does not rest on ticket binding; it
rests on the deeper asymmetry: Kerberos needs a reachable DC, while a token
carries cryptographic authorization that survives any transport. Vol I Book 03
establishes what replaces it:
...
# Vol I Book 05 — Network and Identity Modernization: Entra ID + ZTNA + Active Governance Closes 13 NIST Controls
...
The agency's authentication architecture was designed for a world where all users, all servers, and all applications were co-located inside a data center connected by a private network. Active Directory Kerberos assumes continuous line-of-sight to a domain controller — an assumption that holds inside a data center and degrades once client and server are separated by cloud, VPN, or a WAN whose path to the DC can fail; the logon experience, not the ticket, is what suffers. The agency now operates across four cloud environments simultaneously. The authentication model has not been updated to match.
...
|---|---|---|---|---|---|
| **IA-2 / IA-2(1)** | Identification and Authentication / Multi-factor Authentication to Privileged Accounts | Entra ID Conditional Access | Kerberos-based authentication; MFA optional and inconsistently applied; NTLM fallback generates opaque, unauditable authentication events | Entra ID Conditional Access enforces phishing-resistant MFA (CBA/FIDO2) for every access request; NTLM eliminated; every authentication event generates a structured log | OMB M-22-09; BOD 25-01 |
...
| **SC-20 / SC-21** | Secure Name/Address Resolution Service (Authoritative Source / Recursive or Caching Resolver) | Active Governance + InfoBlox | DNS security dependent on per-environment tools with no unified policy | Unified DNS security enforced via governance control plane; DNSSEC and RPZ applied consistently across all environments | NIST SP 800-53 Rev 5 SC-20, SC-21 |

: Vol I Book 05 — NIST SP 800-53 Rev 5 Control Closure: Entra ID + ZTNA + Active Governance {.striped .hover}

**Controls closed: 13** (IA-2/IA-2(1), IA-3, IA-5, AC-2, AC-3, AC-4, AC-17, CM-8, SI-12, AU-2/AU-12, CA-7, SC-7, SC-20/SC-21)
...
DIA (Direct Internet Access) circuits at field offices solve the latency problem by eliminating the geographic hairpin. However, **DIA alone satisfies zero NIST controls.** DIA is a pipe. The NIST control requirements for information flow enforcement, boundary protection, transmission confidentiality, malware protection, system monitoring, audit logging, and continuous monitoring are satisfied by the SASE and SD-WAN control stack that governs the pipe — not by the pipe itself. And under TIC 3.0, the relationship is not advisory: DIA requires a distributed enforcement point providing protection *commensurate with the agency's risk tolerance* — in practice the applicable security-capabilities catalog entries. The three-step form of this argument — what DIA solves, what it satisfies, what bare DIA breaks — is stated in *The Closure Necessity Doctrine* above.
...
| **SC-8** | Transmission Confidentiality and Integrity | SD-WAN — IPsec | DIA circuit is unencrypted at the network layer; FIPS-validated application-layer TLS closes SC-8 only per flow — non-TLS flows (DNS, legacy protocols, misconfigured applications) ride bare, and closure must be attested flow by flow | SD-WAN IPsec tunnel encrypts every flow on the circuit at the network layer, regardless of each application's TLS state | SD-WAN IPsec overlay |
| **SI-3** | Malicious Code Protection | SASE — SWG SSL Inspection | No inline malware inspection on DIA path; malware reaches endpoints before detection | SSL inspection and malware scanning inline at SASE PoP nearest to field office; sub-100ms inspection latency | SWG with SSL inspection in SASE stack |
...
| **IA-8** | Identification and Authentication (Non-organizational Users) | Federated authentication events (Entra ID to Amazon Connect SAML assertions) must be logged and KSI-verifiable | SAML assertion log from Entra ID federated to Amazon Connect |
...
| **AU-2 / AU-12** | Event Logging / Audit Record Generation | Entra sign-in logs | NTLM auth events not token-level; no user-to-action-to-data correlation; pass-the-hash invisible to SIEM | Token-level audit event for every Entra-authenticated connection; structured, queryable, correlated to user, device, application | FedRAMP 20x KSIs; NIST SP 800-53 |
...
| **Vol IV Book 02** | Supply Chain Risk Management | SR-1, SR-2, SR-3, SR-5, SR-6, SR-8, SR-10, SR-11 | Syft/Grype SBOM pipeline (EO 14028 §4 / SR-family; remediation SLAs per BOD 26-04) |
...
| **Vol IV Book 06** | Authorization Package and ConMon | CA-2, CA-5, CA-6, CA-7, CA-7(1), CA-7(3), RA-3, SA-15 | OSCAL AP+AR+POA&M · FedRAMP VDR/VER (NTC-0014) · ConMon dashboard |
...
Conformance Adapter and makes all 29 internal KSI-decomposition rules evaluable. **Vol IV Book 06 converts
that evaluable state into an authorization package** — assembling the OSCAL bundle,
filing the FedRAMP VDR/VER submission (NTC-0014), and establishing the CA-7 ConMon program.
...
| **Vol IX Book 06** | Identity & Access Governance | AC-2, AC-5, AC-6, IA-4, IA-5, AU-2 | Event-driven JML from the HRIT SSOT, access-certification campaigns, and the IAM evidence feed — Kit Expansion Phase 2, gated on requirements v1.0 |
...
| **SI** — System and Information Integrity | InfoBlox RPZ + TIDE; SASE SWG; Entra ID OPM HRIT; NTLM monitoring | SI-3, SI-4, SI-12 | Vol I Books 01, 05, 06 and Vol II Books 01–02 |
...
|---|---|---|---|---|
| Vol I Book 01 — InfoBlox DDI Landing Zone | InfoBlox DDI (DNSSEC, RPZ, IPAM, PKI, Syslog) | 18 open with manual/no evidence | 18 closed with automated evidence | 16 controls automated |
...
| Vol I Book 05 — Entra ID + ZTNA + Active Governance | Entra ID, Intune MDM, ZTNA, Active Governance | 13 open or partially satisfied | 13 closed with machine-readable evidence | 12 controls automated |
...
## SR — Supply Chain Risk Management (EO 14028 §4 / SR-family)

The SBOM and exploitability (VEX) pipeline is anchored to EO 14028 §4, OMB
M-22-18 / M-23-16 attestation guidance, the SR control family, and NIST SP
800-161 — **not** to BOD 26-04, which is a risk-tiered *remediation* directive
and does not mandate supply-chain artifacts. What activates **December 7, 2026**
is FedRAMP's Vulnerability Detection & Response (VDR) and Vulnerability
Evaluation & Reporting (VER) rule set (NTC-0014), aligned to the same date
BOD 26-04's remediation timelines take effect; the pipeline's blocking-gate SLAs
follow that remediation model. KSI-015 and KSI-016 (KSI-SCR — Supply Chain Risk
Mitigation and Monitoring) are now evaluable through Vol IV Book 02; the same
pipeline additionally supplies the KSI-CMT change-management evidence for
KSI-011 through KSI-014.

**Controls closed:** SR-1, SR-2, SR-3, SR-5, SR-6, SR-8, SR-10, SR-11

**Vol IV Book 02 — Supply Chain Risk Management** delivers: a Syft CycloneDX SBOM
generation pipeline (EO 14028 §4 / SR-3, SR-4), Grype continuous vulnerability
scanning with OpenVEX exploitability assertions, a GitHub Actions blocking gate
whose remediation SLAs follow the BOD 26-04 four-variable risk model, and a
vendor disclosure matrix. KSI-SCR-MIT and KSI-SCR-MON activated at high confidence.
...
all 29 internal KSI-decomposition rules.
...
All 29 internal KSI-decomposition rules are now evaluable.

## Priority 4: CR26 Breadth Gaps — Resolved

FedRAMP 20x Consolidated Rules cover all 29 internal KSI-decomposition rules. Five themes that
...
verdict below is **self-assessed against the series' internal KSI decomposition, not yet
...
> **Deployed** = running against the {{< meta agency.short >}} estate · **Assessed** = independently
...
|-----------|----------|------|
| Conformance Adapter schema | Authored | 29/29 of the series' internal KSI decomposition mapped (not the 46 CR26 indicators) |
| Surface slots 1–8 (all slots) | Ref-impl | Bound to OrgComp evidence; slot-08 training activated by Vol IV Book 05 |
| OSCAL AP | Ref-impl | 10 themes, 46 CR26 indicators |
...
```bash
# Generate the full bundle (run monthly for ConMon) — shown for the reference
# implementation; any engine satisfying the series evidence contract emits the
# same AP/AR/POA&M bundle shape:
<engine> oscal bundle --system-name "{{< meta agency.short >}} GCC Moderate" \
  --output-dir output/artifacts/ksi-bundle
# Result: 29/29 self-assessed satisfied against the series' internal KSI decomposition,
...
| **Phase 3** | November 2026 | ✅ Complete | ⬜ Not started | Vol IV Book 04 PT/PII templates; Vol IV Book 05 AT training program; KSI-022 → active; all 29 internal KSI rules evaluable |
| **Phase 4** | December 2026 | ✅ Complete | ⬜ Not started | FedRAMP VDR/VER submission (NTC-0014) (due Dec 7); Authorization package assembled for the {{< meta agency.short >}} AO; OSCAL AP+AR+POAM bundle (29/29 self-assessed, pending independent SCA) |
...
| OSCAL AP/AR | The governance engine's OSCAL bundle emitter (per the [series evidence contract](OrgComp_Evidence_Contract_Spec.qmd)) — OSCAL document version pinned in Vol IV Book 06 §2.1 (currently 1.0.4); verify FedRAMP's current machine-readable schema target before each authorization-cycle freeze |
...
|---|---|
| [Vol I Book 01 — Federal Agency Cloud Landing Zone](Vol_I_Book_01_OrgComp_Cloud_Landing_Zone_IPAM_FedRAMP.qmd) | Vol I Book 01: InfoBlox DDI, four-environment landing zone, NIST control mapping, FedRAMP 20x KSI evidence |
| [Vol I Book 02 — Network Access Control: 802.1X and Device Identity](Vol_I_Book_02_OrgComp_Network_Access_Control_802_1X.qmd) | Vol I Book 02: 802.1X EAP-TLS, RADIUS/NPS + CoA, Intune SCEP certificate pipeline, NAC policy tiers, DHCP fingerprinting, Voice VLAN precedent, IA-3/CM-8 necessity closures |
| [Vol I Book 03 — Certificates & Tokens: Cryptographic Identity](Vol_I_Book_03_OrgComp_Certificates_Tokens_Cryptographic_Identity.qmd) | Vol I Book 03: PKI as a naming-plane function, ACME lifecycle automation, OAuth 2.0/OIDC and CBA, phishing-resistant MFA, workload identity, IA-5(2)/SC-12/SC-17 closure |
| [Vol I Book 04 — OPM HRIT and the Identity & Organizational SSOT](Vol_I_Book_04_OrgComp_HRIT_Identity_Org_SSOT.qmd) | Vol I Book 04: HR system of record (Oracle HCM Cloud), joiner/mover/leaver lifecycle, API-driven HR inbound provisioning (Microsoft Graph `bulkUpload` + SCIM 2.0), org hierarchy as queryable SSOT, AC-2/PS-4/PS-5/IA-4 closure |
| [Vol I Book 05 — From Mainframe to Application-Aware Modernization](Vol_I_Book_05_OrgComp_Network_Modernization.qmd) | Vol I Book 05: From mainframe to application-aware — the four principles, AD-to-Entra and ZTNA identity modernization, Kerberos logon under multipath failover, load-balancer dissolution |
| [Vol I Book 06 — Federal Telecommunications Modernization](Vol_I_Book_06_OrgComp_Federal_Telecommunications_Modernization.qmd) | Vol I Book 06: Amazon Connect N8NN, Teams Phone, SD-WAN/SASE, G.114 compliance, TIC 3.0 lawful local breakout, DIA vs. SASE distinction |
| [Vol I Book 07 — Network Enforcement Substrate](Vol_I_Book_07_OrgComp_Network_Enforcement_Substrate.qmd) | Vol I Book 07 (extension book): how customer-owned network gear (Cisco, Palo Alto, Juniper) closes NIST controls, and the NIAP / DISA STIG / FIPS 140-3 / DoDIN APL accreditation gate that governs it |
| [Vol II Book 01 — SQL Server Authentication Modernization](Vol_II_Book_01_OrgComp_SQL_Server_Authentication_Modernization.qmd) | Vol II Book 01: Arc Managed Identity, Kerberos/NTLM breakdown, login migration, application chain, NIST closure |
| [Vol II Book 02 — SQL Server Implementation Guide](Vol_II_Book_02_OrgComp_SQL_Server_Implementation_Guide.qmd) | Vol II Book 02: 10-book runbook — estate discovery, Arc deployment, login migration, NTLM elimination, monitoring |
| [Vol II Book 03 — Database Consolidation and Network Physics](Vol_II_Book_03_OrgComp_Database_Consolidation_Network_Physics.qmd) | Vol II Book 03: SSOT architecture, speed-of-light floor, SD-WAN for database traffic, consistency class alignment |
| [Vol III Book 01 — Privileged Access Management](Vol_III_Book_01_OrgComp_Privileged_Access_Management.qmd) | Vol III Book 01: Entra PIM JIT activation, PAW architecture, AC-5/AC-6 least privilege, IA-11 re-authentication |
| [Vol III Book 02 — Vulnerability Management](Vol_III_Book_02_OrgComp_Vulnerability_Management.qmd) | Vol III Book 02: Defender VM, CISA KEV integration, RA-5 continuous scanning, SI-2 flaw remediation |
| [Vol III Book 03 — Patch & Systems Management](Vol_III_Book_03_OrgComp_Patch_Systems_Management.qmd) | Vol III Book 03 (extension book): the SI-2 / CM closure plane — flaw remediation and configuration state across Microsoft, AWS, Oracle, VMware, and Red Hat under one evidence contract |
| [Vol III Book 04 — Cloud-Native Security Posture & Containers](Vol_III_Book_04_OrgComp_Cloud_Native_Posture.qmd) | Vol III Book 04 (extension book): CSPM, CNAPP, and Kubernetes — closing the misconfiguration, exposure, and runtime gaps host scanning misses |
| [Vol III Book 05 — Data Protection (Purview + Key Vault CMK)](Vol_III_Book_05_OrgComp_Data_Protection_Purview.qmd) | Vol III Book 05: Purview DLP and sensitivity labels, Key Vault CMK, MP-2–5, SC-12/13/17/28, AU-10 |
| [Vol III Book 06 — SIEM / XDR / Detection Engineering](Vol_III_Book_06_OrgComp_SIEM_XDR_Detection.qmd) | Vol III Book 06: Microsoft Sentinel, Defender XDR, KQL analytics, SOAR playbooks, SI-4/AU-6/IR-4/6/8 |
| [Vol III Book 07 — Multi-Cloud Evidence Fabric](Vol_III_Book_07_OrgComp_Multi_Cloud_Evidence_Fabric.qmd) | Vol III Book 07 (extension book): the unified evidence contract — telemetry → CDM → FedRAMP 20x KSIs across every CSP, NEM, and OS |
| [Vol IV Book 01 — Business Continuity](Vol_IV_Book_01_OrgComp_Business_Continuity.qmd) | Vol IV Book 01: CP-2 through CP-8 contingency planning, DR runbooks, Azure Site Recovery |
| [Vol IV Book 02 — Supply Chain Risk Management](Vol_IV_Book_02_OrgComp_Supply_Chain_Risk_Management.qmd) | Vol IV Book 02: Syft CycloneDX SBOM pipeline (EO 14028 §4 / SR-family), Grype vulnerability scan + OpenVEX, GitHub Actions BOD 26-04 remediation-SLA gate, SR-family closure |
| [Vol IV Book 03 — Program Management and Governance](Vol_IV_Book_03_OrgComp_Program_Management_Governance.qmd) | Vol IV Book 03: ISPP template, POA&M tracker, SCRM Charter, Leadership RACI matrix, PM-family closure |
| [Vol IV Book 04 — PII Processing and Transparency](Vol_IV_Book_04_OrgComp_PII_Processing_Transparency.qmd) | Vol IV Book 04: Privacy Program Plan, authority-to-process templates, SORN/PIA scaffolding, Purview Data Map, PT-family closure |
| [Vol IV Book 05 — Cybersecurity Awareness and Training](Vol_IV_Book_05_OrgComp_Cybersecurity_Training_Awareness.qmd) | Vol IV Book 05: AT-2/AT-3/AT-4 training program, role-based matrices for six tiers, LMS evidence schema, CP-3 tabletop calendar, IR-2 drill documentation, KSI-022 activation |
| [Vol IV Book 06 — Authorization Package and ConMon](Vol_IV_Book_06_OrgComp_Authorization_Package_ConMon.qmd) | Vol IV Book 06: authorization package structure, OSCAL AP+AR+POA&M bundle (29/29 self-assessed, pending independent SCA), CA-7 ConMon program, FedRAMP VDR/VER submission (NTC-0014), AO authorization checklist, authorization-decision procedure |
| [Vol V Book 00 — Training & Certification (Volume Overview)](Vol_V_Book_00_OrgComp_Training_Certification_Overview.qmd) | Vol V Book 00: the enablement volume — the two-track model, the 10-of-29 coverage-gap thesis, and training as a control (AT-2/AT-3, KSI-CED) |
| [Vol V Book 01 — The Compliance Track](Vol_V_Book_01_OrgComp_Compliance_Track.qmd) | Vol V Book 01: the eight evidence slots as curriculum — book → control → KSI → evidence, modules A0–A8, the eight-slot-walk capstone |
| [Vol V Book 02 — The Implementation Track](Vol_V_Book_02_OrgComp_Implementation_Track.qmd) | Vol V Book 02: eight engineering modules (B1–B8), build-and-land-evidence, labs as worked exercises |
| [Vol V Book 03 — Assessment, Rubrics & Certification](Vol_V_Book_03_OrgComp_Assessment_Certification.qmd) | Vol V Book 03: the four-level rubric ladder, capstone scoring, the KSI Closure Necessity Matrix as exam, and the OrgComp Practitioner / Assessor credentials |
| [Vol V Book 04 — Vendor Training & Lab Environments](Vol_V_Book_04_OrgComp_Vendor_Training_Lab_Environments.qmd) | Vol V Book 04: external courses mapped per volume, and the fixture / trial-tenant / product-eval lab environments (Theme F realization layer) |
| [Vol VI Book 00 — Implementation (Overview)](Vol_VI_Book_00_OrgComp_Implementation_Overview.qmd) | Vol VI Book 00: the deployable layer — architecture operationalized as code; CM-2/CM-3 deploy discipline |
| [Vol VI Book 01 — Landing Zone & Network as Code](Vol_VI_Book_01_OrgComp_Landing_Zone_Network_as_Code.qmd) | Vol VI Book 01: SC-7/SC-20/21/22, CM-8, AC-4 as reviewable Terraform/Bicep |
| [Vol VI Book 02 — Identity & Access as Code](Vol_VI_Book_02_OrgComp_Identity_Access_as_Code.qmd) | Vol VI Book 02: AC-2/5/6, IA-2/3/5/11, SC-17 as policy-as-code |
| [Vol VI Book 03 — Detection Engineering](Vol_VI_Book_03_OrgComp_Detection_Engineering.qmd) | Vol VI Book 03: AU-6, SI-4, AC-7/AC-12 — the Sentinel KQL analytics rule library |
| [Vol VI Book 04 — Response Automation (SOAR)](Vol_VI_Book_04_OrgComp_Response_Automation_SOAR.qmd) | Vol VI Book 04: IR-4/6, AC-12, AC-2(13) — the SOAR playbook library |
| [Vol VI Book 05 — Data Protection Deployment](Vol_VI_Book_05_OrgComp_Data_Protection_Deployment.qmd) | Vol VI Book 05: MP-2–5, SC-12/17/28, AU-10 deployment (Purview, Key Vault) |
| [Vol VI Book 06 — Configuration Baselines & Hardening](Vol_VI_Book_06_OrgComp_Configuration_Baselines_Hardening.qmd) | Vol VI Book 06: CM-2/3/6/7, SI-2 baselines and hardening |
| [Vol VI Book 07 — Evidence & ConMon Pipeline](Vol_VI_Book_07_OrgComp_Evidence_ConMon_Pipeline.qmd) | Vol VI Book 07: AU-6/12, CA-5/7 — the evidence/ConMon pipeline deployment (OSCAL emitter) |
| [Vol VI Book 08 — Validation & Test Harnesses](Vol_VI_Book_08_OrgComp_Validation_Test_Harnesses.qmd) | Vol VI Book 08: CA-2/7, SI-4 — conformance validation and test harnesses |
| [Vol VII Book 00 — ServiceNow Automation (Overview)](Vol_VII_Book_00_OrgComp_ServiceNow_Automation_Overview.qmd) | Vol VII Book 00: the coordination layer — ServiceNow (Gov) + in-boundary MID Server; CMDB reconciled to IPAM/DDI, actuation platform-native |
| [Vol VII Book 01 — CMDB Reconciliation & Asset Identity](Vol_VII_Book_01_OrgComp_CMDB_Reconciliation_Asset_Identity.qmd) | Vol VII Book 01: CM-8 (reconciled to the IPAM/DDI join key), CM-3 change gate |
| [Vol VII Book 02 — M365 Control-Compliance Automation](Vol_VII_Book_02_OrgComp_M365_Control_Compliance_Automation.qmd) | Vol VII Book 02: CM-6, AC-2, CA-7 — SCuBA/baseline drift → tracked task → KSI feed |
| [Vol VII Book 03 — Azure Control-Compliance Automation](Vol_VII_Book_03_OrgComp_Azure_Control_Compliance_Automation.qmd) | Vol VII Book 03: CM-6, SI-2, RA-5 — Azure Policy/Defender findings → ServiceNow remediation |
| [Vol VII Book 04 — Control Attestation, Evidence & KSI](Vol_VII_Book_04_OrgComp_Control_Attestation_Evidence_KSI.qmd) | Vol VII Book 04: CA-2, CA-5, CA-7 — IRM/GRC control tests + POA&M → OSCAL/KSI evidence |
| [Vol VII Book 05 — The ServiceNow Compliance App](Vol_VII_Book_05_OrgComp_ServiceNow_Compliance_App.qmd) | Vol VII Book 05: the scoped app — Flows, ATF tests, Update Set |
| [Vol IX Book 05 — SaaS Integration Governance](Vol_IX_Book_05_OrgComp_SaaS_Integration_Governance.qmd) | Vol IX Book 05: onboarding a third-party SaaS as the governed SA-9 event that closes 24 controls — verify authorization, then configure, never the reverse |
| [Vol VIII Book 00 — Multi-Cloud DDI Landing-Zone Automation (Overview)](Vol_VIII_Book_00_OrgComp_DDI_Automation_Overview.qmd) | Vol VIII Book 00: SC-20/22, CM-8, CM-3/CM-5, AU-2 across Azure/AWS/GCP/OCI/VMware — the InfoBlox DDI kit (multi-CSP exception); full chapters in `infoblox-ddi-book/` |
| [Vol IX Book 00 — Day-2 Operations (Overview)](Vol_IX_Book_00_OrgComp_Day2_Operations_Overview.qmd) | Vol IX Book 00: the governed day-2 catalog — same discipline as Vol VII, reconciled to CM-8 |
| [Vol IX Book 01 — Helpdesk & ITSM Catalog](Vol_IX_Book_01_OrgComp_Helpdesk_ITSM_Catalog.qmd) | Vol IX Book 01: AC-2, IA-5, AC-6 as governed catalog items over Entra/M365/Azure |
| [Vol IX Book 02 — Landing Zone Front Door](Vol_IX_Book_02_OrgComp_Landing_Zone_Front_Door.qmd) | Vol IX Book 02: CM-2, CM-8, CM-3 — catalog → IaC → CMDB, reconciled to IPAM/DDI |
| [Vol IX Book 03 — App Registration Governance](Vol_IX_Book_03_OrgComp_App_Registration_Governance.qmd) | Vol IX Book 03: IA-5(2), SC-17, AC-6 — short-lived app credentials, scoped consent, orphan attestation |
| [Vol IX Book 04 — Teams Telephony Catalog & SCuBA Drift](Vol_IX_Book_04_OrgComp_Teams_Telephony_Catalog.qmd) | Vol IX Book 04: CM-3, CM-6, AU-2 — number/routing changes with E911 validation; Teams SCuBA drift into the M365 loop |
| [Vol IX Book 06 — Identity & Access Governance](Vol_IX_Book_06_OrgComp_Identity_Access_Governance.qmd) | Vol IX Book 06: AC-2, AC-5, AC-6, IA-4, IA-5, AU-2 — event-driven JML, certification campaigns, IAM evidence (Kit Phase 2, gated) |
| [Vol X Book 00 — Governance-Substrate Integration](Vol_X_Book_00_OrgComp_Governance_Substrate_Integration_Overview.qmd) | Vol X Book 00: the optional integration layer — conformance mapping, surface slots, evidence contract; no new control closures |
