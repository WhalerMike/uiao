...
| `atf-sam-negative-jws-binding.xml` | the JWS path binds the SUBJECT and the DECISION, not just the signature — a signed denial and a signed-for-someone-else assertion are both refused (regression: the JWS branch gated on neither, while pull-verify beside it gated on both) |
