...
| System properties | `sam_flavor`, `sam_base_url`, `sam_source_id`, `iiq_verify_endpoint`, `sam_jws_public_key`, `nonprod_instances`, `tbl_integration` | With **neither** `iiq_verify_endpoint` nor `sam_jws_public_key` set, every push is refused by design — that is the safe post-import default, not a broken install. Set exactly one verification method before going live. |
...
| ATF suite | `../atf/atf-sam-*.xml` (7 tests) | Run in `test_mode` before enabling the live connector — see the next section. |
