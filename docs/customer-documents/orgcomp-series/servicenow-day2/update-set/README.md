...
3. In the target instance: create the `x_fed_day2_ops.graph` Connection & Credential
   alias (Graph endpoint, MID Server selected), set the app properties, run the
   control-map CI check against `orgcomp-compliance-spine.yml`, and run the ATF suites in
