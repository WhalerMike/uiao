...
plus Kubernetes — AKS/EKS/OKE/OpenShift) they are where a dominant class of
...
| GCP | Microsoft Defender for Cloud (native GCP ingestion) | Yes (via Defender) | FedRAMP Moderate (GCC) | Native Moderate; Google's own stack (Assured Workloads, Security Command Center) is not currently owned or deployed — adopt only if GCP is procured, per the series' coordination-doctrine scope (Requirements §2) |
| OCI | OCI Cloud Guard | No | FedRAMP-authorized, above Moderate (DISA IL5 / JAB) | Exceeds Moderate, boundary treatment documented; residual-gap closer only — Defender's native coverage does not extend to OCI/OKE |
...
the Moderate-exclusive rule this is the identical flag as the AWS/OCI rows
...
GCP, natively at FedRAMP Moderate. Where an owned or preferred mechanism is
FedRAMP High rather than Moderate — AWS Config/Security Hub, OCI Cloud Guard,
and the agency-preferred **Palo Alto Prisma Cloud** / **Wiz for Government** —
the High-covering-Moderate boundary treatment must be documented; this is not a
disqualifier under the Moderate-exclusive rule. GCP's own native stack
(Assured Workloads, Security Command Center) is out of scope until GCP is
actually procured — Defender for Cloud's GCP ingestion is the only mechanism
documented for that platform today. Verify all authorization claims on the
FedRAMP Marketplace at procurement.
:::
...
| Oracle Cloud Guard (OCI, residual-gap closer, FedRAMP-authorized above Moderate / DISA IL5) | <https://www.oracle.com/security/cloud-security/cloud-guard/> |
