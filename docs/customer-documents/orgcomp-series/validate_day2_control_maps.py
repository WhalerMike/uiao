...
files describe themselves as a "projection of orgcomp-compliance-spine.yml,
...
It enforces, for every catalog item in every control map:
  ...
  - required fields present (title, control, task_type, approval, actuation,
    ksi, slot);
  - every `slot` is a real evidence slot declared in orgcomp-compliance-spine.yml;
...
orgcomp-compliance-spine.yml, CI-checked against it" claim in every map header was
