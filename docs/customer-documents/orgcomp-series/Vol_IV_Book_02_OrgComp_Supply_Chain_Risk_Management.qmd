---
title: "Vol IV Book 02 — Federal Organization Compliance — Supply Chain Risk Management"
...
aliases:
  - /customer-documents/federal-aan-series/Vol_IV_Book_02_FedAAN_Supply_Chain_Risk_Management.html
    ...
    steps:
      ...
      - name: Scan SBOM for Vulnerabilities (VDR)
        ...
        with:
          ...
          output-file: vdr-${{ github.sha }}.xml
          fail-build: false
        id: scan
...
escalated to P0. CISA's KEV catalog assigns each entry its own remediation due
date (the roughly two-week window applies to recently added CVEs; older
entries carry longer deadlines), and the clock runs from the date CISA adds
the CVE to the catalog, not from disclosure. The pipeline drives the SLA off
each entry's `dueDate`.
...
def triage_against_kev(vdr_path: str) -> list[dict]:
    # Each KEV entry carries its own dueDate; the KEV catalog sets a per-entry
    # remediation deadline measured from KEV catalog addition, not a flat window.
    kev = {e["cveID"]: e for e in requests.get(KEV_URL).json()["vulnerabilities"]}
...
| `under_investigation` | Exploitability being analyzed | New CVE, analysis in progress (30-day internal triage SLA) |
...
```json
{
  ...
  "@id": "https://sbom.{{< meta agency.domain >}}/vex/2026-Q3/vex-20260703.json",
  ...
  "statements": [
    {
      ...
      "impact_statement": "{{< meta agency.short >}} API gateway enforces input validation upstream of the affected code path. The vulnerable deserialization function is never called in production configuration.",
...
| Microsoft | Defender VM, Sentinel, Purview, Key Vault | Yes — GCC Moderate (fedramp.gov/marketplace/products/MSO365MT) | MSRC Security Update Guide | Yes (MSRC VEX) |
| Cisco | SD-WAN (IOS XE, vManage) | Yes — FedRAMP Moderate (FR2012534927) | Cisco PSIRT | Yes (OpenVEX) |
| Infoblox | NIOS Grid, BloxOne DDI | Yes — FR2017257053 | Infoblox Security Portal | Yes |
| NIST | NVD data feed | N/A (data source) | NVD JSON feed | N/A |
| CISA | KEV catalog feed | N/A (data source) | KEV JSON feed | N/A |
...
|---|---|---|---|---|
| **SR-1** | Policy and Procedures | No documented SCRM policy; procurement decisions lack security evaluation criteria | SCRM Policy document + SBOM pipeline governance charter | Pipeline run log; policy document in SSP |
...
**Controls closed: 8** (SR-1, SR-2, SR-3, SR-5, SR-6, SR-8, SR-10, SR-11) — pending registration in the compliance spine.
...
The SBOM/VDR/VEX pipeline provides the VER (Vulnerability Evaluation &
Reporting) evidence contract required to upgrade KSI-CMT rules from scaffold
...
| SLSA Level | Requirements | {{< meta agency.short >}} Implementation |
|---|---|---|
...
| where now() > kev_due_date_t
| project CVE = cve_id_s, Component = component_name_s,
          CVSS = cvss_score_d, KEVDate = kev_date_added_t, DueDate = kev_due_date_t,
          DaysOverdue = datetime_diff('day', now(), kev_due_date_t)
...
| KEV Integration | CISA KEV JSON feed | JSON | BOD 26-04 14-day SLA |
...
| [Vol III Book 02 — Vulnerability Management](Vol_III_Book_02_OrgComp_Vulnerability_Management.qmd) | Defender VM + KEV integration; extends to SBOM-sourced CVE triage |
| [Vol III Book 06 — SIEM / XDR / Detection Engineering](Vol_III_Book_06_OrgComp_SIEM_XDR_Detection.qmd) | Sentinel ingestion of SBOM/VDR telemetry; KQL analytic rules |
