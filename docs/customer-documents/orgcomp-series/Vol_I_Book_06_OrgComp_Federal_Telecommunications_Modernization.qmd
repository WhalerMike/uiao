...
aliases:
  - /customer-documents/federal-aan-series/Vol_I_Book_06_FedAAN_Federal_Telecommunications_Modernization.html
...
> **Why SC-8 Cannot Be Inherited — the voice path.** A DIA circuit carries traffic unencrypted at the network layer. SRTP protects the media payload of a call; it does not protect the signaling, the softphone HTTPS session's metadata, or any of the non-voice traffic sharing the circuit — and application-layer TLS covers no non-TLS flows at all. FIPS-validated application-layer encryption (TLS, SRTP) closes SC-8 for the flows that negotiate it; the only mechanism class that encrypts *all* traffic on a DIA path regardless of application behavior is a network-layer overlay, and among overlay classes SD-WAN is the one that also delivers the SC-5 traffic-class and SI-4 telemetry closures this book depends on. The claim is coverage-completeness, not exclusivity. There is no configuration setting, scanner finding, or policy document that closes SC-8 for *every* flow on a bare pipe. The formal SC-8 closure-necessity anchor is registered in [Vol I Book 07](Vol_I_Book_07_OrgComp_Network_Enforcement_Substrate.qmd); this book applies that mechanism on the telecommunications path.
...
Two TIC 3.0 use cases map directly onto the telecommunications populations in this document — the Branch Office use case (elsewhere in this series shorthand-labeled Use Case E) and the Remote User use case:
