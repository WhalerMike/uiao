---
title: "Vol IV Book 03 — Federal Organization Compliance — Program Management & Governance"
...
aliases:
  - /customer-documents/federal-aan-series/Vol_IV_Book_03_FedAAN_Program_Management_Governance.html
...
7. **Supply Chain Risk Management (SCRM) Governance Charter** — organizational
   ...
   for the technical evidence established in Vol IV Book 02 (Supply Chain Risk Management)

Together these artifacts provide the templates and framework to close nine NIST SP 800-53 Rev 5 PM family controls
and provide the PM-30 evidence that, combined with Vol IV Book 02's technical
...
- **PM-30** (SCRM Strategy) is the organizational commitment that makes the
  Vol IV Book 02 SBOM/VDR/VEX pipeline a governance artifact rather than a technical
...
| PM-2 | Info Security Program Leadership Role | §3 Leadership RACI |
| PM-5 | System Inventory | §4 Inventory Framework |
| PM-7 | Enterprise Architecture | §2.2 EA Integration |
| PM-9 | Risk Management Strategy | §5 Risk Register Framework |
| PM-11 | Mission/Business Process Definition | §2.3 Mission Mapping |
| PM-14 | Testing, Training, and Monitoring | §6 T&T&M Plan |
| PM-15 | Security and Privacy Groups and Associations | §7 Contact Registry |
...
{{< meta agency.short >}}'s cloud security program under FedRAMP 20x. It must be maintained by
...
> impact system operated by the {{< meta agency.formal >}} Cloud
> Services Team under an agency RMF authorization (NIST SP 800-37), with
> ConMon evidence modeled on the FedRAMP 20x KSI schema.
>
...
> authorization package presented to the {{< meta agency.short >}} Authorizing Official.]
>
...
> [FILLABLE] The {{< meta agency.short >}} cloud security program is designed to achieve the
> following risk-based objectives, prioritized in alignment with {{< meta agency.short >}}'s
...
The {{< meta agency.short >}} security architecture is integrated into the enterprise architecture
at three levels:

**Technical Reference Architecture (TRA):** Security controls are embedded in
the TRA as mandatory capabilities. The Volume I–II OrgComp books establish the
...
- Target Architecture Document (TAD) — references the Volume I–IV OrgComp control mappings
- Data Architecture — references Vol III Book 05 (Purview/CMK) for data-at-rest controls
- Network Architecture — references the Volume I network books and Vol II Book 03 for network security

## Mission and Business Process Definition (PM-11)

### Critical Mission Functions

{{< meta agency.short >}} operates the following mission-critical processes relevant to this
...
> (CP controls, Vol IV Book 01) and the Business Impact Analysis (BIA).
...
> must be {{< meta agency.short >}} employees; the ISSO may be a contractor with documented oversight.
> The 3PAO must be FedRAMP-recognized.

![Leadership RACI — Accountable, Responsible, and the constraints that make it PM-2 evidence](figs/pmg-fig-02-leadership-raci.png){#fig-pmg02 fig-alt="Three-column card grid titled 'The leadership RACI names who owns each decision' under an Accountability (PM-2) heading. Column 1, Accountable (tagged AO / CISO): the AO owns the ATO decision and organizational risk acceptance, while the CISO is accountable for the assessment, POA&M oversight, KSI monitoring, and training. Column 2, Responsible (tagged ISSO / DevSecOps / SOC): the ISSO runs day-to-day control operation and the POA&M; DevSecOps owns the SBOM pipeline and CI gates; the SOC owns Sentinel monitoring and incident response. Column 3, Constraints (tagged Named individuals): the AO and CISO must be {{< meta agency.short >}} employees, the ISSO may be a contractor with documented oversight, and the 3PAO must be FedRAMP-recognized. A summary banner states a RACI with role titles is a template, but a RACI with named individuals against each activity is the PM-2 evidence artifact. White background. Source: Vol IV Book 03 Program Management & Governance briefing deck, Date Code 2026-07-09 17:00 ET." width="100%"}
...
The {{< meta agency.short >}} system inventory is maintained through a three-layer approach:
...
Vol IV Book 02 establishes the SBOM pipeline using Syft. Every container image
...
Entra ID (Vol I Book 05) provides the authoritative account inventory. The ISSO
reviews privileged account counts weekly (referenced by KSI-003). The
Vol I Book 05 evidence artifacts satisfy CM-8(2) automated inventory maintenance for
the identity plane.
...
{{< meta agency.short >}}'s risk management strategy follows NIST SP 800-39 (Managing Information
Security Risk) at three tiers:

- **Tier 1 (Organization):** Risk framing — risk tolerance, risk assumptions,
  risk constraints, priorities, and trade-offs defined by {{< meta agency.short >}} leadership
...
>
> {{< meta agency.short >}} accepts residual risk at the Moderate impact level for systems that
...
The {{< meta agency.short >}} enterprise risk register is maintained in the POA&M tracker and
...
![Every risk scored, owned, and time-bounded — the scoring model, remediation clock, responses, and tiers](figs/pmg-fig-03-risk-scoring.png){#fig-pmg03 fig-alt="Four-quadrant card grid titled 'Every risk scored, owned, and time-bounded' under a Risk Scoring & Response heading. Card 1, The scoring model: Score equals likelihood (1 rare to 5 certain) times impact (1 negligible to 5 catastrophic), a single number that sets the remediation clock and the escalation path. Card 2, The remediation clock: scores 1-4 accept and monitor; 5-9 mitigate within 90 days; 10-15 mitigate within 30 days; 16-25 escalate to the AO with a POA&M within 15 days. Card 3, Four responses: mitigate by strengthening a control, transfer by shifting to a CSP contractually, accept with AO concurrence and time bounding above score 4, or avoid by removing the component. Card 4, The three tiers: NIST SP 800-39 organizational risk framing, mission-level risk assessment, and system-level risk response, with the register living at the system tier reviewed monthly. A summary banner notes the register is reviewed monthly by the ISSO and quarterly by the CISO, and a score above 4 cannot be accepted indefinitely. White background. Source: Vol IV Book 03 Program Management & Governance briefing deck, Date Code 2026-07-09 17:00 ET." width="100%"}
...
| Vulnerability scan (Defender VM) | Continuous | SOC | Vol III Book 02 Defender VM output |
| SBOM/VDR scan (Grype CI gate) | Every PR merge | DevSecOps | Vol IV Book 02 CI logs |
...
The continuous monitoring program for {{< meta agency.short >}}'s FedRAMP 20x authorization is
operationalized through the Vol III Book 06 Sentinel XDR platform with the following
...
| CISA KEV team | Vulnerability intel | KEV additions affecting {{< meta agency.short >}} SBOM components | kev@cisa.dhs.gov |
| FedRAMP PMO | Inherited CSO authorizations (M365 GCC, GovCloud, DDI SaaS) | Change in an inherited authorization's status or 20x transition timeline | info@fedramp.gov |
| OMB Cyber | Policy guidance | Major policy changes | [OMB POC] |
| {{< meta agency.short >}} IG | Audit coordination | Audit notification, investigation referral | [IG POC] |
...
This Supply Chain Risk Management (SCRM) Governance Charter establishes {{< meta agency.short >}}'s
organizational commitment to identifying, assessing, and managing supply chain
risks across the lifecycle of cloud services used to deliver {{< meta agency.short >}}'s mission.

**Authority:** This charter is approved by the [{{< meta agency.short >}} CISO / CIO] and reviewed
...
{{< meta agency.short >}}'s SCRM program addresses five supply chain risk categories aligned with
NIST SP 800-161r1:

| Category | Examples | Controls | Vol IV Books 02–03 Evidence |
|----------|---------|----------|-------------------|
...
| **Unauthorized access by supplier** | Vendor with excessive access | SR-6, AC-21 | Vol I Book 05 vendor access review |
| **Vendor financial/operational risk** | Vendor bankruptcy, PSIRT closure | SR-8, PM-9 | Vendor disclosure matrix (Vol IV Book 02) |

![SCRM risk categories — five categories each mapped to specific SR controls and concrete evidence](figs/pmg-fig-04-scrm-categories.png){#fig-pmg04 fig-alt="Four-quadrant card grid titled 'Five categories, each mapped to controls and evidence' under an SCRM Risk Categories heading (the fifth category, supplier access, is drawn out as its own card). Card 1, Counterfeit / tampered: fake open-source packages, typosquatting, and compromised build pipelines, countered by SR-3, SR-10, and SA-10/11 through the SBOM plus Cosign verification and SLSA provenance. Card 2, Vulnerable components: known CVEs in SBOM dependencies, countered by SR-5, SR-6, and SR-11 via the Grype VDR and the CISA KEV integration that escalates exploited CVEs. Card 3, Supplier access: a vendor with excessive access to agency systems, countered by SR-6 and AC-21 through the vendor access review in the identity plane (Vol I Book 05). Card 4, Vendor viability: vendor bankruptcy or a closed PSIRT leaving CVEs unaddressed, countered by SR-8 and PM-9 through the vendor disclosure matrix and the risk register. A summary banner notes the charter's five categories align to NIST SP 800-161r1 and each maps to specific SR controls and concrete Vol IV Book 02-03 evidence, not abstract policy. White background. Source: Vol IV Book 03 Program Management & Governance briefing deck, Date Code 2026-07-09 17:00 ET." width="100%"}
...
**Assessment Inputs:**
1. SBOM manifest (Syft output, Vol IV Book 02) — identifies all components and suppliers
2. VDR (Grype output, Vol IV Book 02) — identifies exploitable vulnerabilities
3. Vendor disclosure matrix (Vol IV Book 02 §5) — supplier security posture
4. KEV feed (CISA, integrated via Vol III Book 06 Sentinel) — known exploited CVEs
5. PSIRT/MSRC subscriptions (Vol IV Book 02 §5 Vendor Disclosure Matrix; §8 Continuous Monitoring) — vendor security advisories

**Assessment Output:**
- SCRM risk register update (referenced in PM-9 §5.3 above)
- VEX assertions for each CVE (OpenVEX format, Vol IV Book 02 §5)
...
3. **Notify {{< meta agency.short >}}** within 72 hours of discovery of any security incident that
   may affect components in {{< meta agency.short >}}'s production environment
4. **Submit VEX assertions** for CVEs identified in their components within
   30 days of CVE publication (NVD)
5. **Accept {{< meta agency.short >}} SCRM audit rights** — the right to audit the supplier's
   security practices with 30 days' notice

> [FILLABLE] Insert contract clause language for SR-2/SR-3 requirements.
> Coordinate with {{< meta agency.short >}} Contracts and Procurement to embed SBOM requirement
> in the standard Acquisition/Vendor Agreement template.

## Supply-Chain Evidence Posture (EO 14028 §4)

The SBOM / VDR / VEX evidence artifacts are mandated by EO 14028 §4 and NIST SP
800-161 — not by BOD 26-04, which governs the risk-tiered *remediation*
timelines. The pipeline is built in Vol IV Book 02; FedRAMP's VDR/VER rule sets
(NTC-0014) and BOD 26-04's remediation timelines both take effect **December 7,
2026**.

| Evidence Tier | Artifact | {{< meta agency.short >}} Status | Evidence |
|---------------|----------|------------|----------|
| Tier 1: SBOM | Machine-readable SBOM for all systems | Pipeline built; formal submission due Dec 7 2026 | Vol IV Book 02, Syft pipeline |
| Tier 2: VDR | Vulnerability Disclosure Report (CycloneDX) | Pipeline built; formal submission due Dec 7 2026 | Vol IV Book 02, Grype output |
| Tier 3: VEX | Vulnerability Exploitability eXchange | Pipeline built; formal submission due Dec 7 2026 | Vol IV Book 02, OpenVEX JSON |

{{< meta agency.short >}}'s SCRM program satisfies all three BOD 26-04 tiers through the technical
pipeline established in Vol IV Book 02 (SBOM + VDR + VEX Framework) and the
...
| PM-2 | Info Security Program Leadership Role | §3 | Leadership RACI | ✅ Template provided |
| PM-5 | System Inventory | §4 | Inventory Framework | ✅ Template + automation |
| PM-7 | Enterprise Architecture | §2.2 | EA Integration | ✅ Architecture gates |
| PM-9 | Risk Management Strategy | §5 | Risk Register | ✅ Template provided |
| PM-11 | Mission/Business Process Definition | §2.3 | Mission Map | ✅ Template provided |
| PM-14 | Testing, Training, and Monitoring | §6 | T&T&M Plan | ✅ Template provided |
| PM-15 | Security and Privacy Groups and Associations | §7 | Contact Registry | ✅ Template provided |
| PM-30 | Supply Chain Risk Management Strategy | §8 | SCRM Charter | ✅ Template provided |

---

# Appendix B — Integration with OrgComp Technical Series

| OrgComp Book | PM Control Dependencies |
|----------|------------------------|
| Vol I Book 05 (Entra ID / ZTNA) | PM-5 (system inventory — identity plane) |
| Vol III Book 02 (Defender VM) | PM-14 (testing — vulnerability scan cadence) |
| Vol III Book 05 (Purview / Key Vault) | PM-11 (mission mapping — PII-bearing processes) |
| Vol III Book 06 (Sentinel) | PM-14 (monitoring program — Sentinel workbooks) |
| Vol IV Book 01 (Contingency) | PM-9 (risk register — RTO/RPO inputs) |
| Vol IV Book 02 (SBOM/VDR/VEX) | PM-30 (SCRM charter — technical backbone) |
| **Vol IV Book 03 (this document)** | PM-1, PM-2, PM-5, PM-7, PM-9, PM-11, PM-14, PM-15, PM-30 |

The PM governance artifacts in Vol IV Book 03 provide the organizational layer that
contextualizes the technical evidence from the Volume I–IV technical books that
precede it. Together they form a
complete agency RMF authorization package (ConMon evidence modeled on the
FedRAMP 20x KSI schema) for the PM and SR control families.
