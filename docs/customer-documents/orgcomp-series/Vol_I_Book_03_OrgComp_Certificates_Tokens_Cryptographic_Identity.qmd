---
title: "Vol I Book 03 — Federal Organization Compliance — Certificates & Tokens: Cryptographic Identity"
...
aliases:
  - /customer-documents/federal-aan-series/Vol_I_Book_03_FedAAN_Certificates_Tokens_Cryptographic_Identity.html
...
::: {.aan-important}
**Series Context** — This document is Vol I Book 03 of the Federal Organization Compliance series. [Vol I Book 01](Vol_I_Book_01_OrgComp_Cloud_Landing_Zone_IPAM_FedRAMP.qmd) established the naming and addressing plane — authoritative DNS, DHCP, and IPAM as the landing-zone source of truth. [Vol I Book 02](Vol_I_Book_02_OrgComp_Network_Access_Control_802_1X.qmd) established device identity at the network port — 802.1X, EAP-TLS, and the machine certificate as the admission credential. This part completes the identity plane's cryptographic foundation: how certificates are issued, revoked, and automated as a function of the naming plane, and how tokens replace network sessions as the carrier of identity. [Vol II Books 01](Vol_II_Book_01_OrgComp_SQL_Server_Authentication_Modernization.qmd) and [02](Vol_II_Book_02_OrgComp_SQL_Server_Implementation_Guide.qmd) — SQL Server authentication modernization — are direct applications of the doctrine this part establishes.
:::
...
| **Location binding** | A domain controller is **reachable** — not within any protocol-defined latency bound, but reachable at all, and on a path fast enough that logon-time operations complete. RFC 4120 sets a five-*minute* clock-skew tolerance and no latency requirement | Workloads span on-premises virtualization, two hyperscale government clouds, and an externally managed tenancy; the KDC is co-located with none of them — and while service-side ticket validation is offline, ticket *issuance and renewal* require that reachability |
...
| Latency dependency | KDC reachability at logon, password change, and GPO retrieval — a deployment-era flat-LAN assumption, not a protocol bound | None beyond the request itself |
...
- **"Extend the domain into the cloud."** Site-to-site tunnels carrying domain traffic to cloud regions recreate the hairpin that [Vol I Book 05](Vol_I_Book_05_OrgComp_Network_Modernization.qmd)'s physics argument closed: authentication round-trips ride the tunnel back to wherever the domain controllers live, re-importing the latency floor into every logon. The tunnel moves the problem; it cannot shorten the distance.
...
- **CAA records (RFC 8659)** publish issuance policy *in the zone itself*. A Certification Authority Authorization record declares which CAs are authorized to issue for a domain, and public CAs are required by the CA/Browser Forum Baseline Requirements to check and honor CAA at issuance time. Issuance policy enforcement is therefore a property of the authoritative DNS zone — the zone [Vol I Book 01](Vol_I_Book_01_OrgComp_Cloud_Landing_Zone_IPAM_FedRAMP.qmd) placed under unified DDI control. A policy memo asks CAs nicely; a CAA record is checked by protocol.
...
|---|---|---|
| Device identity at the port | Machine certificates enrolled via SCEP at device provisioning, validated by RADIUS/EAP-TLS | [Vol I Book 02](Vol_I_Book_02_OrgComp_Network_Access_Control_802_1X.qmd) |
| Workload transport identity | Server and service certificates issued at IP allocation time, revoked atomically at decommission | [Vol I Book 01](Vol_I_Book_01_OrgComp_Cloud_Landing_Zone_IPAM_FedRAMP.qmd) |
| User and application authentication | Certificate-based authentication and token signing keys | This book, applied in [Vol II Books 01](Vol_II_Book_01_OrgComp_SQL_Server_Authentication_Modernization.qmd)–[02](Vol_II_Book_02_OrgComp_SQL_Server_Implementation_Guide.qmd) |
...
- [Vol I Book 02](Vol_I_Book_02_OrgComp_Network_Access_Control_802_1X.qmd) covers the machine certificate side — Intune/SCEP enrollment issues a certificate with `CN={{DeviceName}}` at 802.1X admission.
- [Vol I Book 01](Vol_I_Book_01_OrgComp_Cloud_Landing_Zone_IPAM_FedRAMP.qmd) owns the DNS hostname side — the InfoBlox DHCP lease assignment should confirm or create a DNS A/PTR record binding the device's name to its admitted IP.
...
- **Purpose-scoped issuing CAs.** Separate issuing CAs (or at minimum separate certificate profiles with disjoint extended key usage) for device identity, server transport, and user authentication. Scoping limits the blast radius of an issuing-CA compromise to one credential class and makes trust-store policy expressible: the RADIUS infrastructure of [Vol I Book 02](Vol_I_Book_02_OrgComp_Network_Access_Control_802_1X.qmd) trusts the device-identity chain, not every certificate the agency ever minted.
...
- **Verification without callback.** The service validates a signature; it does not open a connection to a domain controller. The KDC-reachability dependency is gone.
...
For the governance framing above this operational layer — why identity has
to be the addressing root before the other ZTMM pillars can mature, and how
ZTMM v2.0 / M-22-09 / SP 800-207 fit together — see
[Zero Trust Governance Principles](../whitepapers/zero-trust-governance-principles.qmd).
...
This is the hinge to the application books. [Vol II Book 01](Vol_II_Book_01_OrgComp_SQL_Server_Authentication_Modernization.qmd) replaces domain credentials in the SQL Server estate with managed identities and token-based authentication; [Vol II Book 02](Vol_II_Book_02_OrgComp_SQL_Server_Implementation_Guide.qmd) is its implementation runbook. Read them as **applications of this chapter** — the database is simply the first large estate where the workload-identity progression is executed end-to-end:
...
- **[Vol I Book 01](Vol_I_Book_01_OrgComp_Cloud_Landing_Zone_IPAM_FedRAMP.qmd)** — the naming and addressing plane: authoritative DNS, DHCP, IPAM, and the landing zone that makes every workload enumerable.
- **[Vol I Book 02](Vol_I_Book_02_OrgComp_Network_Access_Control_802_1X.qmd)** — device identity at the port: the machine certificate as the admission credential, quarantine as the default.
...
Everything after this point is application and evidence. [Vol I Book 05](Vol_I_Book_05_OrgComp_Network_Modernization.qmd) and [Vol I Book 06](Vol_I_Book_06_OrgComp_Federal_Telecommunications_Modernization.qmd) modernize the transport that made this identity model mandatory. [Vol II Books 01](Vol_II_Book_01_OrgComp_SQL_Server_Authentication_Modernization.qmd)–[02](Vol_II_Book_02_OrgComp_SQL_Server_Implementation_Guide.qmd) execute this book's doctrine against the database estate — the first proof that session-bound authentication can be *removed*, not merely deprecated. The privileged access, detection, and continuity parts consume the token-level telemetry this architecture emits, and [Vol IV Book 06](Vol_IV_Book_06_OrgComp_Authorization_Package_ConMon.qmd) assembles it into the authorization package.
...
| **F — Accreditation Substrate** | Does not land in this book. Theme F — the accreditation and ConMon gates that turn control closure into an authorization package — is carried by the governance volumes; this book supplies the certificate and token lifecycle evidence those gates consume |

: The six doctrine themes as they appear in Vol I Book 03 {.striped .hover}
