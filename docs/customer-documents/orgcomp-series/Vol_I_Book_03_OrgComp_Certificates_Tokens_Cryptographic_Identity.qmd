...
- **CAA records (RFC 8659)** publish issuance policy *in the zone itself*. A Certification Authority Authorization record declares which CAs are authorized to issue for a domain, and public CAs are required by the CA/Browser Forum Baseline Requirements to check and honor CAA at issuance time. Issuance policy enforcement is therefore a property of the authoritative DNS zone — the zone [Vol I Book 01](Vol_I_Book_01_OrgComp_SSA_Landing_Zone_IPAM_FedRAMP.qmd) placed under unified DDI control. A policy memo asks CAs nicely; a CAA record is checked by protocol.
...
| Workload transport identity | Server and service certificates issued at IP allocation time, revoked atomically at decommission | [Vol I Book 01](Vol_I_Book_01_OrgComp_SSA_Landing_Zone_IPAM_FedRAMP.qmd) |
...
- [Vol I Book 01](Vol_I_Book_01_OrgComp_SSA_Landing_Zone_IPAM_FedRAMP.qmd) owns the DNS hostname side — the InfoBlox DHCP lease assignment should confirm or create a DNS A/PTR record binding the device's name to its admitted IP.
...
- **[Vol I Book 01](Vol_I_Book_01_OrgComp_SSA_Landing_Zone_IPAM_FedRAMP.qmd)** — the naming and addressing plane: authoritative DNS, DHCP, IPAM, and the landing zone that makes every workload enumerable.
