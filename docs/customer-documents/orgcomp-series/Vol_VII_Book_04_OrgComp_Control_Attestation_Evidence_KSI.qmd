---
title: "Vol VII Book 04 — Federal Organization Compliance — Control Attestation, Evidence & KSI"
...
aliases:
  - /customer-documents/federal-aan-series/Vol_VII_Book_04_FedAAN_Control_Attestation_Evidence_KSI.html
...
Developed by the **CSI Team (Cloud Services Infrastructure)**; not yet reviewed or approved by the {{< meta agency.short >}} CIO Office, OIS, or leadership. **Date Code:** 2026-07-21 07:38 ET
:::
...
# One Pipeline, Every Estate — the Phase 5 Seam

The attestation loop this book defines is estate-agnostic by construction, and
Phase 5 makes that concrete: the **RedHat-on-AWS estate's control state enters
this same pipeline**, not a parallel one. The Patch & Systems Management book's
Phase 5 emission contract delivers AWS remediation records into the same SI-2
task stream this book attests over; the AWS assessment feed ranks in the same
RA-5 queue; Security Hub and GuardDuty telemetry lands in the same Evidence
Fabric the KSI emit reads. An assessed control is assessed over the *whole*
estate keyed to the one CM-8 join key — there is no AWS attestation narrative,
no second scorecard, and no per-cloud OSCAL package. That is the epic's closing
invariant, and this book is where it holds.

# Closure Necessity

::: {.aan-tip}
## Closure Provenance — the doctrine this pipeline enforces

Book 00's **Closure Provenance Doctrine** is the law this book mechanizes: *no
closure counts unless it carries its proof.* Every stage of the pipeline above
is a provenance stage — the control test attaches the posture evidence it
read; the POA&M item closes only on passing re-test evidence; the emit step
publishes machine-readable results, not summaries. The task streams this book
reads (Books 02/03 and the day-2 catalog lanes) honor the same rule at the
task level: a closing update carries its verification payload, and human-only
closures are flagged and counted separately — an attestation over unverified
closures would attest fiction, and a report that cannot be regenerated from
the closure stream is an artifact of effort, not of state. The day-2 scoped
app implements the closure rules (see `servicenow-day2/README.md`).
...
Generated from the series compliance spine (`orgcomp-compliance-spine.yml`) by `render_authorities_table.py`; not hand-edited here.

<!-- authorities:book-sn-attestation — generated from orgcomp-compliance-spine.yml; do not hand-edit -->
...
- **`orgcomp-compliance-spine.yml`** — the SSOT control-to-evidence binding the IRM control map projects.
...
- **Vol III Book 03 (Patch & Systems Management), Phase 5** — the RedHat-on-AWS emission contract and assessment feed whose records this book attests alongside the M365/Azure streams.

# Provenance

Draft. ServiceNow IRM/GRC, OSCAL, and the FedRAMP 20x KSI pipeline are deployed examples of the attestation-and-emit mechanism; the spine-as-SSOT and coordination-feeds-truth disciplines are the invariants. Vendor-source citations for the ServiceNow IRM/GRC capability claims are pending, and once added remain dated, not permanent — see the [Vendor Currency Doctrine](Vol_0_Book_00_OrgComp_Executive_Summary.qmd#vendor-currency); FedRAMP 20x tooling in particular (e.g. ServiceNow-native KSI products) is moving fast enough that this book's own claims should be re-verified before use in a Project Plan. The attestation-pipeline figure is authored as committed house-style SVG (rasterized to PNG).
