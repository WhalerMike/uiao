...
- **The quantitative headlines were reframed.** The unconditional "29/29 · 0 open
  ...
  (`federal-orgcomp-conmon-gap-roadmap.md:22-54`), and Vol IV Book 06
...
- **The ADR-111 → CR26 diff now exists and is honest.**
  `OrgComp_CR26_Reconciliation.md` reports **19 of 46 CR26 indicators explicitly
...
- **Two derived artifacts are actually CI-gated.**
  `.github/workflows/orgcomp-authorities-drift.yml` runs both
...
**`Vol_IV_Book_05_OrgComp_Cybersecurity_Training_Awareness.qmd:651-657` was not
...
`OrgComp_CR26_Reconciliation.md` shows those themes at **0 indicators mapped**
...
- `OrgComp-Training-Program/books/book-18.qmd:72` — "taking **CR26 coverage to
...
coverage 19/46 — see `OrgComp_CR26_Reconciliation.md`," fix the ✓ marks and the
...
- **The `kits:` registry is pure inventory.** Kit rows carry `id/volume/book/
  ...
  (`orgcomp-compliance-spine.yml:211-224`); the kit IDs appear nowhere else in the
...
still lives only in the companion roadmap (`federal-orgcomp-conmon-gap-roadmap.md:174-185`),
