...
> Program** (`OrgComp-Training-Program/`, `Vol_V_*`, `boundary/`) **against the
> external evaluation** committed verbatim as
> `OrgComp_Training_Program_External_Evaluation.md` (2026-07-18, scored
...
> follows `OrgComp_Series_Assessment_Against_Critique.md` (2026-07-13), which
...
all 20 books (`OrgComp-Training-Program/compliance-track.qmd:48-162`, modules
...
"quick start" across `OrgComp-Training-Program/` and `Vol_V_*` return nothing.
...
evidence-complete** (`Vol_V_Book_03_OrgComp_Assessment_Certification.qmd:214-219`)
...
(`OrgComp_Series_Assessment_Against_Critique.md:41` — "SSOTs declared but
...
(`OrgComp-Training-Program/index.qmd:20-27` callout) and the Expansion roadmap
(`OrgComp-Training-Program/index.qmd:158-191`) functions as a changelog — six
✅-marked shipped waves with dates. Two real gaps remain: (i) training
pages stamp `date: today` (`OrgComp-Training-Program/index.qmd:4`, all module
pages), so every
...
`Vol_V_Book_04_OrgComp_Vendor_Training_Lab_Environments.qmd:213`) and **no
...
satisfy" (`index.qmd:29-34`, linking `OrgComp_Evidence_Contract_Spec.qmd`) —
...
`OrgComp-Training-Program/` or any module page into `boundary/`; the
...
evidence flow (`OrgComp-Training-Program/index.qmd:44`, `:141`), the Track A
...
(`Vol_V_Book_03_OrgComp_Assessment_Certification.qmd:214`). The reviewer's
...
(`BUILD-DERIVATIVES.md` / `scripts/build_orgcomp_download.py`), not a source
change.
...
(`Vol_IV_Book_06_OrgComp_Authorization_Package_ConMon.qmd`) is the place
...
across `OrgComp-Training-Program/` and all five `Vol_V_*` books return
...
1. **Define `<engine>` where it is used** (Concern 3) — a short
   ...
   the token stands for, a link to `OrgComp_Evidence_Contract_Spec.qmd`, the
