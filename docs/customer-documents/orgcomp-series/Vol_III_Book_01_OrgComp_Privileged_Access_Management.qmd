---
title: "Vol III Book 01 — Federal Organization Compliance — Privileged Access Management"
...
aliases:
  - /customer-documents/federal-aan-series/Vol_III_Book_01_FedAAN_Privileged_Access_Management.html
...
::: {.aan-important}
**Series Context** — This document is Vol III Book 01 (Privileged Access Management) of the Federal Organization Compliance series. The Volume I network-and-identity books and the Volume II database books establish Landing Zone IPAM/DDI/PKI, Network Access Control, Certificates and Tokens, Network Modernization, Telecommunications Modernization, SQL Server Authentication Modernization, SQL Server Implementation, and Database Consolidation and Network Physics. This part addresses the privileged access layer — the administrative credential surface that remains exploitable even after all token-based authentication controls from the Volume I and Volume II books are in place.
:::

# What This Document Addresses

The Volume I network-and-identity books and the Volume II database books replaced password-hash authentication with Managed Identity tokens, eliminated NTLM from SQL Server connections, removed standing service account credentials from application connection strings, and established Arc as the identity bridge for hybrid server workloads. Each of those changes materially reduced the attack surface for credential theft and lateral movement.
...
The Volume I and Volume II books deploy Entra ID authentication — Managed Identity tokens, Conditional Access, Entra-native OS login. These controls verify identity quality. They do not reduce role scope.
...
![Eligible vs. active: the one PIM distinction that does the work](figs/pam-fig-05-eligible-vs-active.png){#fig-pam05 fig-alt="Two side-by-side panels. Left, ACTIVE assignment in red: the traditional state — role assigned and immediately effective, exercisable at any time with no additional step; appropriate only for automated service principals and break-glass accounts; immediately exploitable if the session is compromised. Right, ELIGIBLE assignment in teal: the PIM state — role assigned but not effective; the user must explicitly activate, satisfying MFA plus justification plus optional approver; grants a time-limited active session that returns to eligible-only at expiry; worthless to an attacker who has the session but has not completed the activation ceremony. Blue bar: the decisive operational difference — an eligible assignment is worthless without the activation ceremony while an active assignment is immediately exploitable; between activations no persistent active scope exists; human accounts hold eligible only, active assignments are reserved for service principals and break-glass. White background. Source: Vol III Book 01 Privileged Access Management briefing deck, Date Code 2026-07-08 13:08 ET." width="100%"}

## PIM Activation Flow

![PIM activation flow: request — MFA — approval — time-limited role — audit showing all steps and the resulting audit trail](figs/pam-fig-02-pim-activation-flow.png){#fig-pam02 fig-alt="Flowchart showing the PIM activation sequence. Step 1: Administrator submits activation request with justification text and requested duration in the Entra ID portal or My Access. Step 2: Entra ID Conditional Access enforces MFA at activation time, requiring a fresh authentication proof. Step 3: If approver is configured, the approver receives an email notification and approves or denies the request. Step 4: Upon approval, Entra ID issues a time-limited active role assignment with an expiration timestamp. Step 5: At expiration the role automatically deactivates with no user action required. Audit logging spans all five stages: every stage generates audit log entries in the Entra ID audit log that feed to Microsoft Sentinel or equivalent SIEM." width="100%"}
...
Every stage generates an audit log entry in the Entra ID audit log under the "PIM" category. These entries are structured and queryable in Microsoft Sentinel (via the Microsoft Entra ID connector) and in Log Analytics.
...
Convert-ToPimEligible `
    -UserUpn "admin.user@{{< meta agency.domain >}}" `
    -RoleDefinitionId "62e90394-69f5-4237-9190-012177145e10"  # Global Administrator
```

## PIM Policy Configuration

PIM policy is configured per role. The following settings represent the recommended baseline for federal environments operating at FedRAMP Moderate:
...
$adminUpn        = "admin.user@{{< meta agency.domain >}}"
...
This is the boundary between authentication-only security (what the Volume I and Volume II books deliver) and device-bound security (what the PAW delivers): the device itself is an additional authentication factor for privileged actions.
...
$pawCompliancePolicy = @{
    ...
    # Minimum OS version -- Windows 10 22H2 baseline (set 10.0.22621 to require Windows 11 22H2)
    osMinimumVersion                         = "10.0.19045"   # Windows 10 22H2 minimum; set 10.0.22621 for Win11 22H2

    # Code integrity / WDAC
    codeIntegrityEnabled                     = $true

    # Mobile Threat Defense: require devices at or below the "secured" MTD risk level
    # (Credential Guard / VBS is enforced separately via a Device Guard configuration profile)
    deviceThreatProtectionRequiredSecurityLevel = "secured"

    # Privileged-device password and idle-lock policy
    # (the non-compliance grace period is configured separately in scheduledActionsForRule)
...
![Break-glass: the deliberate exception to every control](figs/pam-fig-06-break-glass-design.png){#fig-pam06 fig-alt="Red panel on why break-glass must exist: every control that blocks an admin path can itself fail — if Intune is down, the PAW is lost, or PIM has an outage, a PAW-requiring Conditional Access policy would block all access, a permanent tenant lockout; break-glass is the path that survives failure of the control plane that enforces access. Four design cards: two cloud-only accounts (not synced from AD, not federated, reachable when AD/ADFS/Entra Connect is down; two so a single failure is survivable); standing Global Admin (the one exception to eligible-only, because PIM may be the unavailable system); excluded from every Conditional Access policy by group membership; FIDO2 plus an offline credential — a 20-plus character random password in a physical safe, never digital, with a FIDO2 hardware key as unphishable MFA, two sealed envelopes in two safes held by two officials. Naming: follow the normal convention with no indicator of elevated privilege. White background. Source: Vol III Book 01 Privileged Access Management briefing deck, Date Code 2026-07-08 13:08 ET." width="100%"}
...
**Naming convention.** Use an account name that does not identify the purpose. `bg-acct-01@{{< meta agency.domain >}}` is discoverable; a name following the normal account naming convention with no indicator of elevated privilege is less targeted.
...
function New-BreakGlassAccount {
    param(
        [string]$AccountNumber,       # "01" or "02" -- internal reference only, never embedded in the account name
        [string]$Domain,              # Agency tenant domain, e.g. "{{< meta agency.domain >}}"
        [string]$AccountName,         # Local part following the agency's NORMAL user-naming convention -- no "bg"/privilege indicator
        [string]$DisplayName          # Display name matching ordinary accounts -- no "Break Glass" indicator
    )
...
# The names below follow the agency's normal user-naming convention and are deliberately
# NOT self-identifying. Substitute names that blend into your directory, and record the
# mapping (account number -> object ID) offline in the break-glass runbook only.
$bg01Id = New-BreakGlassAccount -AccountNumber "01" -Domain "{{< meta agency.domain >}}" -AccountName "svc-ops-a41" -DisplayName "Operations Service A41"
$bg02Id = New-BreakGlassAccount -AccountNumber "02" -Domain "{{< meta agency.domain >}}" -AccountName "svc-ops-b72" -DisplayName "Operations Service B72"
...
# Drive the alert from the object IDs captured at creation ($bg01Id/$bg02Id), NOT the
# (deliberately non-identifying) account names -- ideally maintain them in a Sentinel
# watchlist so the account names never need to appear in detection content.
$alertQuery = @"
SigninLogs
| where UserId in ("$bg01Id", "$bg02Id")   // object IDs from New-BreakGlassAccount; or _GetWatchlist('BreakGlassAccounts')
...
![Three controls that bound a live privileged session](figs/pam-fig-07-session-controls.png){#fig-pam07 fig-alt="Three stacked rows. AC-11 Device lock in navy, tagged 15 min idle: 15-minute idle screen lock enforced by an Intune configuration profile on the PAW group, not user-overridable, requires re-authentication to unlock. AC-12 Session termination in teal, tagged 1 hr / 8 hr: Conditional Access sign-in frequency bounds session lifetime — 8 hours for the base privileged portal session, 1 hour for PIM-activated role sessions with re-auth forced independent of the activation window. IA-11 Re-authentication in amber, tagged every activation: every PIM activation requires fresh MFA via Authentication Context c1 regardless of session age. Blue bar: together they bound the third dimension — PIM bounds privilege in time between episodes, PAW bounds it by device, session controls bound it within a live episode, so a walked-away workstation, an over-long session, and a stale MFA proof each fail closed. White background. Source: Vol III Book 01 Privileged Access Management briefing deck, Date Code 2026-07-08 13:08 ET." width="100%"}
...
# Privileged Access Beyond Entra — Multi-Cloud and Third-Party PAM

The Entra PIM / PAW / Arc-JIT architecture above governs privileged access to the
Microsoft estate — Azure RBAC, Entra directory roles, and Arc-connected servers.
It does not, by itself, govern privileged identities in **AWS GovCloud** (IAM
roles, IAM Identity Center permission sets), **Oracle OCI** (IAM), or the
standing service, break-glass, and appliance-admin accounts a dedicated PAM
platform vaults. On a multi-cloud estate, **AC-5 / AC-6** least privilege must
hold across every cloud — not only Azure.

**Microsoft-preference is earned, not automatic here.** Entra PIM is the default
just-in-time mechanism for the Microsoft surface it actually governs; it does not
natively broker JIT elevation into AWS or OCI IAM. Multi-cloud privileged access
is therefore where the cross-vendor coordinators earn the role:

| Function (control) | Microsoft-native | Cross-vendor / multi-cloud |
|---|---|---|
| JIT elevation (AC-6) | Entra PIM (Azure/Entra) | AWS IAM Identity Center permission sets; OCI IAM policies |
| Entitlement governance & access review (AC-2, AC-6) | Entra ID Governance | **SailPoint Identity Security Cloud** *(owned, FedRAMP Moderate)* — certifies privileged entitlements across CSPs + on-prem, on the HRIT identity SSOT |
| Credential vaulting, session isolation, rotation (AC-6, IA-5) | Key Vault (secrets) | **CyberArk** — vaults and brokers the standing accounts JIT cannot eliminate |
| Cloud entitlement right-sizing (CIEM, AC-6) | Defender for Cloud CIEM *(Moderate)* | Prisma Cloud / Wiz CIEM *(High)* |

**SailPoint ISC** (owned, Moderate) is the cross-cloud entitlement-governance
layer — it reviews and certifies who holds privileged access across Azure, AWS,
OCI, and on-prem, sitting *on* the HRIT identity SSOT rather than replacing it;
**SailPoint NERM** governs non-employee privileged access (series carve-out).
**CyberArk** vaults and session-isolates the standing privileged accounts JIT
cannot remove — break-glass, service, and appliance-admin credentials. Verify
each on the FedRAMP Marketplace at procurement.

Privileged-access events — PIM activations, SailPoint certifications, CyberArk
vault check-outs — emit the unified evidence-record contract into the Multi-Cloud
Evidence Fabric, keyed to authoritative identity, feeding CISA CDM and KSI-IAM.
...
> **Closure Necessity — AC-6(1) and AC-6(10) on privileged sessions.** Every closure in this table rests on two identity substrates, not on policy text. PIM's activation ceremony is enforceable only because the role grant is a cryptographically validated token claim with a bounded lifetime — the certificate-and-token doctrine of [Vol I Book 03 (Certificates & Tokens)](Vol_I_Book_03_OrgComp_Certificates_Tokens_Cryptographic_Identity.qmd); a password-conveyed admin role carries no expiry the directory can enforce. And the PAW gate on AC-6(1)/AC-6(10) holds only because Conditional Access can verify *device* identity and compliance — the device-identity doctrine of [Vol I Book 02 (Network Access Control: 802.1X)](Vol_I_Book_02_OrgComp_Network_Access_Control_802_1X.qmd); without a device-bound assertion, "activate only from a compliant workstation" is a request, not a control. There is no audit procedure, admin-training artifact, or account-naming convention that bounds a standing credential's scope in time or binds its use to hardware.

![NIST control closure for Vol III Book 01 PAM showing the before and after state for each AC and IA control addressed](figs/pam-fig-04-nist-closure.png){#fig-pam04 fig-alt="Table-style diagram with five columns: Control ID, Control Title, State Without PAM Controls, State With PAM Controls, and Primary Authority. Rows cover AC-5 through AC-6-10 and AC-11, AC-12, IA-11. Color coding: red rows indicate non-compliant state without controls, green rows indicate compliant state with controls." width="100%"}
...
| AC-6(2) | Non-privileged Access for Nonsecurity Functions | Admins performing routine tasks (email, documents) use the same account scope as their administrative role — elevated scope is always in use | Admins use a standard account for routine tasks; privileged roles are activated only when needed through PIM; PAW is used only for admin tasks, never for general productivity | NIST SP 800-53 Rev 5 AC-6(2) |
...
| AC-11 | Device Lock | Screen lock timeout may be configured per-device with no enforcement; users can override or disable | Intune configuration profile enforces 15-minute idle screen lock on PAW devices; policy is not user-overridable; compliance policy flags non-compliant devices | NIST SP 800-53 Rev 5 AC-11 |
...
: NIST SP 800-53 Rev 5 control closure for Vol III Book 01 — Privileged Access Management {.striped .hover}

::: {.aan-note}
**FedRAMP 20x Key Security Indicator Alignment**

FedRAMP 20x KSI-IAM-JIT (Authorizing Just-in-Time) requires a least-privileged, role- and attribute-based, just-in-time authorization approach for privileged roles, with privileged accounts separately inventoried and monitored. The PIM eligible assignment inventory, PIM activation audit trail, and Sentinel alert rules documented in this part directly satisfy KSI-IAM-JIT.

FedRAMP 20x KSI-IAM-APM (Adopting Passwordless Methods) calls for secure passwordless, phishing-resistant authentication. The combination of Entra ID MFA at PIM activation, FIDO2 hardware keys for break-glass accounts, and PAW device compliance for the Conditional Access session satisfies KSI-IAM-APM at the phishing-resistant tier.

FedRAMP 20x's KSI-MLA (Monitoring, Logging, and Auditing) theme covers monitoring privileged workstations with endpoint detection — "EDR" is a capability, not a CR26 indicator. The PAW baseline with WDAC, Credential Guard, and Microsoft Defender for Endpoint provides the coverage; the PAW Intune compliance policy ensures it is continuously verified.
...
*Federal Organization Compliance — Vol III Book 01 of the series. Vol III Book 06 (SIEM/XDR Detection) addresses audit log architecture and SIEM integration: structured log forwarding from Arc servers, SQL Server extended events, Entra ID sign-in logs, and PIM audit events into a centralized Log Analytics workspace with Sentinel analytics rules. CyberArk's Privileged Session Manager (PSM) component — the proxying, time-boxing, and recording layer this book's vaulting/session-isolation closure relies on — is also the compensating-control evidence source [Vol VII Book 07 (VMware)](Vol_VII_Book_07_OrgComp_VMware_Control_Compliance_Automation.qmd) cites for manual, out-of-band vCenter script actuation where no orchestrator yet reaches: a PSM-recorded session, gated by a ServiceNow task's approval, in place of a "run successfully" reply.*
