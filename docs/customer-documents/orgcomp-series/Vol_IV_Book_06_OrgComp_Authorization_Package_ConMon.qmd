---
title: "Vol IV Book 06 — Federal Organization Compliance — Authorization Package & ConMon"
...
date: "2026-07-18T00:56:00-04:00"
format:
  docx:
...
aliases:
  - /customer-documents/federal-aan-series/Vol_IV_Book_06_FedAAN_Authorization_Package_ConMon.html
...
**Date Code:** 2026-07-18 00:56 ET
:::

# Preface

This is the capstone book of the Federal Organization Compliance
(OrgComp) series. The preceding twenty-two content books, spanning Volumes I–IV, generated the evidence layer: infrastructure
...
FedRAMP 20x CR26 KSI schema (29 rules in the series' internal KSI decomposition). This document
assembles those artifacts into the **agency RMF authorization package** — the
formal package presented to the {{< meta agency.short >}} Authorizing Official for an agency
Authorization to Operate (ATO) under NIST SP 800-37, with the FedRAMP-authorized
cloud services {{< meta agency.short >}} consumes documented as inherited (leveraged) authorizations.
{{< meta agency.short >}} is a consuming agency, not a cloud service provider: nothing in this package
is submitted to the FedRAMP PMO. The FedRAMP 20x pipeline (Class A/B/C windows)
applies to the CSOs {{< meta agency.short >}} inherits, and this book tracks those provider timelines
...
RA-3, SA-15 (pending registration in the compliance spine).

---

# 1 The Twenty-Two-Book Journey — All 29 Internal KSI Rules Evaluable at High Confidence

## 1.1 Series Summary

The OrgComp series delivered twenty-two technical books covering the {{< meta agency.short >}} GCC
...
self-assessed evidence for all **29 rules in the series' internal KSI decomposition**
(modeled on the FedRAMP 20x CR26 KSI schema).

::: {.aan-important}
**Self-Assessment Notice.** All KSI verdicts in this document are produced by
the governance engine's conformance adapter — an automated self-assessment tool. They are
advisory and do not constitute a formal authorization. Results must be
independently validated through a security control assessment (SCA) — {{< meta agency.short >}} may
...
**KSI-count reconciliation (initial diff published).** The 29 rules evaluated
here are the series' internal KSI decomposition, not the published CR26 Moderate KSI
catalog. The rule-ID-by-rule-ID diff against the in-repo CR26 OSCAL catalog now
exists — see `OrgComp_CR26_Reconciliation.md`. Result: **19 of 46** CR26 indicators
are explicitly mapped 1:1 to an internal rule (six themes fully covered); **27
are not yet bound to an indicator ID** (the IAM/CNA/SVC/MLA themes, addressed by
the 10 ScuBA baseline rules without indicator-ID binding). So "29/29 satisfied"
is a claim about the internal rule set; CR26 indicator-level coverage is 19/46
explicitly mapped, and the POA&M is expected non-empty once the remaining
indicators are assessed against the finalized CR26 Moderate list (June 25, 2026)
by an independent SCA.
:::
...
: OrgComp Series — Control Coverage Summary {.striped .hover}

## 1.2 KSI Scorecard

The governance engine's conformance adapter evaluates all 29 internal KSI rules against the
OrgComp slot evidence. As of July 4, 2026 (self-assessment — pending independent SCA):

| Evaluation Path | Count | KSI IDs | Verdict |
|----------------|-------|---------|---------|
| CISA ScuBA adapter (M365 continuous) | 10 | KSI-001..010 | **satisfied** |
| OrgComp slot evaluator — high confidence | 19 | KSI-011..029 | **satisfied** |
...
run. Zero open risks identified **in the series' internal KSI decomposition**; zero POA&M
items generated **against that rule set** — the POA&M is expected to be non-empty
once the 27 not-yet-mapped CR26 indicators are reconciled (see
`OrgComp_CR26_Reconciliation.md`) and assessed. These results are advisory pending
independent security control assessment (SCA). The OSCAL
...
![Authorization Package Structure](figs/ato-fig-01-package-structure.png){fig-alt="Hierarchical diagram of the authorization package for {{< meta agency.short >}} GCC Moderate. Top tier: Authorization Package presented to the {{< meta agency.short >}} Authorizing Official (agency RMF ATO) with System Security Plan (SSP), Security Assessment Report (SAR), Plan of Action and Milestones (POA&M), and Authorization Decision (Agency ATO). Second tier: OSCAL Machine-Readable Bundle — AP (Assessment Plan, 10 themes, 29 internal KSI rules), AR (Assessment Results, 29 findings, 0 open in the internal rule set — self-assessed pending CR26 reconciliation and independent SCA, 8 evidence slots), POA&M (0 items in the internal rule set, expected non-empty after CR26 mapping and SCA). Third tier: OrgComp Evidence Layer — the twenty-two content books (Volumes I–IV) providing infrastructure, security, governance, and training artifacts. Bottom tier: Continuous Monitoring Program — monthly OSCAL AR refresh, quarterly KSI review, annual full assessment, and ConMon dashboard via Sentinel." width="100%"}
...
The `<engine> oscal bundle` command generates three interlinked OSCAL 1.0.4 documents
...
```bash
# Generate AP + AR + POA&M for the {{< meta agency.short >}} GCC Moderate boundary
<engine> oscal bundle \
  --system-name "{{< meta agency.short >}} GCC Moderate" \
...
|-------|-----------|-----------------|
| KSI-IAM | KSI-001, KSI-002, KSI-003, KSI-009 | slot-01 identity; ScuBA |
...
| KSI-PIY | KSI-017–021 | slot-05 endpoint |
...
![Ten KSI themes mapped to eight evidence slots, each backed by a series book](figs/ato-fig-03-themes-slots.png){#fig-ato03 fig-alt="Table titled 'Ten KSI themes, eight evidence slots' under a Themes to Slots heading, with columns Theme, KSI Rules, and Evidence Slot(s). KSI-IAM covers rules 001, 002, 003, 009, bound to slot-01 identity plus ScuBA. KSI-SVC / KSI-CNA covers 004, 007, 008, bound to slot-03/04 plus ScuBA. KSI-MLA covers 005, 006, 010, bound to slot-02/04 plus ScuBA. KSI-CMT covers 011 through 014, bound to slot-06 security. KSI-SCR covers 015, 016, bound to slot-06 security. KSI-PIY covers 017 through 021, bound to slot-05 endpoint. KSI-INR / KSI-RPL covers 023 through 029, bound to slot-04/07 continuity. KSI-CED covers 022, bound to slot-08 training. A summary banner states each KSI rule binds to at least one evidence slot a series book built, and the ScuBA path (KSI-001 through 010) is validated continuously against M365. White background. Source: Vol IV Book 06 Authorization Package & ConMon briefing deck, Date Code 2026-07-09 17:00 ET." width="100%"}
...
```
Satisfied     : 29  (all 29 internal KSI-decomposition rules)
Not-satisfied : 0
Not-evaluated : 0
Open risks    : 0
POA&M items   : 0
...
| POA&M status review | ISSO + ISSM | `<engine> oscal bundle` | `ksi-poam.json` (zero-item target) |
...
the build) → `<engine> oscal scuba-verdicts` writes
...
|----------|---------|-----------|
| IR tabletop drill (SOC scenario exercise) | IR-2, KSI-INR | Vol IV Book 05 §6 Incident Response and DR Training |
| CP-3 contingency training exercise | CP-3, KSI-RPL | Vol IV Book 01 §4 Tabletop Calendar |
| SBOM freshness + VDR/VER review | SR-5, SR-6, KSI-SCR | Vol IV Book 02 §3 Vulnerability Disclosure Report (VDR) |
...
<engine> oscal ksi-ar --output-path output/artifacts/ksi-bundle/ksi-ar-$(date +%Y%m).json
...
the 20x KSI-evidence model replaces the ~323-control Rev 5 Moderate baseline audit binder for the CSOs
{{< meta agency.short >}} inherits, and this package adopts the same machine-readable model for the
agency's own ConMon evidence. The authorization package consists of:

| Component | Format | Source |
|-----------|--------|--------|
| System Security Plan (SSP) | OSCAL JSON | `<engine> generate ssp` (Vol I Book 01 architecture as baseline) |
...
| OrgComp Evidence Book Series | DOCX | the preceding twenty-two content books, Volumes I–IV (docs/customer-documents/orgcomp-series/) |
...
| Present authorization package to {{< meta agency.short >}} AO | Nov–Dec 2026 | Planned |
...
vexctl create \
  --author "{{< meta agency.short >}} Cloud Services Team" \
  --product "pkg:docker/ssa-gcc-moderate@1.0.0" \
  --vuln "CVE-YYYY-NNNNN" \
  --status "affected" \
  --action-statement "Remediation in progress; tracked in POA&M" \
...
After ATO is granted, the ConMon program feeds back into the governance
substrate on a recurring basis:

```
[M365 ScuBA Assessment]
         ↓ monthly
[<engine> adapter-run-scuba]
         ↓
[Governance Substrate] ←—— [OrgComp Slot Evidence YAMLs]
         ↓                                   ↑
[<engine> oscal bundle]            [Slot confidence upgrades
...
| **ISSO** | Runs monthly `<engine> oscal bundle`; reviews Sentinel dashboard; owns POA&M |
...
| `risk.mitigating-factors` | Compensating controls from OrgComp evidence |
| `risk.remediation-tracking` | BOD 26-04 milestones or training milestones |

When the AR contains zero open risks against the internal rule set (current
self-assessed state), the risk register entry for KSI controls reads: "All 29
internal KSI rules satisfied — no residual risk from the KSI evaluation
(self-assessed; CR26 indicator coverage 19/46, pending reconciliation and SCA)."
This does not eliminate enterprise risk; it eliminates internal-rule-set
...
- **Mission risk** — impact to {{< meta agency.short >}} service delivery from system failure
...
- **Physical security** — PE family (inherited from the CSP data-center authorization)
...
- [ ] Run `<engine> oscal bundle --system-name "{{< meta agency.short >}} GCC Moderate"` and confirm 29/29 self-assessed satisfied against the series' internal KSI decomposition (not the CR26 catalog)
- [ ] Confirm POA&M has zero open items *in the internal rule set* — expect it to become non-empty once the internal-rules → CR26 diff and independent SCA are complete
- [ ] Archive bundle with timestamp to `output/artifacts/ksi-bundle/history/YYYYMM/`
- [ ] Present bundle (AP + AR + POA&M) to the {{< meta agency.short >}} AO for the RMF authorization decision

**Evidence Books (CA-2):**

- [ ] Confirm all twenty-two content books compiled and available in `docs/customer-documents/orgcomp-series/`
...
<engine> oscal bundle \
  --system-name "{{< meta agency.short >}} GCC Moderate" \
  --ssp-href "#<ssp-uuid-from-engine-ssp-generator>"
...
# 4. Package for the {{< meta agency.short >}} AO review
...
|---------|-------|-------------------|
| **CA-2** | Control Assessments | OSCAL AR (`ksi-ar.json`) is the formal security assessment record for all 29 internal KSI-decomposition rules; generated by `<engine> oscal bundle` on a monthly cadence |
...
| **CA-7** | Continuous Monitoring | Monthly `<engine> oscal bundle` → Sentinel dashboard; quarterly KSI trend review; evidence freshness rules per slot in §6.2 |
| **CA-7(1)** | Independent Assessment | Independent security control assessment (SCA) of the OSCAL AR — {{< meta agency.short >}} may engage a FedRAMP-recognized 3PAO as its assessor — constitutes the independent assessment; ScuBAGear is run by the {{< meta agency.short >}} ISSO as agency self-assessment against the external CISA ScuBA baseline, feeding the same M365 evidence into that engagement |
| **CA-7(3)** | Trend Analyses | Sentinel KSI Status Workbook; AR history archive in `output/artifacts/ksi-bundle/history/`; quarterly trend briefing to AO |
| **RA-3** | Risk Assessment | OSCAL AR risk entries feed PM-9 risk register; residual risk taxonomy in §7.2 |
| **SA-15** | Development Process, Standards, and Tools | The slot evaluator, series origin, and diagram pipeline are documented design decisions of record covering the development process used to build the evidence pipeline; the engine's CI/CD gate (lint, type-check, and unit-test suite) is the SA-15 toolchain evidence |
...
> **Indicator-level mapping is NOT asserted here.** The `CR26 Theme` column gives the CR26 Moderate *theme* each internal rule contributes to; the authoritative rule→indicator reconciliation (19 of the 46 CR26 indicators mapped) lives in [`OrgComp_CR26_Reconciliation.md`](OrgComp_CR26_Reconciliation.md). The `KSI-0NN` ids are the series' internal rules, not CR26 indicator ids.

| KSI ID | CR26 Theme | Evaluation Path | Verdict |
|--------|-------------|----------------|---------|
| KSI-001 | KSI-IAM | ScuBA (M365-ACCT-001) | satisfied |
| KSI-002 | KSI-IAM | ScuBA (M365-AUTH-002) | satisfied |
| KSI-003 | KSI-IAM | ScuBA (M365-PRIV-003) | satisfied |
| KSI-004 | KSI-SVC | ScuBA (M365-EXO-004) | satisfied |
| KSI-005 | KSI-MLA | ScuBA (M365-EXO-005) | satisfied |
| KSI-006 | KSI-MLA | ScuBA (M365-SPO-006) | satisfied |
| KSI-007 | KSI-SVC | ScuBA (M365-ATP-007) | satisfied |
| KSI-008 | KSI-SVC | ScuBA (M365-ATP-008) | satisfied |
| KSI-009 | KSI-IAM | ScuBA (M365-CA-009) | satisfied |
| KSI-010 | KSI-MLA | ScuBA (M365-DLP-010) | satisfied |
| KSI-011 | KSI-CMT | slot-06 high (Vol IV Book 02–03) | satisfied |
| KSI-012 | KSI-CMT | slot-06 high | satisfied |
| KSI-013 | KSI-CMT | slot-06 high | satisfied |
| KSI-014 | KSI-CMT | slot-06 high | satisfied |
| KSI-015 | KSI-SCR | slot-06 high | satisfied |
| KSI-016 | KSI-SCR | slot-06 high | satisfied |
| KSI-017 | KSI-PIY | slot-05 high | satisfied |
| KSI-018 | KSI-PIY | slot-05 high | satisfied |
| KSI-019 | KSI-PIY | slot-05 high | satisfied |
| KSI-020 | KSI-PIY | slot-05 high | satisfied |
| KSI-021 | KSI-PIY | slot-05 high | satisfied |
| KSI-022 | KSI-CED | slot-08 high (Vol IV Book 05) | satisfied |
| KSI-023 | KSI-INR | slot-07 high | satisfied |
| KSI-024 | KSI-INR | slot-04 high | satisfied |
| KSI-025 | KSI-INR | slot-04 high | satisfied |
| KSI-026 | KSI-RPL | slot-07 high | satisfied |
| KSI-027 | KSI-RPL | slot-07 high | satisfied |
| KSI-028 | KSI-RPL | slot-07 high | satisfied |
| KSI-029 | KSI-RPL | slot-07 high | satisfied |

: KSI Findings Summary — July 4, 2026 Bundle Run {.striped .hover}

---

*This document is an internal working artifact. It does not constitute an
authorization package or official assessment. All control closure claims are
advisory pending formal ATO review by the Agency Authorizing Official and the
Office of Information Security — {{< meta agency.short >}} is a consuming agency, so nothing here is
submitted to the FedRAMP PMO.*
