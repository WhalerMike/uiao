---
title: "Vol I Book 02 — Federal Organization Compliance — Network Access Control: 802.1X and Device Identity"
subtitle: "802.1X, EAP-TLS, and Device Identity Enforcement at the Port"
author: "Federal Cloud Services"
date: "2026-07-15T06:26:00-04:00"
format:
  html:
    css: lz-style.css
...
aliases:
  - /customer-documents/federal-aan-series/Vol_I_Book_02_FedAAN_Network_Access_Control_802_1X.html
...
**Scenario B — Physical port access on an uncontrolled network.** An attacker connects a laptop to a wired jack in an {{< meta agency.short >}} conference room. Without NAC, the switch port assigns a DHCP lease and routes traffic immediately. The attacker now has Layer 3 connectivity to {{< meta agency.short >}} internal resources — without presenting any user credential at all. All user identity controls are irrelevant until the attacker chooses to attempt authentication. The window for passive reconnaissance, ARP poisoning, DNS poisoning, and lateral movement scanning is open from the moment the cable is plugged in.

NAC closes Scenario B at the port level. The attacker's laptop presents no machine certificate, 802.1X authentication fails, and the port is placed in the quarantine VLAN. The attacker has Layer 3 connectivity to the quarantine segment only — no routing to {{< meta agency.short >}} resources, no DNS resolution for internal names.
...
- **Conference rooms** are routinely accessible to vendors, contractors, and visitors with valid facility badges who do not have {{< meta agency.short >}} IT authorization.
...
![The federal physical access reality — five exposure classes where a facility badge is the only gate](figs/nac-fig-10-physical-access-reality.png){#fig-nac10 fig-alt="Five cards showing federal facility exposure classes with live, unauthenticated network ports. Conference rooms: routinely accessible to vendors, contractors, and visitors with valid facility badges but no {{< meta agency.short >}} IT authorization. Wiring closets: older federal buildings often lack electronic access control distinct from the floor-level badge reader. Decommissioned areas: live switch ports persist weeks to months after occupancy ends. Hoteling workstations: hot-desk ports are live by design with a different physical occupant daily. IoT/OT devices: printers, badge readers, HVAC, and video endpoints share the same switch fabric as workstations and servers. Banner: each is a live network connection with no authentication gate — NAC makes the gate the switch port itself, independent of the physical access regime. White background. Source: Vol I Book 02 NAC briefing deck, Date Code 2026-07-07 10:15 ET." width="100%"}

## Before and After: The Structural Comparison

![Before and after NAC deployment showing unauthenticated port access versus 802.1X authenticated admission flow](figs/nac-fig-01-before-after.png){#fig-nac01 fig-alt="Two-panel diagram. Left panel labeled BEFORE shows a laptop connecting to a switch port and immediately receiving a DHCP lease and routing to {{< meta agency.short >}} resources with no authentication gate. Right panel labeled AFTER shows the same laptop initiating 802.1X EAP exchange with the switch acting as authenticator, forwarding the EAP-TLS exchange to a RADIUS server, which validates the machine certificate against Entra ID device inventory, and the switch placing the port into the appropriate VLAN — compliant, non-compliant, or quarantine — before any IP address is issued to a routable segment." width="100%"}

**Before this document** — a device connecting to any {{< meta agency.short >}} switch port in a managed facility receives network access determined solely by VLAN configuration on the port. No device identity assertion is required. Physical access to the facility is the only gate.
...
![The voice VLAN precedent — identity by assertion (DHCP option 60) versus identity by proof (802.1X EAP-TLS)](figs/nac-fig-08-voice-vlan-precedent.png){#fig-nac08 fig-alt="Two-column comparison. Left, late 1990s IP telephony before 802.1X: phone boots on the data VLAN and sends DHCP option 60 (vendor class identifier); the DHCP server recognizes the class and answers with the voice VLAN ID and config server; the phone re-tags onto the voice VLAN; the switch never authenticated anything — the naming plane classified the device. Verdict: identity by assertion — any device can transmit a phone's vendor class; nothing is verified. Right, today with 802.1X EAP-TLS: the device asserts identity with a machine certificate whose private key never leaves the TPM; RADIUS plus the Entra inventory verify the assertion cryptographically; the network assigns a segment scoped to trust — production, remediation, or quarantine. Verdict: identity by proof — the assertion lineage survives only in MAB and DHCP fingerprinting, deliberately constrained. Banner: the DHCP server was acting as a policy decision point a decade before the term existed; NAC generalizes the pattern and replaces the assertion with a proof. White background. Source: Vol I Book 02 NAC briefing deck, Date Code 2026-07-07 10:15 ET." width="100%"}
...
![The 802.1X authentication stack — supplicant, authenticator, authentication server](figs/nac-fig-09-authentication-stack.png){#fig-nac09 fig-alt="Three-node flow diagram of the IEEE 802.1X roles. Node 1, supplicant — the device asking: software on the device requesting access; Windows built-in supplicant via Intune, wpa_supplicant on Linux, native supplicant via MDM on macOS. Node 2, authenticator — the switch or WAP enforcing: validates nothing itself; relays the EAP exchange and enforces the VLAN the server returns. Node 3 (highlighted), authentication server — RADIUS deciding: validates the machine certificate against the Entra ID device inventory and returns the access decision — VLAN ID, session timeout, filter rules. Caption: EAP carried end-to-end — supplicant to authenticator over EAPOL, authenticator to authentication server as EAP over RADIUS. White background. Source: Vol I Book 02 NAC briefing deck, Date Code 2026-07-07 10:15 ET." width="100%"}

## EAP Method Selection — Why EAP-TLS for Federal

NIST SP 800-120 (Recommendation for EAP Methods Used in Wireless Network Access Authentication) and NIST SP 800-153 (Guidelines for Securing Wireless Local Area Networks) both recommend EAP-TLS as the strongest EAP method for federal environments. A mutual-certificate, FIPS-approved EAP method is not optional under FedRAMP Moderate — EAP-TLS is recommended, EAP-PEAP/EAP-TLS is acceptable, and the password-based methods below are not:
...
The RADIUS server is the authentication server in the 802.1X architecture. For {{< meta agency.short >}} GCC Moderate, the RADIUS platform choice is governance decision **D3 (NPS vs. Cisco ISE vs. Aruba ClearPass)** — and D3 is **gated by the Entra-migration timeline (Vol I Book 05, Network Modernization), not by preference**. The deciding criterion is Entra-native device authorization:

- **Microsoft Network Policy Server (NPS)** on Windows Server 2025 is viable for the **hybrid-joined phase only**. NPS authorizes a device by resolving its Active Directory computer object; it has no supported path for authorizing cloud-native Entra-joined devices — Microsoft provides no cloud NPS equivalent, and the community workaround (dummy AD computer objects) is unsustainable at fleet scale. NPS works against today's estate precisely because the estate is hybrid-joined and AD objects still exist. **Its viability degrades in direct proportion to Vol I Book 05's AD-to-Entra migration progress.**
- **Cisco Identity Services Engine (ISE)** and **Aruba ClearPass** — the dominant federal alternatives — both terminate EAP-TLS and authorize against the Entra ID / Intune device inventory without requiring an AD computer object. Either closes the Entra-native gap.

The sequencing requirement, stated plainly: **D3 must select a platform that authorizes Entra-native devices before Vol I Book 05's cloud-native cutover retires the AD objects NPS depends on.** An agency may deploy NPS now for the hybrid phase — the NPS configuration in this document is written for that phase — but the D3 decision record must carry the migration trigger rather than treat NPS as the end state.

Cloud-issued device certificates are provisioned via **Microsoft Cloud PKI** (an Intune add-on, licensed via Intune Suite or standalone) **or Intune SCEP against the agency's AD CS** — the certificate infrastructure section below covers both paths.

![RADIUS server architecture showing NPS in high-availability configuration with primary and secondary servers, RADIUS proxy groups for wired and wireless authenticators, and integration with Microsoft Cloud PKI and the Intune Compliance API](figs/nac-fig-02-radius-architecture.png){#fig-nac02 fig-alt="Architecture diagram showing two NPS servers in an active-passive pair. Each switch and WAP in the building is configured with the primary NPS server as the first RADIUS target and the secondary as the failover. NPS servers connect to Active Directory for machine certificate validation and to the Intune Compliance API (via Graph) for real-time device compliance posture. Microsoft Cloud PKI issues device certificates during Intune enrollment via SCEP. A third connection shows RADIUS accounting records flowing to a Log Analytics workspace for Sentinel." width="100%"}
...
NAC with EAP-TLS requires every managed device to hold a machine certificate trusted by the RADIUS server's CA. This certificate is distinct from the user certificate (used for user authentication to Entra ID). The machine certificate proves device identity — that this physical device is an {{< meta agency.short >}}-enrolled managed endpoint — independently of which user is logged in.

The certificate trust chain for {{< meta agency.short >}} GCC Moderate NAC:

```
Microsoft Cloud PKI Root CA — or agency AD CS Root
  └─ {{< meta agency.short >}} Device Identity CA (Intermediate, deployed via Intune)
       └─ Device Machine Certificates (issued per-device at enrollment)
```

Both issuance paths terminate in the same intermediate: **Microsoft Cloud PKI** (the Intune add-on for cloud-issued certificates) can host the issuing CA directly, or the agency's existing **AD CS** issues via the Intune SCEP connector (NDES). The choice is a PKI-operations decision; the RADIUS trust configuration and the EAP-TLS exchange are identical under either.

The RADIUS server (NPS) is configured to trust the {{< meta agency.short >}} Device Identity CA. Any device that presents a certificate signed by this CA and passes the revocation check is recognized as a managed {{< meta agency.short >}} device.
...
```json
{
  ...
  "subjectAlternativeNameValue": "{{DeviceId}}@{{< meta agency.tenant >}}",
  ...
  "scepServerUrls": ["https://ndes.{{< meta agency.tenant >}}/certsrv/mscep/mscep.dll"]
}
```
...
| **Guest** | 500 | MAB — unmanaged device | Internet-only; no {{< meta agency.short >}} routing |
...
1. **Certificate validity** — the machine certificate is valid, not expired, issued by the {{< meta agency.short >}} Device Identity CA, and not revoked (CRL/OCSP check passes).
...
- DNS resolution for those names via the {{< meta agency.short >}} DNS forwarder
- No routing to any {{< meta agency.short >}} internal resource
...
The Quarantine VLAN (VLAN 999) receives devices that fail certificate authentication entirely: no certificate presented, certificate from an unknown CA, expired certificate, or certificate revoked. These are devices that are either not {{< meta agency.short >}}-managed or have been decommissioned.

A captive portal DNS redirect sends HTTP traffic from the quarantine VLAN to an internal page that explains: *"This device is not authorized for {{< meta agency.short >}} network access. If you believe this is in error, contact the {{< meta agency.short >}} Help Desk."* This prevents the user from believing the network is simply down and generating a flood of unrelated help desk calls.
...
- **Approved IoT/OT MAC prefixes** — manufacturer OUI prefixes for approved device categories (e.g., the OUI prefix for the {{< meta agency.short >}}-standard IP phone model, the badge reader vendor). These receive VLAN 300 (IoT VLAN) with restricted routing to their management systems only.
...
The posture check in {{< meta agency.short >}}'s NAC deployment is driven by Intune device compliance state, not by an inline NAC probe. This architectural choice is intentional:
...
The standard 802.1X re-authentication interval (8 hours) means a device that fails compliance mid-day could remain on the production VLAN for up to 8 hours. RADIUS Change of Authorization (CoA, RFC 5176) shrinks this window.

**Why this component exists at all: NPS has no RFC 5176 support.** ISE and ClearPass issue CoA natively from their own session state; NPS cannot. The Azure-Function "NAC policy engine" below is a purpose-built dynamic-authorization client that routes around the NPS gap — and its existence is one of the strongest arguments for ISE/ClearPass in the D3 decision. Deploying it honestly means engineering, not two sentences:

- **Network path:** the function needs reachability to **UDP 3799 on every switch's management interface** — a cloud-to-management-plane path that must be firewalled to exactly this flow.
- **Authenticator trust:** every switch must be configured to trust the function's source IP as a dynamic-authorization client (`aaa server radius dynamic-author`), with its own shared secret under the same Key Vault rotation regime as the RADIUS secrets.
- **Session identity:** a CoA Disconnect/Re-Auth must carry session-identifying attributes (Calling-Station-Id, NAS-IP-Address, Acct-Session-Id) that exist only in **RADIUS accounting data** — so the accounting feed stops being telemetry and becomes **Tier-0-adjacent enforcement infrastructure**: its integrity and availability now gate an enforcement action.

The flow: the Azure Function polls the Intune compliance API every 15 minutes and sends a CoA Disconnect or Re-Auth to the owning switch when a device's state changes from `compliant` to `nonCompliant`, using session attributes resolved from the accounting feed. The switch re-authenticates the device, the RADIUS server returns the Remediation VLAN, and the port moves without manual intervention.

**Qualify the timing claim:** "within 20 minutes" is 20 minutes *after Intune registers the state change* — the polling interval plus CoA propagation. Intune's own compliance evaluation depends on device check-in cadence and can lag the actual on-device state change by hours. The bound that holds unconditionally is the 8-hour re-authentication backstop (see "Session Timeout and Re-Authentication"): even with the CoA agent down or Intune lagging, no non-compliant device survives past the next re-auth.
...
Wireless 802.1X uses the same EAP-TLS supplicant and certificate infrastructure as wired, but the authenticator is the Wireless Access Point (WAP) or Wireless LAN Controller (WLC). The SSID for managed {{< meta agency.short >}} devices is configured as WPA3-Enterprise (SAE + EAP-TLS), which provides Protected Management Frames (PMF) and eliminates the KRACK-class attacks that affect WPA2-Enterprise.

A separate guest SSID (WPA3-Personal or open with a captive portal) is provided for personal devices and visitors on a completely separate VLAN with no routing to {{< meta agency.short >}} resources.

```
# Cisco WLC / Catalyst Center SSID configuration (CLI equivalent)
wlan {{< meta agency.short >}}-Managed 1 {{< meta agency.short >}}-Managed
 security wpa wpa3
 security wpa akm dot1x
 security dot1x eap-profile {{< meta agency.short >}}-EAP-TLS
...
2. **Anomaly detection** — a device that presents a valid {{< meta agency.short >}} machine certificate but whose DHCP fingerprint does not match any known Windows/macOS pattern (e.g., a Linux system or Android device) generates an alert in Sentinel. This catches cases where a valid certificate has been extracted from a managed device and installed on an unauthorized system.
...
This record is the **CM-8 device inventory artifact** for NAC-controlled devices. Every device that has ever connected to the {{< meta agency.short >}} network — managed or not — has a DHCP record in InfoBlox. The NAC authentication result annotates the record with the identity assertion that was presented (or absent).

::: {.aan-note}
**Cross-Book Link — DNS Hostname + Certificate Binding at NAC Admission (Vol I Books 01 and 03):**
For every NAC-admitted endpoint, two compound events should occur together at admission:

1. **Machine certificate issued** — Intune/SCEP enrollment (this book) issues a machine certificate with `CN={{DeviceName}}`. The certificate SAN is bound to the device's Entra ID FQDN.
2. **DNS A/PTR record confirmed in InfoBlox** — the DHCP lease assignment (this section) should trigger InfoBlox to confirm or create a DNS A record binding the device's hostname to its assigned IP address.

When both events complete, every NAC-admitted device has **name, address, and key bound together** — the same compound guarantee that [Vol I Book 01](Vol_I_Book_01_OrgComp_Cloud_Landing_Zone_IPAM_FedRAMP.qmd) achieves for cloud workloads at provisioning time, extended to the endpoint population. The machine certificate architecture is generalized in [Vol I Book 03](Vol_I_Book_03_OrgComp_Certificates_Tokens_Cryptographic_Identity.qmd). The design decision for the hostname-to-cert binding must be validated with OIS.
:::
...
5. Enable the Sentinel analytics rules and configure alert routing to the {{< meta agency.short >}} SOC.
...
![NIST control closure for Vol I Book 02 NAC showing before and after state for IA, AC, SC, and CM controls](figs/nac-fig-07-nist-closure.png){#fig-nac07 fig-alt="Table-style diagram with five columns: Control ID, Control Title, State Without NAC, State With NAC, and Primary Authority. Rows cover IA-3, IA-3(1), AC-3, AC-17, AC-19, AC-20, SC-7(3), SC-7(5), CM-6, CM-7, CM-8. Color coding: red rows indicate non-compliant or uncontrolled state without NAC, green rows indicate controlled state with NAC." width="100%"}

| Control | Title | Without This Document | With This Document | Authority |
|---------|-------|----------------------|-------------------|-----------|
| **IA-3** | Device Identification and Authentication | Devices connect to {{< meta agency.short >}} network ports without presenting any device identity assertion; the network cannot distinguish a managed {{< meta agency.short >}} workstation from an unauthorized device on the same physical port | Every device must present a valid EAP-TLS machine certificate issued by the {{< meta agency.short >}} Device Identity CA before receiving network access; certificate SAN is validated against the Entra ID device inventory; unauthorized devices are quarantined | NIST SP 800-53 Rev 5 IA-3 |
...
| **AC-19** | Access Control for Mobile Devices | Mobile devices and personal laptops connecting to {{< meta agency.short >}} wireless receive the same SSID access as managed devices; no device identity check distinguishes them | WPA3-Enterprise with EAP-TLS restricts the managed SSID to Intune-enrolled devices with valid machine certificates; personal devices and unmanaged mobile devices cannot authenticate to the managed SSID and are directed to the guest SSID | NIST SP 800-53 Rev 5 AC-19 |
| **AC-20** | Use of External Systems | Vendor and contractor devices that connect to {{< meta agency.short >}} conference room ports are treated identically to managed {{< meta agency.short >}} devices by the network | Vendor and contractor devices that cannot present a valid {{< meta agency.short >}} machine certificate are placed in the quarantine VLAN; if vendor NAC access is required, an explicit exception process (MAC registration + IoT VLAN + time-limited ACL) is used with documented approval | NIST SP 800-53 Rev 5 AC-20 |
| **SC-7(3)** | Access Points | Network access points (switch ports, WAPs) are open to any physically connected device; the boundary is the building perimeter, not the network port | Every switch port and WAP SSID (managed) is a boundary enforcement point; 802.1X authentication is the access control mechanism at each access point; unauthorized devices cannot traverse the boundary regardless of physical facility access | NIST SP 800-53 Rev 5 SC-7(3) |
| **SC-7(5)** | Deny by Default — Allow by Exception | Unauthenticated devices receive full network access by VLAN assignment; the default is open | Unauthenticated devices (no 802.1X supplicant, no MAB match) receive VLAN 999 (quarantine) with no routing to {{< meta agency.short >}} resources; the default is deny; access must be explicitly granted by a successful authentication | NIST SP 800-53 Rev 5 SC-7(5) |
| **CM-6** | Configuration Settings | Device configuration compliance (patch level, AV, encryption) is assessed by Intune but is not enforced at the network admission layer; a non-compliant device can connect to the production network | Intune compliance state is evaluated at every 802.1X authentication and on CoA events (15-minute polling); a device that fails Intune compliance is moved to the Remediation VLAN within ~20 minutes of Intune registering the state change, bounded unconditionally by the 8-hour re-authentication interval | NIST SP 800-53 Rev 5 CM-6 |
...
This is a continuum, not a replacement. Vol I Book 03 (Certificates and Tokens) takes the machine certificate this document provisions for the RADIUS exchange and generalizes it into the series' cryptographic identity plane — the same SCEP pipeline, the same TPM-bound keys, the same revocation infrastructure, now issuing identities for users, services, and workloads as well as switch-port admission. Vol I Book 05 (application-aware modernization) then rebuilds the network itself around those identities, so that transport policy, not just admission policy, is decided per-application and per-identity. Posture asserted at the port evolves into posture asserted in the token; the port remains the enforcement point for the one layer no token can reach — the physical wire — while the token carries the identical device identity everywhere the wire does not go.

---

*Federal Organization Compliance — Vol I Book 02 of the series. Vol I Book 01 established the authoritative naming and addressing plane; Vol I Book 02 lays the device identity substrate on top of it, closing the device identity gap at the network port before any higher-layer identity assertion is made. Vol I Book 03 builds the certificate and token identity plane on this substrate, and the books that follow across Volumes I–IV carry those identities through transport, applications, operations, and the authorization package. Together the nineteen parts provide self-assessed evidence for all 29 rules in the series' internal KSI decomposition (modeled on the FedRAMP 20x CR26 KSI schema) and the NIST SP 800-53 Rev 5 controls recorded as closed in the series compliance spine (`orgcomp-compliance-spine.yml`), pending independent security control assessment (SCA).*
