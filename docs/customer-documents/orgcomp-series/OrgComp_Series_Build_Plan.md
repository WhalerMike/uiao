...
Companion updates: `federal-orgcomp-conmon-gap-roadmap.md` (slot bindings cite Part
numbers — all shift), Book 00 series-map tables, the datecoded distribution kit
`OrgComp_Federal_Series_Complete_<DATE>ET.zip` (rebuild last, locally — carries the
...
| **6** | Derived artifacts: regenerate .docx/.html from .qmd; redraw es-fig-01 series figure (committed SVG per ADR-093); rebuild the datecoded kit `OrgComp_Federal_Series_Complete_<DATE>ET.zip` locally (docx + .pptx decks + governance docx); note training-program modules (`docs/publications/aan-training-program/`) as downstream regeneration | Publishable series v2 | Render clean; fig-alt text updated |
