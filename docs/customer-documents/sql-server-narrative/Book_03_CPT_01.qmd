...
A Category 3 instance requires a SQL Server version upgrade as the gate before any Entra-auth migration can occur, and that upgrade timeline must be factored into the overall program sequencing ahead of Microsoft's progressive NTLM defaults.
...
**Microsoft provides:** `sys.dm_exec_connections` with `auth_scheme`; `sys.server_principals` for login enumeration; the Windows registry `LoginMode` value; Windows event log NTLM auditing.

**What only you can provide:** Spec3-D1.8 (`Get-SQLServerAuthAudit.ps1`) — the cross-estate enumeration and Arc-eligibility classification that DMVs alone do not produce; the CCM-BIR integration that makes the output a drift-aware compliance record rather than a point-in-time spreadsheet.
:::
