...
::: {.callout-note}
## In this chapter
Why NTLMv1 is blocked early — in ADR-068's Phase B, by default upon completion of the Phase A assessment (targeted 2026-Q3) — through the LAN Manager Authentication Level policy, while NTLMv2 is phased out as dependencies are remediated ahead of Microsoft's progressive NTLM defaults, and the four distinct SQL Server connection patterns that generate NTLM traffic — a missing or incorrect SPN, cross-site connections where the KDC is unreachable, linked-server passthrough authentication, and non-domain-joined application hosts created by the server transformation.
:::
...
Unlike NTLMv1, NTLMv2 is not removed in Phase B. ADR-068 restricts it via Group Policy to documented exception groups during Phase B, but full disablement waits for the elimination phase — Phase C — which is applied as operational readiness allows. The phased timeline exists to give programs the operational space to identify and remediate NTLMv2-generating connection patterns before the block is applied.

That space narrows with each release as Microsoft tightens defaults, and the program must maintain forward momentum. ADR-068's elimination phase applies domain-wide NTLM restrictions program-wide — the program's own NTLM-elimination step, timed to complete ahead of Microsoft's progressive default-disable. SQL Server NTLMv2 connections must be migrated either to Kerberos as an interim posture — using Cloud Kerberos trust per ADR-068 (Cloud Kerberos trust) — or to Entra ID OAuth 2.0 authentication as the final destination. (ADR-068)
...
Any application running on a host that has been migrated to a non-domain-joined state — as part of the ADR-002 server transformation — but still connecting to SQL Server using Windows Integrated Security will generate NTLM traffic. A non-domain-joined host has no Kerberos TGT from the KDC and cannot negotiate Kerberos for SQL Server connections. SSPI falls back to NTLM using local credentials when the application host is in a workgroup.

This pattern becomes more prevalent as the ADR-002 server migration progresses. As more application servers are domain-unjoined, the applications running on them that still use Windows Integrated Security become new NTLM sources. This is a strong operational argument for completing the SQL Server connection-string migration in parallel with, or before, the domain unjoin of application hosts.

An application host that has been domain-unjoined and whose connection string uses Managed Identity authentication generates no NTLM traffic and is fully compliant with both ADR-002 and ADR-068. An application host that has been domain-unjoined but whose connection string still uses Windows Integrated Security is now an NTLM source that did not exist before the domain unjoin. (ADR-002, ADR-068)

::: {.callout-tip title="Division of labor"}
**Microsoft provides:** the LAN Manager Authentication Level policy for NTLMv1 blocking; Windows event log NTLM auditing (events 4624 and 4625); Defender for Identity NTLM signals; the product deprecation timeline.

**What only you can provide:** the four connection-pattern classification across your estate — missing SPN, cross-site KDC unreachability, linked-server passthrough, ADR-002 domain-unjoin interaction; the SPN corrections, KDC routing fixes, linked-server migrations, and connection-string updates that each pattern class requires.
:::

::: {.callout-note title="What Book 06 establishes — Chapter 1"}
NTLM is a protocol, not a setting, so there is no single switch that removes it from SQL Server connections. NTLMv1 is blocked early — in ADR-068's Phase B, through the LAN Manager Authentication Level policy — and should never appear in a modern estate; NTLMv2 is restricted to exception groups in Phase B and fully blocked at the Phase C elimination step, applied ahead of Microsoft's progressive NTLM default-disable (ADR-068). Four connection patterns generate NTLM at the SQL layer — a missing or incorrect SPN, cross-site connections where the KDC is unreachable, linked-server passthrough authentication, and non-domain-joined application hosts created by the ADR-002 transformation — and each must be detected before it can be remediated.
:::
