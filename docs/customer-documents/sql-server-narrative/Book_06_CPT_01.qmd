...
That space is not infinite, and the timeline is not flexible. ADR-068 sets 2027-04-01 as the date on which domain-wide NTLM restrictions take effect program-wide, in its elimination phase — the backstop the program aligned to the CCM BIR continuous-monitoring adoption deadline (FedRAMP Notice 0009, via ADR-043). SQL Server NTLMv2 connections must be migrated either to Kerberos as an interim posture — using Cloud Kerberos trust per ADR-068 (Cloud Kerberos trust) — or to Entra ID OAuth 2.0 authentication as the final destination, before that date. (ADR-068)
