...
**Microsoft provides:** `Find-DbaInstance` / Azure Migrate MAP / DMA as discovery instruments; the `MSSQLSvc` SPN trail in AD as an estate-presence indicator; Azure Migrate assessment tooling.

**What only you can provide:** the multi-instrument discovery coordination — no single tool covers every instance; the Spec3-D1.x estate-discovery instrument that ADR-091 §1 identifies as not yet authored; the inventory that gates the authentication audit in Book 03.
:::
