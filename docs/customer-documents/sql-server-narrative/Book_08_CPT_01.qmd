...
**Microsoft provides:** OIDC/SAML federation (Class 3/2); Azure App Proxy (Class 1); Entra Domain Services (Class 4 last resort); Spec3-D1.9 (`Get-AppChainAudit.ps1`) as the application discovery instrument.

**What only you can provide:** the per-application four-class classification across your estate; the two-track migration coordination — user authentication track plus SQL connection track — for each class; the correlation of Spec3-D1.9 application data with Spec3-D1.8 instance data to reveal the complete authentication chain.
:::
