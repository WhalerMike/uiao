...
**Microsoft provides:** the Arc enrollment process; the Azure extension for SQL Server installation; the IMDS token endpoint; the Spec3-D1.8 Arc-eligibility classification as the prioritization input.

**What only you can provide:** outbound HTTPS connectivity verification through the GCC-Moderate egress proxy; the per-host deployment sequence that respects linked-server dependency order; the IMDS token test that serves as the gate authorizing login migration.
:::
