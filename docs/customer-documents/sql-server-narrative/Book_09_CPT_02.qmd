...
**Microsoft provides:** the six evidence artifact categories — CCM-BIR records, Entra sign-in logs, Defender for SQL telemetry, `sa`-disabled report, Conditional Access MFA report, exception registry; the 2027-04-01 network block (platform-level, fixed, non-negotiable).

**What only you can provide:** migration completion well before 2027-04-01 so the final months are reserved for continuous-audit validation, not last-minute migration; the exception registry with Certificate-Based Authentication as the authorized interim posture; the documented sunset dates for every exception that must resolve before the block.
:::
