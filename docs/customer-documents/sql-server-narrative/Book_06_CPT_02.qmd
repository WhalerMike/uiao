...
::: {.callout-note}
## In this chapter
How ADR-068's three-phase structure applies to SQL Server: Phase A discovery using Spec3-D1.8 alongside Windows event-log NTLM analysis, Phase B restriction that converts NTLM connections to Kerberos and migrates the remaining patterns, and Phase C elimination applied ahead of Microsoft's progressive NTLM default-disable — and why an instance migrated to Entra ID logins is immune to that block because it was never generating NTLM in the first place.
:::

ADR-068 structures NTLM elimination into three phases — discover, restrict, eliminate — and this chapter applies that structure specifically to SQL Server workloads. The phases are sequential, and the elimination phase is applied as operational readiness allows — ahead of Microsoft's progressive NTLM default-disable, not waiting for a fixed calendar date.

Once the elimination phase is applied, any SQL Server connection that requires NTLM will fail at the LSA layer — inside the host's Windows security subsystem, where the Local Security Authority (LSA) enforces the policy. The SQL Server engine will not receive an authenticated connection; the NTLM exchange is rejected by the LSA before the engine evaluates the login. SQL Server NTLM elimination is therefore not decoupled from the broader ADR-068 posture. Every NTLM-generating connection pattern must be resolved before the elimination phase, not after. (ADR-068)

![The three NTLM-elimination phases for SQL Server — discover, restrict, eliminate — sequenced ahead of Microsoft's progressive NTLM defaults](images/book06cpt02-diagram-01-three-phase-timeline.png){#fig-book-06-cpt-02-diagram-01 fig-alt="A horizontal engineering timeline diagram on a white background showing three sequential phase bands in federal navy. Phase A discovery band labeled Spec3-D1.8 audit plus event-log NTLM events 4624 and 4625 logon type 3. Phase B restriction band in navy with teal accents labeled NTLMv1 block, SPN remediation, Cloud Kerberos trust, linked-server security contexts, connection-string migration. Phase C elimination band ending at a navy vertical marker labeled domain-wide NTLM block applied as dependencies are remediated. A teal callout above the marker reads instances migrated to type-E logins are immune. Engineering blueprint style, white background, federal navy (#1F3A5F), teal (#1A9E8F) for Kerberos and Entra-safe annotations, monospace labels, no human figures, 16:9 landscape." width="100%"}
...
After Phase B, the only remaining NTLM-generating SQL Server connections should be those that require the elimination phase's block to remove. These are typically applications that cannot be modified before Phase C is applied, which become exception cases requiring formal exception documentation per ADR-068. (ADR-068)
...
The sequencing imperative is clear: complete the SQL Server login migration before Phase C is applied. Phase C applies as dependencies are remediated, and the SQL Server login migration must be complete for every production instance in the estate before that step is taken. (ADR-068)

::: {.callout-tip title="Division of labor"}
**Microsoft provides:** the NTLM enforcement controls for each phase; Windows event log analysis instrumentation; the 2027/2028 platform-level network block that arrives regardless.

**What only you can provide:** the discovery/restriction/elimination phase sequencing; the instance-by-instance rollout with defined rollback procedures; the login migration that must be complete before Phase C is applied — because the block does not wait for unresolved dependencies.
:::

::: {.callout-note title="What Book 06 establishes — Chapter 2"}
ADR-068 elimination runs in three phases for SQL Server. Discovery uses Spec3-D1.8 plus Windows event-log analysis of NTLM events 4624 and 4625 at logon type 3 to baseline every NTLM-generating pattern. Restriction blocks NTLMv1, remediates SPNs, deploys Cloud Kerberos trust, migrates linked-server security contexts, and updates connection strings on domain-unjoined hosts. Elimination applies the domain-wide NTLM block as dependencies are remediated, ahead of Microsoft's progressive NTLM default-disable — and any instance migrated to type-E Entra ID logins is immune, because it never generated NTLM to begin with.
:::
