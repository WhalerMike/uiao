...
After Phase B, the only remaining NTLM-generating SQL Server connections should be those that require the elimination phase's hard block to remove. These are typically applications that cannot be modified before the 2027-04-01 deadline, which become exception cases requiring formal exception documentation. (ADR-068)
