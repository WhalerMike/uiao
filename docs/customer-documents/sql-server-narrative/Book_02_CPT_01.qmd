...
The order matters because effort is finite and each release narrows the window. Migrating the logins of an instance to Entra ID, only to retire that instance three months later, is wasted work that also delays the instances that most need attention. ADR-091 §1 makes the sequencing normative: the consolidation assessment classifies every instance **retain**, **consolidate**, or **retire** before any login is touched.
...
**Microsoft provides:** Arc-based SQL Server discovery for enrolled hosts; Azure Resource Graph inventory; Defender for SQL assessments; Azure Migrate and MAP for migration assessment.

**What only you can provide:** discovery of instances Arc cannot reach — unenrolled, firewalled, cross-domain, vendor-installed, Express-edition instances on application servers; the utilization and ownership determination for every discovered instance; the retire/consolidate/retain decision that gates authentication migration.
:::
