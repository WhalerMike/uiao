...
A Category 3 instance that cannot be upgraded to SQL Server 2022 as dependencies are remediated will face increasing friction as Microsoft tightens NTLM defaults, regardless of exception status. The NTLM block, when applied, is network-level and does not distinguish between instances that are excepted from the transformation and instances that are not.
...
**Microsoft provides:** the Arc-eligibility prerequisites — Windows Server 2019+, SQL Server 2022, Connected Machine agent — and the capability envelope each category defines.

**What only you can provide:** the per-instance triage that maps Arc-eligibility category to action sequence; the critical clarification that SQL-engine Entra authentication (Layer 1) does not depend on OS-level domain unjoin (Layer 2) and must never be blocked waiting for it.
:::

::: {.callout-note title="What Book 03 establishes — Chapter 2"}
The three Arc-eligibility categories map directly to triage actions: a Category 1 instance migrates now from its login inventory, a Category 2 instance is prioritized for Arc deployment and then re-classifies to Category 1, and a Category 3 instance must be upgraded to SQL Server 2022 or formally excepted, with upgrade timing tracked ahead of Microsoft's progressive NTLM defaults. The decisive clarification is the Layer 1 versus Layer 2 distinction: SQL engine Entra authentication depends only on Arc, the Azure extension, and SQL Server 2022 — it can proceed on a still-domain-joined host, independent of the ADR-002 OS-level Entra login transformation.
:::
