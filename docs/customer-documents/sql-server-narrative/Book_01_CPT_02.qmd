...
The first driver is a decision this program has made and recorded: ADR-068 (Kerberos / NTLM Elimination), accepted 2026-05-13, commits the program to eliminating NTLM — driven by Microsoft's deprecation of the protocol (Windows Server 2025 / Windows 11) and the zero-trust posture this boundary requires. ADR-068 distinguishes NTLMv1 from NTLMv2 because they present different risk profiles and require different timelines, but both are eliminated.
...
NTLMv2 has a longer runway but a firm backstop: 2027-04-01. On that date, ADR-068 Phase C blocks NTLM domain-wide — the date the program set to land its transformed posture before the CCM BIR continuous-monitoring adoption deadline (FedRAMP Notice 0009, via ADR-043). Any SQL Server connection that relies on NTLMv2 — because an SPN is missing, because the client cannot negotiate Kerberos, because the connection crosses a trust boundary, or because the host has already been migrated to a non-domain-joined state — will fail after 2027-04-01.
