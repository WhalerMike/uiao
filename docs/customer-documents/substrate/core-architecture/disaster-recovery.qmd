...
![Backup-and-recovery topology (primary/secondary/GitHub mirror with RPO/RTO overl...](images/disaster-recovery-image-01-backup-and-recovery-topology-pri.png){#fig-disaster-recovery-image-01 fig-alt="Backup-and-recovery topology (primary/secondary/GitHub mirror with RPO/RTO overlays). Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for primary structural elements, teal (#1A9E8F) for secondary/integration elements, amber (#D4A017) for governance/audit/registry overlays, monospace labels for identifiers, no human figures, 16:9 landscape." width="85%"}

![recovery decision tree](images/disaster-recovery-image-02-recovery-decision-tree.png){#fig-disaster-recovery-image-02 fig-alt="recovery decision tree. Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for primary structural elements, teal (#1A9E8F) for secondary/integration elements, amber (#D4A017) for governance/audit/registry overlays, monospace labels for identifiers, no human figures, 16:9 landscape." width="85%"}
...
These objectives span two tiers of recovery. **High-availability failover** (the hot-standby posture in **ADR-090**) handles node, disk, power, and network failures automatically: a quorum witness gates promotion so exactly one node ever binds commits, the failed node is fenced, and synchronous replication yields near-zero RPO with a single-digit-minute RTO. **Disaster recovery** --- the runbooks in this playbook --- is the cold-restore floor for total-site loss, rebuilding from the backup chain and the off-premises mirror. The component values below are the disaster-recovery targets; where high availability is enabled, the everyday failure path is the faster ADR-090 failover, not a restore.
...
1. **Active-Passive Git Replica** --- Secondary server (git-dr.uiao.local) receives real-time repository mirrors via git push \--mirror. Triggered by post-receive hooks on every push to primary. Provides near-zero RPO for Git data. In an HA-enabled deployment (**ADR-090**) this secondary is a *hot* standby kept current by synchronous replication of both the relational database and the Git repositories with their LFS objects, with automatic, quorum-gated, fenced promotion --- never active-active.
...
## Compliance Amendment — Business Impact Analysis, DR Testing, and Stakeholder Communication (`CP-2`, `CP-4`)

### Business Impact Analysis (`CP-2`)

The contingency plan is informed by a Business Impact Analysis identifying mission-essential functions, their maximum tolerable downtime, and the recovery objectives (RPO/RTO) that follow. The BIA is reviewed at least annually and after significant architecture change, and it drives the prioritization of the recovery scenarios in this playbook.

### Annual DR test (`CP-4`)

The disaster-recovery plan is exercised at least annually via a documented test (tabletop or functional), with results, gaps, and corrective actions recorded. Corrective actions are tracked as POA&M items (`UIAO_189`).

### Stakeholder communication plan (`CP-2`)

A communication plan identifies the stakeholders to be notified during a contingency event, the notification triggers and channels, and the escalation path. It is aligned with the Incident Response Plan (`UIAO_186`) reporting timelines.
