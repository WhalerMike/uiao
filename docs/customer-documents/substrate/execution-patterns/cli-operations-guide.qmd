...
![Dual-remote repository architecture (DIAG-001 placeholder)](images/cli-operations-guide-image-01-dual-remote-repository-architect.png){#fig-cli-operations-guide-image-01 fig-alt="Dual-remote repository architecture (DIAG-001 placeholder). Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for primary structural elements, teal (#1A9E8F) for secondary/integration elements, amber (#D4A017) for governance/audit/registry overlays, monospace labels for identifiers, no human figures, 16:9 landscape." width="100%"}

![cross-platform integration topology (AD/Entra/Intune/Arc)](images/cli-operations-guide-image-02-cross-platform-integration-topol.png){#fig-cli-operations-guide-image-02 fig-alt="cross-platform integration topology (AD/Entra/Intune/Arc). Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for primary structural elements, teal (#1A9E8F) for secondary/integration elements, amber (#D4A017) for governance/audit/registry overlays, monospace labels for identifiers, no human figures, 16:9 landscape." width="100%"}
...
\$groupParams = \@{ DisplayName = \"UIAO-Tier0-GCC-Servers\" Description = \"Dynamic group for Tier 0 GCC-Moderate governance servers\" MailEnabled = \$false MailNickname = \"uiao-tier0-gcc\" SecurityEnabled = \$true GroupTypes = @(\"DynamicMembership\") MembershipRule = \'(device.extensionAttribute11 -eq \"Tier0\") and (device.extensionAttribute13 -eq \"Production\")\' MembershipRuleProcessingState = \"On\" } \$group = New-MgGroup -BodyParameter \$groupParams Write-Host \"Created group: \$(\$group.DisplayName) (ID: \$(\$group.Id))\"
...
Only OrgPath facets are stamped here. The site a device sits at is not among them: physical place is the LocPath axis, and per ADR-102 D5 LocPath claims no extensionAttribute slot, so there is no site value to write. Site scoping comes from the LocPath site groups and Administrative Units instead.

\$deviceId = (Get-MgDevice -Filter \"displayName eq \'UIAO-GIT01\'\").Id Update-MgDevice -DeviceId \$deviceId -BodyParameter \@{ extensionAttributes = \@{ extensionAttribute1 = \"EASTUS\" extensionAttribute2 = \"IT\" extensionAttribute3 = \"Governance\" extensionAttribute11 = \"Tier0\" extensionAttribute13 = \"Production\" extensionAttribute12 = \"GitServer\" } } Write-Host \"Extension attributes updated for UIAO-GIT01\"
