...
![Quarto pipeline end-to-end flow (Gitea push to IIS + GitHub Pages)](images/quarto-pipeline-image-01-quarto-pipeline-end-to-end-flow.png){#fig-quarto-pipeline-image-01 fig-alt="Quarto pipeline end-to-end flow (Gitea push to IIS + GitHub Pages). Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for primary structural elements, teal (#1A9E8F) for secondary/integration elements, amber (#D4A017) for governance/audit/registry overlays, monospace labels for identifiers, no human figures, 16:9 landscape." width="85%"}

![dual-publishing architecture](images/quarto-pipeline-image-02-dual-publishing-architecture.png){#fig-quarto-pipeline-image-02 fig-alt="dual-publishing architecture. Engineering blueprint style, neutral slate background, federal navy (#1F3A5F) for primary structural elements, teal (#1A9E8F) for secondary/integration elements, amber (#D4A017) for governance/audit/registry overlays, monospace labels for identifiers, no human figures, 16:9 landscape." width="85%"}
...
1. **Author pushes** --- A contributor commits changes to .qmd files and pushes to the Gitea repository.

2. **Post-receive hook fires** --- The Gitea post-receive hook detects changes in docs/ and triggers the rendering pipeline.

3. **Quarto CLI renders** --- The render-docs.ps1 script invokes quarto render against the project, producing static HTML in docs/\_site/.

4. **Validation** --- The validate-links.ps1 script scans rendered output for broken links, missing images, and cross-reference failures.

5. **Internal deploy** --- Rendered HTML is copied to D:\\UIAO\\Docs\\site\\, which IIS serves at docs.uiao.local.

6. **External deploy** --- On push to main on GitHub, the quarto-publish.yml GitHub Actions workflow renders and deploys to GitHub Pages.
...
2.3 Dual Publishing Architecture
  ...
  **Target**   **URL**                          **Trigger**                      **Authentication**                  **Hosting**
  ...
  External     whalermike.github.io/uiao   GitHub Actions on push to main   Public (GitHub Pages)               GitHub Pages
  -----------------------------------------------------------------------------------------------------------------------------------------------
...
4.2 Directory Purpose Reference
  ...
  **Directory / File**            **Purpose**                                                                                                                                        **Approx. Files**
  ...
  docs/customer-documents/reference-architecture/             Modernization playbooks --- identity migration plans, DNS restructuring, PKI modernization, certificate lifecycle, and hybrid identity.            16
...
4.3 File Distribution Across Directories
  ...
  **Directory**         **.qmd Count**   **Primary Content**
  ...
  docs/customer-documents/reference-architecture/   16               Identity, DNS, PKI modernization, hybrid identity, migration plans
...
## Compliance Amendment — Pipeline Integrity and Artifact Signing (`SA-10`, `SI-7`)

### Pipeline integrity verification (`SI-7`)

The documentation and build pipeline verifies the integrity of its inputs and outputs. Source content is integrity-checked against the governed repository, and generated artifacts carry recorded content hashes. Unexpected modification of a pipeline input or output surfaces as a `DRIFT-PROVENANCE` finding.

### Artifact signing (`SA-10`)

Released build artifacts are signed and accompanied by a signed manifest of SHA-256 hashes, establishing developer-configuration-management integrity (`SA-10`). Consumers verify signatures before trusting an artifact; signing keys are protected consistent with the Supply Chain Risk Management Plan (`UIAO_191`).
