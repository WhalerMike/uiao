...
finding_id: "FINDING-PGM-001"
status: Awaiting-External-Remediation
severity: P2
created_at: "2026-04-27"
updated_at: "2026-04-27"
owner: "Michael Stratton"
related_canon: ["UIAO_003", "UIAO_020", "UIAO_022", "UIAO_023", "UIAO_109", "UIAO_110", "UIAO_113", "UIAO_131", "UIAO_132", "UIAO_133", "UIAO_138", "UIAO_139"]
related_adrs: ["ADR-047", "ADR-061", "ADR-106"]
supersedes: []
superseded_by: []
---
...
documented in [FINDING-001](./fedramp-gcc-moderate-informed-network-routing.qmd)
...
1. **Native KSI emission language.** Phase 2 substrate output
   ([Phase 2 — Governance OS Deployment](../customer-documents/operational-guides/uiao-modernization-program/03-phase2-governance-os.qmd))
...
2. **Continuous ATO crosswalk.** Phase 3 cATO alignment
   ([Phase 3 — Optimization & Continuous ATO Alignment](../customer-documents/operational-guides/uiao-modernization-program/04-phase3-optimization-cato.qmd))
...
1. **KSI Emission Surface section in Phase 2
   ([UIAO_022 — Phase 2 Governance OS Deployment](../customer-documents/operational-guides/uiao-modernization-program/03-phase2-governance-os.qmd)).**
...
2. **20x cATO Crosswalk in Phase 3
   ([UIAO_023 — Phase 3 Optimization & cATO](../customer-documents/operational-guides/uiao-modernization-program/04-phase3-optimization-cato.qmd)).**
...
3. **Forward-looking note in Phase 0 strategic master plan
   ([UIAO_020 — Phase 0 Modernization Master Plan](../customer-documents/operational-guides/uiao-modernization-program/01-phase0-master-plan.qmd)).**
...
5. **Promote substrate-side decision and operational mechanics
   to canon.** Land [ADR-047 — FedRAMP 20x Integration](https://github.com/WhalerMike/uiao/blob/main/src/uiao/canon/adr/adr-106-fedramp-20x-integration.md)
...
6. **Findings registry.** Land
   ...
   FINDING-001 and FINDING-PGM-001 with their cross-references, per
...
7. **ADR-106 gate 3 dry-run — PASSED (2026-06-25).** `uiao fedramp
   dryrun` executed against a synthetic GCC-Moderate IR covering all
   46 CR26 KSI controls. All three criteria passed with zero P0/P1
   events:

   | Criterion | Result |
   |---|---|
   | 1. All 10 KSI themes present | PASS |
   | 2. Zero P0/P1 stale artifacts | PASS |
   | 3. cATO package (row 11) present | PASS |

   Run ID: `79de8c93-f8df-406d-b39f-d0616015c222` |
   Baseline: `gcc-moderate` |
   CR26 snapshot: `c31eb04c082d6d578a26a00de9a482707ab7a00c` |
   Theme coverage: CMT(6) CNA(3) CED(1) IAM(3) INR(1) MLA(9) PIY(1) RPL(1) SVC(3) SCR(1)

   This satisfies the final ratification gate for ADR-106. ADR-106
   may now be promoted from PROPOSED to ACCEPTED by the canon steward.
   See [ADR-106](../../src/uiao/canon/adr/adr-106-fedramp-20x-integration.md).
...
- **2026-04-27** — Finding lands under `docs/findings/` with
  ...
  offerings.
- **2026-06-25** — ADR-106 gate 3 dry-run executed and PASSED
  (run ID `79de8c93-f8df-406d-b39f-d0616015c222`). UIAO_138 and
  UIAO_139 landed in PR #1028; ADR-061 ratified ACCEPTED in PR
  #1029. ADR-106 final ratification gate cleared; promotion to
  ACCEPTED pending canon-steward signoff.
- **2026-06-25** — UIAO_205 (FedRAMP 20x Agency Registration and
  Sponsor Onboarding) authored, filling the agency-side gap deferred
  by UIAO_133. §7 of this finding designated as the registration
  record artifact store per UIAO_205 §4.

## 6. Related

- [FINDING-001 — FedRAMP GCC-Moderate Informed Network Routing
  unavailable](./fedramp-gcc-moderate-informed-network-routing.qmd)
...
- [ADR-047 — FedRAMP 20x Integration decision](https://github.com/WhalerMike/uiao/blob/main/src/uiao/canon/adr/adr-106-fedramp-20x-integration.md)
...
- [UIAO_205 — FedRAMP 20x Agency Registration and Sponsor Onboarding](https://github.com/WhalerMike/uiao/blob/main/src/uiao/canon/specs/fedramp-20x-agency-registration.md)
  — agency-side registration workflow spec; §4 gate sequence determines
  when the modernized pathway migration ceremony fires; §7 of this finding
  is the artifact store for that gate sequence.
- [docs/findings/registry.yaml](./registry.yaml) — cross-finding
  index enumerating FINDING-001 and FINDING-PGM-001.
- [Phase 0 — Modernization Master Plan](../customer-documents/operational-guides/uiao-modernization-program/01-phase0-master-plan.qmd)
...
This section is the artifact store for UIAO_205 §4 gate artifacts. It is
empty until the agency initiates FedRAMP 20x registration.

### 7.1 Substrate readiness (UIAO_205 §2.1 gate S-3)

Dry-run PASSED: run ID `79de8c93-f8df-406d-b39f-d0616015c222`, baseline
`gcc-moderate`, snapshot `c31eb04`, executed 2026-06-25. All three criteria
passed with zero P0/P1 events. See §4.7 for the full result table.

### 7.2 Open scope disputes (UIAO_205 §3.3 step 5 / UIAO_138 §4.3)

No open scope disputes as of 2026-06-25.

### 7.3 Migration ceremony record (UIAO_205 §4.1)

Not yet executed. This subsection will record the three sign-offs
(canon-steward, governance-steward, agency AO) and the date of
transition to the modernized pathway when the ceremony completes.
...
| 2026-06-25 | Awaiting-External-Remediation | ADR-106 gate 3 dry-run PASSED; UIAO_138/139 + ADR-061 landed; internal remedies complete; external remedy (RFC-0010, CSP 20x package) still open |
