...
- **OMB M-26-14 maturity Level 3 (Advanced)**: requires extended retention — six months actively searchable and one year retrievable. **At
  Audit Standard, the directive's spirit is not met for the
  long-tail.** Premium or 10-year add-on (or agency-side long-term
  store) is required.
- **CISA [BOD 25-01](https://cyber.dhs.gov/bod/25-01/)**: core requirement met for recent activity.
...
- **Compensating stack**: `methodology.md` §13.2 — long-term forensic
  store is item 7.
- **OMB M-26-14**: this is the primary mandate driver (it rescinded M-21-31 on 22 May 2026).
- **ADR-058** — `src/uiao/canon/adr/adr-058-microsoft-purview-conformance-adapter-coverage.md`
  reserves the `purview-audit` conformance adapter slot whose `scope`
  includes `unified-audit-retention-tier`. When that slot is
  activated, this finding's posture becomes a continuous drift signal
  in the ConMon program (UIAO_132 / ADR-047) rather than a one-shot
  static finding. The conformance adapter is **not the remedy** — it
  is the continuous-detection surface that turns the §4 remedy
  decision into a measurable posture per agency.
- **Tracking issue**: WhalerMike/uiao#322 (canon change request that
  scoped the slot).
...
| 2026-05-07 | Awaiting-Internal-Remediation | ADR-058 reserved `purview-audit` conformance slot; activation will convert this finding to a live drift signal. Remediation status unchanged (slot is read-only detection, not remedy). |
