---
title: "GCC Moderate — Solution Architecture"
subtitle: "How UIAO addresses the GCC Moderate compliance contradiction"
---
...
![The GCC-Moderate invisible boundary — what the portal shows versus what actually functions — and the four UIAO components that resolve each gap.](../images/canonical/UIAO-FIG-024-gcc-boundary-solution.png){#fig-gcc-boundary fig-alt="A red top panel titled invisible boundary compares what the M365 GCC-Moderate portal shows (teal checks for Endpoint Analytics, Device Health Attestation, Locations, ARC Server Monitoring, Expedited Updates) against what actually functions (red crosses — telemetry blocked, planning phase, explicitly unavailable). A teal arrow labelled UIAO resolves each gap leads to an ice panel holding four navy boxes — Boundary Probe, In-Boundary Telemetry, Gap Registry, OrgPath Device Targeting." width="100%"}
...
The AD survey adapter writes the governance-layer facets to their codebook
slots on computer objects, and the derived OrgPath to `extensionAttribute15`.
Entra Connect syncs these to Entra ID device objects. Dynamic device groups
are built from the same rules as user groups — and which rule shape you use
depends on the question being asked:

```
OrgTree-IT-CyberOps (devices) — node-exact: boolean facet composition
  Rule: (device.extensionAttribute2 -eq "IT") and
        (device.extensionAttribute3 -eq "CyberOps")

OrgTree-FIN-All (devices) — cross-cutting: every Finance device, any region
  Rule: (device.extensionAttribute2 -eq "Finance")

OrgTree-NCR-IT-Branch (devices) — subtree: this node and every descendant
  Rule: (device.extensionAttribute15 -startsWith "Region=NCR|Department=IT|")
```

The third form is the only one that uses the derived path, and it must be
anchored at `Region=` — the path is composed region-first, so a prefix
starting at `Department=` matches nothing. "All of Finance, everywhere" is a
cross-cutting question, not a subtree, so it is a facet predicate. The
trailing `|` is mandatory in any prefix (ADR-127).
