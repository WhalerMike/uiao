...
[DIAGRAM-01: Top-down three-tier resilience evaluation diagram. Top tier is a labeled cluster "Chaos Inputs" containing six chaos-injection boxes arranged in two rows of three: CI Chaos, Webhook Chaos, Workflow Chaos, API Chaos, DB Chaos, Dashboard Chaos. Middle tier contains five resilience-domain boxes: Drift Detection Resilience (fed by CI Chaos + Webhook Chaos); SLA Timer Resilience (fed by Workflow Chaos); Automation Resilience (fed by API Chaos); Schema Resilience (fed by DB Chaos); Dashboard Resilience (fed by Dashboard Chaos). All five resilience-domain boxes converge into a single terminal box at the bottom: "Resilience Score". Engineering blueprint style, neutral slate background, red (#C74040) for chaos inputs, federal navy (#1F3A5F) for resilience domains, amber (#D4A017) for the terminal score, monospace labels, 16:9 landscape.]
