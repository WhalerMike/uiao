...
|---|---|
| [D-01: Evidence Determinism](d-01-evidence-determinism.qmd) | No-silent-drops, write-once, ordered sequence, and idempotent write guarantees |
| [D-02: Evidence Lifecycle](d-02-evidence-lifecycle.qmd) | ACTIVE → ARCHIVED → SEALED phases and retention policies |
| [D-03: Evidence Signing](d-03-evidence-signing.qmd) | Two-layer signing model, key management, and severity-based signing requirements |
| [D-04: Evidence Correlation](d-04-evidence-correlation.qmd) | Correlation dimensions, evidence diffing, and the correlation API |

## Key Principles

**Write-once:** Evidence records are never modified or deleted. Corrections are appended.

**Origin authentication:** Every evidence record carries an HMAC-SHA256 integrity tag keyed to its originating adapter. To holders of the shared key, the tag proves the claim came from that adapter and was not altered. Because HMAC is a symmetric MAC, this is *not* third-party non-repudiation — an auditor must trust the key holder. Asymmetric signing (true non-repudiation) is on the roadmap.
