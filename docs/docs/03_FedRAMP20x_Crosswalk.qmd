...
For an agency in GCC-Moderate today: **very little until Microsoft
actually applies the rule to GCC-Moderate's blocked pipelines** — filing
a 20x-aligned package or opting into the Rev5 Balance Improvement
Releases for the GCC-Moderate offering. The rule itself is no longer
hypothetical: MAS launched 2026-06-24 as part of FedRAMP's Consolidated
Rules for 2026 and became required for 20x authorizations on 2026-07-04
(Rev5 optional adoption opened the same date, required to obtain from
2027-01-01) — but a live rule Microsoft *can* invoke is not the same as
Microsoft having invoked it for these specific signals.

What it changes strategically: the **negotiation surface**. Agency
requests for descoped telemetry now have a named, live mechanism to
point at — MAS-CSO-MDI for measurement-only metadata, and the Rev5
opt-in path as the realistic near-term route for currently-authorized
CSPs — not a future one.

### 12.2 Cross-references

- **All five MAS-CSO rules, eleven KSI families, and the Phase 2 Pilot
  framing**:
  [`04_FedRAMP20x_Phase2_Summary.qmd §12`](04_FedRAMP20x_Phase2_Summary.html)
  (this section covers only MAS-CSO-IIR and MAS-CSO-MDI, the two rules
  the signal-class analysis above turns on; §12 there also carries the
  confirmed launch/effective dates this section now cites).
- **Per-row gap matrix**:
  [`gcc-moderate-telemetry-gaps.yaml`](../../src/uiao/canon/data/gcc-moderate-telemetry-gaps.yaml).
- **Source memo**:
...
- **Boundary assessment (the canonical gap matrix anchor)**:
  [`B1-gcc-moderate-boundary-model.qmd`](../customer-documents/orgcomp-series/boundary/B1-gcc-moderate-boundary-model.html).
- **The scoped MAS petition this signal-class analysis supports (and
  what it doesn't)**:
  [`B1-3-rebuilding-boundary-blocked-analytics.qmd §12`](../customer-documents/orgcomp-series/boundary/B1-3-rebuilding-boundary-blocked-analytics.html).
- **ThousandEyes Networks-pillar adoption decision**:
  [`ADR-057`](../../src/uiao/canon/adr/adr-057-thousandeyes-networks-pillar-scope.html)
  (this section previously cited "ADR-047," a different, unrelated ADR —
  corrected).
...
| 1.2 | 2026-08-06 | Michael Stratton | §12.1: added confirmed MAS launch/effective dates (2026-06-24 launch, 2026-07-04 required for 20x and Rev5 optional-adoption open, 2027-01-01 Rev5 required-to-obtain); clarified the rule being live is not the same as Microsoft having applied it to GCC-Moderate's pipelines. §12.2: fixed a broken cross-reference (cited "ADR-047," an unrelated ADR, instead of ADR-057 ThousandEyes); added pointers to 04_FedRAMP20x_Phase2_Summary.qmd's full 5-rule MAS-CSO table and B.1.3 §12's scoped petition. |
