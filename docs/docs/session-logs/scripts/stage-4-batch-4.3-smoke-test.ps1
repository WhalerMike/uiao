...
# -----------------------------------------------------------------------------
# Prereqs:
#   - uiao-impl: v0.1.0 tag pushed to origin.
#   - uiao-core: Batch 4.2 commit pushed to origin.
# What this does:
#   - Creates a throwaway venv and installs uiao-impl from the v0.1.0 tag.
#   - Runs `uiao --help` to confirm the CLI entry point resolves.
#   - Runs a minimal canon-side check against uiao-core/tools/*.py to verify
...
$VenvDir  = Join-Path $env:TEMP 'uiao-impl-smoke-venv'
...
Write-Step 'Check 1 — remote tag v0.1.0 on uiao-impl'
...
Write-Step 'Check 2 — uiao-core main reaches the MIGRATE commit'
...
Write-Step 'Check 3 — install uiao-impl from v0.1.0 tag in fresh venv'
...
& $pip install "git+https://github.com/WhalerMike/uiao-impl.git@v0.1.0" 2>&1 | Tee-Object -Variable installOut | Out-Host
...
Write-Step 'Check 6 — uiao-core/tools/*.py import without uiao_impl installed'
...
# --- 7. Cross-repo smoke — uiao-impl CLI reading canon from uiao-core --------
