...
Emit a reciprocity record for a consuming agency. Calls `uiao.oscal.reciprocity_record.emit_reciprocity_record`, writes the signed JSON to `<out-dir>/<consuming-agency>/reciprocity-record.json`.
...
| `--legal-basis` | ENUM | Yes | Legal basis for reciprocity. One of: `interagency-mou`, `economy-act`, `fisma-shared-service`, `fedramp-authorization`, `other-statutory-authority`. |
...
```json
{
  ...
  "reciprocity_basis": "interagency-reciprocity",
  "effective_at": "2026-05-11T00:00:00+00:00",
  "expires_at": "2027-05-11T00:00:00+00:00",
  "configuration_latitude_ref": "ssp-2026-latitude-baseline-v1",
  "signature": {
    "algorithm": "HMAC-SHA256",
    "value": "<hex-digest>",
    "signed_at": "2026-05-11T00:00:00+00:00",
    "signer": "uiao-reciprocity-operator"
  },
  "provenance": { "...": "..." }
}
```
...
Walk `<records-dir>/**/reciprocity-record.json` and print a table of (controlling_ato_id, consuming_agency_code, effective_at, expires_at, status). Status is `Active` or `Expired` relative to current UTC time.

**Synopsis:**

```
uiao reciprocity list-records [OPTIONS]
```
...
| Option | Type | Required | Description |
|--------|------|----------|-------------|
| `--records-dir` | PATH | Yes | Root directory containing per-agency `reciprocity-record.json` files. |
| `--json` | FLAG | No | Emit records as a JSON array on stdout instead of a rich table. |
...
**Expected output shape (table):**

```
 controlling_ato_id   consuming_agency_code  effective_at        expires_at          status
 OPM-HRIT-2026-001    TREAS                  2026-05-11T00:00:00 2027-05-11T00:00:00 Active
 OPM-HRIT-2026-001    IRS                    2026-05-11T00:00:00 2027-05-11T00:00:00 Active
2 record(s) found
```
...
Verify the HMAC-SHA256 signature of a reciprocity record. Calls `uiao.oscal.reciprocity_record.verify_signature`.

**Synopsis:**

```
uiao reciprocity verify [OPTIONS]
```
...
| Option | Type | Required | Description |
|--------|------|----------|-------------|
| `--record` | PATH | Yes | Path to a `reciprocity-record.json` file to verify. |
| `--signing-key-env` | TEXT | No | Name of the env var that holds the HMAC signing key (default: `UIAO_RECIPROCITY_HMAC_KEY`). |

**Exit codes:** `0` signature valid, `1` signature invalid or record unreadable, `3` WS-A2 verifier not yet merged.
...
# Non-zero exit on invalid signature
uiao reciprocity verify --record /tmp/hrit-recip/TREAS/reciprocity-record.json
# → Signature INVALID.   (exit code 1)
```

---

### 3.6 Continuous Monitoring (ConMon)

Continuous monitoring is the operational heartbeat of FedRAMP compliance. These commands process live security telemetry (Sentinel alerts), manage Plans of Action & Milestones (POA&Ms), export ongoing authorization evidence, produce KSI (Key Security Indicator) dashboards, and validate ATO lifecycle SLA cadence. The ConMon pipeline transforms raw security events into governance-grade artifacts.
...
| conmon ato-cadence-check | Validate ATO lifecycle SLA cadence (SSP windows + reauthorization). | SLA enforcement; CI-friendly PASS/WARN/FAIL verdict. |
...
**conmon ato-cadence-check** — Validates the three ATO lifecycle SLAs defined in UIAO_140 §4 and ADR-054 Q&A #44: draft SSP within 30 days of award (`SSP-Draft-30`), final SSP within 45 days of award (`SSP-Final-45`), and reauthorization package due at least 30 days before ATO expiry (`Reauthorization-30`). Verdicts are `PASS`, `WARN`, `FAIL`, or `N/A`; the overall verdict is the worst-case severity. Emits a rich table by default; use `--json` for machine-readable output.
...
**Expected output shape (JSON with `--json`):**
...
```json
{
  ...
  "verdicts": [
    {"name": "SSP-Draft-30", "threshold_days": 30, "actual_days": 24, "verdict": "PASS", "message": "..."},
    {"name": "SSP-Final-45", "threshold_days": 45, "actual_days": 40, "verdict": "PASS", "message": "..."},
    {"name": "Reauthorization-30", "threshold_days": 30, "actual_days": null, "verdict": "N/A", "message": "..."}
  ]
}
```
...
### 3.9 IR (Intermediate Representation) Pipeline
...
### 3.10 CQL (Compliance Query Language) Engine
