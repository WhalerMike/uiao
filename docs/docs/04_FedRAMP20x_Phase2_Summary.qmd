...
MAS officially launched 2026-06-24 as part of FedRAMP's Consolidated
Rules for 2026 and became required for 20x authorizations on
2026-07-04 (required to maintain for 20x, and required to obtain for
Rev5, on 2027-01-01; optional adoption for Rev5 opened 2026-07-04). It
replaces the prior FedRAMP authorization-boundary construct with a
...
|---|---|---|
| MAS-CSO-IIR | Identify Information Resources | Two-prong inclusion test: in scope if "likely to handle federal customer data or likely to impact the confidentiality, integrity, or availability of federal customer data" |
| MAS-CSO-MDI | Metadata Inclusion | Metadata is in scope only if MAS-CSO-IIR applies to the underlying resource |
| MAS-CSO-FLO | Information Flows and Security Categories | Providers MUST document information flows and security categories for every information resource in the cloud service offering |
| MAS-CSO-TPR | Third-Party Information Resources | Applies only if MAS-CSO-IIR applies; providers MUST document usage/configuration, justification, mitigations, and compensating controls for each third-party resource |
| MAS-CSO-SUP | Supplemental Information | Providers MAY include materials about out-of-scope resources in a marked, separate package supplement; not FedRAMP Certified |
...
posture rollups — has a credible path to falling outside MAS-CSO-IIR's
own two-prong test, and therefore outside the cloud service offering it
defines. Telemetry that handles federal customer data routinely
(sign-in events, mailbox-access events, sensitivity-label content) does
not; MAS-CSO-MDI then keeps any metadata describing that in-scope
resource in scope alongside it.
...
- **MITRE Chains, gap matrix, and ZTMM ceiling**:
  [`docs/customer-documents/orgcomp-series/boundary/B1-gcc-moderate-boundary-model.qmd`](../customer-documents/orgcomp-series/boundary/B1-gcc-moderate-boundary-model.html).
...
- **Crosswalk by signal class**: see §X of
  [`03_FedRAMP20x_Crosswalk.qmd`](03_FedRAMP20x_Crosswalk.html).
- **Source memo**:
...
| 1.2 | 2026-08-06 | Michael Stratton | §12.1: corrected MAS-CSO-FLO/TPR/SUP names and tests (were placeholder/mislabeled); added confirmed launch/effective dates (2026-06-24 launch, 2026-07-04 required for 20x, 2027-01-01 required to maintain/Rev5-obtain); reworded the MAS-CSO-MDI "descoping" claim to ground it in MAS-CSO-IIR's own test. §12.3: dated the Rev5 Balance Improvement Releases row. |
