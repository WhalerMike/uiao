---
title: "UIAO Substrate Roadmap"
subtitle: "Platform substrate development and release roadmap"
aspirational: true
---
...
Work required:
- Capture ScubaGear v1.8.0 output against the M365 developer tenant (same tenant used for tier-1 above).
...
- `scripts/tools/spec_test_coverage_check.py` — the CI gate. Re-runs
  ...
  `docs/docs/governance/spec-test-coverage.qmd`. Fails the PR if any
  ...
  rewritten, etc.).
- `docs/docs/governance/spec-test-coverage.qmd` — the tracking artifact.
...
- `src/uiao/substrate/walker.py::_scan_tenants` — registry-hygiene
  scan on `src/uiao/canon/tenants` (proposed `.yaml`). Active tenants
  must declare a non-empty `credential_scope:` (P2 finding tagged
  `DRIFT-SCHEMA` if missing or empty). The file is optional —
  single-tenant deployments without a tenants.yaml produce zero
  findings.
...
- `src/uiao/canon/policies/README` (proposed `.md`) — policy
  authoring guide documenting the schema and the
  `epl:<verb>-<subject>` id convention.
...
- ~~Compliance landing pages (8 sub-categories under `customer-documents/compliance/`)~~ — resolved by the July 2026 compliance consolidation: the tree retired; the Federal Organization Compliance Series is the compliance offering and the engine's compliance docs live in its companion volume (old URLs redirect).
