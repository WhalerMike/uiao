...
routing the write through the `ldap` binding-profile transport to the provider of
record — it requires `--enable-writes`, an authenticated bind, and a `--provider-*`
target:
...
# Gated actuation (L3): routes accepted writes to the provider directory.
uiao directory serve --snapshot snapshot.json --enable-writes --apply --bind admin=$PW \
    --provider-host dc01.contoso.com --provider-bind-dn 'cn=svc,dc=contoso,dc=com' \
    --provider-password $PROV_PW --tls-cert agd.pem --tls-key agd.key
```
