...
**Status:** **DRAFT — all four open questions resolved; implementation PR unblocked**
**Date:** 2026-04-23
...
| **Q1.4** — Connect.gov credential handling | **UIAO never touches Connect.gov credentials — operator session is 100% outside the system.** Receipt flow is purely operator-paste-into-issue. *Revisitable* if portal automation ever becomes worth the fragility cost (per user "default for now"). | Recommended option. Matches `ssot-mutation: never` + `object-identity-only: true` canon invariants. Any future automation becomes its own enhancement, effectively revisiting Q1.1. |
...
4. ~~**Security posture for the operator's Connect.gov credential state:** is there any part of the submission workflow that needs to handle a Connect.gov session token or portal credential (e.g. if a future sub-task auto-logs-in to check upload history), or is the credential material 100% on the operator side and never touches UIAO infrastructure?~~ **RESOLVED — credentials stay operator-side; revisitable as a future enhancement if portal automation becomes worth it.**

**All four E1 open questions are resolved.** The implementation PR is unblocked and can start once you merge this design-draft PR.
