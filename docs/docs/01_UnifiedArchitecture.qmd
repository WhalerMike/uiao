...
The "unified architecture" described here is Tier B of the reconciled layer model declared by [ADR-030 §2](https://github.com/WhalerMike/uiao/blob/main/src/uiao/canon/adr/adr-030-pre-uiao-terminology-reconciliation.md) — the six-plane decomposition that lives **inside** the Control Plane of the Tier A four-layer prose model (Authority Plane / Control Plane / Overlay Fabric / Physical Underlay). Neither framing supersedes the other; they describe the same substrate at different granularity.
:::
