...
**Attestation** — An integrity-protected record (HMAC-SHA256 over a canonicalized content hash) produced by the Evidence Fabric confirming that a specific governance event occurred at a specific time. Attestations are immutable once written. HMAC is a symmetric MAC: it authenticates origin to holders of the shared key but does not provide third-party non-repudiation; asymmetric digital signatures are on the roadmap.
