---
title: "RFC-0026-E5 — Multi-Agency Design"
subtitle: "E5 multi-agency architectural design specification"
aspirational: true
---

# E5 Design Memo — Multi-Agency Artifact Distribution Layer

**Roadmap entry:** [`docs/docs/uiao-rfc-0026-roadmap.qmd`](uiao-rfc-0026-roadmap.qmd) § E5
...
| **Q2.5** — Canon placement | **`src/uiao/canon/agency-customer-registry`** (proposed; `.yaml` ext) alongside `adapter-registry.yaml`. Agency customers are first-class canon citizens. | User selected default (option a). |
...
[FedRAMP RFC-0026](https://www.fedramp.gov/rfcs/0026/) RV5-CA07-CCM obliges a CSP with multiple agency ATOs to demonstrate that ConMon artifacts were *made available* to every agency customer, regardless of whether those agencies actually consume them. The RFC forum thread ([FedRAMP community discussion #130](https://github.com/FedRAMP/community/discussions/130)) converged on *"facilitate and document"* as the correct framing — the CSP's obligation is to distribute, not to guarantee consumption.
...
1. `src/uiao/canon/agency-customer-registry` (proposed `.yaml`) exists and passes schema validation in CI
...
### `src/uiao/canon/agency-customer-registry` (proposed `.yaml`)
...
5. ~~**Canon placement.**~~ **RESOLVED — `src/uiao/canon/agency-customer-registry` (proposed `.yaml`).**
