...
### Option A — Online install (default)
...
### Option B — Offline / proxy-restricted install (`uiao.zip`)

If your environment blocks PyPI (for example proxy `407`), download and
extract `uiao.zip` from the download page, then run:
...
Mapped 191 KSIs to IR Controls + Policies
SCuBA Transform
  Total KSI results : 5
  PASS              : 2
  WARN              : 1
...
The available route surfaces (per [M5 public-surface audit](../reports/public-surface-audit-v0.5.0.qmd)):
