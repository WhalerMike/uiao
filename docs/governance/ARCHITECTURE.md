...
**Axis 2 — Doctrinal mission class (`mission-class`).** The canonical UIAO_003 Adapter Segmentation Overview class the adapter realizes against SSOT. The five canonical values are `identity`, `telemetry`, `policy`, `enforcement`, `integration` (all five NEW (Proposed) per `canon/UIAO_003_Adapter_Segmentation_Overview_v1.0.md` — the fifth, `integration`, was added per resolved ODA-15 on 2026-04-15). Two escape values are also defined:
- `unmapped` — no current doctrinal class cleanly covers the adapter's role. Requires `mission-class-notes` and MUST appear as a §13 open decision. (No seeded adapter currently uses this value; ODA-15 retired the last occurrences when the `integration` class was ratified.)
- `mixed` — adapter straddles two canonical roles. Requires `mission-class-notes`.

**Why two axes.** The operational class answers *what the adapter does at runtime* (auditable, unambiguous, safe to encode in workflows). The mission class answers *which governance role the adapter plays against SSOT* (auditable against UIAO_003 canon, but partially proposed). Separating them prevents the operational choice from accidentally ratifying the doctrinal one, and lets the doctrinal model evolve without forcing a registry migration — as demonstrated by the 2026-04-15 ratification of the `integration` class (ODA-15, Option a), which was absorbed into the schema and registries without changing the operational `class` axis.
...
- id: entra-id
  class: modernization
  mission-class: integration       # UIAO_003 §4.7 (ODA-15 resolved 2026-04-15)
  mission-class-notes: >-
    Modernization adapter — performs change-making actions against identity
    systems. Classified per UIAO_003 §4.7 Integration Adapter Class.
  status: active
...
|---|---|---|---|
| 1 | ~~`uiao-gos` role~~ **RESOLVED** (superseded 2026-04-17) — See ADR-028: firewall retired, directory-migration subsystem integrated under `src/uiao/directory_migration/`; IPAM adapters registered in `core/canon/modernization-registry.yaml`. | — | — |
...
| 15 | ~~**Doctrinal mission-class gap for modernization adapters (ODA-15).**~~ **RESOLVED 2026-04-15 (Option a — new class `integration`).** Added fifth canonical doctrinal mission-class `integration` to UIAO_003 §4.7 covering change-making actions against a target environment (create/update/delete in the authoritative system). Schema enum extended; all five seeded modernization adapters (`entra-id`, `m365`, `service-now`, `palo-alto`, `scuba`) migrated from `mission-class: unmapped` → `mission-class: integration`. Rationale: keeps the doctrinal axis clean and orthogonal to the operational axis; explicitly distinguishes modernization adapters (change-makers) from enforcement adapters (assurance artifact producers); avoids re-interpreting `enforcement`; avoids the permanent `unmapped` hack; future-proofs the registry for additional change-making adapters (Terraform/OpenTofu, CyberArk credential rotation, etc.). | Michael | Resolved 2026-04-15 |
...
2. ~~Commit companion `uiao-gos/ARCHITECTURE.md`~~ — superseded by ADR-028: `uiao-gos` dissolved into `src/uiao/directory_migration/`; no separate architecture document needed.
