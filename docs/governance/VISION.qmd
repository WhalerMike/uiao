...
> **Canonical source & scope.** This file is the authoritative mission statement
> for the **federal vertical** of `uiao` — the most mature adapter pack. The
> product-level positioning of the *core engine* is vertical-agnostic and is owned
> by [ADR-085](https://github.com/WhalerMike/uiao/blob/main/src/uiao/canon/adr/adr-085-universal-enterprise-positioning.md);
> per ADR-085 §D5 this document is the federal-vertical articulation of that core,
> not a redefinition of it. It is referenced by the README, the uiao-docs canon,
> and the UIAO Codex.
...
UIAO (Unified Identity-Addressing-Overlay Architecture) is a **vertical-agnostic
governance substrate**; this Vision is written from its most mature vertical —
**federal compliance** (see [ADR-085](https://github.com/WhalerMike/uiao/blob/main/src/uiao/canon/adr/adr-085-universal-enterprise-positioning.md)).
Applied to civilian-agency modernization, it is purpose-built to eliminate the
manual, error-prone, and perpetually out-of-date compliance machinery that
governs agency infrastructure today.
...
continuously generates every required artifact, evaluates every required control,
and contains drift — automatically and repeatably, on a fast (target sub-two-minute)
pipeline run.
...
### 2. CISA SCuBA / [BOD 25-01](https://cyber.dhs.gov/bod/25-01/) Two-Way Governance Envelope
...
### 4. Evidence Fabric
Every compliance claim is backed by an evidence bundle: raw collector output,
normalized overlay, KSI evaluation result, and a commit-hash reference, linked
into OSCAL back-matter. The substrate flags any adapter that declares
`certificate-anchored` evidence without a declared trust anchor. Auditors and
authorizing officials get a verifiable, timestamped record — not a screenshot in
a folder. *(Full issuer-chain / signature verification of the provenance chain is
on the roadmap; today the substrate enforces trust-anchor declaration, not
cryptographic chain validation.)*

### 5. Drift Detection & Containment
...
is detected, classified, and raised as a structured finding (the design target is
a sub-two-minute pipeline run; this is an engineering goal, not a gated SLA).
