...
ADAPTERS = {
    "bluecat-address-manager": BlueCatAdapter(
        {"bam_host": "bam.test", "configuration": "test", "auth_method": "api-token"}
    ),
...
class TestDomain23QueryNormalization:
    ...
    def test_execution_plan_hash_deterministic(self, adapter_id, adapter):
        assert (
            adapter.execute_query({"from": "resources", "filter": {}}).execution_plan_hash
            == adapter.execute_query({"from": "resources", "filter": {}}).execution_plan_hash
        )
...
class TestDomain24DataNormalization:
    ...
    def test_claim_id_has_adapter_prefix(self, adapter_id, adapter):
        ...
        if result.claims:
            ...
            short = adapter.ADAPTER_ID.split("-")[0]
            assert cid.startswith(adapter.ADAPTER_ID) or cid.startswith(short)
