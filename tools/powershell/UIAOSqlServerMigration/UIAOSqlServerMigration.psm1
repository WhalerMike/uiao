...
function New-UIAOEntraLoginMapping {
    ...
    $auditRows = Get-UIAORow -Data $auditData -PreferredArrayKeys @('records', 'Logins', 'logins', 'WindowsLogins')
    ...
    if (-not [string]::IsNullOrWhiteSpace($ReconciliationPath)) {
        ...
        foreach ($r in (Get-UIAORow -Data $reconEnv.data.records)) {
...
function New-UIAOEntraLoginScript {
    ...
    $rows = Get-UIAORow -Data $env.data.records
...
function Test-UIAOLoginParallelRun {
    ...
    $rows = Get-UIAORow -Data $data -PreferredArrayKeys @('records', 'observations')
...
function New-UIAOSpnRemediationPlan {
    ...
    $collisions = Get-UIAORow -Data $collisionSource -PreferredArrayKeys @('Collisions', 'records')
...
function Get-UIAONtlmGpoReport {
    ...
    if (-not [string]::IsNullOrWhiteSpace($GpoReportPath)) {
        ...
        $data = Read-UIAOSourceData -Path $full
        $gpoRows = Get-UIAORow -Data $data -PreferredArrayKeys @('records', 'gpos', 'GPOs')
        $sourceRef = "snapshot:$(Split-Path -Leaf $full)"
    }
    elseif ($Live) {
...
function New-UIAOEstateInventory {
    ...
    $auditData = Read-UIAOSourceData -Path $auditFull
    foreach ($r in (Get-UIAORow -Data $auditData -PreferredArrayKeys @('InstanceAudits', 'records', 'Instances'))) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($SpnInventoryPath)) {
        ...
        foreach ($r in (Get-UIAORow -Data $spnData -PreferredArrayKeys @('SPNs', 'records', 'Orphans'))) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($FindInstancePath)) {
        ...
        foreach ($r in (Get-UIAORow -Data $fiData -PreferredArrayKeys @('records', 'Instances'))) {
...
function New-UIAOEntraGroupClassification {
    ...
    $rows = if (($data.PSObject.Properties.Name -contains 'data') -and $data.data -and ($data.data.PSObject.Properties.Name -contains 'records')) {
        Get-UIAORow -Data $data.data.records
    }
    else {
        Get-UIAORow -Data $data -PreferredArrayKeys @('records', 'groups', 'Groups')
    }
...
function New-UIAOAppConnectionPlan {
    ...
    $data = Read-UIAOSourceData -Path $full
    $rows = Get-UIAORow -Data $data -PreferredArrayKeys @('LDAPBindAccounts', 'ldapAccounts', 'records', 'accountGroups')
    ...
    if (-not [string]::IsNullOrWhiteSpace($AuthAuditPath)) {
        ...
        foreach ($inst in (Get-UIAORow -Data $auditData -PreferredArrayKeys @('InstanceAudits', 'records', 'Instances'))) {
...
function Get-UIAOInstanceLoginName {
...
function New-UIAOClosureRecord {
    ...
    $data = Read-UIAOSourceData -Path $full
    $rows = Get-UIAORow -Data $data -PreferredArrayKeys @('InstanceAudits', 'records', 'Instances')
    ...
    if (-not [string]::IsNullOrWhiteSpace($ExceptionRegistryPath)) {
        ...
        foreach ($e in (Get-UIAORow -Data $excData -PreferredArrayKeys @('records', 'exceptions', 'instances'))) {
    ...
    $records = foreach ($inst in $rows) {
        ...
        $sqlLogins = Get-UIAOInstanceLoginName -Instance $inst -ArrayNames @('SQLLogins', 'sql_logins')
        $winLogins = Get-UIAOInstanceLoginName -Instance $inst -ArrayNames @('WindowsLogins', 'windows_logins')
...
function Compare-UIAOClosureRun {
    ...
    function Read-Run {
        ...
        $rows = Get-UIAORow -Data (Read-UIAOSourceData -Path $full) -PreferredArrayKeys @('InstanceAudits', 'records', 'Instances')
        $idx = @{}
        foreach ($inst in $rows) {
            ...
            $idx[$key] = [ordered]@{
                ...
                sql_logins   = Get-UIAOInstanceLoginName -Instance $inst -ArrayNames @('SQLLogins', 'sql_logins')
                win_logins   = Get-UIAOInstanceLoginName -Instance $inst -ArrayNames @('WindowsLogins', 'windows_logins')
...
function New-UIAOSqlCertificatePlan {
    ...
    if ($certs.Count -eq 0) { $certs = @(Get-UIAORow -Data $audit -PreferredArrayKeys @('Certificates', 'certificates', 'records')) }
    ...
    $roots = Get-CaName (Get-UIAOArrayProp -Row $audit -Names @('EnterpriseRootCAs', 'enterprise_root_cas'))
    $subs = Get-CaName (Get-UIAOArrayProp -Row $audit -Names @('EnterpriseSubCAs', 'enterprise_sub_cas'))
    if ($roots.Count -eq 0 -and $subs.Count -eq 0) {
        ...
        $roots = Get-CaName (Get-UIAOArrayProp -Row $audit -Names @('EnterpriseCAs', 'enterprise_cas'))
    }
    $sequence = @()
    $order = 1
