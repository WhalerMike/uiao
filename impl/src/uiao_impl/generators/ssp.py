...
Migrated from scripts/generate_ssp.py into the uiao_impl package.
...
from uiao_impl.utils.context import get_settings, load_context
...
def _build_back_matter(template: dict[str, Any]) -> dict[str, Any]:
    ...
    resources = []
    laws = template.get("laws_and_regulations", [])
...
def build_ssp_skeleton(
    ...
    if enhanced:
        from uiao_impl.generators.narrative_loader import load_control_library
