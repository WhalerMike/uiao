"""uiao_impl.cli.evidence — Typer sub-app for the `uiao evidence` command group.

Mount point
-----------
    # in uiao_impl/cli/app.py
    from uiao_impl.cli.evidence import evidence_app
...
Or via module invocation:
    python -m uiao_impl.cli evidence build \\
        --input  ./output/ksi/tenant-a.ksi.json \\
        --output ./output/evidence/tenant-a/
"""
...
def build_command(
    ...
    `uiao_impl.evidence.builder.build_evidence`, which is kept
