...
>
> **Out of scope (owned by the Documentation & Training lane):** anything under `docs/`, the Quarto site, narrative markdown (README / CLAUDE.md / ARCHITECTURE.md / VISION.md / CONMON.md / PROJECT-CONTEXT.md), tutorials, onboarding guides, CHANGELOG authoring.
>
> **Origin:** seeded from three Explore-agent gap surveys (`core/`, `impl/`, repo-root) on 2026-04-17, branch `claude/assess-repo-gaps-EO8Yq`.
...
- [x] **ADP-001** [P0] Consolidate duplicate adapter code  ✓ 2026-04-20 — @agent-ci — PR #142
      Paths: impl/adapters/cloud/*, impl/src/uiao_impl/adapters/
...
- [ ] **ADP-001** [P0] Consolidate duplicate adapter code
      Paths: `impl/adapters/cloud/{m365,entra_id,intune}.py`, `impl/adapters/database/sql_adapter.py`, `impl/adapters/government/__init__.py`, `impl/src/uiao_impl/adapters/`
...
- [ ] **ADP-002** [P0] Build adapter registry + dynamic loader (`@register()` decorator)
      Paths: `impl/src/uiao_impl/adapters/__init__.py`, `impl/src/uiao_impl/adapters/registry.py`
...
- [ ] **ADP-003** [P0] Implement the conformance-check module referenced by `adapter-conformance.yml`
      Path: `impl/src/uiao_impl/adapters/conformance_check.py`
...
- [ ] **CNT-001** [P0] Promote `BaseAdapter` contract into `core/` (currently only lives in `impl/`)
      ...
      Done-when: `impl/src/uiao_impl/adapters/base_adapter.py` imports / references the canon contract.
...
- [ ] **TST-003** [P1] Export conformance test suite from `core/` so impl adapters can import it
      Pattern: `from uiao_core.tests.conformance import BaseAdapterTest`
...
- `core/PROJECT-CONTEXT.md`, `core/CLAUDE.md`, `core/ARCHITECTURE.md` — canon intent (read-only reference; do **not** edit, that's the docs lane).
- `impl/PROJECT-CONTEXT.md`, `impl/CLAUDE.md` — impl intent.
...
- `impl/src/uiao_impl/adapters/base_adapter.py` — current BaseAdapter (to be promoted per **CNT-001**).
