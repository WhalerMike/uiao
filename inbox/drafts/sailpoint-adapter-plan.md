...
> **Date:** 2026-05-07
> **Status:** draft (partially landed — Option A landed in ADR-059 + `sailpoint-nerm` slot; remaining 4 ISC/Machine-Identity slots stay in this draft as future work pending Option-B decision)
> **Target ADR:** `src/uiao/canon/adr/adr-059-sailpoint-adapter-family.md` (new; ADR-058 was concurrently allocated to `microsoft-purview-conformance-adapter-coverage` by PR #323 while this work was in flight, so SailPoint NERM moved to ADR-059)
...
> - `src/uiao/canon/adr/index.md` — list ADR-059
...
# ADR-059: SailPoint Adapter Family — Boundary-Exception Carve-Out and Slot Allocation
...
5. Update `src/uiao/canon/adr/index.md` to list ADR-059 under
...
```yaml
  # ──────────────────────────────────────────────────────────────────
  # Reserved adapter slots allocated by ADR-059 (2026-05-07)
  ...
  - id: sailpoint-nerm
    ...
    notes: >-
      ...
      Commercial-exception carve-out per ADR-059 §Decision 1; the
      `gcc-boundary: fedramp-moderate-aws-govcloud` value is a new
      enum value that ADR-059 introduces and that
  ...
  - id: sailpoint-isc-governance
    ...
    notes: >-
      ...
      ADR-059 §Notes. Read-only governance-plane observer for ISC:
      ...
      ADR-059 §Decision 5, UIAO holds SSOT for Entra writes; SailPoint
  ...
  - id: sailpoint-machine-identity
    ...
    notes: >-
      Reserved pending Option-B boundary-expansion decision per
      ADR-059 §Notes. Cross-source machine / service / bot / RPA /
...
```yaml
  ...
  - id: sailpoint-isc-actions
    ...
    notes: >-
      Reserved pending Option-B boundary-expansion decision per
      ADR-059 §Notes. SSOT-conflict resolution per ADR-059 §Decision
  ...
  - id: sailpoint-machine-identity-actions
    ...
    notes: >-
      Reserved pending Option-B boundary-expansion decision per
      ADR-059 §Notes. Operational counterpart to UIAO_139's
...
|---|---|---|
| 1 | Land ADR-059 (`src/uiao/canon/adr/adr-059-sailpoint-adapter-family.md`) + update `adr/index.md` | Governance Board (boundary-exception decision) |
...
5. **Ordering vs. ADR-057 (ThousandEyes)** — ADR-057 was just merged; this is ADR-059. No content conflict, but flagging for awareness.
