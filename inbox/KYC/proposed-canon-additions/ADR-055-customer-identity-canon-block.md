...
# ADR-055 (DRAFT): Customer Identity Canon Block — KYC Protocol & Reciprocal Attribute Exchange

> **Draft status.** Held in `inbox/KYC/proposed-canon-additions/`. Promote to
> `src/uiao/canon/adr/` and allocate the real ADR-NNN identifier at canon-merge
> governance review per repo invariant I5.
...
4. **Authority-of-record adapter slots.** Reserved adapter entries in
   ...
   | `loginGov-federation-service` | modernization | integration | GSA Login.gov — FAL-2 federated authentication for citizen portals |
   | `idMe-federation-service` | modernization | integration | ID.me — IAL-2/AAL-2 verification (commercial; federal-authorized) |
...
- The workforce-identity canon block (UIAO_129 / UIAO_130 / ADR-051..054 /
  ...
  (`loginGov-federation-service`, `ssa-attribute-service`) participate in
