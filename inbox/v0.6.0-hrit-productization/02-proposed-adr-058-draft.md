...
> **Status:** Draft for maintainer review. Will be promoted to
> `src/uiao/canon/adr/adr-058-hrit-productization-mission.md` (Accepted)
> in Phase 0 if ratified.
...
The ratification is anchored to OPM Solicitation 24322626R0007 (single
OPM ATO covers all ~24 CFO-Act agencies). Spec2-D6.1 enumerates the
twelve federal HRIT systems that constitute the integration surface.
...
candidates were surveyed in the v0.5.0+1 reconciliation report
...
1. **Allocate UIAO_143** as the *HRIT Productization Operational Spec*
   under `src/uiao/canon/specs/hrit-productization.md`. The spec is the
   operational counterpart to UIAO_140 (which is the model) and Spec2-D6.1
   (which is the federal-integration runbook).

2. **Close every deferred item from ADR-054 §Implementation** as Batch A
   workstreams. Ten file-scoped, parallelizable workstreams enumerated
   in `inbox/v0.6.0-hrit-productization/03-batch-plan.md`.
...
3. **Acceptance for v0.6.0 release** (binding):
   ...
   - 8 blocking CI gates remain green (ruff, mypy strict, pytest, schema
     validation, substrate drift, metadata validator, adapter conformance,
     quarto)
...
5. **Out of scope for v0.6.0** (deliberate):
   ...
   - Microsoft Tier-1 adapter completion (#299)
   - Transport plane implementation (ADR-057)
...
- New ADR-058 (this document, when promoted from inbox to canon)
- New UIAO_143 spec (HRIT Productization Operational Spec)
- UIAO_113 v1.2 amendment (add `ato-decision` + `reciprocity-record` node types)
- New schema: `reciprocity-record.schema.json`
- KSI rules: KSI-RECIP-* family

### Runtime work in scope (Batch A — see `03-batch-plan.md`)
...
detector, fixture, quickstart, tests, KSI rules, narrative doc.
...
PIV cert chains observed at TLS endpoints; APIM logs ingested as
telemetry). No new adapter code in v0.6.0 for these — Phase 1 of each
ADR's own activation work is independent.
...
- The transferable-ATO framing used informally in earlier
  documentation/conversation now points to ADR-054 + UIAO_140 + (this
  ADR's) productization spec UIAO_143.
...
- **Microsoft Tier-1 adapter completion** — narrower; better as a parallel
  batch within HRIT productization (e.g., ServiceNow as a downstream
  consumer of reciprocity records).
