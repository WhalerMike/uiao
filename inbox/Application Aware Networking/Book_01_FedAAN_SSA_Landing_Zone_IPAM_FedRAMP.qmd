...
::: {.callout-note}
**Cross-Book Link — NAC Endpoints and DNS/Certificate Binding (Books 02 and 03):**
...
1. **Intune/SCEP enrollment** (covered in [Book 02](Book_02_FedAAN_Network_Access_Control_802_1X.qmd)) issues the machine certificate with `CN={{DeviceName}}` and SAN bound to the device's Entra ID FQDN.
...
This linkage — that every NAC-admitted device carries both a DNS hostname and a machine certificate, and that InfoBlox holds the authoritative binding record — is the shared foundation of [Book 02](Book_02_FedAAN_Network_Access_Control_802_1X.qmd) (NAC/802.1X) and [Book 03](Book_03_FedAAN_Certificates_Tokens_Cryptographic_Identity.qmd) (Cryptographic Identity). The design decision for this binding must be validated with OIS.
:::
...
| **R-12** | **SIEM integration** — must export asset inventory and change events to the SIEM (Book 13) for enrichment of security alerts with asset context | SI-4, CA-7 | Must |
| **R-13** | **ConMon evidence generation** — must produce CM-8-compliant inventory exports (OSCAL format preferred) for the ConMon pipeline (Book 19) | CA-7 | Must |
...
- Migrate Windows Server DHCP scopes to InfoBlox Grid DHCP; enable DHCP fingerprinting (the device-identity feed that [Book 02](Book_02_FedAAN_Network_Access_Control_802_1X.qmd) builds the NAC device inventory on)
