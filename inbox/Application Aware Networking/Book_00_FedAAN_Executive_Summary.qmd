...
**What this is:** A comprehensive technical roadmap — mapping twenty implementation areas to specific control families, identifying closure mechanisms, and sequencing the work against the regulatory timeline. It is an attempt at a true enterprise-wide view, not a CSI-scoped one.
...
This series comprises nineteen companion implementation guides for the Federal Application-Aware Networking program, organized in four bands:

**Books 01–04 — Substrate** (naming, identity, and trust foundations):

- **Book 01** — DDI/IPAM naming-and-addressing plane — 16 NIST SP 800-53 Rev 5 controls
- **Book 02** — Network Access Control (802.1X EAP-TLS, RADIUS, device identity at the port)
- **Book 03** — Certificates & Tokens — the cryptographic identity layer every session-less architecture depends on
- **Book 04** — OPM HRIT Identity & Org SSOT — the HR system of record whose worker records underpin every token, account, and access decision

**Books 05–09 — Modernization Tracks** — closing **34 distinct NIST SP 800-53 Rev 5 controls** across the original six tracks:

- **Book 05** — Network and Identity Modernization (Entra ID, ZTNA, Active Governance)
- **Book 06** — Federal Telecommunications Modernization (SD-WAN, SASE)
- **Books 07–08** — SQL Server Authentication Modernization and Implementation Guide
- **Book 09** — Database Consolidation and Network Physics

**Books 10–14 — Security Operations** — approximately **55 additional controls** across the AC, RA, MP, SC, AU, SI, IR, and CP families:

- **Book 10** — Privileged Access Management
- **Book 11** — Vulnerability Management
- **Book 12** — Data Protection and Purview
- **Book 13** — SIEM/XDR Detection Engineering
- **Book 14** — Business Continuity

**Books 15–17 — Risk and Program Governance** (SR, PM, and PT control families):

- **Book 15** — Supply Chain Risk Management (SBOM/VDR/VEX pipeline)
- **Book 16** — Program Management and Governance (POA&M/SCRM/ISPP)
- **Book 17** — PII Processing and Transparency (PT family templates, Purview enforcement)

**Book 18 — Training and Awareness** — AT family cybersecurity training program — activates KSI-022 (KSI-CED-RAT), making all 29 KSI rules in the series' ADR-111 rule set evaluable at high confidence.

**Book 19 — Authorization Package and ConMon** — OSCAL AP+AR+POA&M bundle (29/29 rules self-assessed, pending independent security control assessment (SCA)), continuous monitoring program, and BOD 26-04 VDR/VER submission — closing CA-2, CA-5, CA-6, CA-7, RA-3, and SA-15.

---

**Together, the nineteen books constitute the complete evidence-generation and authorization-package layer for the agency's RMF authorization, with continuous-monitoring evidence structured in the FedRAMP 20x KSI idiom.** Each book maps its implementations directly to the NIST SP 800-53 Rev 5 controls it closes, and Book 19 assembles those mappings into a machine-readable OSCAL authorization package ready for independent SCA. The FedRAMP cloud services SSA consumes (M365 GCC, AWS GovCloud, DDI SaaS) are inherited as leveraged authorizations; SSA itself does not submit to the FedRAMP PMO.
...
**The nineteen implementations described in this series are not optional modernization projects.** They are the structural prerequisites for an agency Moderate authorization whose evidence survives the FedRAMP 20x transition. CR26 is mandatory for the cloud service offerings SSA inherits effective January 1, 2027; SSA's obligation is to ensure inherited services are on the 20x path and to structure its own ConMon evidence on the same KSI schema so inheritance stays clean.
:::
...
| Part 4 | Federal AAN — OPM HRIT and the Identity & Organizational SSOT | HR system of record · joiner/mover/leaver lifecycle · org hierarchy as queryable truth · HR-driven provisioning | Oracle HCM Cloud + Entra inbound provisioning |
...
| Part 19 | Federal AAN — Authorization Package and ConMon | Agency RMF authorization package (ConMon evidence modeled on the FedRAMP 20x KSI schema) · OSCAL AP+AR+POA&M bundle · BOD 26-04 VDR/VER submission · CA-7 ConMon program | CA-2/5/6/7 · RA-3 · SA-15 |
...
|------|---------|--------------------|
| **Aug 7, 2026** | BOD 26-04 agency policies due | Vulnerability-management policy must support risk-based remediation (Part 11) |
| **Sep 21, 2026** | FIPS 140-2 certificates → NIST Historical List | No new procurement reliance on 140-2-only modules; verify FIPS 140-3 tracking (SC-13 — Parts 3, 12) |
| **Dec 7, 2026** | BOD 26-04 remediation timelines in force | Four-variable risk model live: 3-day KEV+exposed tier through deferral (Part 11) |
...
| ~Aug 2027 | Next NIST SP 800-53 scheduled release | 2-year cadence from Release 5.2.0 (Aug 27, 2025, which added SA-15(13), SA-24, SI-2(7) — not yet mapped in Parts 11/15; tracked in the roadmap) |
| 2030 / 2035 | Quantum-vulnerable crypto deprecated / prohibited | M-23-02 annual crypto inventories run through the 2035 migration horizon (Parts 3, 12) |
...
![The Six Functional Planes — functions, not organizations](figs/es-fig-05-functional-planes.png){fig-alt="Stacked diagram of the six functional planes, each with role, function description, and owning Parts. 1 Transport (enforcement, navy): move packets between sites and clouds — MPLS to DIA, SD-WAN overlay, path selection, QoS; Parts 6 and 9. 2 Naming and addressing (truth, teal): assert what things are called and where they live — DNS, DHCP, IPAM, DNSSEC, RPZ, certificate issuance; Parts 1 and 3. 3 Identity (truth, teal): assert who and what is requesting — NAC 802.1X, Entra ID, workload identity, OAuth OIDC, CBA; Parts 2-4 and 6. 4 Policy enforcement (enforcement, navy): decide and enforce per session — SASE (SWG CASB FWaaS), ZTNA, TIC 3.0 PEPs, Conditional Access; Parts 5 and 6. 5 Application and experience (consumes both, ice blue): deliver the mission service — contact center, unified communications, business apps, databases; Parts 6-9. 6 Evidence and telemetry (truth, teal): prove continuously what happened — SIEM XDR, per-session records, OSCAL bundle, KSI evidence pipeline; Parts 13 and 19. Footer axiom: enforcement planes can be re-platformed; truth planes can only be depended on. White background. Source: Book 00 Executive Summary briefing deck, Date Code 2026-07-07 09:58 ET." width="100%"}
...
![Architecture Before Assessment — six phases; assessment tools arrive at Phase 5](figs/es-fig-07-architecture-before-assessment.png){fig-alt="Chevron flow of six phases: 1 Understand — gaps and root causes (this series, Books 00-17); 2 Design — closure patterns (domain books 01-14); 3 Implement — runbooks and deploys (Part 8 SQL, domain books); 4 Govern — SCRM, POA&M, ISPP (Books 15-17); 5 Validate — confirmation only (SCuBA, ZTTT, Secure Score, teal); 6 Authorize (highlighted, blue) — KSI-ready evidence via the OSCAL AP AR POA&M bundle. Caption: authorize on evidence the architecture generates — assessment tools arrive at Phase 5 as the final proof, not the starting point. White background. Source: Book 00 Executive Summary briefing deck, Date Code 2026-07-07 09:58 ET." width="100%"}
...
![The Coverage Arithmetic — 10 of 29 KSI rules attestable by conformance tooling alone](figs/es-fig-08-coverage-arithmetic.png){fig-alt="Diagram of the 29 KSI rules in the ADR-111 rule set by what closes them, all verdicts self-assessed pending independent SCA. Left stat: 10 of 29 KSI rules attestable by conformance tooling alone — M365 GCC tenant configuration, SCuBAGear plus ScubaDrift dispositions — shown as a teal row of rule chips 001-010. Right stat: 19 of 29 evaluable only because the architecture exists, bound to evidence slots built by Parts 1-19 — shown as a grey row of rule chips 011-029 with grouping brackets: SCR supply chain (SBOM VDR VEX, slot-06, Parts 15-16), PIY privacy (data-flow maps, slot-05, Part 17), CED-RAT training (slot-08, Part 18), and CED INR RPL incident and continuity (playbooks plus DR, slots 04 and 07, Parts 13-14). Highlighted bottom line: a conformance tool measures drift from an intended state — it cannot author one; run against an environment with no architecture, it measures drift from nothing. White background. Source: Book 00 Executive Summary briefing deck, Date Code 2026-07-07 09:58 ET." width="100%"}
...
Book 19 documents the evidence procedures; this matrix is the citable summary.
All verdicts are self-assessed pending independent security control assessment
(SCA).

> **KSI-count reconciliation (open item).** The 29 rules in this matrix are the
...
> method). Before any coverage claim is carried into an authorization decision,
> the 29-rule set must be diffed rule-ID-by-rule-ID against the finalized CR26
> Moderate list (June 25, 2026) and the mapping stated explicitly. Until that
> reconciliation is documented, "29/29 satisfied" is a claim about the internal
> rule set — not about the full CR26 indicator set.
...
- **NAC → ZTNA continuum** — posture asserted at the port evolves into posture asserted in the token (Part 3)
...
- **PKI as a naming-plane function** — certificate issuance, revocation, CAA, and ACME DNS-01 live in the same substrate as Part 1's DDI
...
Parts 7–8's SQL Server authentication modernization is a direct application of this book's cryptographic identity doctrine to the database tier.
...
# Part 4 — OPM HRIT and the Identity & Organizational SSOT
...
| **AC** — Access Control | SASE CASB; ZTNA; Entra ID OPM HRIT; SQL Entra groups | AC-2, AC-3, AC-4, AC-17 | Parts 5, 6, 7-8 |
...
| **SI** — System and Information Integrity | InfoBlox RPZ + TIDE; SASE SWG; Entra ID OPM HRIT; NTLM monitoring | SI-3, SI-3(1), SI-4, SI-12 | Parts 1, 5, 6, 7-8 |
...
|-------|----------|-----------|----------------|--------------|
| **Phase 0** | July 2026 | ✅ Complete | ⬜ Not started | Parts 1 and 5–14 delivered; Conformance Adapter slots 1–4 bound |
| **Phase 1** | August 2026 | ✅ Complete | ⬜ Not started | Part 15 SR/SBOM/VDR; Adapter SR slot bound; KSI-011–014 → active |
| **Phase 2** | Sep–Oct 2026 | ✅ Complete | ⬜ Not started | OSCAL Emitter AR output; Part 16 PM governance artifacts; CR26 SR/PM rules |
| **Phase 3** | November 2026 | ✅ Complete | ⬜ Not started | Part 17 PT/PII templates; Part 18 AT training program; KSI-022 → active; all 29 ADR-111 KSI rules evaluable |
...
| **Phase 5** | January 2027 | ✅ Complete | ⬜ Not started | CR26 mandatory for inherited CSOs; All 19 books operational; Continuous monitoring cadence established |
...
| OSCAL AP/AR | `uiao oscal bundle` Python emitter — OSCAL document version pinned in Book 19 §2.1 (currently 1.0.4); verify FedRAMP's current machine-readable schema target before each authorization-cycle freeze |
...
|---|---|
| [Book 01 — Federal Agency Cloud Landing Zone](Book_01_FedAAN_SSA_Landing_Zone_IPAM_FedRAMP.qmd) | Part 1: InfoBlox DDI, four-environment landing zone, NIST control mapping, FedRAMP 20x KSI evidence |
| [Book 02 — Network Access Control: 802.1X and Device Identity](Book_02_FedAAN_Network_Access_Control_802_1X.qmd) | Part 2: 802.1X EAP-TLS, RADIUS/NPS + CoA, Intune SCEP certificate pipeline, NAC policy tiers, DHCP fingerprinting, Voice VLAN precedent, IA-3/CM-8 necessity closures |
| [Book 03 — Certificates & Tokens: Cryptographic Identity](Book_03_FedAAN_Certificates_Tokens_Cryptographic_Identity.qmd) | Part 3: PKI as a naming-plane function, ACME lifecycle automation, OAuth 2.0/OIDC and CBA, phishing-resistant MFA, workload identity, IA-5(2)/SC-12/SC-17 closure |
| [Book 04 — OPM HRIT and the Identity & Organizational SSOT](Book_04_FedAAN_OPM HRIT_Identity_Org_SSOT.qmd) | Part 4: HR system of record (Oracle HCM Cloud), joiner/mover/leaver lifecycle, HR-driven inbound provisioning per Spec2-D3.1 (UIAO_136), org hierarchy as queryable SSOT, AC-2/PS-4/PS-5/IA-4 closure |
| [Book 05 — From Mainframe to Application-Aware Modernization](Book_05_FedAAN_Network_Modernization.qmd) | Part 5: From mainframe to application-aware — the four principles, AD-to-Entra and ZTNA identity modernization, Kerberos on multipath, load-balancer dissolution |
| [Book 06 — Federal Telecommunications Modernization](Book_06_FedAAN_Federal_Telecommunications_Modernization.qmd) | Part 6: Amazon Connect N8NN, Teams Phone, SD-WAN/SASE, G.114 compliance, TIC 3.0 lawful local breakout, DIA vs. SASE distinction |
| [Book 07 — SQL Server Authentication Modernization](Book_07_FedAAN_SQL_Server_Authentication_Modernization.qmd) | Part 7: Arc Managed Identity, Kerberos/NTLM breakdown, login migration, application chain, NIST closure |
| [Book 08 — SQL Server Implementation Guide](Book_08_FedAAN_SQL_Server_Implementation_Guide.qmd) | Part 8: 10-book runbook — estate discovery, Arc deployment, login migration, NTLM elimination, monitoring |
| [Book 09 — Database Consolidation and Network Physics](Book_09_FedAAN_Database_Consolidation_Network_Physics.qmd) | Part 9: SSOT architecture, speed-of-light floor, SD-WAN for database traffic, consistency class alignment |
| [Book 10 — Privileged Access Management](Book_10_FedAAN_Privileged_Access_Management.qmd) | Part 10: Entra PIM JIT activation, PAW architecture, AC-5/AC-6 least privilege, IA-11 re-authentication |
| [Book 11 — Vulnerability Management](Book_11_FedAAN_Vulnerability_Management.qmd) | Part 11: Defender VM, CISA KEV integration, RA-5 continuous scanning, SI-2 flaw remediation |
| [Book 12 — Data Protection (Purview + Key Vault CMK)](Book_12_FedAAN_Data_Protection_Purview.qmd) | Part 12: Purview DLP and sensitivity labels, Key Vault CMK, MP-2–5, SC-12/13/17/28, AU-10 |
| [Book 13 — SIEM / XDR / Detection Engineering](Book_13_FedAAN_SIEM_XDR_Detection.qmd) | Part 13: Microsoft Sentinel, Defender XDR, KQL analytics, SOAR playbooks, SI-4/AU-6/IR-4/6/8 |
| [Book 14 — Business Continuity](Book_14_FedAAN_Business_Continuity.qmd) | Part 14: CP-2 through CP-8 contingency planning, DR runbooks, Azure Site Recovery |
| [Book 15 — Supply Chain Risk Management](Book_15_FedAAN_Supply_Chain_Risk_Management.qmd) | Part 15: Syft CycloneDX SBOM pipeline, Grype VDR/VEX, GitHub Actions BOD 26-04 gate, SR-family closure |
| [Book 16 — Program Management and Governance](Book_16_FedAAN_Program_Management_Governance.qmd) | Part 16: ISPP template, POA&M tracker, SCRM Charter, Leadership RACI matrix, PM-family closure |
| [Book 17 — PII Processing and Transparency](Book_17_FedAAN_PII_Processing_Transparency.qmd) | Part 17: Privacy Program Plan, authority-to-process templates, SORN/PIA scaffolding, Purview Data Map, PT-family closure |
| [Book 18 — Cybersecurity Awareness and Training](Book_18_FedAAN_Cybersecurity_Training_Awareness.qmd) | Part 18: AT-2/AT-3/AT-4 training program, role-based matrices for six tiers, LMS evidence schema, CP-3 tabletop calendar, IR-2 drill documentation, KSI-022 activation |
| [Book 19 — Authorization Package and ConMon](Book_19_FedAAN_Authorization_Package_ConMon.qmd) | Part 19: authorization package structure, OSCAL AP+AR+POA&M bundle (29/29 self-assessed, pending independent SCA), CA-7 ConMon program, BOD 26-04 VDR/VER submission, AO authorization checklist, authorization-decision procedure |
