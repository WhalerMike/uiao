---
title: "Book 07 — Federal Application-Aware Networking: SQL Server Authentication Modernization"
...
**Date Code:** 2026-07-07 16:26 ET
:::

::: {.callout-important}
**Series Context** — This document is Part 7 of the Federal Application-Aware Networking series. Parts 1–6 cover Landing Zone IPAM/DDI/PKI, Network Access Control, Certificates and Tokens, OPM HRIT and the Identity & Organizational SSOT, Network Modernization, and Telecommunications Modernization. This part addresses the authentication layer of SQL Server — the protocol, credential, and identity surface that persists after network controls are in place.
:::
...
- **BOD 25-01 and OMB M-22-09.** Both directives require elimination of NTLM in federal environments. The transition from Kerberos to Entra ID token-based authentication is the mechanism, not the goal. The goal is the elimination of NTLM.
...
**NTLMv1 block.** At the end of Phase B, NTLMv1 is blocked at the domain level via Group Policy (`Network security: LAN Manager authentication level = Send NTLMv2 response only/refuse LM and NTLM`). NTLMv1 has no legitimate use case in a federal environment and is prohibited under BOD 25-01.
...
**Phase 1 — Create.** `CREATE LOGIN [user@agency.gov] FROM EXTERNAL PROVIDER` — or for a group: `CREATE LOGIN [GroupName] FROM EXTERNAL PROVIDER` with the Entra Object ID.
...
![Entra dynamic groups: SQL authorization from HR attributes](figs/san-fig-08-entra-perimeter.png){#fig-san08 fig-alt="Three numbered step cards left to right with arrows. Step 1, HR attribute changes: department, job code, location, management chain maintained in the HR system of record, the Part 4 SSOT. Step 2, dynamic group reconciles: Entra ID continuously re-evaluates membership rules against the attributes — no ticket, no manual step. Step 3, SQL access follows: CREATE USER group FROM EXTERNAL PROVIDER maps the group to a database role — membership is the authorization. Teal consequence bar: when an employee's department changes in the HR system, group membership changes automatically and SQL Server access changes with it — access derived from employment status and role, not a ticket to a DBA. Blue series bar: this is Book 04's HR-driven identity SSOT arriving at the database layer — the leaver whose HR record closes loses SQL access in the same reconciliation pass. White background. Source: Book 07 SQL Auth Modernization briefing deck, Date Code 2026-07-07 16:23 ET." width="100%"}
...
**Before:**
```
Server=sql01.agency.gov;Database=ApplicationDB;Integrated Security=SSPI;
```

**After:**
```
Server=sql01.database.usgovcloudapi.net;Database=ApplicationDB;Authentication=Active Directory Default;
```
...
The PKI surface is addressed in Part 1 of this series (Landing Zone IPAM/DDI/PKI). The certificate authority decisions made in Part 1 apply directly to the SQL Server certificate migration described here.
...
| AU-2 | Audit Events | NTLM elimination | NTLM eliminated; all authentication produces auditable token events |
...
These artifacts satisfy the IA-2, IA-5, AU-2, AU-12, CA-7, and CM-8 KSI requirements under FedRAMP 20x.
