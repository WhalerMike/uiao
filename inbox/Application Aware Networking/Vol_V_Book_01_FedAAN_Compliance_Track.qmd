---
title: "Vol V Book 01 — Federal Application-Aware Networking — The Compliance Track"
...
state; it cannot author one.** Ten of the twenty-nine CR26 KSIs are attestable
...
1. **Read** — the AAN book(s), by volume coordinate, bound to the slot.
...
![Each module reads the corpus as book to control to KSI to evidence](figs/vol5b01-fig-01-read-control-ksi-evidence.png){#fig-vol5b01-01 fig-alt="A four-panel numbered grid defining the four things each compliance module gives, in order. Panel 1, Read: the AAN book or books, cited by volume coordinate, bound to each evidence slot. Panel 2, Control: the NIST SP 800-53 Rev 5 anchor controls the slot closes. Panel 3, KSI: the FedRAMP 20x Key Security Indicator category that scores the closure. Panel 4, Evidence: the concrete artifact or CLI run proving the chain end-to-end. A footer banner states that ten of twenty-nine KSIs are attestable by ScuBA-class tooling alone while the other nineteen bind to slots a workforce built — the tracing skill this track certifies. The title reads 'Tooling measures drift; it cannot author the intended state.' White background. Source: Vol V Book 01 The Compliance Track briefing deck, Date Code 2026-07-09 17:00 ET." width="100%"}
...
- **Learn:** the FedRAMP 20x timeline (Class A window opened August 2026; 20x
  mandatory for new authorizations January 2027), the BOD 26-04 VDR/VER deadline
  (December 7, 2026), and the split the whole program turns on — the **evidence
...
- **Controls:** IA-2 (+enhancements 1, 2, 11), IA-5, IA-11, AC-2 (+1), AC-3,
...
- **Read:** Vol I Book 01 (SSA Landing Zone, IPAM & FedRAMP) — the Infoblox DDI
...
- **Evaluate:** **KSI-CMT** (configuration / change-management evidence);
  ...
  contract — the BOD 26-04 submission (December 7, 2026) remains the operational
  cadence Vol IV Book 06 runs.
- **Checkpoint:** state the BOD 26-04 submission story — which SBOM artifacts
  Vol IV Book 02 requires, which KSI rules they activated, and how the Vol IV
...
Produce a single document (or an annotated `uiao oscal ksi-ar` export) that, for
**all eight slots**:
...
3. shows the per-KSI verdict (satisfied / not-satisfied / not-evaluated) from a
   real `uiao oscal ksi-ar` run, and
...
it depends on architecture a workforce built.

| Slot | Feeding books | Anchor controls | KSI category | Tool-attestable alone? |
|---|---|---|---|---|
| 1 — Identity | Vol I Bk 03/04/05 · Vol II Bk 01 · Vol III Bk 01 | IA-2, AC-2 | KSI-IAM | No |
| 2 — Addressing | Vol I Bk 01 (Bk 02 pending) | CM-8, AC-4 | KSI-PIY | No |
| 3 — Network | Vol I Bk 05/06/07 · Vol II Bk 03 | SC-8, SC-7, CA-7 | KSI-CNA | No |
...
| 6 — Security / Change | Vol III Bk 04/06 · Vol IV Bk 02/03 | CM-2, AC-4 | KSI-CMT | Partial |
...
With slot 8 active, all 29 FedRAMP 20x CR26 KSIs are evaluable. Ten are
...
*Federal Application-Aware Networking — Vol V Book 01 of the series. Volume V is
...
and KSI claims are self-assessed against the series' ADR-111 rule set (modeled on
