...
> **Correction (2026-07 review — figure pending regeneration):** the figure's right panel overstates the multipath mechanism. The SD-WAN overlay resequences packets and holds the transport session across a path switch — an in-flight Kerberos exchange *survives* active-active multipath, as the callout below states. What actually breaks session-bound authentication on the modern estate is what the path contains and spans: inline decrypt/re-encrypt and NAT rebinding at stateful middleboxes, and KDC unreachability for ticket issuance across cloud boundaries. [Book 03](Book_03_FedAAN_Certificates_Tokens_Cryptographic_Identity.qmd) carries the corrected argument in full. The figure's bottom-line comparison — token-based authentication survives every case — stands.
