...
7. **Supply Chain Risk Management (SCRM) Governance Charter** — organizational
   ...
   for the technical evidence established in Part 12

Together these artifacts close nine NIST SP 800-53 Rev 5 PM family controls
and provide the PM-30 evidence that, combined with Part 12's technical
...
- **PM-30** (SCRM Strategy) is the organizational commitment that makes the
  Part 12 SBOM/VDR/VEX pipeline a governance artifact rather than a technical
...
the TRA as mandatory capabilities. The AAN Part 1–6 series establishes the
...
- Target Architecture Document (TAD) — references AAN Part 1-13 control mappings
- Data Architecture — references Part 9 (Purview/CMK) for data-at-rest controls
- Network Architecture — references Part 1-3, 6 for network security
...
> (CP controls, Part 11) and the Business Impact Analysis (BIA).
...
Part 12 establishes the SBOM pipeline using Syft. Every container image
...
Entra ID (Part 3) provides the authoritative account inventory. The ISSO
reviews privileged account counts weekly (referenced by KSI-003). The
Part 3 evidence artifacts satisfy CM-8(1) automated inventory updates for
the identity plane.
...
| Vulnerability scan (Defender VM) | Continuous | SOC | Part 8 Defender VM output |
| SBOM/VDR scan (Grype CI gate) | Every PR merge | DevSecOps | Part 12 CI logs |
...
operationalized through the Part 10 Sentinel XDR platform with the following
...
| Category | Examples | Controls | Part 12/13 Evidence |
|----------|---------|----------|-------------------|
...
| **Unauthorized access by supplier** | Vendor with excessive access | SR-6, AC-21 | Part 3 vendor access review |
| **Vendor financial/operational risk** | Vendor bankruptcy, PSIRT closure | SR-8, PM-9 | Vendor disclosure matrix (Part 12) |
...
**Assessment Inputs:**
1. SBOM manifest (Syft output, Part 12) — identifies all components and suppliers
2. VDR (Grype output, Part 12) — identifies exploitable vulnerabilities
3. Vendor disclosure matrix (Part 12 §7) — supplier security posture
4. KEV feed (CISA, integrated via Part 10 Sentinel) — known exploited CVEs
5. PSIRT/MSRC subscriptions (Part 12 §6.3) — vendor security advisories

**Assessment Output:**
- SCRM risk register update (referenced in PM-9 §5.2 above)
- VEX assertions for each CVE (OpenVEX format, Part 12 §5)
...
|---------------|-------------|------------|----------|
| Tier 1: SBOM | Machine-readable SBOM for all systems | Implemented | Part 12, Syft pipeline |
| Tier 2: VDR | Vulnerability Disclosure Report | Implemented | Part 12, Grype output |
| Tier 3: VER | Vulnerability Exploitability eXchange | Implemented | Part 12, OpenVEX JSON |

SSA's SCRM program satisfies all three BOD 26-04 tiers through the technical
pipeline established in Part 12 (SBOM + VDR + VEX Framework) and the
...
|----------|------------------------|
| Part 3 (Entra ID / ZTNA) | PM-5 (system inventory — identity plane) |
| Part 8 (Defender VM) | PM-14 (testing — vulnerability scan cadence) |
| Part 9 (Purview / Key Vault) | PM-11 (mission mapping — PII-bearing processes) |
| Part 10 (Sentinel) | PM-14 (monitoring program — Sentinel workbooks) |
| Part 11 (Contingency) | PM-9 (risk register — RTO/RPO inputs) |
| Part 12 (SBOM/VDR/VEX) | PM-30 (SCRM charter — technical backbone) |
| **Part 13 (this document)** | PM-1, PM-2, PM-5, PM-7, PM-9, PM-11, PM-14, PM-15, PM-30 |

The PM governance artifacts in Part 13 provide the organizational layer that
contextualizes the technical evidence from Parts 1–12. Together they form a
