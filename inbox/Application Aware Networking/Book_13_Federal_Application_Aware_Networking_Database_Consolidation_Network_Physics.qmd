---
title: "Book 13 — Federal Application-Aware Networking: Database Consolidation and Network Physics"
...
::: {.callout-important}
**Series Context** — This document is Part 13 of the Federal Application-Aware Networking series. Parts 1–5 address network infrastructure (IPAM/DDI/PKI, SD-WAN/SASE, network modernization) and SQL Server identity migration. This part addresses the architectural substrate beneath the identity layer: where databases should live, why network physics constrain that decision, and how SD-WAN and token-based security change the trade-off calculus.
:::
...
Parts 2 and 3 of this series addressed SD-WAN overlay architecture for network traffic and application layer protocols. The same overlay principles apply to database replication traffic, extending the traffic classification described earlier in this document.
...
| SC-8 | Transmission Confidentiality and Integrity | TLS + Arc-managed certificate | All SQL connections over TLS 1.2+ with agency-PKI certificate |
...
Together with the SQL Server authentication modernization (Parts 4 and 5) and the network controls established in Parts 1 through 3, this architectural framework closes the FedRAMP 20x gap in the database and data management layer.
