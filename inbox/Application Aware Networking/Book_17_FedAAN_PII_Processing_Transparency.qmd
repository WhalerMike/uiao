...
Privacy (PT) control family codifies those obligations as auditable, ATO-ready
...
This document closes the Privacy Transparency (PT) control family gap for
...
- **Part 12 (Purview + Key Vault)** — established the technical data
  ...
  through auto-labeling rules defined in Part 12.
- **Part 16 (Program Management Governance)** — established the organizational
  ...
  The RACI defined in Part 16 §3 assigns Privacy Officer accountability across
  all PT controls.

![PII Processing Lifecycle — Privacy Controls and Authority Chain](figs/pii-fig-01-pii-lifecycle.png){fig-alt="Horizontal lifecycle diagram with six stages: 1 Collect (PT-2 Authority, PT-3 Purpose, PT-5 Notice, PT-4 Consent — statute/SORN/OMB M-03-22 notice, consent mechanism); 2 Process (PT-3 Purpose limit, PT-6 Dissemination, PT-7 De-ID — Purview data map enforcement); 3 Store (SI-12, MP-6, SC-28 at-rest encryption — retention schedule, AES-256, accessibility log); 4 Share/Disclose (PT-6, PT-8 SORN Records, CA-3 ISA — inter-agency ISA, ROUTINE USE match, recipient audit log); 5 Retain (PT-3 retention, SI-12, AU-11 — NARA schedule, audit log min 3 years); 6 Dispose (MP-6, SR-12, PT-3 purpose end — NIST 800-88 purge, disposal certificate). Return arrow shows annual SORN review cycle. Bottom left: Special Categories (SSN, medical, financial, biometric) requiring heightened protection. Bottom right: Microsoft Purview integration for automated purpose-limitation enforcement." width="100%"}

## Scope

This Part applies to any SSA GCC Moderate system that processes PII, including
but not limited to:
...
**Linkage:** PT-1 is the privacy parallel to PM-1 (ISPP, Part 16 §2). The
...
as the ISPP. Cross-reference Part 16 §3 RACI for the Privacy Officer role.
...
> §552a), the E-Government Act of 2002 (44 U.S.C. §3501), OMB Circular A-130
...
- E-Government Act of 2002, 44 U.S.C. §3501, Title III (FISMA)
...
| Privacy control assessment | ISSO + 3PAO | Annual (aligned with security assessment) | Privacy Assessment Report |
...
2. SAOP submits to OMB and Congress for 40-day comment period (5 U.S.C. §552a(e)(4) and (11))
3. Published in the Federal Register at least **30 days before** live PII collection begins
...
follows. This integration (established in Part 12) enables automated enforcement
...
> Label" references the sensitivity label applied in Part 12.
...
FedRAMP 20x requires that the privacy notice be machine-accessible — a stable,
publicly reachable URL that 3PAOs and FedRAMP PMO reviewers can validate
without manual intervention. The SSP must include:
...
# Special Categories of PII (PT-7)
...
|-------------|-------------------|-------------------------------------|
| Social Security Number (SSN) | Yes | OMB M-17-12; OMB M-07-16; SSN limitation laws |
...
| Citizenship / immigration status | Yes | INA §274B; OMB guidance; heightened sensitivity |
| Mental health records | Yes | HIPAA; 42 C.F.R. Part 2 (substance use records) |
| Genetic information | Yes | GINA (29 U.S.C. §1182); HIPAA |

## Special Category Control Matrix

> [FILLABLE] For each special category processed by the system, document the
> Purview sensitivity label (from Part 12), encryption requirement, access
...
|-------------|--------------------------|----------------------|----------------|---------------|
| SSN (full) | `Restricted-SSN` | AES-256 at rest; TLS 1.3 in transit; CMK-managed (Part 12 §4) | Role-based; least privilege; MFA required; no service account access | All reads and writes logged to Sentinel; 7-year retention |
...
> AZURE] must be on file. Reference Part 12 §5 for the Azure BAA status under
...
4. **Breach Response:** SSN exposure triggers OMB M-17-12 breach notification
   procedures; see the Breach Response SOP (reference Part 16 §5 for incident
...
Part 12 §6 established auto-labeling rules in Microsoft Purview. The following
...
Microsoft Purview Data Map lineage (Part 12 §3) provides automated discovery of
...
auto-labeling triggers on intake (Part 12 §6).
...
Moderate requirements (TLS 1.3 minimum; mutual TLS preferred for cross-agency
API calls).
...
800-53 Rev 5 Privacy controls for a GCC Moderate authorization:

| Control | Title | Closing Artifact | Document Section |
|---------|-------|-----------------|-----------------|
| PT-1 | Privacy Policy and Procedures | Privacy Program Plan (PPP) | §2 |
| PT-2 | Authority to Process PII | Legal Authority Matrix + SORN Registry | §3 |
| PT-3 | Purpose Specification | Purpose Specification Matrix + Purview Attribute Mapping | §4 |
| PT-4 | Consent Requirements | Consent Mechanism Matrix | §5 |
...
> Purview labeling and DLP (Part 12) *enforce* the purposes and categories those
...
Security Indicator rules. However, PT controls underpin the **KSI-PIY (Privacy
Inventory)** theme, which assesses whether the agency has a current, accurate
understanding of its PII holdings. The following indirect connections apply:

| KSI Theme | Indirect PT Dependency | Mechanism |
|-----------|----------------------|-----------|
| KSI-PIY (Privacy Inventory) | PT-2, PT-3, PT-7 | Purview data inventory populated through PT-3 purpose mapping and PT-7 special category labeling |
| KSI-DAT (Data Protection) | PT-7 | Special category encryption and access control requirements feed into data protection KSI evidence |
| KSI-IDM (Identity Management) | PT-4 | Consent records linked to identity; withdrawal workflows touch IAM |
| KSI-CMT (Continuous Monitoring) | PT-1 | Privacy oversight cadence integrates with the ConMon plan |

> **Note for FedRAMP Package Reviewers:** Until FedRAMP PMO publishes explicit
> PT → KSI mappings, document PT control evidence in the SSP privacy section
> and reference this document's section numbers. The 3PAO privacy assessment
> report should include PT-1 through PT-7 findings as a distinct section,
...
| Data Flow Map (§8) | All four required scenarios documented | Before ATO submission | DevSecOps + SPO |
| First Annual Privacy Review | Full review of all PT artifacts; 3PAO assessment | Within 365 days of ATO grant | SAOP + 3PAO |

---

*End of Part 17 — PII Processing and Transparency*

*Previous: [Part 16 — Program Management Governance](Book_16_FedAAN_Program_Management_Governance.qmd)*
