...
![Keys first, then labels, then DLP, then audit — the deployment order](figs/vol6b05-fig-03-deployment-order.png){#fig-vol6b05-order fig-alt="A four-step horizontal sequence titled 'Keys first, then labels, then DLP, then audit,' with the fourth step highlighted and all connectors filled to show the completed order. Step 1 (navy), Keys — CMK / BYOK. Step 2 (teal), Labels — taxonomy as code. Step 3 (amber), DLP — test / notify then enforce. Step 4 (red, highlighted), Audit — premium tier plus export. The detail panel for step 4 reads: set the audit tier and immutable export so retention meets the high-impact bar — the retention-cliff fix the detection layer depends on for long-tail forensics (AU-10 non-repudiation). The sequence encodes the dependency chain: labels' encryption depends on the keys, DLP acts on the labels, and audit retention closes the evidence trail. White background. Source: Vol VI Book 05 — Federal Application-Aware Networking — Data Protection Deployment briefing deck, Date Code 2026-07-09 17:00 ET." width="100%"}
