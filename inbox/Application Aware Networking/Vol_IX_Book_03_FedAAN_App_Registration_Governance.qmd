...
Draft. Entra app registrations, ACME, and certificate-based service-principal auth are deployed examples; the request→scoped-consent→short-lived-credential→rotate→attest discipline is the invariant. House-style figures per ADR-093 (rendered at 3× for crisp `.docx` zoom) are added in the figure pass; this draft is text-and-table-first.
