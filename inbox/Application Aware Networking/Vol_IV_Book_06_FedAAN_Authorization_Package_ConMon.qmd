...
here are the ADR-111 rule decomposition, not the published CR26 Moderate KSI
...
run. Zero open risks identified **in the internal ADR-111 rule set**; zero POA&M
...
internal ADR-111 rules satisfied — no residual risk from the KSI evaluation
...
- [ ] Run `uiao oscal bundle --system-name "SSA GCC Moderate"` and confirm 29/29 self-assessed satisfied against the internal ADR-111 rule set (not the CR26 catalog)
- [ ] Confirm POA&M has zero open items *in the internal rule set* — expect it to become non-empty once the ADR-111 → CR26 diff and independent SCA are complete
