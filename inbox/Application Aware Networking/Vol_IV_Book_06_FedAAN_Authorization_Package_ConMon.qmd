...
The Federal AAN series delivered twenty-two technical books covering the SSA GCC
...
- [ ] Run `uiao oscal bundle --system-name "SSA GCC Moderate"` and confirm 29/29 self-assessed satisfied against the series' internal KSI decomposition (not the CR26 catalog)
...
| **CA-7(1)** | Independent Assessment | Independent security control assessment (SCA) of the OSCAL AR — SSA may engage a FedRAMP-recognized 3PAO as its assessor — constitutes the independent assessment; ScuBAGear is run by the SSA ISSO as agency self-assessment against the external CISA ScuBA baseline, feeding the same M365 evidence into that engagement |
...
Office of Information Security — SSA is a consuming agency, so nothing here is
