...
- **SSA edition (this document)** — carries a **hard scope rule (§2)** and a
  pre-filled block of known {{< meta agency.short >}} tools (§3). Everything the SSA edition
  recommends or documents must be **FedRAMP Moderate** (see §2).
- **Federal-agnostic edition (to follow)** — the same discovery tables (§4)
  with the {{< meta agency.short >}} pre-fills (§3) removed and the level rule relaxed to
