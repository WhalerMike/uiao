---
title: "Book 18 — Federal Application-Aware Networking — Part 16"
...
This is the sixteenth and final book in the Federal Application-Aware Networking
(AAN) series. Parts 1–15 generated the evidence layer: infrastructure
...
31 CR26 KSIs. This document assembles those artifacts into the **authorization
package** — the formal submission that requests a FedRAMP Authorization to
Operate (ATO).

Part 16 addresses four deliverables:
...
# 1 The Fifteen-Book Journey — All 29 KSIs Evaluable at High Confidence

## 1.1 Series Summary

The Federal AAN series delivered fifteen technical books covering the SSA GCC
...
| Part 4 | SQL Server Authentication Modernization | IA family (depth) |
| Part 5 | SQL Server Implementation Runbook | Procedural evidence |
| Part 6 | Database Architecture / Network Physics | SC/SI families (depth) |
...
| Part 12 | Supply Chain Risk / SBOM / VDR | SR-1–3/5/6/8/10/11 + BOD 26-04 framework |
| Part 13 | Program Management Governance | PM-1/2/5/7/9/11/14/15/30 |
| Part 14 | PII Processing and Transparency | PT-1/2/3/4/5/7 |
| Part 15 | Cybersecurity Awareness and Training | AT-2, AT-2(1)/2, AT-3, AT-3(3)/5, AT-4, CP-3, IR-2 |
...
|----------|---------|-----------|
| IR tabletop drill (SOC scenario exercise) | IR-2, KSI-INR | Part 15 §3 IR Drill Cadence |
| CP-3 contingency training exercise | CP-3, KSI-RPL | Part 11 §4 Tabletop Calendar |
| SBOM freshness + VDR/VER review | SR-5, SR-6, KSI-SCR | Part 12 §5 VDR Pipeline |
...
| AAN Evidence Book Series | DOCX | Parts 1–15 (inbox/Application Aware Networking/) |
| BOD 26-04 VDR Submission | CycloneDX 1.6 JSON | Part 12 SBOM pipeline |
| Training Effectiveness Record | JSON | `governance.training-effectiveness-record` (Part 15) |
...
Part 12 of this series established the VDR/VER framework. This section
documents the submission procedure.

## 5.2 SBOM Artifact Chain

The SBOM pipeline (from Part 12) produces a chain of machine-readable artifacts:
...
The Part 12 `vdr-submission-record.json` schema generates the cover document
...
Part 10 established the Sentinel XDR platform as the operational ConMon
...
(PM-9, established in Part 13). The risk structure follows OSCAL 1.0.4:
...
- [ ] Confirm all 15 books compiled and available in `inbox/Application Aware Networking/`
...
: Controls Closed by Part 16 {.striped .hover}
...
| KSI-011 | KSI-CMT-LMC | slot-06 high (Part 12/13) | satisfied |
...
| KSI-022 | KSI-CED-RAT | slot-08 high (Part 15) | satisfied |
