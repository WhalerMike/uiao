...
This is the eighteenth and final book in the Federal Application-Aware Networking
(AAN) series. Parts 1–17 generated the evidence layer: infrastructure
...
Part 18 addresses four deliverables:
...
# 1 The Seventeen-Book Journey — All 29 KSIs Evaluable at High Confidence

## 1.1 Series Summary

The Federal AAN series delivered seventeen technical books covering the SSA GCC
...
| Part 4 | SD-WAN, SASE, UC (Teams Phone, Amazon Connect) | 7 |
| Part 5 | Entra ID, ZTNA, Active Governance | 12 |
| Part 6 | SQL Server Authentication Modernization | IA family (depth) |
| Part 7 | SQL Server Implementation Runbook | Procedural evidence |
| Part 8 | Database Architecture / Network Physics | SC/SI families (depth) |
| Part 9 | Privileged Access Management (Entra PIM + PAW) | AC-5, AC-6(×7), AC-11/12, IA-11 |
| Part 10 | Vulnerability Management (Defender VM + KEV) | RA-5(×3), SI-2(×2), SI-5, CM-7(×2) |
| Part 11 | Data Protection (Purview + Key Vault CMK) | MP-2–5, SC-12/13/17/28, AU-10 |
| Part 12 | SIEM / XDR / Detection Engineering (Sentinel) | AU-6(×3), AU-7/8/11, SI-4(×4), IR-4/6/8 |
| Part 13 | Business Continuity / Contingency Planning | CP-2(×3), CP-3/4(×2), CP-6/7(×3), CP-8(×2) |
| Part 14 | Supply Chain Risk / SBOM / VDR | SR-1–3/5/6/8/10/11 + BOD 26-04 framework |
| Part 15 | Program Management Governance | PM-1/2/5/7/9/11/14/15/30 |
| Part 16 | PII Processing and Transparency | PT-1/2/3/4/5/7 |
| Part 17 | Cybersecurity Awareness and Training | AT-2, AT-2(1)/2, AT-3, AT-3(3)/5, AT-4, CP-3, IR-2 |
...
|----------|---------|-----------|
| IR tabletop drill (SOC scenario exercise) | IR-2, KSI-INR | Part 17 §3 IR Drill Cadence |
| CP-3 contingency training exercise | CP-3, KSI-RPL | Part 13 §4 Tabletop Calendar |
| SBOM freshness + VDR/VER review | SR-5, SR-6, KSI-SCR | Part 14 §5 VDR Pipeline |
...
| AAN Evidence Book Series | DOCX | Parts 1–17 (inbox/Application Aware Networking/) |
| BOD 26-04 VDR Submission | CycloneDX 1.6 JSON | Part 14 SBOM pipeline |
| Training Effectiveness Record | JSON | `governance.training-effectiveness-record` (Part 17) |
...
Part 14 of this series established the VDR/VER framework. This section
documents the submission procedure.

## 5.2 SBOM Artifact Chain

The SBOM pipeline (from Part 14) produces a chain of machine-readable artifacts:
...
The Part 14 `vdr-submission-record.json` schema generates the cover document
...
Part 12 established the Sentinel XDR platform as the operational ConMon
...
(PM-9, established in Part 15). The risk structure follows OSCAL 1.0.4:
...
- [ ] Confirm all 17 books compiled and available in `inbox/Application Aware Networking/`
...
: Controls Closed by Part 18 {.striped .hover}
...
| KSI-011 | KSI-CMT-LMC | slot-06 high (Part 14/15) | satisfied |
...
| KSI-022 | KSI-CED-RAT | slot-08 high (Part 17) | satisfied |
