...
Every stack in the actuation matrix is a **CSP service**, so its gate is
**FedRAMP** — verify each stack's authorization status on the FedRAMP Marketplace
at procurement, and never claim above Moderate. One boundary nuance recurs: a
...
**KSI-MLA** (Monitoring, Logging, Alerting); the CM family feeds **KSI-CM**
(Configuration Management). In each case the KSI is attested from the unified
