---
title: "Vol I Book 07 — Federal Application-Aware Networking — Network Enforcement Substrate: Cisco, Palo Alto, Juniper and the NIAP/STIG/FIPS Path"
...
authenticator function performed by the access switch and a NAC policy service —
ISE or Mist Access Assurance. No cloud identity provider authenticates a switch
port; the enforcement is on the wire, in gear the agency owns.

**SC-7 boundary protection at the branch (necessity under TIC 3.0).** Under TIC
3.0, a branch running Direct Internet Access is permissible *only when* the
security-capabilities catalog is satisfied at a distributed enforcement point.
...
> 802.1X function on customer-owned access gear; no cloud identity provider
> reaches the switchport. This is the same necessity the Network Access Control
> book argues, viewed from the equipment that performs it.
...
(`aan-compliance-spine.yml`) and regenerated by `render_authorities_table.py`;
