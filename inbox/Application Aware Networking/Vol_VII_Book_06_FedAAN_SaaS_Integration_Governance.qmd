---
title: "Vol VII Book 06 — Federal Application-Aware Networking — SaaS Integration Governance"
subtitle: "The ServiceNow implementation: onboarding a third-party SaaS as the moment 24 controls are closed — verify authorization, then configure, never the reverse"
...
Developed by the **CSI Team (Cloud Services Infrastructure)**; not yet reviewed or approved by the SSA CIO Office, OIS, or leadership. **Date Code:** 2026-07-14 10:17 ET
:::

::: {.callout-important}
**Series Context** — Volume VII Book 06. Books 02 and 03 automate control compliance *on* the M365 and Azure surfaces. This book implements the governed catalog for the moment an administrator connects a **third-party SaaS** to those surfaces. It inherits the coordination doctrine and the boundary treatment from Book 00 without restating them, reuses the Book 05 scoped-app pattern, and ships as **Lane F** of the Volume IX day-2 kit (`servicenow-day2/saas-control-map.json`) rather than forking a second app.
:::
...
| Decision | Rationale |
|---|---|
| **Lane F in `servicenow-day2`**, not a Vol VII kit | `validate_day2_control_maps.py` globs `servicenow-day2/*-control-map.json` — the lane is CI-validated from its first commit with zero wiring |
...
::: {.callout-important}
**Honest consequence.** A Volume VII book's implementation ships in `Vol_IX_Day2_Operations/kits/servicenow-day2/`. That is a real awkwardness, and it is a signal this book may architecturally belong in Volume IX. The alternative — forking the scoped app to satisfy a volume label — would duplicate Script Includes, Flow, ATF and update set for one lane, which is exactly what Book 05 warns against. Reuse wins; the cross-volume pointer is the price.

**Related debt, not paid here.** Volume VII currently registers **no kit at all**, and the `x_ssa_fed_compliance` app that Book 05 describes as "the deployable counterpart to Books 00–04" does not exist on disk or in the spine's kit registry. That is Book 05's debt. It is recorded here because a reader of Book 05 will look for an artifact and not find one.
:::
...
: Phase 1 — closes **AC-20**, **SA-9** †, **SA-9(2)**, **AC-4**, **CA-3**, **AC-5** {.striped .hover}
...
| `saas.identifier` | One enterprise identifier per person from the HR-derived worker record (Book 04). The vendor never mints its own identifier for an organizational user | automated | **IA-4** |

: Phase 2 — closes **CM-8**, **CM-3**, **IA-2**, **IA-4** {.striped .hover}
...
| `saas.credential.rotate` | Automated rotation ahead of expiry, SaaS-side update tracked as a change. The trust never depends on a human remembering a three-year certificate | automated | **IA-5** |
...
: Phase 3 — closes **IA-5(2)**, **IA-5**, **SC-17** {.striped .hover}
...
: Phase 4 — closes **AC-2(1)**, **AC-3**, **AC-6** {.striped .hover}
...
| Catalog item | What happens | Approval | Control |
|---|---|---|---|
| `saas.leaver` | The separation event drives disable/remove in the SaaS **on the effective date**, evidenced per step. Separation-to-revocation interval inside the SaaS becomes a ConMon metric | automated | **AC-2(3)** |
...
: Phase 5 — closes **AC-2(3)**, **PS-5** {.striped .hover}
...
: Phase 6 — closes **AC-2(4)**, **AU-2**, **AU-6** {.striped .hover}
...
: Phase 7 — closes **CA-7**, **SI-12**, **AC-2** {.striped .hover}
...
- every `slot` is a real evidence slot in `aan-compliance-spine.yml`;
...
`validate_day2_control_maps.py` globs `servicenow-day2/*-control-map.json`. Adding Lane F required no wiring — the existing gate picked it up and reported `5 day-2 control maps, 47 catalog items, all consistent with the spine`. That is the concrete payoff of reusing the day-2 app instead of forking one.
:::
...
3. **`x_ssa_fed_compliance` does not exist.** Book 05 describes it as the deployable artifact; Volume VII registers no kit. Lane F sidesteps this by living in the day-2 app, but a Book 05 reader still finds nothing to import.
