...
**Related debt — since paid.** When this book was first drafted, Volume VII registered no kit and the `x_ssa_fed_compliance` app Book 05 describes existed only as prose. As of 2026-07-14 the DRAFT skeleton ships at `x_ssa_fed_compliance/` and is registered in the spine's `kits:` section (`kit-fed-compliance`), so the distribution build bundles it and its control map is CI-validated. Lane F's placement decision stands on its own merits (reuse of the day-2 plumbing), not on the kit's absence.
:::
