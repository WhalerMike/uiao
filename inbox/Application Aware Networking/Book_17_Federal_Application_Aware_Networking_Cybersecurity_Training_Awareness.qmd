...
dimensions. This is the last remaining KSI that the sixteen-book series
...
5. **Contingency Training (CP-3)** — tabletop exercise calendar and
   attendance documentation linking to Part 13 DR runbooks
6. **Incident Response Training (IR-2)** — drill cadence, scenario library,
   and after-action documentation linking to Part 12 Sentinel playbooks
7. **KSI-CED-RAT Activation** — evidence contract wiring for KSI-022,
   completing all 29 CR26 KSIs to evaluable status
...
| Secure delivery | Developers / engineers | AT-3(3), SA-16 | Training certification or course transcript |
...
## The Relationship to Parts 13 and 15

Part 13 (Business Continuity) established the CP-2 Contingency Plan and
defined RTO/RPO/MTD targets. Part 15 (Program Management) established the
...
The PM-14 T&T&M Plan names training as a required activity. Part 17
...
| **AT-3(5)** | Processing PII | PII awareness module for data stewards, connecting to Part 16 PT family |
...
| **IR-2** | Incident Response Training | Semi-annual IR drill; scenario library maintained by SOC; connects to Part 12 Sentinel playbooks |

: AT Family Control Closure — Book 17 {.striped .hover}
...
> plane (Book 01), device identity at the port (Book 02), and certificate- and
> token-based cryptographic identity (Book 03) — so the people operating the
...
| **Data Classification and Handling** | CUI markings, GCC-only sharing, Purview sensitivity labels | Part 11, CUI policy |
| **Incident Reporting** | How to report a suspected incident; SOC contact; IR playbook entry points | IR-6, Part 12 |
| **Acceptable Use and Legal** | System use notice acknowledgment; FISMA obligations; personal device policy | PL-4, AC-8 |
| **Remote Work Security** | VPN, ZTNA, home network risks, shoulder surfing | AC-17, Part 4 |
...
|-------------|---------|-------------|
| InfoBlox DDI Security | DNSSEC key management, RPZ rule maintenance, IPAM audit log review | Part 1 |
| SD-WAN / SASE Administration | IPsec key rotation, SASE policy updates, DIA circuit failover procedures | Part 4 |
| Entra ID and Conditional Access | Conditional Access policy design, MFA enforcement exceptions, break-glass account procedures | Part 5 |
| Azure Arc and SQL Security | Managed Identity token lifecycle, Arc extension updates, SQL login audit review | Parts 6–7 |
| Microsoft Sentinel Operations | Connector health monitoring, analytics rule tuning, UEBA threshold calibration | Part 12 |
| Privileged Access Management | Entra PIM role activation, PAW maintenance, JIT request approval | Part 9 |
| Vulnerability Management | Defender MDVM dashboard interpretation, KEV remediation SLA tracking, WUfB ring management | Part 10 |
| Backup and DR Operations | Azure Site Recovery test procedures, SQL AG failover validation, RTO/RPO measurement | Part 13 |

: Tier 1 Sysadmin Training — AAN-Aligned Curriculum {.striped .hover}

## Tier 3 — Developer / Engineer Secure Delivery Training (AT-3(3), SA-16)

Developer training under AT-3 and SA-16 covers the secure delivery
practices established by the SBOM/VDR pipeline in Part 14:
...
| **SBOM Generation (Syft)** | Running Syft against container images and repositories, CycloneDX format, attestation signing | Part 14 pipeline competency |
| **Vulnerability Triage (Grype)** | Interpreting Grype output, VEX exploitability assertions, BOD 26-04 D1–D5 SLA obligations | Part 14 pipeline competency |
| **GitHub Actions Security** | Secrets management, workflow permissions, OIDC token scoping, supply chain attack patterns | Part 14 pipeline integrity |
...
| **Incident Reporting from Dev** | How to report a suspected supply chain event; who to contact | IR-6, Part 12 |
...
information. This connects to Part 16 (PII Processing and Transparency):
...
- **DLP policy understanding** — what the DLP policies in Part 11 block
...
by Part 14's SBOM pipeline and Part 10's Defender VM integration — requires
...
(Part 14) with the human competency layer (this document). Together they
...
connects directly to Part 12 (SIEM/XDR Detection Engineering) — the
...
5. **Ransomware encryption event** — endpoint detection via Defender MDE,
   ...
   failover to DR replica (Part 13 procedure)

## Contingency Training (CP-3)

CP-3 requires that contingency plan personnel are trained on their specific
roles and responsibilities. Part 13 established the Contingency Plan,
...
(Part 15) and the CP-3 evidence record that the UIAO substrate ingests.
...
![AT-4 Evidence Collection Pipeline — governance.training-effectiveness-record](figs/ced-fig-02-evidence-pipeline.png){fig-alt="Flow diagram showing four stages of the AT-4 evidence collection pipeline. Stage 1 (dark navy): LMS (Viva Learning or equivalent) stores enrollment and completion records. Stage 2 (federal blue): JSON Schema Export produces the training-effectiveness-record document with six role_tier entries carrying completion percentages and dates. Stage 3 (dark green): UIAO Governance Substrate ingests the record via the Conformance Adapter (ADR-111). Stage 4 (dark red): KSI-CED-RAT Verdict is produced by calling slot_binding(KSI-CED-RAT, confidence=high) — result is PASS when all thresholds are met. Below the flow, a detail panel shows required schema fields: top-level fields include overall_completion_pct (must be at least 95), snapshot_date, cp3_tabletop_date (within 12 months), ir2_drill_date (within 12 months), and role_tiers array; per-tier fields include tier_name, tier_completion_pct (must be at least 95 per tier), last_completion_date (within 12 months), nist_controls array, and ir2_drill_date for IR responder tier. Six tiers required: sysadmins, privileged_access, developers, ir_responders, data_stewards, executives." width="100%"}
...
| **AT-3(5)** | PII processing training | Purview DLP and sensitivity label training for Tier 5 data stewards; connects to Part 16 |
| **AT-4** | Training records | Viva Learning Graph API export; JSON schema defined; monthly automated pull to UIAO governance substrate |
...
## Net New Controls Closed by Part 17
...
: Part 17 — Net New NIST SP 800-53 Rev 5 Controls Closed {.striped .hover}
...
- **CR26 KSI coverage:** 28/29 active → **29/29 active**
- **All CR26 KSI themes:** IAM ✓ · MLA ✓ · SVC ✓ · SCR ✓ · PIY ✓ · INR ✓ · RPL ✓ · **CED ✓**

**This completes the KSI coverage across all eight CR26 themes.** The
Federal Application-Aware Networking series — seventeen books, nine NIST
control families, 29 CR26 Key Security Indicators — is fully evaluable
under FedRAMP 20x Consolidated Rules.

## Series Completion Summary

| Book | Domain | Controls Closed | KSIs Activated |
|------|--------|----------------|----------------|
| Part 1 | InfoBlox DDI Landing Zone | 16 | IAM, MLA, SVC (addressing) |
| Part 4 | Telecommunications (SD-WAN/SASE) | 7 | SVC (boundary), MLA |
| Part 5 | Network and Identity Modernization | 12 | IAM, SVC (network) |
| Parts 6–7 | SQL Server Authentication | auth surface | IAM (SQL tier) |
| Part 8 | Database Architecture and Network Physics | 7 | SVC (data) |
| Part 9 | Privileged Access Management | AC-5/6/11/12, IA-11 | IAM-ELP |
| Part 10 | Vulnerability Management | RA-5, SI-2, CM-7 | MLA-ALA, SVC |
| Part 11 | Data Protection and Purview | MP, SC-12/13/28 | SVC (data protection) |
| Part 12 | SIEM / XDR / Detection Engineering | AU-6/7/8, SI-4, IR-4/6/8 | MLA (primary) |
| Part 13 | Business Continuity | CP-2 through CP-10 | RPL |
| Part 14 | Supply Chain Risk Management | SR-1 through SR-11 | SCR-MIT, SCR-MON |
| Part 15 | Program Management and Governance | PM-1/2/5/7/9/11/14/15/30 | SCR (governance) |
| Part 16 | PII Processing and Transparency | PT-1 through PT-7 | PIY-RSD |
| Part 17 | Cybersecurity Training and Awareness | AT-2/3/4, CP-3, IR-2 | **CED-RAT** |
...
|----------|-------------|
| Federal AAN — SSA Landing Zone IPAM / FedRAMP | Part 1: InfoBlox DDI, four-environment landing zone, NIST control mapping, FedRAMP 20x KSI evidence |
| Federal AAN — Federal Telecommunications Modernization | Part 4: Amazon Connect, Teams Phone, SD-WAN/SASE, G.114 compliance |
| Federal AAN — Network Modernization | Part 5: AD-to-Entra migration, ZTNA, Active Governance, FedRAMP 20x alignment |
| Federal AAN — SQL Server Authentication Modernization | Part 6: Arc Managed Identity, Kerberos/NTLM elimination, login migration |
| Federal AAN — SQL Server Implementation Guide | Part 7: 10-book runbook — estate discovery, Arc deployment, monitoring |
| Federal AAN — Database Consolidation and Network Physics | Part 8: SSOT architecture, speed-of-light floor, SD-WAN for database traffic |
| Federal AAN — Privileged Access Management | Part 9: Entra PIM JIT, PAW architecture, AC-6 least privilege |
| Federal AAN — Vulnerability Management | Part 10: Defender MDVM, CISA KEV, BOD 22-01 SLAs, RA-5 continuous scanning |
| Federal AAN — Data Protection and Purview | Part 11: Purview DLP, sensitivity labels, Key Vault CMK |
| Federal AAN — SIEM / XDR / Detection Engineering | Part 12: Sentinel connectors, KQL analytics, UEBA, Logic App playbooks |
| Federal AAN — Business Continuity | Part 13: CP-2 Contingency Plan, BIA, SQL AG and DDI DR integration |
| Federal AAN — Supply Chain Risk Management | Part 14: Syft SBOM, Grype VDR, OpenVEX, GitHub Actions pipeline |
| Federal AAN — Program Management and Governance | Part 15: ISPP, POA&M, SCRM Charter, PM-30 closure |
| Federal AAN — PII Processing and Transparency | Part 16: PT family templates, Privacy Program Plan, Purview Data Map |
