...
dimensions. This is the last remaining KSI that the fourteen-book series
...
5. **Contingency Training (CP-3)** — tabletop exercise calendar and
   attendance documentation linking to Part 11 DR runbooks
6. **Incident Response Training (IR-2)** — drill cadence, scenario library,
   and after-action documentation linking to Part 10 Sentinel playbooks
...
## The Relationship to Parts 11 and 13

Part 11 (Business Continuity) established the CP-2 Contingency Plan and
defined RTO/RPO/MTD targets. Part 13 (Program Management) established the
...
The PM-14 T&T&M Plan names training as a required activity. Part 15
...
| **AT-3(5)** | Processing PII | PII awareness module for data stewards, connecting to Part 14 PT family |
...
| **IR-2** | Incident Response Training | Semi-annual IR drill; scenario library maintained by SOC; connects to Part 10 Sentinel playbooks |

: AT Family Control Closure — Book 15 {.striped .hover}
...
directly to the RACI established in Part 13:
...
| **Data Classification and Handling** | CUI markings, GCC-only sharing, Purview sensitivity labels | Part 9, CUI policy |
| **Incident Reporting** | How to report a suspected incident; SOC contact; IR playbook entry points | IR-6, Part 10 |
| **Acceptable Use and Legal** | System use notice acknowledgment; FISMA obligations; personal device policy | PL-4, AC-8 |
| **Remote Work Security** | VPN, ZTNA, home network risks, shoulder surfing | AC-17, Part 2 |
...
| SD-WAN / SASE Administration | IPsec key rotation, SASE policy updates, DIA circuit failover procedures | Part 2 |
| Entra ID and Conditional Access | Conditional Access policy design, MFA enforcement exceptions, break-glass account procedures | Part 3 |
| Azure Arc and SQL Security | Managed Identity token lifecycle, Arc extension updates, SQL login audit review | Parts 4–5 |
| Microsoft Sentinel Operations | Connector health monitoring, analytics rule tuning, UEBA threshold calibration | Part 10 |
| Privileged Access Management | Entra PIM role activation, PAW maintenance, JIT request approval | Part 7 |
| Vulnerability Management | Defender MDVM dashboard interpretation, KEV remediation SLA tracking, WUfB ring management | Part 8 |
| Backup and DR Operations | Azure Site Recovery test procedures, SQL AG failover validation, RTO/RPO measurement | Part 11 |
...
practices established by the SBOM/VDR pipeline in Part 12:
...
| **SBOM Generation (Syft)** | Running Syft against container images and repositories, CycloneDX format, attestation signing | Part 12 pipeline competency |
| **Vulnerability Triage (Grype)** | Interpreting Grype output, VEX exploitability assertions, BOD 26-04 D1–D5 SLA obligations | Part 12 pipeline competency |
| **GitHub Actions Security** | Secrets management, workflow permissions, OIDC token scoping, supply chain attack patterns | Part 12 pipeline integrity |
...
| **Incident Reporting from Dev** | How to report a suspected supply chain event; who to contact | IR-6, Part 10 |
...
information. This connects to Part 14 (PII Processing and Transparency):
...
- **DLP policy understanding** — what the DLP policies in Part 9 block
...
by Part 12's SBOM pipeline and Part 8's Defender VM integration — requires
...
(Part 12) with the human competency layer (this document). Together they
...
connects directly to Part 10 (SIEM/XDR Detection Engineering) — the
...
5. **Ransomware encryption event** — endpoint detection via Defender MDE,
   ...
   failover to DR replica (Part 11 procedure)

## Contingency Training (CP-3)

CP-3 requires that contingency plan personnel are trained on their specific
roles and responsibilities. Part 11 established the Contingency Plan,
...
(Part 13) and the CP-3 evidence record that the UIAO substrate ingests.
...
| **AT-3(5)** | PII processing training | Purview DLP and sensitivity label training for Tier 5 data stewards; connects to Part 14 |
...
## Net New Controls Closed by Part 15
...
: Part 15 — Net New NIST SP 800-53 Rev 5 Controls Closed {.striped .hover}
...
Federal Application-Aware Networking series — fifteen books, nine NIST
...
| Part 2 | Telecommunications (SD-WAN/SASE) | 7 | SVC (boundary), MLA |
| Part 3 | Network and Identity Modernization | 12 | IAM, SVC (network) |
| Parts 4–5 | SQL Server Authentication | auth surface | IAM (SQL tier) |
| Part 6 | Database Architecture and Network Physics | 7 | SVC (data) |
| Part 7 | Privileged Access Management | AC-5/6/11/12, IA-11 | IAM-ELP |
| Part 8 | Vulnerability Management | RA-5, SI-2, CM-7 | MLA-ALA, SVC |
| Part 9 | Data Protection and Purview | MP, SC-12/13/28 | SVC (data protection) |
| Part 10 | SIEM / XDR / Detection Engineering | AU-6/7/8, SI-4, IR-4/6/8 | MLA (primary) |
| Part 11 | Business Continuity | CP-2 through CP-10 | RPL |
| Part 12 | Supply Chain Risk Management | SR-1 through SR-11 | SCR-MIT, SCR-MON |
| Part 13 | Program Management and Governance | PM-1/2/5/7/9/11/14/15/30 | SCR (governance) |
| Part 14 | PII Processing and Transparency | PT-1 through PT-7 | PIY-RSD |
| Part 15 | Cybersecurity Training and Awareness | AT-2/3/4, CP-3, IR-2 | **CED-RAT** |
...
| Federal AAN — Federal Telecommunications Modernization | Part 2: Amazon Connect, Teams Phone, SD-WAN/SASE, G.114 compliance |
| Federal AAN — Network Modernization | Part 3: AD-to-Entra migration, ZTNA, Active Governance, FedRAMP 20x alignment |
| Federal AAN — SQL Server Authentication Modernization | Part 4: Arc Managed Identity, Kerberos/NTLM elimination, login migration |
| Federal AAN — SQL Server Implementation Guide | Part 5: 10-book runbook — estate discovery, Arc deployment, monitoring |
| Federal AAN — Database Consolidation and Network Physics | Part 6: SSOT architecture, speed-of-light floor, SD-WAN for database traffic |
| Federal AAN — Privileged Access Management | Part 7: Entra PIM JIT, PAW architecture, AC-6 least privilege |
| Federal AAN — Vulnerability Management | Part 8: Defender MDVM, CISA KEV, BOD 22-01 SLAs, RA-5 continuous scanning |
| Federal AAN — Data Protection and Purview | Part 9: Purview DLP, sensitivity labels, Key Vault CMK |
| Federal AAN — SIEM / XDR / Detection Engineering | Part 10: Sentinel connectors, KQL analytics, UEBA, Logic App playbooks |
| Federal AAN — Business Continuity | Part 11: CP-2 Contingency Plan, BIA, SQL AG and DDI DR integration |
| Federal AAN — Supply Chain Risk Management | Part 12: Syft SBOM, Grype VDR, OpenVEX, GitHub Actions pipeline |
| Federal AAN — Program Management and Governance | Part 13: ISPP, POA&M, SCRM Charter, PM-30 closure |
| Federal AAN — PII Processing and Transparency | Part 14: PT family templates, Privacy Program Plan, Purview Data Map |
