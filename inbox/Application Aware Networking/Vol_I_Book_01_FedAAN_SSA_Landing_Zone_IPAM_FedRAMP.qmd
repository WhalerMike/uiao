...
::: {.callout-tip}
**Key Point (Path A only):** When SSA uses the cloud-managed control plane, it is hosted on AWS GovCloud infrastructure with its own FedRAMP High authorization. SSA inherits both the InfoBlox CSO (FR2017257053) and the AWS GovCloud infrastructure authorization — layered FedRAMP coverage. Under Path B neither inheritance applies; the agency carries the component.
:::
