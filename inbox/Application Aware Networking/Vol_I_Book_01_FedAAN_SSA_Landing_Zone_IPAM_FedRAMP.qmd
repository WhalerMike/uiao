...
**Unified DDI** — an authoritative DNS/DHCP/IPAM plane with DNSSEC and RPZ — solves all four problems from a single control plane. **InfoBlox DDI** is the deployed example this document evaluates for SSA's estate. Under Path A (cloud-managed, below) the control plane is InfoBlox's FedRAMP-authorized offering, listed on the Marketplace as **Infoblox Government Cloud** (CSO FR2017257053, hosted on AWS GovCloud — verify current scope and authorization date on the FedRAMP Marketplace); under Path B (agency-operated, the better fit for SSA's topology) the agency carries the component in its own boundary and no inheritance applies.

> **Mechanism, not product.** Per series doctrine (Vol 0 Book 00, Executive Summary), closure necessity names mechanism classes, never products. The mechanism argued for throughout this document is unified DDI; InfoBlox is the deployed example chosen for SSA's current estate, **BlueCat Integrity** is the principal mechanism-class peer, and the ConMon roadmap's alternatives matrix documents what changes on substitution. Every closure claim below is a claim about the mechanism class.
...
> **SSA topology note:** For SSA's multi-cloud architecture spanning AWS GovCloud, Azure Government, Oracle OCI, and on-premises VMware, Path B — NIOS Grid with an on-premises Grid Master — more closely matches SSA's topology. No single cloud provider is the hub; all four environments are equal spokes of the on-premises control plane. The decision between Path A and Path B should involve OIS and the CIO Office, as it determines whether InfoBlox is a shared-responsibility SaaS or an agency-operated infrastructure component. Agency selection pending.

![Deployment Paths — Path A (BloxOne DDI, cloud-managed) vs. Path B (NIOS Grid, agency-operated)](figs/lz-fig-21-deployment-paths.png){fig-alt="Side-by-side comparison of the two InfoBlox deployment paths. Path A — BloxOne DDI (cloud-managed): InfoBlox operates the management plane on AWS GovCloud (CSO FR2017257053, FedRAMP Moderate); agency deploys lightweight On-Prem Hosts in each environment; FedRAMP Moderate authorization inherited from InfoBlox's ATO; procured via AWS GovCloud Marketplace with no separate IDIQ; best fit for a managed control plane and simplest inheritance. Path B — NIOS Grid (agency-operated, highlighted): Grid Master virtual appliance on on-premises VMware as SSA-governed hub; Grid Members in AWS, Azure Gov, OCI, and on-prem sites as equal spokes; documented inside SSA's own FedRAMP Moderate boundary with agency-owned evidence; procured via GSA Schedule GS-35F or agency IDIQ; best fit when no single cloud is the hub and the trust plane stays in-house. Amber note: Path B more closely matches SSA topology; the A/B decision belongs to OIS and the CIO Office; agency selection pending. White background. Source: Book 01 InfoBlox DDI Briefing deck, Date Code 2026-07-07 09:11 ET." width="100%"}
...
For SSA, the InfoBlox NIOS Grid Master should be deployed as a **virtual machine on on-premises VMware infrastructure** (Path B from the Executive Summary). This is not only a technical topology choice — it is a trust architecture decision. DNS, DHCP, and IPAM together constitute the network naming and addressing control plane: every device's address, every workload's hostname, and every certificate's subject name flows through them. Placing the Grid Master on-premises means SSA retains:

- **Hypervisor-layer control** — the Grid Master VM runs under SSA infrastructure team governance, not a cloud provider's control plane
...
- **Boundary clarity** — the NIOS Grid falls within SSA's own FedRAMP authorization boundary; the agency controls the ATO evidence rather than inheriting it from a vendor CSO

::: {.aan-important}
**Trust rationale:** DNS poisoning, DHCP scope hijacking, and IPAM record manipulation are among the highest-impact attack vectors against network infrastructure. An adversary who controls the Grid Master can redirect any FQDN, drain any IP subnet, or suppress any workload's DNS record — effectively owning the naming plane for all four SSA environments simultaneously. On-premises VMware deployment keeps this function under SSA's direct institutional control.
...
::: {.aan-warning}
**Without InfoBlox automation:** Each of Microsoft's 8–12 annual endpoint changes requires four separate change tickets across NSG, Security Groups, RPZ, and VPN config — during which SSA has a boundary gap window. With InfoBlox, all four update within minutes of the next daily poll of Microsoft's API — a worst-case gap of roughly 24 hours rather than the days-to-weeks a manual four-ticket cycle takes.
:::

### AWS Partnership: BloxOne Runs ON AWS GovCloud {#sec-aws}

![AWS GovCloud × InfoBlox — Infoblox Government Cloud Hosted on AWS, LZA DNS Alignment](figs/lz-fig-07-aws-lza.png){fig-alt="Diagram showing Infoblox Government Cloud (CSO FR2017257053, verify scope and date on the FedRAMP Marketplace) hosted inside AWS GovCloud container, with connections to AWS LZA, Security Hub, ACM, AWS IPAM, and GovCloud Marketplace, plus SSA environments on the right" width="100%"}
...
::: {.aan-tip}
**Key Point (Path A only):** When SSA uses the cloud-managed control plane, InfoBlox operates it on AWS GovCloud under **InfoBlox's own FedRAMP Moderate authorization** (CSO FR2017257053) — that CSO's authorization already accounts for its AWS hosting. SSA inherits the InfoBlox CSO it consumes; it does **not** separately inherit the underlying AWS IaaS authorization (a consumer inherits the service it consumes, not the provider's own supply chain). Under Path B this inheritance does not apply; the agency operates and carries the component itself.
:::
...
OPM is migrating OPM HRIT (federal HR and benefits processing) to Oracle HCM Cloud on OCI. As OPM's OPM HRIT moves to OCI-hosted SaaS, SSA's integration path changes from a mainframe handoff to a cloud API call from SSA's environment to OPM's OCI-hosted service endpoints.
...
SSA currently operates **four asset management surfaces** with no unified coordinator:
...
The following requirements must be satisfied by whichever platform SSA selects. Requirements are grouped by compliance driver. **All requirements must be satisfied regardless of product selection.**
...
| **R-11** | **FedRAMP Moderate or High authorization** — the platform must carry its own FedRAMP ATO at Moderate or High impact, or be deployed within SSA's own FedRAMP boundary with appropriate documentation | FedRAMP | Must |
...
The following platforms have federal government deployments and FedRAMP authorization status relevant to this requirement. **SSA must verify current FedRAMP Marketplace listings at time of procurement** — authorization status changes and this list reflects a point-in-time assessment. No selection is made here; this is a landscape to inform the CIO Office's evaluation.

| Platform | FedRAMP Status | Primary Strength | Key Gap for SSA | Requirements Met |
|---|---|---|---|---|
...
SSA should evaluate candidates against requirements R-01 through R-15 in a structured proof-of-concept across three specific scenarios:
...
```{=html}
<div class="fouo-banner">UNCLASSIFIED // FOR OFFICIAL USE ONLY — SSA Cloud Services Team — Cloud Landing Zone IPAM/DDI/PKI Architecture — July 8, 2026</div>
```
