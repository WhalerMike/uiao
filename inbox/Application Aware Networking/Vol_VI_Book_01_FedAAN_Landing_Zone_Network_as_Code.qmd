...
![The DNSSEC-signed authoritative zone as code — authority, query telemetry, and protective-DNS RPZ are properties of source, not portal settings](figs/vol6b01-fig-03-dnssec-authoritative-zone.png){#fig-vol6b01-dnssec fig-alt="Three side-by-side cards describing the load-bearing ddi naming module declared as code. Card 1, Authoritative plus signing — the zone declares dnssec = on, so SC-20(1) signed responses are an attestable property of source; the resolver being authoritative is diffable, not a portal claim; tagged SC-20(1). Card 2, Every query is telemetry — query_log = on makes every resolution an SI-4 / AU-12 record, so the naming plane emits monitoring evidence by construction; tagged SI-4 / AU-12. Card 3, Protective-DNS RPZ — an RPZ response policy fed by CISA Protective DNS blocks known-bad resolution under SC-21, run in log_only mode first to baseline benign traffic, then flipped to nxdomain enforcing; tagged SC-21. A banner beneath the cards states the DHCP scopes and address plan come from the same ipam source, so a device's lease, its address, and its inventory record are one chain of custody rather than three guesses. White background. Source: Vol VI Book 01 — Federal Application-Aware Networking — Landing Zone & Network as Code briefing deck, Date Code 2026-07-09 17:00 ET." width="100%"}

```hcl
# ddi module — authoritative, DNSSEC-signed zone with protective-DNS RPZ (deployed example: cloud DNS)
resource "dns_zone" "agency" {
  name        = var.zone_fqdn          # e.g. agency.gov
  dnssec      = "on"                    # SC-20(1): signed responses
...
resource "dns_rpz_policy" "protective" {
  ...
  action          = "nxdomain"                # SC-21: block known-bad resolution
...
| SC-20/20(1) | Secure Name/Address Resolution (Authoritative) | DNSSEC-signing authoritative resolver as code |
| SC-21 | Secure Name/Address Resolution (Recursive) | Validating recursive resolver + RPZ protective DNS |
| SC-22 | Architecture & Provisioning for Name/Address | Redundant resolver topology in the `ddi` module |
