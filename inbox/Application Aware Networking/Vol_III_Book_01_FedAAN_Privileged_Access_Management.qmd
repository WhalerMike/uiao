---
title: "Vol III Book 01 — Federal Application-Aware Networking — Privileged Access Management"
...
> **Closure Necessity — AC-6(1) and AC-6(10) on privileged sessions.** Every closure in this table rests on two identity substrates, not on policy text. PIM's activation ceremony is enforceable only because the role grant is a cryptographically validated token claim with a bounded lifetime — the certificate-and-token doctrine of [Vol I Book 03 (Certificates & Tokens)](Vol_I_Book_03_FedAAN_Certificates_Tokens_Cryptographic_Identity.qmd); a password-conveyed admin role carries no expiry the directory can enforce. And the PAW gate on AC-6(1)/AC-6(10) holds only because Conditional Access can verify *device* identity and compliance — the device-identity doctrine of [Vol I Book 02 (Network Access Control: 802.1X)](Vol_I_Book_02_FedAAN_Network_Access_Control_802_1X.qmd); without a device-bound assertion, "activate only from a compliant workstation" is a request, not a control. There is no audit procedure, admin-training artifact, or account-naming convention that bounds a standing credential's scope in time or binds its use to hardware.
