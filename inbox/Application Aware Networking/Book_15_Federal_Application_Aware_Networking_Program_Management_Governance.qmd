...
7. **Supply Chain Risk Management (SCRM) Governance Charter** — organizational
   ...
   for the technical evidence established in Part 14

Together these artifacts close nine NIST SP 800-53 Rev 5 PM family controls
and provide the PM-30 evidence that, combined with Part 14's technical
...
- **PM-30** (SCRM Strategy) is the organizational commitment that makes the
  Part 14 SBOM/VDR/VEX pipeline a governance artifact rather than a technical
...
the TRA as mandatory capabilities. The AAN Part 1–8 series establishes the
...
- Target Architecture Document (TAD) — references AAN Part 1-15 control mappings
- Data Architecture — references Part 11 (Purview/CMK) for data-at-rest controls
- Network Architecture — references Part 1-5, 8 for network security
...
> (CP controls, Part 13) and the Business Impact Analysis (BIA).
...
Part 14 establishes the SBOM pipeline using Syft. Every container image
...
| Vulnerability scan (Defender VM) | Continuous | SOC | Part 10 Defender VM output |
| SBOM/VDR scan (Grype CI gate) | Every PR merge | DevSecOps | Part 14 CI logs |
...
operationalized through the Part 12 Sentinel XDR platform with the following
...
| Category | Examples | Controls | Part 14/15 Evidence |
|----------|---------|----------|-------------------|
...
| **Vendor financial/operational risk** | Vendor bankruptcy, PSIRT closure | SR-8, PM-9 | Vendor disclosure matrix (Part 14) |
...
**Assessment Inputs:**
1. SBOM manifest (Syft output, Part 14) — identifies all components and suppliers
2. VDR (Grype output, Part 14) — identifies exploitable vulnerabilities
3. Vendor disclosure matrix (Part 14 §7) — supplier security posture
4. KEV feed (CISA, integrated via Part 12 Sentinel) — known exploited CVEs
5. PSIRT/MSRC subscriptions (Part 14 §6.3) — vendor security advisories

**Assessment Output:**
- SCRM risk register update (referenced in PM-9 §5.2 above)
- VEX assertions for each CVE (OpenVEX format, Part 14 §5)
...
|---------------|-------------|------------|----------|
| Tier 1: SBOM | Machine-readable SBOM for all systems | Implemented | Part 14, Syft pipeline |
| Tier 2: VDR | Vulnerability Disclosure Report | Implemented | Part 14, Grype output |
| Tier 3: VER | Vulnerability Exploitability eXchange | Implemented | Part 14, OpenVEX JSON |

SSA's SCRM program satisfies all three BOD 26-04 tiers through the technical
pipeline established in Part 14 (SBOM + VDR + VEX Framework) and the
...
| Part 10 (Defender VM) | PM-14 (testing — vulnerability scan cadence) |
| Part 11 (Purview / Key Vault) | PM-11 (mission mapping — PII-bearing processes) |
| Part 12 (Sentinel) | PM-14 (monitoring program — Sentinel workbooks) |
| Part 13 (Contingency) | PM-9 (risk register — RTO/RPO inputs) |
| Part 14 (SBOM/VDR/VEX) | PM-30 (SCRM charter — technical backbone) |
| **Part 15 (this document)** | PM-1, PM-2, PM-5, PM-7, PM-9, PM-11, PM-14, PM-15, PM-30 |

The PM governance artifacts in Part 15 provide the organizational layer that
contextualizes the technical evidence from Parts 1–14. Together they form a
