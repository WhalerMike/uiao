...
Draft. ServiceNow, Microsoft Graph, and Entra are deployed examples; the request→approve→actuate-native→evidence discipline is the invariant. Vendor claims cite vendor documentation. The figure above is authored as committed house-style SVG per ADR-093, rendered to PNG at 3× for crisp `.docx` zoom.
