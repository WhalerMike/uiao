---
title: "Book 21 — Federal Application-Aware Networking — Part 21"
...
│  Feedback loop: new CVEs trigger Defender VM alert (Part 8 chain)   │
└─────────────────────────────────────────────────────────────────────┘
```
...
: Part 12 — NIST SP 800-53 Rev 5 Control Closure: SR Family {.striped .hover}
...
| KSI Rule | KSI Title | NIST Controls | Evidence from Part 12 Pipeline |
|---|---|---|---|
| **KSI-011** | Change Logging and Monitoring | CM-3, AU-6 | SBOM diff between releases: every dependency change is logged as a structured artifact with author, timestamp, and before/after hash. Ingested by Sentinel (Part 10) as a structured change event. |
...
: KSI-CMT Evidence Contract — Part 12 Provides VER Evidence for KSI-011–014 {.striped .hover}
...
The SBOM/VDR/VEX pipeline integrates with Microsoft Sentinel (Part 10) via a
...
: Part 12 — Implementation Checklist {.striped .hover}
...
| Monitoring | Microsoft Sentinel (Part 10) | KQL analytics | KSI-MLA evidence chain |
| KEV Integration | CISA KEV JSON feed | JSON | BOD 22-01 14-day SLA |

: Part 12 — Technology Stack {.striped .hover}
...
| [Book 08 — Vulnerability Management](Book_08_Federal_Application_Aware_Networking_Vulnerability_Management.qmd) | Defender VM + KEV integration; extends to SBOM-sourced CVE triage |
| [Book 10 — SIEM / XDR / Detection Engineering](Book_10_Federal_Application_Aware_Networking_SIEM_XDR_Detection.qmd) | Sentinel ingestion of SBOM/VDR telemetry; KQL analytic rules |
