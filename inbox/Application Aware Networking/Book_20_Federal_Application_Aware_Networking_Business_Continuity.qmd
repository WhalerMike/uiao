---
title: "Book 20 — Federal Application-Aware Networking — Part 20"
...
Parts 1 through 10 of this series built the infrastructure substrate for a resilient federal network: DNS with Grid replication (Part 1), SD-WAN with dual underlay circuits and automatic path failover (Part 2), identity with break-glass access procedures (Part 3 and Part 7), SQL Server Always On Availability Groups with geo-replicated disaster recovery replicas (Parts 5 and 6), and a centralized logging and alerting fabric (Part 10). Each part produced runbooks, configuration templates, and tested recovery procedures. A FedRAMP assessor reviewing all ten parts would find a technically impressive architecture. They would also find that the agency cannot demonstrate compliance with CP-2.

The reason is structural. CP-2 is not satisfied by capable infrastructure. CP-2 is satisfied by a document: a Contingency Plan that has been approved by a designated official, distributed to all recovery personnel, tested on a defined cycle, and updated when the system changes or after every test. The runbook in Part 5 that walks a DBA through failing over an Availability Group is a recovery procedure. It is not a Contingency Plan. The SD-WAN configuration in Part 2 that automatically selects the backup DIA circuit when MPLS degrades is a resilience capability. It is not a Contingency Plan. An assessor who opens the Plan of Action and Milestones and sees CP-2 open can turn off the screens showing a perfectly functioning DR failover and write a High finding, because the artifact does not exist.

This document closes that gap. It assembles the infrastructure components from Parts 1 through 10 into a formal CP-2 Contingency Plan structure aligned to NIST SP 800-34 Rev 1, adds the organizational and testing layers that infrastructure alone cannot provide, and closes the full CP control family through CP-10. Agencies can use this document as a direct template: the section outlines are populated with enough specificity to guide authors, the Business Impact Analysis methodology produces the RTO/RPO values that anchor the plan, and the testing framework produces the artifacts an assessor expects to see.
...
The DR runbooks in Parts 5 and 6 are Section 5 content for the Contingency Plan. They are not the Contingency Plan itself. The difference is visible in what each artifact contains.
...
Appendix A — Network Recovery Procedures (SD-WAN failover and DDI Grid failover; derived from the runbooks in Parts 1 and 2 of this series)

Appendix B — Database Recovery Procedures (SQL AG failover and point-in-time restore; derived from the runbooks in Parts 5 and 6 of this series)

Appendix C — Identity Recovery Procedures (break-glass account activation and Conditional Access adjustment; derived from Parts 3 and 7 of this series)

Appendix D — Application and Cloud Service Recovery Procedures (application tier failover to secondary region)

Appendix E — Audit Logging Recovery Procedures (Sentinel workspace validation and log analytics continuity; derived from Part 10 of this series)
...
The SD-WAN dual-underlay architecture from Part 2 provides automatic alternate path selection that satisfies CP-7 for WAN connectivity. The critical documentation requirements:
...
For cloud contact center platforms (such as the Amazon Connect deployment referenced in Part 2), CP-7 requires documented multi-region failover. The failover mechanism uses DNS-based health checks with automatic routing policy changes.
...
The SD-WAN architecture from Part 2 directly implements CP-8. The dual underlay consists of:
...
**Backup path (DIA — Direct Internet Access):** Broadband Internet circuit from a different provider, providing backup connectivity when MPLS is degraded or unavailable. Characteristics to document:
...
**Automatic failover.** The SD-WAN fabric monitors both paths continuously via BFD (Bidirectional Forwarding Detection) or equivalent liveness detection. When the primary path degrades below configured thresholds (packet loss, latency, jitter), traffic is automatically shifted to the backup path without manual intervention. The failover detection and cutover time is configurable and must be documented.
...
| DIA-BACKUP-01 | [Carrier B] | Broadband Internet | [N] Mbps / [N] Mbps | 99.5% | Backup (automatic failover) |
...
## Integration with Parts 1 Through 10
...
| Part 2 (SD-WAN) | Dual underlay (MPLS + DIA) with automatic path failover | CP-8: primary and alternate telecommunications services; CP-7: alternate network path to cloud processing site |
| Part 3 (Identity — Entra ID) | Cloud-hosted globally redundant identity; Conditional Access | CP-7: identity is inherently multi-region; CP-2: break-glass accounts ensure identity access independent of Conditional Access availability |
| Part 4 (PKI and Certificate Services) | Certificate revocation and issuance continuity | CP-7: certificate authority failover documentation; ensures TLS-dependent services can be restored at alternate site |
| Part 5 (SQL — on-premises) | AG synchronous replica for RPO near zero | CP-6: synchronous replica as alternate storage; CP-7: synchronous failover for minimal RTO SQL recovery |
| Part 6 (SQL — cloud DR) | AG async replica in secondary region | CP-6(1): geographically separated alternate storage; CP-7(1): geographically separated alternate processing; CP-9: backup of structured data |
| Part 7 (Zero Trust and MFA) | Break-glass accounts for emergency access | CP-2: ensures recovery teams can authenticate during Conditional Access disruption or MFA outage |
| Part 8 (Network Segmentation) | Firewall and micro-segmentation policy backup | CP-6: configuration baseline for firewall rules stored in alternate region; CP-7: network policy can be restored at alternate site |
| Part 9 (Compliance Automation) | Configuration state and compliance baseline exports | CP-6: configuration baselines (device management policy exports, compliance rules) stored in alternate Key Vault |
| Part 10 (Sentinel/SIEM) | Geo-redundant Log Analytics workspace | CP-9: audit logs replicated to alternate region; AU-9: integrity of audit records maintained during primary site outage |
...
| CP-9 | System Backup | Addressed in Parts 5 and 6; extended here to audit logs and config baselines | Audit log backup via geo-redundant Log Analytics (Part 10) explicitly referenced in CP-2 as CP-9 evidence; config baseline exports in secondary Key Vault documented | NIST SP 800-53 Rev 5 CP-9 |
...
: NIST SP 800-53 Rev 5 Control Closure — Part 11 Business Continuity {.striped .hover}
...
**KSI: Incident Response and Continuity (maps to CP-2, CP-4, IR-4).** FedRAMP 20x assessors will look for evidence that contingency plans are living documents connected to operational telemetry, not static PDFs filed at authorization time. The connection points in this framework are the Sentinel alerts (Part 10) that trigger the activation threshold evaluation, the SD-WAN monitoring dashboards that evidence CP-8 circuit health, and the AAR tracking table that shows findings from prior tests are being resolved. Automate the evidence collection: a scheduled export of circuit uptime, replication lag, and backup age to the ISCM continuous monitoring system satisfies KSI evidence requirements without manual artifact assembly at assessment time.

The Contingency Plan document is the assessor-facing artifact. The monitoring, replication, and failover capabilities from Parts 1 through 10 are the automated evidence substrate. Part 11 closes the gap between having the capability and being able to demonstrate it.
:::
