...
- **"Extend the domain into the cloud."** Site-to-site tunnels carrying domain traffic to cloud regions recreate the hairpin that [Book 05](Book_05_Federal_Application_Aware_Networking_Network_Modernization.qmd)'s physics argument closed: authentication round-trips ride the tunnel back to wherever the domain controllers live, re-importing the latency floor into every logon. The tunnel moves the problem; it cannot shorten the distance.
...
| User and application authentication | Certificate-based authentication and token signing keys | This book, applied in [Books 07](Book_07_Federal_Application_Aware_Networking_SQL_Server_Authentication_Modernization.qmd)–[08](Book_08_Federal_Application_Aware_Networking_SQL_Server_Implementation_Guide.qmd) |
...
This is the hinge to the application books. [Book 07](Book_07_Federal_Application_Aware_Networking_SQL_Server_Authentication_Modernization.qmd) replaces domain credentials in the SQL Server estate with managed identities and token-based authentication; [Book 08](Book_08_Federal_Application_Aware_Networking_SQL_Server_Implementation_Guide.qmd) is its implementation runbook. Read them as **applications of this chapter** — the database is simply the first large estate where the workload-identity progression is executed end-to-end:
...
Everything after this point is application and evidence. [Book 05](Book_05_Federal_Application_Aware_Networking_Network_Modernization.qmd) and [Book 06](Book_06_Federal_Application_Aware_Networking_Federal_Telecommunications_Modernization.qmd) modernize the transport that made this identity model mandatory. [Books 07](Book_07_Federal_Application_Aware_Networking_SQL_Server_Authentication_Modernization.qmd)–[08](Book_08_Federal_Application_Aware_Networking_SQL_Server_Implementation_Guide.qmd) execute this book's doctrine against the database estate — the first proof that session-bound authentication can be *removed*, not merely deprecated. The privileged access, detection, and continuity parts consume the token-level telemetry this architecture emits, and [Book 19](Book_19_Federal_Application_Aware_Networking_Authorization_Package_ConMon.qmd) assembles it into the authorization package.
