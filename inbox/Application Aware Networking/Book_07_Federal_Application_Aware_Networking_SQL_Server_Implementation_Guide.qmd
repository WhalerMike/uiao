---
title: "Book 07 — Federal Application-Aware Networking: SQL Server Implementation Guide"
...
::: {.callout-important}
**Series Context** — This document is Part 5 of the Federal Application-Aware Networking series and the implementation companion to Part 4 (SQL Server Authentication Modernization). Part 4 covers the conceptual framework; this document provides the operational runbooks, command references, and configuration templates for execution.
:::
...
![The ten books run in sequence. The consolidation gate at Book 04 determines which instances proceed. Only retained instances are processed through Books 03–10.](figs/imp-fig-01-book-run-order.png){#fig-imp01 fig-alt="Flowchart showing 10-book run order with consolidation gate at Book 04"}

**The consolidation gate** at Book 04 is a real gate, not a formality. Instances that do not meet the retain criteria are decommissioned before migration resources are spent on them. An instance that is retired is not migrated — it is turned off, its databases are archived or transferred, and it is removed from the estate inventory.

**Safety model.** Every destructive action in this guide is preceded by a documented undo path. `ALTER LOGIN DISABLE` can be reversed with `ALTER LOGIN ENABLE`. SPNs can be removed with `setspn -D`. Arc enrollment can be unenrolled. No action in Books 01–07 is irreversible. The first irreversible action is `DROP LOGIN` in Book 07 — and that action is explicitly gated behind a 30-day observation period.

**Test environment requirement.** Books 03–06 should be validated in a test environment before execution against production instances. The test environment must be Arc-enrolled and running SQL Server 2022 to produce meaningful validation results. Testing against SQL Server 2019 with the Entra extension is acceptable for Books 03 and 04, but Book 07 (login migration) must be tested against 2022.
...
# Book 04 — Estate Discovery and Consolidation Gate
...
SELECT
    ...
    DATEADD(ms, -1 * (sys.dm_os_ring_buffers.timestamp - cpu_tick_count), GETDATE()) AS event_time
FROM sys.dm_os_ring_buffers
CROSS APPLY (SELECT CAST(ring_buffer_address AS XML) AS record) AS xRecord(record)
...
Document the disposition decision, authorizing manager, and expected completion date for each instance before proceeding to Book 05.

---

# Book 05 — Authentication Audit

The authentication audit produces the pre-migration baseline: every login, its type, its last use, and its associated permissions. This baseline is the reference document for the login migration in Book 07.
...
$events = Get-WinEvent -FilterHashtable @{
    ...
    $_.Properties[8].Value -eq 'NTLM'  # AuthenticationPackageName
}
...
![The five closure conditions gate progression from authentication audit to Arc deployment. All five must be documented before Book 06 begins.](figs/san-fig-03-ntlm-elimination.png){#fig-imp03 fig-alt="Diagram showing NTLM elimination phases and source patterns"}
...
# Book 06 — Arc Deployment and Managed Identity
...
CREATE LOGIN [SQLDBAs@agency.gov] FROM EXTERNAL PROVIDER;
ALTER SERVER ROLE sysadmin ADD MEMBER [SQLDBAs@agency.gov];
GO
```

---

# Book 07 — Login Migration and Parallel Run
...
```sql
-- For a user login:
CREATE LOGIN [firstname.lastname@agency.gov] FROM EXTERNAL PROVIDER;
...
CREATE USER [firstname.lastname@agency.gov] FOR LOGIN [firstname.lastname@agency.gov];
ALTER ROLE db_datareader ADD MEMBER [firstname.lastname@agency.gov];
```
...
$connectionString = "Server=sql01.agency.gov;Database=ApplicationDatabase;Authentication=Active Directory Interactive;"
...
# Book 08 — NTLM Detection and Remediation

## SPN Fix

For each NTLM source classified as "missing SPN" in Book 05:
...
EXEC sp_addlinkedsrvlogin
    ...
    @useself      = 'FALSE',
    @locallogin   = NULL,
    @rmtuser      = 'svc-linkedserver@agency.gov',  -- Entra service account
...
# Book 09 — Groups and Conditional Access Perimeter
...
# Book 10 — Application Chain and Connection Strings
...
**Before:**
```
Data Source=SQL01.agency.gov;Initial Catalog=ApplicationDB;Integrated Security=SSPI;
```

**After:**
```
Data Source=SQL01.agency.gov;Initial Catalog=ApplicationDB;Authentication=Active Directory Default;
```
...
var connection = new SqlConnection("Server=sql01.agency.gov;Database=ApplicationDB;Authentication=Active Directory Default;");
...
# Book 11 — Continuous Monitoring and Compliance Record
...
# Book 12 — Certificate Discovery and ADCS Dependencies
...
Upon completion of all 10 books for all retained instances, the SQL Server estate is in the transformed posture described in Part 4 of this series.
