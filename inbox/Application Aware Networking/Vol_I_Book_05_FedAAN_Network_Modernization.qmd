...
> **Closure Necessity — SC-8 on DIA circuits.** A DIA circuit carries traffic unencrypted at the network layer. Application-layer TLS provides no network-layer guarantee and no coverage for non-TLS flows. The only mechanism that encrypts *all* traffic on a DIA path regardless of application behavior is an IPsec overlay — i.e., SD-WAN. There is no configuration setting, scanner finding, or policy document that closes SC-8 on a bare pipe.
