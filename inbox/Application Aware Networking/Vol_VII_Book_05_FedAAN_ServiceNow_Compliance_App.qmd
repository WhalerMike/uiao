...
4. Set app properties: `x_ssa_fed_compliance.mid_server`, `x_ssa_fed_compliance.graph_cloud` (`gcc`|`gcc-high`|`commercial`), `x_ssa_fed_compliance.arm_cloud`, `x_ssa_fed_compliance.mid_scripts_dir`, and `x_ssa_fed_compliance.acknowledge_saas_boundary`.
...
Draft. `x_ssa_fed_compliance`, Microsoft Graph, and Azure Resource Manager are deployed examples; the in-boundary-MID, least-privilege-connector, control-map-checked-against-the-spine disciplines are the invariants. This is a vendor-integration skeleton independent of UIAO governance canon. House-style figures per ADR-093 are added in the PPTX→DOCX sync pass; this draft is text-and-table-first.
