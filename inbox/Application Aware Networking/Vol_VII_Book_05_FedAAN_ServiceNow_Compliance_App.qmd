...
subtitle: "The deployable artifact: a scoped app with connectors, Flows, catalog items, ATF tests, and one importable update set"
...
::: {.callout-note}
**Starter skeleton, not a signed product.** The records described here are an illustrative scoped app named `x_ssa_fed_compliance`. Review them, complete the connector credential aliases and the control map per tenant, pin API versions, and test in a sub-prod instance before any production use. This mirrors the disposition of the DDI app skeleton already in the repo.
:::
...
| `rest/sys_rest_message_graph.xml`, `..._arm.xml` | `sys_rest_message` (+ `_fn`) | Outbound REST to Microsoft Graph and Azure Resource Manager — endpoint/auth from a credential alias, **routed via the MID Server** |
| `data/control-map.json` | app data (machine-readable) | The finding→(control, task type, severity, SLA, KSI slot) binding — a projection of `aan-compliance-spine.yml`; CI-checked against the spine |
...
| `atf/*.xml` | ATF test suites | Happy-path (drift→task→close→evidence) + negative (unreconciled asset, self-approval block, SLA breach escalation), runnable via `test_mode` with no live cloud |
| `update-set/x_ssa_fed_compliance-update-set.xml` | update set | The whole app assembled into one importable XML |
...
The finding-to-task binding lives in `data/control-map.json`, not in code, so it is reviewable and CI-checked. Each entry mirrors a closure row in `aan-compliance-spine.yml`:

```jsonc
// control-map.json — one entry per (finding class → task). Projection of the spine.
{
  "m365.scuba.fail": {
    "control": "CM-6", "task_type": "change", "severity": "moderate",
    "sla_class": "baseline-drift", "ksi": ["KSI-CMT", "KSI-SVC"], "slot": "slot-06-security"
  },
  "azure.updatemgr.missing_critical": {
    "control": "SI-2", "task_type": "change", "severity": "high",
    "sla_class": "kev-exposed", "ksi": ["KSI-SVC", "KSI-MLA"], "slot": "slot-05-endpoint"
  },
  "azure.defender.finding": {
    "control": "RA-5", "task_type": "vulnerability_response", "severity": "by-exposure",
    "sla_class": "risk-based", "ksi": ["KSI-MLA"], "slot": "slot-06-security"
  }
}
```

A CI check (the same regen-and-diff pattern as `render_authorities_table.py --check`) fails when the app's control map diverges from the spine — so the deployable app and the series SSOT cannot silently drift.
...
Draft. `x_ssa_fed_compliance`, Microsoft Graph, and Azure Resource Manager are deployed examples; the in-boundary-MID, least-privilege-connector, control-map-checked-against-the-spine disciplines are the invariants. This is a vendor-integration skeleton independent of UIAO governance canon. The record-model figure is authored as committed house-style SVG per ADR-093 (rasterized to PNG).
