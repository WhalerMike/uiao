...
::: {.callout-note}
**What this appendix is for.** Book 02's main narrative describes SD-WAN in platform-neutral terms. This appendix addresses the specific combination that most federal Microsoft 365 agencies encounter in practice: **Cisco Catalyst SD-WAN** as the branch transport platform and **Microsoft Intelligent Network Routing (INR)** as the M365 traffic optimization layer. It covers what each does, how they interact, and what a FedRAMP-compliant implementation of the combination requires.
:::
...
| Cisco SD-WAN + Azure Virtual WAN | Azure gov or GCC High presence | Excellent — traffic on Microsoft backbone from Azure VWan hub |
...
: Cisco Catalyst SD-WAN + Microsoft INR Integration Paths {.striped .hover}
...
| Microsoft M365 GCC / GCC High | **Outside** — FedRAMP-authorized external service | Microsoft CSP ISA; FedRAMP package reference |
| Azure Virtual WAN hub | **Outside** — FedRAMP-authorized external service | Azure Government FedRAMP package reference; data flow to/from |
...
## A.6 — GCC vs. GCC High: What Changes for Microsoft INR

Federal agencies operating under **GCC High** (handling CUI or DoD data) face one additional routing constraint that commercial and GCC deployments do not.

**M365 GCC High uses dedicated Microsoft endpoints.** GCC High Microsoft 365 services are hosted on Microsoft's sovereign cloud infrastructure, with dedicated endpoints distinct from the commercial M365 service. The Microsoft-published IP/domain prefix lists for GCC High are different from the commercial lists. Cisco Catalyst SD-WAN Cloud OnRamp policies must reference the GCC High endpoint lists — not the commercial lists. Using the wrong list is the most common GCC High SD-WAN misconfiguration and results in M365 traffic not being steered as Optimize-category traffic.

**INR backbone optimization is available in GCC High.** Microsoft's private backbone routing intelligence applies to GCC High traffic. The local breakout + nearest-front-door approach is equally valid and equally beneficial. The Azure Virtual WAN integration path — where Cisco Catalyst SD-WAN tunnels to Azure Government regions and then onto the Microsoft sovereign backbone — is the architecturally preferred path for GCC High agencies, as it eliminates any public internet segment after the branch WAN Edge.

**Azure Virtual WAN in Azure Government.** For GCC High agencies, the Azure Virtual WAN hub should be deployed in **Azure Government** (not commercial Azure). Cisco Catalyst SD-WAN IPsec tunnels terminate at the Azure Government VWan hub; traffic then traverses Microsoft's sovereign backbone to GCC High M365 endpoints. This is the configuration that satisfies the DoD IL2/IL4 data-handling requirements for in-transit traffic and produces the cleanest FedRAMP boundary diagram.

| Deployment | M365 Tenant | INR Architecture | Cisco SD-WAN Target |
|---|---|---|---|
| Civilian agency, general federal use | GCC | Cloud OnRamp + DIA/MAPS | Microsoft commercial front door endpoints |
| Agency handling CUI or DoD data | GCC High | Cloud OnRamp + Azure VWan (Azure Government) | GCC High dedicated endpoints; Azure Government region |
| DoD component | IL4/IL5 (GCC High or DoD 365) | Cloud OnRamp + Azure VWan (Azure Government DoD) | DoD-specific endpoints; Azure Government DoD region |

: M365 Government Tier vs. Cisco Catalyst SD-WAN INR Architecture {.striped .hover}

## A.7 — Implementation Sequence for a Greenfield FedRAMP Deployment

The following sequence reflects what a Cisco Catalyst SD-WAN + Microsoft INR deployment looks like in a FedRAMP-governed environment, from authorization to production.

**Step 1 — Boundary documentation.** Classify every SD-WAN component against the FedRAMP boundary table in §A.4.1. Determine whether SD-WAN Manager will be self-hosted (GovCloud) or cloud-delivered (Cisco SaaS); verify FedRAMP authorization status of the cloud-delivered option if chosen. Draft ISAs for all external service connections.
...
**Step 6 — Azure Virtual WAN integration (if GCC High).** Deploy Azure Government Virtual WAN hub in the appropriate region. Create Cisco Catalyst SD-WAN site configurations targeting the VWan hub. Validate that M365 GCC High traffic exits via the Azure Government backbone rather than public internet.

**Step 7 — FedRAMP 20x KSI evidence validation.** Using the evidence contracts in §A.5, verify that SIEM receives the required structured events for each KSI. Confirm the evidence passes the format and completeness requirements of the `uiao oscal bundle` assessment results generator.
