---
title: "Vol V Book 02 — Federal Application-Aware Networking — The Implementation Track"
...
> where a `fedramp_aan_catalog` slot mapping can bind it, and a `uiao oscal
...
- **Read:** Vol I Book 01 (SSA Landing Zone, IPAM & FedRAMP).
- **Build:** the DDI core — authoritative IPAM, DNS, DHCP, and PKI issuance for
  the landing zone. The reference stack is Infoblox; the UIAO modernization
  adapters for BlueCat and Infoblox cover directory migration into it.
- **Lab:** stand up (or simulate with fixtures) an IPAM inventory export and bind
  it as **slot-02 addressing** evidence.
...
bound and the slot-02 verdict reads it.
...
# Module B6 — Detect & Protect (Vol III Books 02, 04–07)
...
- **Build:** the telemetry spine — Defender VM assessment with KEV
  prioritization; CSPM/CNAPP posture and container runtime coverage; Purview
...
- **Lab:** one end-to-end detection — seed a vulnerable/exposed condition, watch
  it surface in Defender VM, alert in Sentinel, and close with an evidence export
...
- **Lab:** produce a `training-effectiveness-record` JSON that passes the KSI-022
  thresholds and bind it as **slot-08** evidence; then run `uiao oscal
...
is the whole bundle: 29/29 KSI verdicts self-assessed against the internal
ADR-111 rule set (pending CR26 reconciliation and independent SCA), a zero-item
POA&M as the *target* state (expected non-empty until then),
...
**your** artifact — the same bar as UIAO_125's Operator track ("clean
walker/drift run"), applied to the AAN vertical. Score the result against Vol V
Book 03's rubric ladder.

![The Track B capstone — one module, evidence-complete](figs/vol5b02-fig-02-capstone-one-module.png){#fig-vol5b02-02 fig-alt="A three-panel numbered diagram describing the implementation-track capstone. Panel 1, Implement end-to-end: pick one module from B1 through B8 and build or lab-simulate it fully; tagged 'B1–B8.' Panel 2, Land and regenerate: place the artifact where the slot binds it, then run uiao ksi evaluate and uiao oscal ksi-ar; tagged 'ksi-ar.json.' Panel 3, Your artifact, your verdict: the Assessment Results must show your module's slot verdict fed by your artifact — the same bar as UIAO_125's Operator track; tagged 'Scored in Bk 03.' A footer banner pairs each B-module with its A-module: B1 with A2, B2 with A1/A5, B3 with A1/A3, B4 with A1, B5 with A1/A5, B6 with A4/A6, B7 with A6/A7, and B8 with A8. White background. Source: Vol V Book 02 The Implementation Track briefing deck, Date Code 2026-07-09 17:00 ET." width="100%"}
...
|---|---|---|
| B1 | Vol I Bk 01 | slot 2 (addressing) |
...
| B6 | Vol III Bk 02, 04–07 | slots 4 + 6 |
...
*Federal Application-Aware Networking — Vol V Book 02 of the series. Volume V is
...
claims are self-assessed against the series' ADR-111 rule set (modeled on the
