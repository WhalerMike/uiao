...
function Get-FamilyIndexStub {
    ...
    $links = ($SubItems | ForEach-Object { "- [$_](./$($_.ToLower() -replace '\s+','-').md)" }) -join "`n"
    return @"
---
title: "$FamilyTitle"
family_id: "$FamilyId"
...
\`\`\`plantuml
@startuml
' Source: diagrams/$PumlFile
' Replace this block with the full PlantUML source or use the MkDocs plantuml plugin.
@enduml
\`\`\`
"@
}
...
The UIAO Governance Canon — all human-readable documentation, appendices, ADRs,
diagrams, glossary, and onboarding guides — lives exclusively in:
...
For the full governance canon, see: https://github.com/WhalerMike/uiao-docs
"@
}
...
* [Migration Plan — Flat to Hierarchical](canon/migration-plan.md)
...
* [A-03 Adapter Hot-Swap & Rollback](appendix/a-adapter-plane/a-03-adapter-hot-swap-rollback.md)
* [A-04 Adapter Health & Liveness](appendix/a-adapter-plane/a-04-adapter-health-liveness.md)
...
* [ADR-025 Adapter Health & Liveness Guarantees](adr/adr-025-adapter-health-liveness.md)
* [ADR-026 Canonical Evidence Lifecycle Guarantees](adr/adr-026-evidence-lifecycle-guarantees.md)
* [ADR-027 Adapter Retirement & Deprecation](adr/adr-027-adapter-retirement.md)
...
$Adrs = @(
    ...
    @{ Id="ADR-025"; Title="Adapter Health & Liveness Guarantees";        Family="A — Adapter Plane";     File="adr-025-adapter-health-liveness" }
    ...
    @{ Id="ADR-027"; Title="Adapter Retirement & Deprecation";            Family="A — Adapter Plane";     File="adr-027-adapter-retirement" }
)
...
# ── Root-level files ─────────────────────────────────────────────────────────
Write-Host ""
Write-Host "── Root-level files (uiao-docs) ──" -ForegroundColor Magenta
...
New-Stub -Path (Join-Path $exportsDir "README.md") -Content @"
# exports/

Drop the following files here after downloading from Copilot:

- \`UIAO Governance Canon - Master Publishing Document.docx\`
- \`UIAO Governance Canon - Canonical Index.xlsx\`

These files are tracked in .gitignore (binary exports).
"@

# ── diagrams/ (root, raw .puml) ───────────────────────────────────────────────
Write-Host ""
...
foreach ($puml in $pumlFiles) {
    New-Stub -Path (Join-Path $diagramsRoot $puml) -Content @"
@startuml
' PLACEHOLDER — Replace with full PlantUML source from Copilot task.
' File: $puml
title $($puml -replace '\.puml$','' -replace 'uiao-','UIAO ' -replace '-',' ')
note center
  Paste the PlantUML source block
  from the Copilot publishing task here.
end note
@enduml
"@
}
...
$canonDir = Join-Path $DocsRoot "docs\canon"
...
New-Stub -Path (Join-Path $canonDir "migration-plan.md")  -Content (Get-CanonStub "Migration Plan — Flat Appendices to Hierarchical A-01 through E-03")
...
foreach ($fam in $Families) {
    $famDir = Join-Path $DocsRoot "docs\appendix\$($fam.Id)"
    New-Dir -Path $famDir
...
# ── docs/adr/ ─────────────────────────────────────────────────────────────────
Write-Host ""
Write-Host "── docs/adr/ (new hierarchical ADR stubs) ──" -ForegroundColor Magenta
$adrDir = Join-Path $DocsRoot "docs\adr"
New-Dir -Path $adrDir  # likely already exists — guarded

# ADR index
$adrIndexLinks = ($Adrs | ForEach-Object { "- [$($_.Id) — $($_.Title)](./$($_.File).md)" }) -join "`n"
New-Stub -Path (Join-Path $adrDir "index.md") -Content @"
---
title: "ADR Index — ADR-005 through ADR-027"
---
...
> **Note:** ADR-001 through ADR-004 are pre-canon and not included in the hierarchical canon.
"@
...
$docsDiagDir = Join-Path $DocsRoot "docs\diagrams"
New-Dir -Path $docsDiagDir

New-Stub -Path (Join-Path $docsDiagDir "index.md") -Content @"
---
...
| Diagram | Description |
|---|---|
| [Master Governance Hierarchy](./governance-hierarchy.md) | Full hierarchical taxonomy A through E |
| [Adapter Plane](./adapter-plane.md) | Appendix A internal & cross-plane dependencies |
| [Truth Fabric](./truth-fabric.md) | Appendix B internal & cross-plane dependencies |
| [Drift Fabric](./drift-fabric.md) | Appendix C internal & cross-plane dependencies |
| [Evidence Fabric](./evidence-fabric.md) | Appendix D internal & cross-plane dependencies |
| [Governance Plane](./governance-plane.md) | Appendix E internal & cross-plane dependencies |
"@
...
$onboardDir = Join-Path $DocsRoot "docs\onboarding"
New-Dir -Path $onboardDir  # likely exists

New-Stub -Path (Join-Path $onboardDir "index.md") -Content @"
---
title: "Onboarding"
---
...
Write-Host ""
Write-Host "── uiao-core canon pointer ──" -ForegroundColor Magenta
...
Write-Host "  1. Paste the 6 PlantUML sources into diagrams/*.puml" -ForegroundColor Yellow
Write-Host "  2. Drop DOCX + XLSX into exports/" -ForegroundColor Yellow
Write-Host "  3. Update mkdocs.yml nav (appendix/ and adr/ singular paths)" -ForegroundColor Yellow
