...
- Test skeleton: [`atf-provision-happy-path.xml`](./atf-provision-happy-path.xml)
...
| `deleteObject(...)` | `true` |
| `InfobloxDDIGate.runGate(...)` | `{ overall: 'pass', checks: [dns, discovery-sync, ipam-conflict] }` |
...
- **This is a happy path.** Add negative tests next: a gate **failure** routes back to
  approval (not close); a `universal_ddi` request without `acknowledge_saas_boundary`
  hard-fails; a requester = approver attempt is blocked.
