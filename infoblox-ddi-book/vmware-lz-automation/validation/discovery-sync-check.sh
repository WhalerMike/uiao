...
#
# Proves that Infoblox's VMware discovery — Cloud Network Automation (CNA)
# connecting to vCenter (and NSX Manager) to sync clusters/hosts, VMs, port
# groups/segments and assigned IPs into IPAM using the least-privilege READ-ONLY
# service account from _module-contract.md §5 — has completed successfully and
# recently. A stale or errored sync means IPAM no longer reflects vSphere reality,
# so the script fails the pipeline `validate` stage.
#
...
#       WAPI on the Grid Master, object `vdiscoverytask` (the VMware/vCenter
#       discovery job). Fields such as `state` and the last-run timestamp are read
#       to judge freshness. NOTE: WAPI object/field names are version-specific.
#       `state` is documented (IDLE/RUNNING/COMPLETE/WARNING/ERROR/...); the exact
#       "last successful run" epoch field varies by NIOS/WAPI release. Confirm
#       against YOUR Grid Master at https://<gm>/wapidoc/ before relying on it.
#       The parsing below is intentionally defensive.
#
#   * Universal DDI / Infoblox Portal (deployment_model=universal_ddi, SaaS —
#       OUTSIDE the SDDC/ATO boundary, gated on acknowledge_saas_boundary per §1):
...
#   INFOBLOX_PASSWORD  (required for nios) WAPI password. Inject from Vault/CI.
...
#   INFOBLOX_CSP_TOKEN (required for universal_ddi) CSP API token. From Vault/CI.
#
...
check_nios() {
  ...
  : "${INFOBLOX_PASSWORD:?set INFOBLOX_PASSWORD (inject from Vault/CI)}"
  ...
  # Request the discovery task(s). For VMware this is the vCenter/VMware
  # vdiscoverytask. `state` is a documented basic field; we also ask for common
  # status/timestamp fields, but which ones exist depends on the NIOS/WAPI
  # version — hence the defensive parsing below.
  ...
  log "Querying WAPI ${base} for vdiscoverytask objects (VMware/vCenter discovery)"
  local resp
  ...
  [ "${count}" != "0" ] || fail "no vdiscoverytask objects found (is vCenter/NSX discovery configured in CNA?)"
...
check_universal_ddi() {
  ...
  # which is OUTSIDE the SDDC/ATO boundary (contract §1). It should only run when
  # deployment_model=universal_ddi and acknowledge_saas_boundary=true.
  local csp="${INFOBLOX_CSP_URL:-https://csp.infoblox.com}"
  : "${INFOBLOX_CSP_TOKEN:?set INFOBLOX_CSP_TOKEN (CSP API token from Vault/CI)}"
