#!/usr/bin/env bash
#
# ipam-conflict-check.sh — Stage 3 (validation) IPAM overlap/conflict gate (VMware).
#
# Proves that the networks Infoblox knows about (including those just synced from
# vCenter/NSX by CNA discovery) do not overlap or conflict. Overlapping CIDRs in
# IPAM are the classic symptom of two teams claiming the same address space, or a
# workload-domain segment colliding with an on-prem/enterprise supernet — either
# of which will cause routing/DNS breakage. Any conflict fails the pipeline
# `validate` stage.
#
# DHCP scope note (VMware-specific): because DHCP is genuinely Infoblox's job here
# (contract §4/§8), an overlapping `network` object also means an ambiguous DHCP
# scope — the DHCP member could allocate from the wrong range for an NSX-relayed
# segment. Set CANDIDATE_NETWORK to a new segment's CIDR BEFORE you enable its
# NSX DHCP relay to confirm the range is conflict-free first.
#
# Strategy (NIOS WAPI, deployment_model=grid — the FedRAMP-Moderate default):
#   1. If CANDIDATE_NETWORK is set, fetch IPAM networks and confirm the candidate
#      does not overlap any of them (server-side prefetch + local overlap math) —
#      the "does my new segment collide with anything already in IPAM?" answer.
...
# inline. Confirm against https://<grid-master>/wapidoc/ for your release.
#
# ---------------------------------------------------------------------------
...
#   INFOBLOX_PASSWORD  (required) WAPI password. Inject from Vault/CI.
...
#                                 (e.g. the mgmt network or a new tenant segment
#                                 before enabling its NSX DHCP relay).
