...
# DNS for (a) an enterprise/authoritative A record and (b) a Google-service /
# Cloud DNS private name that the DDI members conditionally forward to the Cloud
# DNS inbound forwarder IP (see _module-contract.md §8, 03-gcp.md §7).
#
...
#   PRIVATELINK_FQDN   (optional) A Google-service / Cloud DNS private name to
#                                 resolve through the DDI conditional-forward path
#                                 to the Cloud DNS inbound forwarder IP, e.g. a
#                                 name under a Cloud DNS private zone. (Name kept
#                                 as PRIVATELINK_FQDN for parity with the Azure
#                                 package's pipeline wiring.)
#   PRIVATELINK_EXPECTED_IP (optional) Expected private IP for PRIVATELINK_FQDN.
#                                 If PRIVATELINK_FQDN is set but this is unset, the
#                                 script asserts only that resolution succeeds and
#                                 returns a private (RFC1918) A.
...
is_rfc1918() {
  # crude private-range check for the optional forward-path assertion
  case "$1" in
    10.*|192.168.*) return 0 ;;
...
# --- Check 2: Cloud DNS conditional-forward path (optional) -------------------
if [ -n "${PRIVATELINK_FQDN:-}" ]; then
  log "Check 2: resolving ${PRIVATELINK_FQDN} via DDI -> Cloud DNS inbound forward path"
  pl_answers="$(resolve_a "${PRIVATELINK_FQDN}")"
  [ -n "${pl_answers}" ] || fail "no A answer for ${PRIVATELINK_FQDN} (conditional-forward to the Cloud DNS inbound forwarder IP may be misconfigured)"
