...
#
# Proves that Infoblox's GCP cloud-discovery / vDiscovery job (the process that
# syncs GCP VPCs/subnets/instances into Infoblox IPAM using the least-privilege
# discovery service account from _module-contract.md §5) has completed
# successfully and recently. A stale or errored sync means IPAM no longer reflects
# GCP reality, so the script fails the pipeline `validate` stage.
#
...
#       WAPI on the Grid Master, object `vdiscoverytask`. Fields such as `state`
#       and the last-run timestamp are read to judge freshness.
#       NOTE: WAPI object/field names are version-specific. `state` is documented
#       (IDLE/RUNNING/COMPLETE/WARNING/ERROR/...); the exact "last successful run"
#       epoch field name varies by NIOS/WAPI release. Confirm against YOUR Grid
#       Master at <grid-master>/wapidoc/ (over HTTPS) before relying on it. The
...
#   INFOBLOX_PASSWORD  (required for nios) WAPI password. Inject from Secret Manager.
...
#   INFOBLOX_CSP_TOKEN (required for universal_ddi) CSP API token. From Secret Manager.
#
...
check_nios() {
  ...
  [ "${count}" != "0" ] || fail "no vdiscoverytask objects found (is GCP cloud discovery configured?)"
...
check_universal_ddi() {
  ...
  : "${INFOBLOX_CSP_TOKEN:?set INFOBLOX_CSP_TOKEN (CSP API token from Secret Manager)}"
