...
#
# Proves that Infoblox's AWS cloud-discovery / vDiscovery job (the process that
# syncs AWS VPCs/subnets/ENIs/instances into Infoblox IPAM using the least-
# privilege cross-account discovery role from _module-contract.md §5) has
# completed successfully and recently. A stale or errored sync means IPAM no
# longer reflects AWS reality, so the script fails the pipeline `validate` stage.
#
...
#   INFOBLOX_PASSWORD  (required for nios) WAPI password. Inject from Secrets Manager.
...
#   INFOBLOX_CSP_TOKEN (required for universal_ddi) CSP API token. From Secrets Manager.
#
...
check_nios() {
  ...
  [ "${count}" != "0" ] || fail "no vdiscoverytask objects found (is AWS cloud discovery configured?)"
...
check_universal_ddi() {
  ...
  : "${INFOBLOX_CSP_TOKEN:?set INFOBLOX_CSP_TOKEN (CSP API token from Secrets Manager)}"
