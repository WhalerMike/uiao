...
# AWS by cloud discovery) do not overlap or conflict. Overlapping CIDRs in IPAM
# are the classic symptom of two teams claiming the same address space, or an AWS
# VPC colliding with an on-prem/enterprise supernet — either of which will cause
# routing/DNS breakage. Any conflict fails the pipeline `validate` stage.
#
...
#   INFOBLOX_PASSWORD  (required) WAPI password. Inject from Secrets Manager.
...
#                                 (e.g. a ddi_subnet CIDR or a new spoke range).
...
: "${INFOBLOX_PASSWORD:?set INFOBLOX_PASSWORD (inject from Secrets Manager)}"
