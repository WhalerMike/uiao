...
#
# Proves that the networks Infoblox knows about (including those just synced
# from Azure by cloud discovery) do not overlap or conflict. Overlapping CIDRs
# in IPAM are the classic symptom of two teams claiming the same address space,
# or an Azure VNet colliding with an on-prem/enterprise supernet — either of
# which will cause routing/DNS breakage. Any conflict fails the pipeline
# `validate` stage.
#
# Strategy (NIOS WAPI, deployment_model=grid — the GCC-Moderate default):
#   1. If CANDIDATE_NETWORK is set, ask WAPI directly for networks that overlap
#      it using the `network` search modifier `network_container` / a contains
#      query — the authoritative, server-side answer for "does my new subnet
#      collide with anything already in IPAM?".
#   2. Otherwise, enumerate all `network` objects in the (optional)
#      NETWORK_VIEW and detect pairwise CIDR overlaps locally. This catches
#      pre-existing overlaps across the whole view.
#
...
#   INFOBLOX_PASSWORD  (required) WAPI password. Inject from Key Vault.
...
if [ -n "${CANDIDATE_NETWORK:-}" ]; then
  ...
  # The exact modifier is version-specific; `network~` (regex) is NOT an
  # overlap test. Common approaches:
  ...
  for ((i=0; i<count; i++)); do
    for ((j=i+1; j<count; j++)); do
      # Distinct entries with the identical CIDR are also a conflict
      # (duplicate declaration), and overlaps() returns true for them too.
