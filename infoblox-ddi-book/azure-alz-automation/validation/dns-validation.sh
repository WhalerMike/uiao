...
# answering DNS for (a) an enterprise/authoritative A record and (b) an Azure
# `privatelink.*` name that the DDI members conditionally forward to the Azure
# DNS Private Resolver inbound endpoint (see _module-contract.md §8).
#
...
#   PRIVATELINK_FQDN   (optional) An Azure privatelink name to resolve through
#                                 the DDI conditional-forward path, e.g.
...
is_rfc1918() {
  # crude private-range check for the optional privatelink assertion
  case "$1" in
    10.*|192.168.*) return 0 ;;
...
# --- Check 2: Azure privatelink conditional-forward path (optional) ----------
if [ -n "${PRIVATELINK_FQDN:-}" ]; then
  log "Check 2: resolving privatelink ${PRIVATELINK_FQDN} via DDI forward path"
  pl_answers="$(resolve_a "${PRIVATELINK_FQDN}")"
  [ -n "${pl_answers}" ] || fail "no A answer for ${PRIVATELINK_FQDN} (conditional-forward to Azure Private Resolver may be misconfigured)"
