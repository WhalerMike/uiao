...
non-zero exit**. Together they answer: *is DNS actually resolving, is Azure→IPAM
discovery current, and is the address space clean?*

## What each check proves

| Script | Proves | Fails when |
|---|---|---|
| `dns-validation.sh` | The DDI anycast VIP (contract output `ddi_anycast_vip`) answers DNS: an enterprise/authoritative A record resolves to its expected IP, and an Azure `privatelink.*` name resolves through the conditional-forward path to the Azure DNS Private Resolver (contract §8). | No answer, wrong answer, or a privatelink name that returns a public IP (forward path bypassed). |
| `discovery-sync-check.sh` | Infoblox cloud-discovery / vDiscovery (the Azure→IPAM sync using the least-privilege discovery identity, contract §5) completed successfully and recently. | A task is in `ERROR`/`WARNING`/`FAILED` state, or the last successful run is older than `STALE_THRESHOLD_MIN`. |
| `ipam-conflict-check.sh` | The networks in IPAM (including freshly-discovered Azure VNets/subnets) do not overlap — no two teams or clouds claiming the same CIDR. | Any overlapping or duplicate `network` object is found (server-side candidate query or whole-view pairwise scan). |
...
- **`deployment_model = universal_ddi`**: `discovery-sync-check.sh` has a second
  ...
  the ATO boundary** (contract §1) and should only be exercised when
  `acknowledge_saas_boundary = true`. The pipeline passes the flavor through.

## Required environment variables

Secrets (WAPI/CSP creds) come from **Azure Key Vault** in the pipeline, never
...
- `PRIVATELINK_FQDN`, `PRIVATELINK_EXPECTED_IP` (opt) — privatelink forward test
...
Both pipelines run the scripts after DDI apply; secrets are injected from Key
Vault, non-secret inputs from Stage 2 remote-state outputs:
...
`dig`/`nslookup` (present on the pipeline runners; see pipeline READMEs).
