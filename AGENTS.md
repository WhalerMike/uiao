...
- **Cloud boundary (current deployment):** GCC-Moderate (Microsoft 365 SaaS only). This is the boundary the federal vertical is deployed against today; it is not a property of the core engine. Three named Commercial exceptions: Amazon Connect Contact Center; SailPoint Non-Employee Risk Management (FedRAMP Moderate on AWS GovCloud, per ADR-059); and SailPoint Identity Security Cloud Governance (FedRAMP Moderate on AWS GovCloud, per ADR-059 Option B, ratified by ADR-135). Each exception is encoded as a discrete enum value in the `gcc-boundary` schema; new exceptions are added in lockstep with their authorizing ADR. Non-federal vertical adapter packs (commercial-regulated, state/local, generic enterprise) will introduce their own boundary enums under separate ADRs as they ship.
...
4. **Schema-first governance** — forty JSON Schemas under `src/uiao/schemas/` validate every registry, manifest, and frontmatter edit in CI.
...
> **Version-by-version surface history lives in a companion file.** The
> incremental "Public surface additions" waves (v0.6.0; ADR-096 Azure SaaS;
> ADR-098 OrgPath multi-cloud; ADR-100/101 Active Governance Directory;
> ADR-102 LocPath) were split out to
> [`AGENTS-public-surface.md`](AGENTS-public-surface.md) to keep this entry
> point lean. The M5 inventory above is the current-state snapshot; the
> companion is the append-only changelog of additions since v0.5.0 — append
> new waves there, not here.
...
- **PRs squash-merge into `main`.** One PR = one squash commit; the branch's individual commits are not preserved on `main`.
- **Never stack a PR on another PR's branch.** Because the base is squash-merged, the stacked PR's commits never re-target `main` — they merge into the now-stale branch and silently vanish (this is the #937 incident). Branch every PR off `main`.
- **Rebase onto fresh `main` before merging, not the stale base you branched from.** After any other PR lands, `git fetch origin main && git rebase origin/main`. The squash makes your original copy of an already-merged commit redundant — rebase drops it automatically; a plain merge instead reports a phantom conflict on the files both touched.
- **Require "branches up to date" in branch protection** so a stale base can't merge ahead of CI run against the real post-merge tree.
