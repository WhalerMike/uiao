...
def _load_scuba(src: Path) -> Dict[str, Any]:
      """Load a SCuBA file (JSON or YAML) and return the raw dict."""
      ...
      if not isinstance(data, dict):
                raise ValueError(f"SCuBA file must deserialize to a dict, got {type(data)}")
            return data  # type: ignore[return-value]


def _load_config(config_path: Optional[str]) -> Dict[str, Any]:
      """Load optional transform config (JSON or YAML).  Returns {} on None."""
...
def _apply_config_overrides(
    ...
    if not cfg:
              return scuba
          result = dict(scuba)
    # Config key: tenant_boundary_id → propagated downstream
...
def transform_scuba_to_ir(
      ...
      """Transform a SCuBA assessment file into canonical IR JSON.

          Parameters
              ----------
                  input_path:
                          Path to a SCuBA JSON or YAML file.
                              output_path:
                                                                          ...
                                                                          ------
                                                                              FileNotFoundError
                                                                                      When *input_path* does not exist.
                                                                                          ValueError
                                                                                                  When the SCuBA file cannot be parsed as a dict.
                                                                                                      """
      src = Path(input_path)
    ...
    logger.info("SCuBA→IR transform started")
    ...
    # 1. Load SCuBA source
    logger.debug("Loading SCuBA file …")
    scuba_raw = _load_scuba(src)

    # 2. Load config
    cfg = _load_config(config_path)
    if cfg:
              logger.debug("Config loaded: %s", list(cfg.keys()))

    # 3. Apply config overrides (tenant_boundary_id, drop_statuses, …)
    tenant_boundary_id: str = cfg.get(
              "tenant_boundary_id", "boundary:tenant:m365:default"
    )
    scuba_data = _apply_config_overrides(cfg, scuba_raw)
    logger.debug(
              "ksi_results count after config filter: %d",
              len(scuba_data.get("ksi_results", [])),
    )

    # 4. Write patched SCuBA to a temp file so _core_transform can read it
    #    (the core transformer expects a file path, not an in-memory dict)
    import tempfile
