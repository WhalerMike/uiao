"""uiao_core.cli.evidence — Typer sub-app for the `uiao evidence` command group.

Mount point
-----------
    # in uiao_core/cli/app.py
    from uiao_core.cli.evidence import evidence_app
...
Or via module invocation:
    python -m uiao_core.cli evidence build \\
        --input  ./output/ksi/tenant-a.ksi.json \\
        --output ./output/evidence/tenant-a/
"""
...
from uiao_core.evidence.builder import build_evidence
...
def build_command(
    ...
    `uiao_core.evidence.builder.build_evidence`, which is kept
