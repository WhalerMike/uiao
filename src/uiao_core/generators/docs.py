"""Document generator. Migrated from scripts/generate_docs.py into the uiao_core package.

Renders Jinja2 templates against canon YAML + data overlays to produce Markdown
documents for docs/ and site/ directories. After template rendering, diagram
generation is triggered automatically via :func:`generate_diagrams_from_canon`.

Diagrams are now PlantUML (.puml); the former Mermaid post-processing step has
been removed. Rendered Markdown that embeds ``plantuml`` fenced blocks can be
referenced as ``<img>`` tags pointing to pre-rendered PNGs in
...
# ---------------------------------------------------------------------------
# PlantUML block post-processing
# ---------------------------------------------------------------------------
...
def replace_plantuml_blocks_with_images(
        markdown_text: str,
        images_dir: str = "assets/images/plantuml",
        alt_prefix: str = "diagram",
) -> str:
        """Replace fenced ``plantuml`` code blocks with ``<img>`` references.

            This post-processing step enables DOCX/PPTX/PDF renderers (which cannot
                execute PlantUML server calls at render time) to embed pre-rendered PNG
                    images in place of raw PlantUML source blocks.
                                        ...
                                        images_dir: Relative path prefix for PNG image URLs.
                                                alt_prefix: Fallback prefix for ``alt`` text when no title can be
                                                            inferred from the diagram source.
                                                                ...
                                                                Returns:
                                                                        Markdown text with each ``plantuml`` fence replaced by an ``<img>``
                                                                                referencing ``<images_dir>/<slug>.png``.
                                                                                    """
    counter = 0
    ...
    def _replace(match: re.Match[str]) -> str:
        ...
        # Try to infer a slug from the first non-empty line after @startuml
    ...
    return _PLANTUML_FENCE_RE.sub(_replace, markdown_text)
...
def build_docs(
        ...
        """Build all documentation from templates.
            ...
            After rendering templates, optionally auto-generates PlantUML diagrams from
                ``generation-inputs/diagrams.yaml`` and replaces fenced ``plantuml`` blocks
                    in the rendered Markdown with ``<img>`` references to pre-rendered PNGs.
                                                                        ...
                                                                        template_mapping: Override the default template->output mapping.
                                                                                generate_diagrams: When ``True`` (default), call
                                                                                            :func:`generate_diagrams_from_canon` before template rendering.
                                                                                                    force_visuals: Passed through to the diagram renderer; forces re-render
                                                                                                                even when a cached PNG already exists.
    ...
    for tpl_name, (docs_name, site_name) in template_mapping.items():
        ...
        # Post-process: replace plantuml fences with <img> tags for PDF/DOCX
        processed = replace_plantuml_blocks_with_images(output)
