...
Migrated from scripts/generate_docs.py into the uiao.impl package.
...
``plantuml`` fenced code blocks in rendered Markdown with ``<img>`` references
...
from uiao.utils.context import get_settings, load_canon
...
def render_template(
    ...
    # Jinja2 render() is typed as Any in some envs; coerce to str for the
...
def build_docs(
    ...
    After rendering templates, optionally auto-generates PlantUML diagrams
    from ``generation-inputs/diagrams.yaml`` and replaces fenced ``plantuml`` blocks in
