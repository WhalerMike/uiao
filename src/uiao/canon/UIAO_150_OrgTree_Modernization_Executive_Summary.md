...
Per ADR-079 (src/uiao/canon/adr/adr-079-governance-principle-reconciliation.md), the Governance OS declares 3 universal substrate principles plus 3 tier-specific principles, canonically declared in src/uiao/canon/substrate-manifest.yaml (UIAO_200). The substrate manifest is the code SSOT.

3.1 Universal Substrate Principles

These apply at all 5 ADR-076 conformance tiers. Every Governance OS deployment honors all three.

Principle 1: Single Source of Truth (SSOT). Canonical governance artifacts under src/uiao/canon/ define structure, policy intent, and registry authority exactly once. Downstream systems consume those artifacts via importlib.resources rather than duplicating policy logic. Schema is fixed; values are flexible — the structure of the OrgPath codebook, JSON schemas, and dynamic group rules is immutable once canonized, and only values within defined enumerations change through governed workflows defined in Appendix E.
...
Two-Brain Execution (Tier 3+, Active Substrate): Copilot governs (canonical review, policy enforcement, validation); Execution Substrate executes (PowerShell, Graph API, tenant provisioning); governance logic and execution logic never co-mingle. At Tiers 1–2 there is no active runtime to split between governance and execution; the principle becomes operational only when UIAO writes agency state.

Boundary Enforcement (Tier 4+, Active Services / Platform Server): No governance artifact, automation script, API call, or execution path may extend beyond the M365 GCC-Moderate SaaS boundary. Any artifact referencing an out-of-scope service is non-canonical and rejected at the validation gate (Appendix V). At Tiers 1–3, boundary is a documented scope; at Tier 4 it becomes an enforced runtime contract on agency-deployed UIAO services.

Tenant Agnosticism (Tier 5, Embedded Libraries): All artifacts are portable across any M365 GCC-Moderate tenant. No tenant-specific identifiers, UPNs, or GUIDs. All environment values are injected at deployment time. At Tiers 1–4, UIAO runs agency-side and may carry tenant-local context; at Tier 5, code shipped to third parties (PyPI, NuGet, adapter SDK) MUST be portable.

3.3 Historical names ("Seven Non-Negotiable Principles")

Earlier framings of this canon listed seven principles. Per ADR-079, four of those names were partial expressions of the universal three and have been collapsed:
...
The other three names (Boundary Enforcement, Two-Brain Execution, Tenant Agnosticism) remain canonical, now scoped to the tiers where they bind. The "seven principles" phrasing should not be cited in new material; refer to the 3 + 3 split above or to the substrate manifest directly.
...
Copilot (Governance Brain): Responsible for canonical review of all governance artifacts; enforcement of the substrate governance principles per Section 3 (the 3 universal + the tier-specific principles applicable at the declared ADR-076 tier); validation of schemas, boundary rules, and hierarchical integrity; generation of deterministic instruction sets for execution; provenance recording; and drift classification.
