...
Record migration status IN-PROGRESS in the migration log. Status is *not* written to a facet slot: extensionAttribute4 is the Role facet, and the fifteen slots are a governed, tenant-wide resource (ADR-078/ADR-121).

Validate all attribute writes completed successfully.

# Provision OrgPath for each user foreach ($Mapping in $UserOrgPathMappings) {     try {         $UserParams = @{             UserId = $Mapping.EntraUserId             OnPremisesExtensionAttributes = @{                 extensionAttribute1  = $Mapping.Region                 extensionAttribute2  = $Mapping.Department                 extensionAttribute3  = $Mapping.Division                 extensionAttribute15 = New-OrgPath -Region $Mapping.Region -Department $Mapping.Department -Division $Mapping.Division             }         }         Update-MgUser @UserParams         Write-Verbose "Updated user $($Mapping.EntraUserId) with OrgPath $($Mapping.OrgPath)"     }     catch {         Write-Error "Failed to update user $($Mapping.EntraUserId): $_"     } }  # Verify provisioning $ProvisionedUsers = Get-MgUser -All -Property "Id,OnPremisesExtensionAttributes" |     Where-Object { $_.OnPremisesExtensionAttributes.ExtensionAttribute1 -match "^ORG" } Write-Verbose "Provisioned users: $($ProvisionedUsers.Count)"  # Identify failures $UnprovisionedUsers = Get-MgUser -All -Property "Id,OnPremisesExtensionAttributes" |     Where-Object { [string]::IsNullOrEmpty($_.OnPremisesExtensionAttributes.ExtensionAttribute1) } Write-Verbose "Unprovisioned users: $($UnprovisionedUsers.Count)"  # Update migration status for successful provisions foreach ($User in $ProvisionedUsers) {     Write-MigrationLog -UserId $User.Id -Status "COMPLETED"     # status belongs in the migration log, not in the Role facet slot }

Validation Criteria: every user carries its projected facet values, each valid in its own codebook enumeration; extensionAttribute15 equals the recomputation from those facets (Test-OrgPathDrift reports no divergence); zero provisioning errors; the migration log records COMPLETED for all provisioned users.
...
# Confirm every user's derived path matches its facets, and record the result # in the migration log rather than in a facet slot. $AllUsers = Get-MgUser -All -Property "Id","OnPremisesExtensionAttributes" -Filter "userType eq 'Member'" foreach ($User in $AllUsers) {     try {         $ea = $User.OnPremisesExtensionAttributes         $expected = New-OrgPath -Region $ea.ExtensionAttribute1 -Department $ea.ExtensionAttribute2 -Division $ea.ExtensionAttribute3         if ($ea.ExtensionAttribute15 -ne $expected) {             Write-Error "Derived-path drift for $($User.Id): stored '$($ea.ExtensionAttribute15)', expected '$expected'"         }     }     catch {         Write-Error "Failed to validate $($User.Id): $_"     } }  # Create post-cutover snapshot Export-OrgTreeSnapshot -TenantId $TenantId -OutputPath "$OutputPath\cutover-snapshot.json"  # Compare against baseline $DriftReport = Compare-OrgTreeSnapshots -BaselinePath "$OutputPath\baseline-snapshot.json" `     -CurrentPath "$OutputPath\cutover-snapshot.json" Write-Verbose "Drift entries since baseline: $($DriftReport.Count)"

Validation Criteria: Drift detection engine is active; telemetry is flowing; every user's stored derived path equals its recomputation, and the migration log records VALIDATED for all users.
