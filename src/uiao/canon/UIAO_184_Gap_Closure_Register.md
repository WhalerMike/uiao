...
| Implement UIAOImportAdapters (producer) | `.psd1`+`.psm1`+Pester under `tools/powershell/`, signed | **Implemented (signing pending)** — [`tools/powershell/UIAOImportAdapters/`](../../../tools/powershell/UIAOImportAdapters/); offline, full roster, content-hash-sealed envelope, 21 Pester tests gated in CI. Authenticode signing is the residual maintainer step. |
...
| 2026-06-05 | Workstream B: **UIAOImportAdapters implemented** (first producer, ADR-094 build order) at `tools/powershell/UIAOImportAdapters/` — `.psd1` + `.psm1` + 21 Pester tests, full UIAO_182 roster, offline content-hash-sealed provenance envelope with canonical-JSON parity to `canonical_hash` (DRIFT-PROVENANCE round-trip verified). CI Pester extended to the new suite. Row flipped Open → Implemented (signing pending). Gap 3 stays In progress — UIAOIdentityAssessment and UIAOPlanGenerators remain. |
