...
| 4 | OrgPath drift detection engine only partially implemented | C | **Closed** | [ADR-084](adr/adr-084-phase5-consumer-architecture.md); UIAO_163; `src/uiao/governance/drift{,_engine}.py` |
...
The plan is [ADR-084](adr/adr-084-phase5-consumer-architecture.md)'s Phase 5 sequence plus two classifier-level sub-gaps. The engine is built (`src/uiao/governance/drift_engine.py`) — DRIFT-SCHEMA / DRIFT-SEMANTIC / DRIFT-AUTHZ / DRIFT-PROVENANCE / per-facet DRIFT-IDENTITY all detecting — and the ADR-084 Phase 5 consumer program (C1–C9) is complete (see the conformance-audit note below).
...
| ADR-084 consumer rebuilds (phases 1–5) | Each consumer module rebuilt (Model C) | **Closed** — five Model C modules under `src/uiao/modernization/orgtree/` conform to the ADR-084 contract: **C1** every consumer takes a `Codebook` (zero direct YAML parsing), **C2** all four emit `FacetOperation`s, **C3** the three rule consumers (`dynamic_groups`, `admin_units`, `policy_targeting`) call the shared `rule_renderer.render_rule`, **C6** the canonical 10-facet fixture is in `tests/conftest.py`. Plus **C7** the per-facet API (`src/uiao/api/routes/orgpath.py`) and **C8** the PS helper (`OrgPathTools.Test-OrgPathFacets`). 124 `tests/orgtree` tests pass; no stubs/TODOs. |
| DriftEngine Model C orchestrator integration (phase 6) | Wired to rebuilt consumers | **Closed** — `DriftEngine` (`src/uiao/governance/drift_engine.py`) orchestrates the four adapters + `Codebook` over the snapshot→plan→classify→remediate loop with per-facet findings and `halt_on_critical`; `orgpath_runtime.OrgPathGovernanceRuntime.with_canon_adapters()` wires the canonical Phase 5 adapter set (detection-only, dry-run-safe defaults). Covered by `tests/orgtree/test_drift_engine.py`. |

**Closure criterion for Gap 4:** `DRIFT-PROVENANCE` and per-facet `DRIFT-IDENTITY` both detecting; ADR-084 phases 1–6 complete. **Met:** both classifiers detect (rows 1–2); the ADR-084 C1–C9 consumer program and the phase-6 orchestrator are complete and tested (rows 3–4). **Gap 4 is Closed.**

> **Conformance-audit note (2026-06-05).** The phases 1–5 / phase-6 rows were marked Open/In-progress while the implementation already existed in `src/` — a register-vs-code lag, not missing work. This close is the reconciliation: each ADR-084 contract clause (C1–C9) was verified against the code and the `tests/orgtree` suite (124 passing) before flipping. **Derived-view follow-up:** Book 15 / Chapter 23 still renders "only partially implemented" and its gap diagram (`book15cpt23-diagram-01`) still lists Gap 4 — per Workstream D §2 the chapter is *derived* from this register, so its prose and that rendered figure should be refreshed in a separate change (the figure is a committed raster and needs regeneration, so it is intentionally not edited here).
...
| 2026-06-05 | Workstream C: **conformance audit + reconciliation — Gap 4 Closed.** Verified the ADR-084 C1–C9 contract against code: consumers take a `Codebook` (C1, no direct YAML), emit `FacetOperation`s (C2), the three rule consumers call shared `render_rule` (C3), canonical fixture in `tests/conftest.py` (C6); per-facet API route (C7) and `Test-OrgPathFacets` PS helper (C8) present; `DriftEngine` orchestrates the four adapters + `Codebook` and is wired via `orgpath_runtime.with_canon_adapters()` (phase 6). 124 `tests/orgtree` tests pass, no stubs. **Consumer-rebuilds and phase-6 rows flipped → Closed; Gap 4 row flipped In progress → Closed.** All four register gaps are now Closed. Derived-view follow-up: refresh Book 15 / Ch 23 prose + gap diagram (separate change; the figure is a committed raster needing regeneration). |
| 2026-06-19 | Compliance follow-up (post-Gap-2): **Risk Assessment Methodology (UIAO_198, `RA-3`/`RA-2`/`RA-5`) authored and registered** — closes the SSOT/`DRIFT-PROVENANCE` gap where `RA-3` asserted NIST SP 800-30 Rev 1 alignment but the methodology lived only in external SharePoint. Ships the canonical likelihood × impact matrix (Table I-2) as a committed blueprint SVG (`docs/canon/images/uiao-198-diagram-01-risk-matrix.svg`, ADR-093). Also corrected a `UIAO_195` ID collision: that ID was first allocated to the Addressing-Plane Drift Taxonomy (ADR-108, merged 2026-06-18) but was never recorded in `document-registry.yaml`; this change registers it and moves the risk methodology to `UIAO_198`. |
| 2026-06-19 | Compliance-canon maturation: **promoted the nine compliance deliverables (UIAO_185–192, 198) from `Draft` v0.1 → `Current` v1.0** — frontmatter, `document-registry.yaml`, and the `docs/canon/` publication wrappers all flipped. These are now authoritative templates rather than drafts: UIAO-canonical identity fields are pre-filled for the reference deployment (UIAO Governance OS, v0.7.0, GCC-Moderate boundary) and the remaining bracketed `[…]` fields are explicitly labelled agency-ATO instantiation parameters (system owner, authorizing official, agency policy refs) completed at ATO time. Addresses the compliance-corpus lifecycle lag relative to the `Current` governance corpus — compliance and governance are co-equal pillars. |
