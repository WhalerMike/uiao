...
class PkiCaAdapter(DatabaseAdapterBase):
    ...
    def discover_schema(self) -> SchemaMappingObject:
        ...
        canonical_schema = {
            "identity": f"pki-ca:{self._cert_store}:<serial_number>",
            "control_id": "SC-12",
            "evidence.source": "pki-ca",
        }
        ...
        for cert in raw_rows:
            ...
            claims.append(
                ClaimObject(
                    ...
                    fields={
                        "identity": f"pki-ca:{self._cert_store}:{serial}",
                    ...
                    provenance_hash=self._hash(cert),
                )
            )
    ...
    def detect_drift(self) -> DriftReport:
        return DriftReport(
            drift_type="pki-certificate-state",
            severity="info",
            first_observed=self._now(),
            last_observed=self._now(),
            details={
                "message": "Drift scaffold — check certificate expiry and revocation.",
