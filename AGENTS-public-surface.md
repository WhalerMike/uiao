...
| **AGD write actuation** | `uiao.directory.actuation` | `uiao directory serve --enable-writes --apply --provider-*` | CLI | ADR-109 §3 / ADR-092 §3; `FacetActuator` is the gated `apply_fn` seam — it routes an approved write plan into a Phase-5-shaped adapter's `apply(operations, dry_run=False)`. `FacetWriteAdapter` adapts a per-principal directory transport (`LdapTransport.modify`) into that `FacetApplyAdapter` protocol so a facet write reaches the provider of record. Actuation is **off by default**: `FacetActuator(adapter, enabled=False)` raises `ActuationDisabled` (the L3 opt-in); `governance_review_ops` are always held; each apply is audited. The CLI `--apply` path constructs `LdapTransport` → `FacetWriteAdapter` → `FacetActuator(enabled=True)` → `WriteRouter(dry_run=False, apply_fn=…)`, deferred past `--check` so validation opens no connection |
